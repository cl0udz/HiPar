(function(sandbox) {
    function FlowAnalysis() {
        ...
        var traceLogFile =  path.resolve("./trace_log.json");
	...
	var taint_trace = {};
	    ...
	    //console.log("traceText: " + entryText);
        }
	    ...
	    //if(val != undefined && val.tainted == true)
		//console.log("Reading from: " + name + ", location: " + sandbox.iidToLocation(iid));
            return val;
        };
		...
		//console.log("Adding: from " + oldValue + " to " + val + " on " + name + " id: " + iid);
		//console.log("id to position: " + sandbox.iidToLocation(iid));
	    ...
	    //console.log("Testing pre_taint: " + pre_taint);
	    //console.log("Now val is: " +  val + ", indexOf: " + pre_taint.indexOf("" + val));
	    //if(pre_taint.indexOf("" + val) != -1){
	    //console.log("keys: " + JSON.stringify(Object.keys(tainted_values)) + ", index: " + tainted_values[val.id] === undefined);
	    if(val != undefined && (tainted_values.hasOwnProperty(val.id) || val.tainted == true)){
	    //if(true){
		console.log("Writing to(id) " + iid + ", name: " + name + " with value: " + val + ", tainted: " + val.tainted);
	    	console.log("Writing to " + sandbox.iidToLocation(iid) + " with valueid " + val + ", old value: " + oldValue);
		tainted_values[iid] = sandbox.iidToLocation(iid);
		console.log("tainted_values: " + JSON.stringify(tainted_values));
	    }
	    ...
	    if((left != undefined &&left.hasOwnProperty("tainted")) || (right != undefined && right.hasOwnProperty("tainted")))
		result.tainted = true;
            return result;
        };
	    ...
	    var callerFileName = _getCallerFile();
	    if(callerFileName != undefined && callerFileName.indexOf("analysis.js") == -1 && callerFileName.indexOf("WrappedPrimitivesFlowAnalysis.js") == -1 )
		console.log("Enter function " + f.name + " from " + callerFileName);
            if (!f) //|| f.name === "cleanPC"
                return;
        ...
        this.invokeFun = function(iid, f, base, args, val, isConstructor) {
            ...
            if (f.name === "addSource") {
                console.log("New source:" + args[0] + " with " + args[1]);
                if (args[1]) {
		...
		pre_taint.push(args[0]);
                //logger.log("1Variable marked:"  + result.id + " with " + args[1] + ", source " + args[2], LoggerUtils.WARNING_LEVEL);
                addTraceEntry([TraceConstants.SOURCE_OP, result.id, args[2], iid]);
		tainted_values[result.id] = sandbox.iidToLocation(result.id);
		console.log("args[0]: " + args[0] + ", args[2]: " + args[2] + ", result.id: " + result.id + ", result: " + result + ", base: " + base);
		result.tainted = true;
		return result;
            }
		...
		upperfile = "";
		    ...
		    //console.log(callerfile);
                    if(upperfile == "" && currentfile != callerfile) {
			    //console.log(currentfile + " -> " + callerfile);
			    upperfile = callerfile;
		    }//{return callerfile;}
		    if(upperfile != "" && upperfile != callerfile){
			//console.log("Got you");
			return callerfile;
		    }
                }
