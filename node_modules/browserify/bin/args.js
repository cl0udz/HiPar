...
module.exports = function (args, opts) {
  var argv = subarg(args, {
    'boolean': ['deps', 'pack', 'ig', 'dg', 'im', 'd', 'list', 'builtins', 'commondir', 'bare', 'full-paths', 'bundle-external', 'bf', 'node', 'preserve-symlinks'],
  ...
  var entries = argv._.concat(argv.entry).filter(Boolean).map(function (entry) {
    if (entry === '-') {
      var s = process.stdin;
      if (typeof s.read === 'function') return s; // only needed for 0.8, remove at some point later:
  ...
  b.argv = argv;
  [].concat(argv.p).concat(argv.plugin).filter(Boolean).forEach(function (p) {
    var pf = p,
        pOpts = {};
  ...
  [].concat(argv.ignore).filter(Boolean).forEach(function (i) {
    b._pending++;
  ...
  [].concat(argv.exclude).filter(Boolean).forEach(function (u) {
    b.exclude(u);
    b._pending++;
  ...
  [].concat(argv.require).filter(Boolean).forEach(function (r) {
    var xs = splitOnColon(r);

    b.require(xs[0], {
      expose: xs.length === 1 ? xs[0] : xs[1]
    });
  }); // resolve any external files and add them to the bundle as externals

  [].concat(argv.external).filter(Boolean).forEach(function (x) {
    ...
    function add(x, opts) {
      if (/^[\/.]/.test(x)) b.external(path.resolve(x), opts);else b.external(x, opts);
    }
  });
  [].concat(argv.transform).filter(Boolean).forEach(function (t) {
    addTransform(t);
  });
  [].concat(argv.g).concat(argv['global-transform']).filter(Boolean).forEach(function (t) {
    addTransform(t, {
      global: true
    });
  });
  ...
  function addTransform(t, opts) {
    ...
    } else error('unexpected transform of type ' + _typeof(t));
  }

  [].concat(argv.command).filter(Boolean).forEach(function (c) {
