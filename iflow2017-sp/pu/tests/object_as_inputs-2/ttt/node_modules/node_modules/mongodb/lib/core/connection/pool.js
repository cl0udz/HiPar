...
function attemptReconnect(self) {
  return function () {
    ...
    connect(self.options, function (err, connection) {
      self.connectingConnections--;

      if (err) {
        if (self.logger.isDebug()) {
          self.logger.debug("connection attempt failed with error [".concat(JSON.stringify(err), "]"));
        }
      ...
      handlers.forEach(function (event) {
        return connection.removeAllListeners(event);
      });
...
Pool.prototype.reset = function (callback) {
  ...
  eachAsync(connections, function (conn, cb) {
    ...
    resetPoolState(_this); // create an initial connection, and kick off execution again

    _createConnection(_this);
...
Pool.prototype.write = function (command, options, cb) {
  ...
  if (self.options.monitorCommands) {
    ...
    operation.cb = function (err, reply) {
      if (err) {
        self.emit('commandFailed', new apm.CommandFailedEvent(_this2, command, err, operation.started));
      } else {
        if (reply && reply.result && (reply.result.ok === 0 || reply.result.$err)) {
          self.emit('commandFailed', new apm.CommandFailedEvent(_this2, command, reply.result, operation.started));
        } else {
          self.emit('commandSucceeded', new apm.CommandSucceededEvent(_this2, command, reply, operation.started));
        }
      }
