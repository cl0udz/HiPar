...
function (_EventEmitter) {
  ...
  function Connection(stream, options) {
    ...
    _this4.address = streamIdentifier(stream);
    _this4.bson = options.bson;
    _this4.socketTimeout = typeof options.socketTimeout === 'number' ? options.socketTimeout : 360000;
    _this4.monitorCommands = typeof options.monitorCommands === 'boolean' ? options.monitorCommands : false;
    _this4.closed = false;
    _this4.destroyed = false;
    _this4[kDescription] = new StreamDescription(_this4.address, options);
    _this4[kGeneration] = options.generation;
    _this4[kLastUseTime] = Date.now(); // retain a reference to an `AutoEncrypter` if present
