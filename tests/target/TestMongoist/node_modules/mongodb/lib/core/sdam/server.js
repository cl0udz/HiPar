...
function (_EventEmitter) {
  ...
  function Server(description, options, topology) {
    ...
    _this7 = _possibleConstructorReturn(this, _getPrototypeOf(Server).call(this));
    ...
    var addressParts = _this7.description.address.split(':');
    ...
    _this7.s.pool = new ConnectionPool(poolOptions);
    relayEvents(_this7.s.pool, _assertThisInitialized(_this7), ['commandStarted', 'commandSucceeded', 'commandFailed'].concat(CMAP_EVENT_NAMES));

    _this7.s.pool.on('clusterTimeReceived', function (clusterTime) {
      _this.clusterTime = clusterTime;
    }); // create the monitor


    _this7[kMonitor] = new Monitor(_assertThisInitialized(_this7), _this7.s.options);
    relayEvents(_this7[kMonitor], _assertThisInitialized(_this7), ['serverHeartbeatStarted', 'serverHeartbeatSucceeded', 'serverHeartbeatFailed', // legacy events
    'monitoring']);

    _this7[kMonitor].on('resetConnectionPool', function () {
      _this.s.pool.clear();
    });

    _this7[kMonitor].on('resetServer', function (error) {
      // Revert to an `Unknown` state by emitting a default description with no isMaster, and the
      // error from the heartbeat attempt
      _this.emit('descriptionReceived', new ServerDescription(_this.description.address, null, {
        error: error
      }));
    });

    _this7[kMonitor].on('serverHeartbeatSucceeded', function (event) {
      ...
      if (_this.s.state === STATE_CONNECTING) {
        stateTransition(_this, STATE_CONNECTED);
  ...
  _createClass(Server, [{
    ...
    value: function command(ns, cmd, options, callback) {
      ...
      this.s.pool.withConnection(function (err, conn, cb) {
        ...
        conn.command(ns, cmd, options, function (err, result) {
          if (err) {
    ...
    value: function query(ns, cmd, cursorState, options, callback) {
      ...
      this.s.pool.withConnection(function (err, conn, cb) {
        if (err) return cb(err);
        conn.query(ns, cmd, cursorState, options, function (err, result) {
          if (err) {
            if (options.session && err instanceof MongoNetworkError) {
              options.session.serverSession.isDirty = true;
            }
    ...
    value: function getMore(ns, cursorState, batchSize, options, callback) {
      ...
      this.s.pool.withConnection(function (err, conn, cb) {
        if (err) return cb(err);
        conn.getMore(ns, cursorState, batchSize, options, function (err, result) {
          if (err) {
            if (options.session && err instanceof MongoNetworkError) {
              options.session.serverSession.isDirty = true;
            }
    ...
    value: function killCursors(ns, cursorState, callback) {
      ...
      this.s.pool.withConnection(function (err, conn, cb) {
        if (err) return cb(err);
        conn.killCursors(ns, cursorState, function (err, result) {
...
function executeWriteOperation(args, options, callback) {
  ...
  server.s.pool.withConnection(function (err, conn, cb) {
    if (err) return cb(err);
    conn[op](ns, ops, options, function (err, result) {
      if (err) {
        if (options.session && err instanceof MongoNetworkError) {
          options.session.serverSession.isDirty = true;
        }
