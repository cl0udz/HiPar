...
var writableServerSelector = serverSelection.writableServerSelector; // const selectServers = serverSelection.selectServers;
// Global state
...
var LOCAL_SERVER_EVENTS = ['error', 'connect', 'descriptionReceived', 'close', 'ended'];
...
function (_EventEmitter) {
  ...
  function Topology(seedlist, options) {
    ...
    _this7 = _possibleConstructorReturn(this, _getPrototypeOf(Topology).call(this));
    ...
    _this7.s = {
      ...
      servers: new Map(),
      // Server Session Pool
      sessionPool: new ServerSessionPool(_assertThisInitialized(_this7)),
    ...
    if (options.srvHost) {
      _this7.s.srvPoller = options.srvPoller || new SrvPoller({
        heartbeatFrequencyMS: _this7.s.heartbeatFrequencyMS,
      ...
      _this7.s.detectTopologyDescriptionChange = function (ev) {
        ...
        if (previousType !== TopologyType.Sharded && newType === TopologyType.Sharded) {
          _this.s.handleSrvPolling = srvPollingHandler(_this);
      ...
      _this7.on('topologyDescriptionChanged', _this7.s.detectTopologyDescriptionChange);
    } // NOTE: remove this when NODE-1709 is resolved


    _this7.setMaxListeners(Infinity);
...
function processWaitQueue(topology) {
  ...
  for (var i = 0; i < topology[kWaitQueue].length; ++i) {
