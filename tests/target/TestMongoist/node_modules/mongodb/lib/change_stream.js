...
function (_EventEmitter) {
  ...
  function ChangeStream(parent, pipeline, options) {
    ...
    _this5 = _possibleConstructorReturn(this, _getPrototypeOf(ChangeStream).call(this));
    ...
    _this5.pipeline = pipeline || [];
    _this5.options = options || {};
    _this5.parent = parent;
    _this5.namespace = parent.s.namespace;

    if (parent instanceof Collection) {
      _this5.type = CHANGE_DOMAIN_TYPES.COLLECTION;
      _this5.topology = parent.s.db.serverConfig;
    } else if (parent instanceof Db) {
      _this5.type = CHANGE_DOMAIN_TYPES.DATABASE;
      _this5.topology = parent.serverConfig;
    } else if (parent instanceof MongoClient) {
      _this5.type = CHANGE_DOMAIN_TYPES.CLUSTER;
      _this5.topology = parent.topology;
    } else {
      throw new TypeError('parent provided to ChangeStream constructor is not an instance of Collection, Db, or MongoClient');
    }

    _this5.promiseLibrary = parent.s.promiseLibrary;

    if (!_this5.options.readPreference && parent.s.readPreference) {
      _this5.options.readPreference = parent.s.readPreference;
    } // Create contained Change Stream cursor


    _this5.cursor = createChangeStreamCursor(_assertThisInitialized(_this5), options); // Listen for any `change` listeners being added to ChangeStream

    _this5.on('newListener', function (eventName) {
    ...
    _this5.on('removeListener', function (eventName) {
...
function (_Cursor) {
  ...
  function ChangeStreamCursor(topology, operation, options) {
    ...
    _this6 = _possibleConstructorReturn(this, _getPrototypeOf(ChangeStreamCursor).call(this, topology, operation, options));
    options = options || {};
  ...
  _createClass(ChangeStreamCursor, [{
    ...
    value: function _initializeCursor(callback) {
      ...
      _get(_getPrototypeOf(ChangeStreamCursor.prototype), "_initializeCursor", this).call(this, function (err, result) {
        ...
        if (_this3.startAtOperationTime == null && _this3.resumeAfter == null && _this3.startAfter == null && maxWireVersion(_this3.server) >= 7) {
          _this3.startAtOperationTime = response.operationTime;
        }

        var cursor = response.cursor;

        if (cursor.postBatchResumeToken) {
          _this3.cursorState.postBatchResumeToken = cursor.postBatchResumeToken;

          if (cursor.firstBatch.length === 0) {
            _this3.resumeToken = cursor.postBatchResumeToken;
          }
        }
