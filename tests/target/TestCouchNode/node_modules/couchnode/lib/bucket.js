...
var Bucket = function Bucket(bucket) {
  this.bucket = bucket;
};

Bucket.prototype.maxReadsPerOp = 0;
Bucket.prototype.maxWritesPerOp = 0; // -----------------------------------------------------------------------------
...
function _handleMultiResults(config, callback, err, results) {
  ...
  return _parseResults(config, results, function __handleMapKeysToResults(err, values, cas, misses) {
    ...
    var cb = callback.bind(null, err); // if should return the values, curry the callback
...
function _parseResults(config, results, callback) {
  var keysArray = _array(config.keys),
      ...
      v,
      misses = [],
      err = null; // for each key


  for (i = 0; i < total; i++) {
    k = keysArray[i];
    v = results[i]; // if there was an error

    if (v.error) {
      // if there's a CAS failure check function use it
      if (config.casFailureCheck) {
        // if it's a CAS failure
        if (config.casFailureCheck(v.error)) {
          // set the flag
          v.error.casFailure = true;
        }
      } // if the error was keyNotFound, key was a miss


      if (v.error.code === cbErrors.keyNotFound
      /*|| v.error.message === 'key not found'*/
      ) {
          misses.push(k);
        } else {
      ...
      // no error, just extract the CAS and value
      if (config.extractCas) {
        cas[k] = v.result.cas;
      }
...
function _applyCasToOptions(opt, cas, key) {
  ...
  if (cas[key] && _typeof(cas[key]) === 'object' && Object.keys(cas[key]).length) {
    opt.cas = cas[key];
  } else {
    delete opt.cas;
  }
...
function _errorIsKeyAlreadyExists(error) {
  return error.code === cbErrors.keyAlreadyExists;
}

function _errorIsTemporaryError(error) {
  return error.code === cbErrors.temporaryError;
} // -----------------------------------------------------------------------------
...
Bucket.prototype.append = function (keys, fragment, options, callback) {
  ...
  this._asyncMapWrite(_array(keys), function __handleMap(key, done) {
    ...
    this.bucket.append(key, fragment, opt, function __handleAppend(err, res) {
      if (err && err.code === 18) {
        // when a key doesn't exist, error 18 / LIBCOUCHBASE_NOT_SUPPORTED is returned
    ...
    extractValues: false,
    extractCas: true,
    casFailureCheck: _errorIsKeyAlreadyExists
  }, callback));
...
Bucket.prototype.get = function (keys, callback) {
  ...
  this._asyncMapRead(_array(keys), function __handleMap(key, done) {
...
Bucket.prototype.getAndLock = function (keys, options, callback) {
  ...
  this._asyncMapWrite(_array(keys), function __handleMap(key, done) {
    ...
    extractValues: true,
    extractCas: true,
    casFailureCheck: _errorIsTemporaryError
  }, callback));
...
Bucket.prototype.getReplica = function (keys, options, callback) {
  ...
  this._asyncMapRead(_array(keys), function __handleMap(key, done) {
    this.bucket.getReplica(key, options.index && options.index[key] ? {
      index: options.index[key]
    } : {}, _handleResult.bind(null, done));
  }.bind(this), _handleMultiResults.bind(null, {
    keys: keys,
    extractValues: true,
    extractCas: true
...
Bucket.prototype.insert = function (tuples, options, callback) {
  ...
  this._asyncMapWrite(keys, function __handleMap(key, done) {
...
Bucket.prototype.replace = function (tuples, options, callback) {
  ...
  this._asyncMapWrite(keys, function __handleMap(key, done) {
    // if there is a cas set for this key, use it.
    _applyCasToOptions(opt, options.cas, key); // console.log('replace', tuples, keys, key, tuples[key], opt, done);
...
Bucket.prototype.unlock = function (keys, cas, callback) {
  ...
  this._asyncMapWrite(_array(keys), function __handleEach(key, done) {
    this.bucket.unlock(key, tokens.cas[key], function (err) {
      _handleResult(done, err, {
        value: !err || err.code === cbErrors.keyNotFound
      });
    });
  }.bind(this), _handleMultiResults.bind(null, {
    keys: keys,
...
Bucket.prototype._asyncMapRead = function (arr, iterator, callback) {
  if (this.maxReadsPerOp) {
    return async.mapLimit(arr, this.maxReadsPerOp, iterator, callback);
  }

  return async.map(arr, iterator, callback);
};

Bucket.prototype._asyncMapWrite = function (arr, iterator, callback) {
  if (this.maxWritesPerOp) {
    return async.mapLimit(arr, this.maxWritesPerOp, iterator, callback);
  }
...
module.exports.tuple = function (k, v) {
  var res = {}; // if it's a set of keys and respective values
