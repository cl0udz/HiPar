...
function bt_assert(root, comparator) {
  if (root === null) {
    ...
    var ln = root.left;
    var rn = root.right; // invalid binary search tree

    assert.equal(ln !== null && comparator(ln.data, root.data) >= 0 || rn !== null && comparator(rn.data, root.data) <= 0, false, "binary tree violation");
...
function rb_assert(root, comparator) {
  if (root === null) {
    ...
    var ln = root.left;
    var rn = root.right; // red violation
    ...
    var rh = rb_assert(rn, comparator); // invalid binary search tree

    assert.equal(ln !== null && comparator(ln.data, root.data) >= 0 || rn !== null && comparator(rn.data, root.data) <= 0, false, "binary tree violation"); // black height mismatch

    assert.equal(lh !== 0 && rh !== 0 && lh !== rh, false, "black violation"); // count black links
