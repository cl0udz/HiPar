...
function setup(app, options, cb) {
  var mongoUrl = process.env.MONGO_URL || process.env.MONGOHQ_URL || 'mongodb://' + (process.env.MONGO_HOST || 'localhost') + ':' + (process.env.MONGO_PORT || 27017) + '/' + (process.env.MONGO_DB || 'derby-' + (app.name || 'app'));
  ...
  var expressApp = express().use(favicon(publicDir + '/favicon.ico')) // Gzip dynamically rendered content
  .use(compression()).use(express["static"](publicDir));
  expressApp // Adds req.model
  .use(backend.modelMiddleware()).use(session({
  ...
  })).use(handlers.middleware).use(createUserId);
  ...
  expressApp // Creates an express middleware from the app's routes
  .use(app.router()).use(errorMiddleware);
  expressApp.all('*', function (req, res, next) {
    next('404: ' + req.url);
  });
  app.writeScripts(backend, publicDir, {
    extensions: ['.coffee']
  }, function (err) {
