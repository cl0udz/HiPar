    ...
    setupModel.whenNothingPending(function () {
      ...
      model1.subscribe(model1Query, function () {
        model1.bundle(function (err, bundleData) {
          ...
          var model2 = backend.createModel(); // Unbundle should load data into model.

          model2.unbundle(bundleData);
          expect(model2.get('dogs.fido.name')).to.eql('Fido'); // Unloaded data available until after `unloadDelay` ms has elapsed.

          model2.unloadDelay = 4;
          model2.unload();
          expect(model2.get('dogs.fido.name')).to.eql('Fido'); // Another unbundle should re-increment subscribe count, so the data
