...
RacerBackend.prototype.createModel = function (options, req) {
  if (this.modelOptions) {
    options = options ? util.mergeInto(options, this.modelOptions) : this.modelOptions;
  }
...
RacerBackend.prototype.modelMiddleware = function () {
  ...
  function modelMiddleware(req, res, next) {
    ...
    if (req.model) return next(); // Create a new model for this request

    req.model = backend.createModel({
      fetchOnly: true
    }, req); // DEPRECATED:
    ...
    function closeModel() {
      ...
      if (req.model) req.model.close(); // DEPRECATED:
