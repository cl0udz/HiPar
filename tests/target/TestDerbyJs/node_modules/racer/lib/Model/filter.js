...
Model.INITS.push(function (model) {
  ...
  function filterListener(segments, eventArgs) {
    ...
    for (var path in map) {
      ...
      if (util.mayImpact(filter.segments, segments) || filter.inputsSegments && util.mayImpactAny(filter.inputsSegments, segments)) {
        filter.update(pass);
      }
    }
  }
...
Model.prototype.filter = function () {
  ...
  return this.root._filters.add(parsed.path, parsed.fn, null, parsed.inputPaths, parsed.options);
};

Model.prototype.sort = function () {
  var args = Array.prototype.slice.call(arguments);
  var parsed = parseFilterArguments(this, args);
  return this.root._filters.add(parsed.path, null, parsed.fn || 'asc', parsed.inputPaths, parsed.options);
};
...
Filters.prototype.toJSON = function () {
  ...
  for (var from in this.fromMap) {
    var filter = this.fromMap[from]; // Don't try to bundle if functions were passed directly instead of by name
...
Filter.prototype._slice = function (results) {
  ...
  var begin = this.skip || 0; // A limit of zero is equivalent to setting no limit
...
Filter.prototype.callFilter = function (items, key, inputs) {
  var item = items[key];
  return inputs ? this.filterFn.apply(this.model, [item, key, items].concat(inputs)) : this.filterFn.call(this.model, item, key, items);
};
