...
Model.prototype._mutate = function (segments, fn, cb) {
  ...
  if (!collectionName || !id) {
    var message = fn.name + ' must be performed under a collection ' + 'and document id. Invalid path: ' + segments.join('.');
...
Model.prototype._set = function (segments, value, cb) {
  ...
  function set(doc, docSegments, fnCb) {
    var previous = doc.set(docSegments, value, fnCb); // On setting the entire doc, remote docs sometimes do a copy to add the
...
Model.prototype._create = function (segments, value, cb) {
  ...
  if (segments.length !== 2) {
    var message = 'create may only be used on a document path. ' + 'Invalid path: ' + segments.join('.');
    cb = this.wrapCallback(cb);
    return cb(new Error(message));
  }
...
Model.prototype._del = function (segments, cb) {
  ...
  function del(doc, docSegments, fnCb) {
    var previous = doc.del(docSegments, fnCb); // When deleting an entire document, also remove the reference to the
...
Model.prototype._move = function (segments, from, to, howMany, cb) {
  ...
  function move(doc, docSegments, fnCb) {
    // Cast to numbers
    from = +from;
    to = +to; // Convert negative indices into positive
...
Model.prototype._subtypeSubmit = function (segments, subtype, subtypeOp, cb) {
  ...
  function subtypeSubmit(doc, docSegments, fnCb) {
    ...
    })._pass; // Emit undefined for the previous value, since we don't really know
