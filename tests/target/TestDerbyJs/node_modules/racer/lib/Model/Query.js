...
Model.prototype.sanitizeQuery = function (expression) {
  if (expression && _typeof(expression) === 'object') {
...
Queries.prototype.add = function (query) {
  ...
  var collection = this.collections[query.collectionName] || (this.collections[query.collectionName] = {});
...
Queries.prototype.remove = function (query) {
  ...
  delete collection[query.hash]; // Check if the collection still has any keys
...
function Query(model, collectionName, expression, options) {
  ...
  this._pendingSubscribeCallbacks = []; // These are used to help cleanup appropriately when calling unsubscribe and
  ...
  this.fetchCount = 0;
  this.created = false;
  this.shareQuery = null; // idMap is checked in maybeUnload to see if the query is currently holding
...
Query.prototype.fetch = function (cb) {
  ...
  this.model.root.connection.createFetchQuery(this.collectionName, this.expression, this.options, fetchCb);
  return this;
};
...
Query.prototype.subscribe = function (cb) {
  ...
  options.results = this._getShareResults(); // When doing server-side rendering, we actually do a fetch the first time
...
Query.prototype._shareFetchedSubscribe = function (options, cb) {
  this.model.root.connection.createFetchQuery(this.collectionName, this.expression, options, this._subscribeCb(cb));
};

Query.prototype._shareSubscribe = function (options, cb) {
  var query = this; // Sanity check, though this shouldn't happen

  if (this.shareQuery) {
    this.shareQuery.destroy();
  }

  this.shareQuery = this.model.root.connection.createSubscribeQuery(this.collectionName, this.expression, options, this._subscribeCb(cb));
...
Query.prototype.unfetch = function (cb) {
  ...
  this.model._context.unfetchQuery(this); // No effect if the query is not currently fetched
  ...
  function finishUnfetchQuery() {
    ...
    if (count) return cb(null, count); // Cleanup when no fetches or subscribes remain
...
Query.prototype.unsubscribe = function (cb) {
  ...
  this.model._context.unsubscribeQuery(this); // No effect if the query is not currently subscribed
  ...
  function unsubscribeQueryCallback(err) {
    if (err) return cb(err); // Cleanup when no fetches or subscribes remain
...
Query.prototype.serialize = function () {
  ...
  var serialized = [counts, this.collectionName, this.expression, results, this.options, this.getExtra()];
