...
require("core-js/modules/web.dom-collections.iterator"); // @ts-check
...
var Model = require('./Model'); // These events are re-emitted as 'all' events, and they are queued up and
...
Model.INITS.push(function (model) {
  EventEmitter.call(this); // Set max listeners to unlimited
...
Model.prototype._emitError = function (err, context) {
  var message = err.message ? err.message : typeof err === 'string' ? err : 'Unknown model error';
...
Model.prototype.emit = function (type) {
  ...
  if (Model.MUTATOR_EVENTS[type]) {
    if (this._silent) return this; // `segments` is almost definitely an array of strings.
    //
...
Model.prototype.addListener = Model.prototype.on = function (type, pattern, options, cb) {
...
Model.prototype.removeAllListeners = function (type, subpattern) {
  ...
  var pattern = this.path(subpattern); // If no pattern is specified, remove all listeners like normal
  ...
  var segments = pattern.split('.'); // Make sure to iterate in reverse, since the array might be
...
Model.prototype.pass = function (object, invert) {
  ...
  model._pass = invert ? new Passed(object, this._pass) : new Passed(this._pass, object);
  return model;
};
...
Model.prototype.removeContextListeners = function (value) {
  ...
  for (var type in this._events) {
    var listeners = this.listeners(type); // Make sure to iterate in reverse, since the array might be
    // mutated as listeners are removed
...
function eventListener(model, eventType, arg2, arg3, arg4) {
  ...
  if (subpattern) {
    ...
    return useEventObjects ? modelEventListener(eventType, pattern, cb, model._eventContext) : modelEventListenerLegacy(pattern, cb, model._eventContext);
  ...
  if (path) {
    return useEventObjects ? modelEventListener(eventType, path, cb, model._eventContext) : modelEventListenerLegacy(path, cb, model._eventContext);
  } // For signature:
...
function testPatternFn(pattern, patternSegments) {
  ...
  return function testPattern(segments) {
    ...
    if (patternLen > segments.length) return; // A pattern with the same number of segments matches if each
