...
function diffDeep(model, segments, before, after, group) {
  if (_typeof(before) !== 'object' || !before || _typeof(after) !== 'object' || !after) {
  ...
  if (Array.isArray(before) && Array.isArray(after)) {
    ...
    if (!diff.length) return; // If the only change is a single item replacement, diff the item instead

    if (diff.length === 2 && diff[0].index === diff[1].index && diff[0] instanceof arrayDiff.RemoveDiff && diff[0].howMany === 1 && diff[1] instanceof arrayDiff.InsertDiff && diff[1].values.length === 1) {
