...
function RemoteDoc(model, collectionName, id, snapshot, collection) {
  ...
  this.model = model.pass({
    $remote: true
  });
  this.debugMutations = model.root.debug.remoteMutations; // Get or create the Share document. Note that we must have already added
...
RemoteDoc.prototype._initShareDoc = function () {
  ...
  var id = this.id;
  var shareDoc = this.shareDoc; // Override submitOp to disable all writes and perform a dry-run
  ...
  shareDoc.on('load', function () {
    ...
    var value = shareDoc.data; // If we subscribe to an uncreated document, no need to emit 'load' event
...
RemoteDoc.prototype._updateCollectionData = function () {
  ...
  if (_typeof(data) === 'object' && !Array.isArray(data) && data !== null) {
    data.id = this.id;
  }
...
RemoteDoc.prototype.create = function (value, cb) {
  ...
  this.shareDoc.create(data, cb); // The id value will get added to the data that was passed in
...
RemoteDoc.prototype.set = function (segments, value, cb) {
  ...
  var op = util.isArrayIndex(lastSegment) ? [new ListReplaceOp(segments.slice(0, -1), lastSegment, previous, value)] : [new ObjectReplaceOp(segments, previous, value)];
...
RemoteDoc.prototype.increment = function (segments, byNumber, cb) {
  ...
  if (previous == null) {
    var lastSegment = segments[segments.length - 1];
    var op = util.isArrayIndex(lastSegment) ? [new ListInsertOp(segments.slice(0, -1), lastSegment, byNumber)] : [new ObjectInsertOp(segments, byNumber)];
    this.shareDoc.submitOp(op, cb);
...
RemoteDoc.prototype.stringInsert = function (segments, index, value, cb) {
  ...
  if (previous == null) {
    var lastSegment = segments[segments.length - 1];
    var op = util.isArrayIndex(lastSegment) ? [new ListInsertOp(segments.slice(0, -1), lastSegment, value)] : [new ObjectInsertOp(segments, value)];
    this.shareDoc.submitOp(op, cb);
...
RemoteDoc.prototype._arrayApply = function (segments, fn, cb) {
  ...
  if (arr == null) {
    var lastSegment = segments[segments.length - 1];
    var op = util.isArrayIndex(lastSegment) ? [new ListInsertOp(segments.slice(0, -1), lastSegment, [])] : [new ObjectInsertOp(segments, [])];
...
RemoteDoc.prototype._onOp = function (op) {
  ...
  var model = this.model; // ObjectReplaceOp, ObjectInsertOp, or ObjectDeleteOp

  if (defined(item.oi) || defined(item.od)) {
    var value = item.oi;
    var previous = item.od;
    model.emit('change', segments, [value, previous, model._pass]); // ListReplaceOp
  } else if (defined(item.li) && defined(item.ld)) {
    var value = item.li;
    var previous = item.ld;
    model.emit('change', segments, [value, previous, model._pass]); // ListInsertOp
  } else if (defined(item.li)) {
    var index = segments[segments.length - 1];
    var values = [item.li];
    model.emit('insert', segments.slice(0, -1), [index, values, model._pass]); // ListRemoveOp
  } else if (defined(item.ld)) {
    var index = segments[segments.length - 1];
    var removed = [item.ld];
    model.emit('remove', segments.slice(0, -1), [index, removed, model._pass]); // ListMoveOp
  } else if (defined(item.lm)) {
    var from = segments[segments.length - 1];
    var to = item.lm;
    var howMany = 1;
    model.emit('move', segments.slice(0, -1), [from, to, howMany, model._pass]); // StringInsertOp
    ...
    model.emit('change', segments, [value, previous, pass]); // StringRemoveOp
    ...
    model.emit('change', segments, [value, previous, pass]); // IncrementOp
  } else if (defined(item.na)) {
    var value = this.get(item.p);
    var previous = value - item.na;
    model.emit('change', segments, [value, previous, model._pass]); // SubtypeOp
  } else if (defined(item.t)) {
    var value = this.get(item.p); // Since this is generic to all subtypes, we don't know how to get a copy
