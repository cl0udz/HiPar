...
function addIndexListeners(model) {
  ...
  function onIndexChange(segments, patch) {
    ...
    for (var from in fromMap) {
      ...
      if (!(ref.updateIndices && util.contains(segments, ref.toSegments) && ref.toSegments.length > segments.length)) continue;
...
function refChange(model, dereferenced, eventArgs, segments) {
  var value = eventArgs[0]; // Detect if we are deleting vs. setting to undefined
...
function addListener(model, type, fn) {
  ...
  function refListener(segments, eventArgs) {
    ...
    for (var i = 0, len = segments.length; i < len; i++) {
      subpath = subpath ? subpath + '.' + segments[i] : segments[i]; // If a ref is found pointing to a matching subpath, re-emit on the
      // place where the reference is coming from as if the mutation also
      // occured at that path

      var refs = toMap[subpath];
      if (!refs) continue; // Shallow clone refs in case a ref is removed while going through
      // the loop
      ...
      for (var refIndex = 0, numRefs = refs.length; refIndex < numRefs; refIndex++) {
        ...
        var dereferenced = ref.fromSegments.concat(remaining); // The value may already be up to date via object reference. If so,
...
Model.prototype.ref = function () {
  ...
  var toPath = this.path(to); // Make ref to reffable object, such as query or filter

  if (!toPath) return to.ref(fromPath);
  var ref = new Ref(this.root, fromPath, toPath, options);

  if (ref.fromSegments.length < 2) {
    throw new Error('ref must be performed under a collection ' + 'and document id. Invalid path: ' + fromPath);
  }
...
Model.prototype._dereference = function (segments, forArrayMutator, ignore) {
  ...
  } while (doAgain); // If a dereference fails, return a path that will result in a null value
