...
Fns.prototype.toJSON = function () {
  ...
  for (var from in this.fromMap) {
    var fn = this.fromMap[from]; // Don't try to bundle non-named functions that were started via
...
function Fn(model, name, from, inputPaths, fns, options) {
  ...
  this.copyOutput = copy === 'output' || copy === 'both'; // Mode can be 'diffDeep', 'diff', 'arrayDeep', or 'array'
