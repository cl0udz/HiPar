...
function addListener(model, type) {
  ...
  function refListListener(segments, eventArgs) {
    var pass = eventArgs[eventArgs.length - 1]; // Check for updates on or underneath paths
...
function patchFromEvent(type, segments, eventArgs, refList) {
  ...
  var model = refList.model.pass(pass, true); // Mutation on the `from` output itself
  ...
  if (segmentsLength === fromLength) {
    ...
    if (type === 'remove') {
      ...
      var ids = model._remove(refList.idsSegments, index, howMany); // Delete the appropriate items underneath `to` if the `deleteRemoved`
      // option was set true
  ...
  var toSegments = refList.toSegmentsByItem(value); // Mutation underneath a child of the `from` object.

  if (segmentsLength > fromLength + 1) {
    throw new Error('Mutation on descendant of refList `from`' + ' should have been dereferenced: ' + segments.join('.'));
  ...
  if (type === 'insert' || type === 'remove' || type === 'move') {
    throw new Error('Array mutation on child of refList `from`' + 'should have been dereferenced: ' + segments.join('.'));
  }
}
/**
 * @private
...
function setNewToValues(model, refList, values) {
  ...
  for (var i = 0; i < values.length; i++) {
    ...
    if (id === undefined && _typeof(value) === 'object') {
...
function patchToEvent(type, segments, eventArgs, refList) {
  ...
  var segmentsLength = segments.length;
  var pass = eventArgs[eventArgs.length - 1];
  var model = refList.model.pass(pass, true); // Mutation on the `to` object itself
...
function patchIdsEvent(type, segments, eventArgs, refList) {
  ...
  var segmentsLength = segments.length;
  var pass = eventArgs[eventArgs.length - 1];
  var model = refList.model.pass(pass, true); // An array mutation of the ids should be mirrored with a like change in
  // the output array
