...
var _require = require('./lib/middleware'),
...
function Yargs(processArgs, cwd, parentRequire) {
  ...
  self.pkgConf = function pkgConf(key, rootPath) {
    ...
    var obj = pkgUp(rootPath || cwd); // If an object exists in the key, add it to options.configObjects
  ...
  function pkgUp(rootPath) {
    ...
    try {
      var startDir = rootPath || require('require-main-filename')(parentRequire || require); // When called in an environment that lacks require.main.filename, such as a jest test runner,
  ...
  self.positional = function (key, opts) {
    ...
    opts = objFilter(opts, function (k, v) {
      var accept = supportedOpts.indexOf(k) !== -1; // type can be one of string|number|boolean.
  ...
  self._parseArgs = function parseArgs(args, shortCircuit, _calledFromCommand, commandIndex) {
    ...
    var parsed = Parser.detailed(args, Object.assign({}, options, {
      ...
      if (argv._.length) {
        if (handlerKeys.length) {
          ...
          if (command.hasDefaultCommand() && !skipDefaultCommand) {
            ...
            return populateDoubleDash ? _innerArgv : self._copyDoubleDash(_innerArgv);
        ...
        return populateDoubleDash ? _innerArgv2 : self._copyDoubleDash(_innerArgv2);
  ...
  function guessLocale() {
    ...
    try {
      var _process = process,
          env = _process.env;
      var locale = env.LC_ALL || env.LC_MESSAGES || env.LANG || env.LANGUAGE || 'en_US';
