...
var SavedContext = function SavedContext(state, lookAhead) {
  _classCallCheck(this, SavedContext);
...
function () {
  ...
  _createClass(Context, [{
    ...
    key: "save",
    value: function save(copy) {
      var state = copy !== false ? (0, _modes.copyState)(this.doc.mode, this.state) : this.state;
      return this.maxLookAhead > 0 ? new SavedContext(state, this.maxLookAhead) : state;
    }
  }], [{
    key: "fromSaved",
    value: function fromSaved(doc, saved, line) {
...
function getLineStyles(cm, line, updateFrontier) {
  if (!line.styles || line.styles[0] != cm.state.modeGen) {
    var context = getContextBefore(cm, (0, _utils_line.lineNo)(line));
    var resetState = line.text.length > cm.options.maxHighlightLength && (0, _modes.copyState)(cm.doc.mode, context.state);
...
function getContextBefore(cm, n, precise) {
  ...
  var saved = start > doc.first && (0, _utils_line.getLine)(doc, start - 1).stateAfter;
  var context = saved ? Context.fromSaved(doc, saved, start) : new Context(doc, (0, _modes.startState)(doc.mode), start);
...
function processLine(cm, text, context, startAt) {
  ...
  var stream = new _StringStream["default"](text, cm.options.tabSize, context);
...
function callBlankLine(mode, state) {
  ...
  var inner = (0, _modes.innerMode)(mode, state);
...
function takeToken(cm, pos, precise, asArray) {
  ...
  var line = (0, _utils_line.getLine)(doc, pos.line),
      context = getContextBefore(cm, pos.line, precise);
  var stream = new _StringStream["default"](line.text, cm.options.tabSize, context),
      tokens;
  if (asArray) tokens = [];
...
function findStartLine(cm, n, precise) {
  ...
  for (var search = n; search > lim; --search) {
    ...
    var line = (0, _utils_line.getLine)(doc, search - 1),
        after = line.stateAfter;
    if (after && (!precise || search + (after instanceof SavedContext ? after.lookAhead : 0) <= doc.modeFrontier)) return search;
    var indented = (0, _misc.countColumn)(line.text, null, cm.options.tabSize);
...
function retreatFrontier(doc, n) {
  ...
  for (var line = n - 1; line > start; line--) {
    var saved = (0, _utils_line.getLine)(doc, line).stateAfter; // change is on 3
