...
function () {
  function DisplayUpdate(cm, viewport, force) {
    _classCallCheck(this, DisplayUpdate);
  ...
  _createClass(DisplayUpdate, [{
    key: "signal",
    value: function signal(emitter, type) {
    ...
    value: function finish() {
      for (var i = 0; i < this.events.length; i++) {
        _event.signal.apply(null, this.events[i]);
      }
    }
  }]);
...
function restoreSelection(snapshot) {
  ...
  if (snapshot.anchorNode && (0, _dom.contains)(document.body, snapshot.anchorNode) && (0, _dom.contains)(document.body, snapshot.focusNode)) {
    var sel = window.getSelection(),
...
function updateDisplayIfNeeded(cm, update) {
  ...
  var toUpdate = (0, _view_tracking.countDirtyView)(cm);
...
function postUpdateDisplay(cm, update) {
  ...
  for (var first = true;; first = false) {
    ...
    var barMeasure = (0, _scrollbars.measureForScrollbars)(cm);
...
function patchDisplay(cm, updateNumbersFrom, dims) {
  ...
  for (var i = 0; i < view.length; i++) {
    ...
    if (lineView.hidden) {} else if (!lineView.node || lineView.node.parentNode != container) {
      // Not drawn yet
      var node = (0, _update_line.buildLineElement)(cm, lineView, lineN, dims);
