...
function () {
  function PastClick(time, pos, button) {
    _classCallCheck(this, PastClick);
  ...
  _createClass(PastClick, [{
    key: "compare",
    value: function compare(time, pos, button) {
...
function onMouseDown(e) {
  ...
  var pos = (0, _position_measurement.posFromMouse)(cm, e),
...
function handleMappedButton(cm, button, pos, repeat, event) {
  ...
  return (0, _key_events.dispatchKey)(cm, (0, _keymap.addModifierNames)(name, event), event, function (bound) {
...
function configureMouse(cm, repeat, event) {
  ...
  if (value.unit == null) {
    var rect = _browser.chromeOS ? event.shiftKey && event.metaKey : event.altKey;
...
function leftButtonStartDrag(cm, event, pos, behavior) {
  var display = cm.display,
      moved = false;
  var dragEnd = (0, _operations.operation)(cm, function (e) {
    ...
    if (!moved) {
      ...
      if (_browser.webkit || _browser.ie && _browser.ie_version == 9) setTimeout(function () {
...
function leftButtonSelect(cm, event, start, behavior) {
  ...
  function extendTo(pos) {
    ...
    if (behavior.unit == "rectangle") {
      ...
      var startCol = (0, _misc.countColumn)((0, _utils_line.getLine)(doc, start.line).text, start.ch, tabSize);
      var posCol = (0, _misc.countColumn)((0, _utils_line.getLine)(doc, pos.line).text, pos.ch, tabSize);
      var left = Math.min(startCol, posCol),
          right = Math.max(startCol, posCol);

      for (var line = Math.min(start.line, pos.line), end = Math.min(cm.lastLine(), Math.max(start.line, pos.line)); line <= end; line++) {
        var text = (0, _utils_line.getLine)(doc, line).text,
            leftPos = (0, _misc.findColumn)(text, left, tabSize);
        if (left == right) _ranges.push(new _selection.Range((0, _pos.Pos)(line, leftPos), (0, _pos.Pos)(line, leftPos)));else if (text.length > leftPos) _ranges.push(new _selection.Range((0, _pos.Pos)(line, leftPos), (0, _pos.Pos)(line, (0, _misc.findColumn)(text, right, tabSize))));
      }

      if (!_ranges.length) _ranges.push(new _selection.Range(start, start));
      (0, _selection_updates.setSelection)(doc, (0, _selection.normalizeSelection)(cm, startSel.ranges.slice(0, ourIndex).concat(_ranges), ourIndex), {
        origin: "*mouse",
        scroll: false
      });
      ...
      var _range = rangeForUnit(cm, pos, behavior.unit);

      var anchor = oldRange.anchor,
          head;

      if ((0, _pos.cmp)(_range.anchor, anchor) > 0) {
        head = _range.head;
        anchor = (0, _pos.minPos)(oldRange.from(), _range.anchor);
      } else {
        head = _range.anchor;
        anchor = (0, _pos.maxPos)(oldRange.to(), _range.head);
      }

      var _ranges2 = startSel.ranges.slice(0);

      _ranges2[ourIndex] = bidiSimplify(cm, new _selection.Range((0, _pos.clipPos)(doc, anchor), head));
      (0, _selection_updates.setSelection)(doc, (0, _selection.normalizeSelection)(cm, _ranges2, ourIndex), _misc.sel_mouse);
    }
  }
  ...
  function extend(e) {
    ...
    var cur = (0, _position_measurement.posFromMouse)(cm, e, true, behavior.unit == "rectangle");
    if (!cur) return;

    if ((0, _pos.cmp)(cur, lastPos) != 0) {
      cm.curOp.focus = (0, _dom.activeElt)();
      extendTo(cur);
      var visible = (0, _update_lines.visibleLines)(display, doc);
      if (cur.line >= visible.to || cur.line < visible.from) setTimeout((0, _operations.operation)(cm, function () {
        if (counter == curCount) extend(e);
      }), 150);
    } else {
      var outside = e.clientY < editorSize.top ? -20 : e.clientY > editorSize.bottom ? 20 : 0;
      if (outside) setTimeout((0, _operations.operation)(cm, function () {
  ...
  var move = (0, _operations.operation)(cm, function (e) {
    if (e.buttons === 0 || !(0, _event.e_button)(e)) done(e);else extend(e);
  });
  var up = (0, _operations.operation)(cm, done);
...
function bidiSimplify(cm, range) {
  ...
  var order = (0, _bidi.getOrder)(anchorLine);
  if (!order) return range;
  var index = (0, _bidi.getBidiPartAt)(order, anchor.ch, anchor.sticky),
      part = order[index];
  ...
  if (head.line != anchor.line) {
    ...
    var headIndex = (0, _bidi.getBidiPartAt)(order, head.ch, head.sticky);
...
function gutterEvent(cm, e, type, prevent) {
  ...
  for (var i = 0; i < cm.display.gutterSpecs.length; ++i) {
    ...
    if (g && g.getBoundingClientRect().right >= mX) {
      var line = (0, _utils_line.lineAtHeight)(cm.doc, mY);
