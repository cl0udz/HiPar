...
function extendRange(range, head, other, extend) {
  if (extend) {
    ...
    if (other) {
      var posBefore = (0, _pos.cmp)(head, anchor) < 0;
...
function extendSelections(doc, heads, options) {
  ...
  var newSel = (0, _selection.normalizeSelection)(doc.cm, out, doc.sel.primIndex);
  ...
  var obj = {
    ...
    update: function update(ranges) {
      ...
      for (var i = 0; i < ranges.length; i++) {
        this.ranges[i] = new _selection.Range((0, _pos.clipPos)(doc, ranges[i].anchor), (0, _pos.clipPos)(doc, ranges[i].head));
      }
    },
...
function setSelectionNoUndo(doc, sel, options) {
  ...
  var bias = options && options.bias || ((0, _pos.cmp)(sel.primary().head, doc.sel.primary().head) < 0 ? -1 : 1);
  setSelectionInner(doc, skipAtomicInSelection(doc, sel, bias, true));
  if (!(options && options.scroll === false) && doc.cm) (0, _scrolling.ensureCursorVisible)(doc.cm);
}
