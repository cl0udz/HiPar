...
function makeChange(doc, change, ignoreReadOnly) {
  ...
  var split = _saw_special_spans.sawReadOnlySpans && !ignoreReadOnly && (0, _spans.removeReadOnlyRanges)(doc, change.from, change.to);
...
function makeChangeInner(doc, change) {
  ...
  var selAfter = (0, _change_measurement.computeSelAfterChange)(doc, change);
  ...
  var rebased = [];
  (0, _document_data.linkedDocs)(doc, function (doc, sharedHist) {
...
function makeChangeFromHistory(doc, type, allowSelectionOnly) {
  ...
  var filter = (0, _event.hasHandler)(doc, "beforeChange") || doc.cm && (0, _event.hasHandler)(doc.cm, "beforeChange");

  var _loop = function _loop(_i) {
    var change = event.changes[_i];
    change.origin = type;
    ...
    var after = _i ? (0, _change_measurement.computeSelAfterChange)(doc, change) : (0, _misc.lst)(source);
    makeChangeSingleDoc(doc, change, after, (0, _history.mergeOldSpans)(doc, change));
    if (!_i && doc.cm) doc.cm.scrollIntoView({
      from: change.from,
  ...
  for (var _i = event.changes.length - 1; _i >= 0; --_i) {
    var _ret = _loop(_i);
...
function shiftDoc(doc, distance) {
  ...
  doc.first += distance;
  doc.sel = new _selection.Selection((0, _misc.map)(doc.sel.ranges, function (range) {
    return new _selection.Range((0, _pos.Pos)(range.anchor.line + distance, range.anchor.ch), (0, _pos.Pos)(range.head.line + distance, range.head.ch));
  }), doc.sel.primIndex);
  ...
  if (doc.cm) {
    ...
    for (var d = doc.cm.display, l = d.viewFrom; l < d.viewTo; l++) {
      (0, _view_tracking.regLineChange)(doc.cm, l, "gutter");
    }
  }
...
function makeChangeSingleDocInEditor(cm, change, spans) {
  ...
  if (!cm.options.lineWrapping) {
    doc.iter(checkWidthStart, from.line + change.text.length, function (line) {
      var len = (0, _spans.lineLength)(line);
  ...
  var changesHandler = (0, _event.hasHandler)(cm, "changes"),
...
function rebaseHistArray(array, from, to, diff) {
  for (var i = 0; i < array.length; ++i) {
    ...
    for (var _j = 0; _j < sub.changes.length; ++_j) {
      var cur = sub.changes[_j];
