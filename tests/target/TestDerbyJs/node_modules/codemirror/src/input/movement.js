...
function moveCharLogically(line, ch, dir) {
  var target = (0, _misc.skipExtendingChars)(line.text, ch + dir, dir);
...
function endOfLine(visually, cm, lineObj, lineNo, dir) {
  if (visually) {
    ...
    if (order) {
      var part = dir < 0 ? (0, _misc.lst)(order) : order[0];
      ...
      if (part.level > 0 || cm.doc.direction == "rtl") {
        var prep = (0, _position_measurement.prepareMeasureForLine)(cm, lineObj);
        ch = dir < 0 ? lineObj.text.length - 1 : 0;
        var targetTop = (0, _position_measurement.measureCharPrepared)(cm, prep, ch).top;
        ch = (0, _misc.findFirst)(function (ch) {
          return (0, _position_measurement.measureCharPrepared)(cm, prep, ch).top == targetTop;
...
function moveVisually(cm, line, start, dir) {
  var bidi = (0, _bidi.getOrder)(line, cm.doc.direction);
  ...
  var partPos = (0, _bidi.getBidiPartAt)(bidi, start.ch, start.sticky),
      part = bidi[partPos];
  ...
  var mv = function mv(pos, dir) {
    return moveCharLogically(line, pos instanceof _pos.Pos ? pos.ch : pos, dir);
  };
  ...
  var searchInVisualLine = function searchInVisualLine(partPos, dir, wrappedLineExtent) {
    var getRes = function getRes(ch, moveInStorageOrder) {
      return moveInStorageOrder ? new _pos.Pos(start.line, mv(ch, 1), "before") : new _pos.Pos(start.line, ch, "after");
    };

    for (; partPos >= 0 && partPos < bidi.length; partPos += dir) {
      var _part = bidi[partPos];

      var _moveInStorageOrder = dir > 0 == (_part.level != 1);

      var _ch = _moveInStorageOrder ? wrappedLineExtent.begin : mv(wrappedLineExtent.end, -1);

      if (_part.from <= _ch && _ch < _part.to) return getRes(_ch, _moveInStorageOrder);
      _ch = _moveInStorageOrder ? _part.from : mv(_part.to, -1);
      if (wrappedLineExtent.begin <= _ch && _ch < wrappedLineExtent.end) return getRes(_ch, _moveInStorageOrder);
    }
