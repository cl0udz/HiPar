...
require("core-js/modules/web.url.to-json"); // A basic type that uses a custom linked list object type as its snapshot
...
exports.Node = Node; // When type.createDeserialized is defined, it will be called on the client
...
function apply(data, op) {
  return data instanceof Node ? deserializedApply(data, op) : serializedApply(data, op);
...
function transform(op1, op2, side) {
  if (typeof op1.insert === 'number' && typeof op2.insert === 'number') {
