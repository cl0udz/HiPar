  ...
  describe('client submit', function () {
    ...
    it('ops submitted sync get composed', function (done) {
      ...
      doc.submitOp({
        ...
        doc.submitOp({
          ...
          doc.del(function (err) {
            if (err) return done(err);
            expect(doc.data).eql(undefined); // del DOES NOT get composed
    ...
    it('resumes composing after doc.preventCompose is set back to false', function (done) {
      ...
      doc.submitOp({
        ...
        expect(doc.version).eql(3); // Reset back to start composing ops again
    ...
    function delayedReconnect(backend, connection) {
      ...
      process.nextTick(function () {
        connection.close(); // Reconnect once the server has a chance to save the op data
    ...
    it('submits fail above the backend.maxSubmitRetries threshold', function (done) {
      ...
      doc.create({
        ...
        doc2.fetch(function (err) {
          if (err) return done(err);
          var docCallback;
          var doc2Callback; // The submit retry happens just after an op is committed. This hook into the middleware
          ...
          backend.use('commit', function (request, callback) {
            ...
            if (request.op.op[0].na === 7) doc2Callback = callback; // Wait until both ops have been applied to the same snapshot and are about to be committed
          ...
          doc.submitOp({
            ...
            if (err) return done(err); // When we know the first op has been committed, we try to commit the second op, which will
