  ...
  describe('client subscribe', function () {
    ...
    ['fetch', 'subscribe'].forEach(function (method) {
        ...
        it(method + ' single with readSnapshots rejectSnapshotRead ' + label, function (done) {
          ...
          connection.get('dogs', 'fido').create({
            ...
            fido[method](function (err) {
              verifyClientError(err); // An error for 'fido' means the data shouldn't get loaded.

              expect(fido.data).eql(undefined); // For subscribe, also test that further remote ops will not get sent for the doc.
              ...
              fido.on('before op', function (op) {
                done(new Error('fido on connection2 should not have received any ops, got:' + JSON.stringify(op)));
              }); // Issue an operation on connection1.

              connection.get('dogs', 'fido').submitOp([{
                p: ['age'],
                na: 1
              }], function (err) {
                if (err) return done(err); // Do a manual fetch on connection2, which should be enough time for it to receive
      ...
      testSingleSnapshotSpecificError('normal error', {
        rejectSnapshot: function rejectSnapshot(context, snapshot) {
          context.rejectSnapshotRead(snapshot, new Error('Failed to fetch fido'));
        },
        verifyClientError: function verifyClientError(err) {
      ...
      it(method + ' bulk on same collection', function (done) {
        ...
        async.parallel([function (cb) {
          ...
          connection.get('dogs', 'spot').create({
            age: 5
          }, cb);
        }, function (cb) {
          connection.get('cats', 'finn').create({
            age: 2
          }, cb);
        }], function (err) {
          if (err) return done(err);
          ...
          async.parallel([function (cb) {
            ...
            if (err) return done(err);
            expect(fido.data).eql({
              age: 3
            });
            expect(spot.data).eql({
              age: 5
            });
            expect(finn.data).eql({
              age: 2
            });
            done();
          });
      ...
      it(method + ' bulk with readSnapshots full error', function (done) {
        ...
        async.parallel([function (cb) {
          ...
          async.parallel([function (cb) {
            ...
            expect(err).to.be.an('error')["with"].property('message', 'Failed to fetch dogs'); // Error should mean data doesn't get loaded.
        ...
        it(method + ' bulk with readSnapshots rejectSnapshotRead ' + label, function (done) {
          ...
          async.parallel([function (cb) {
            ...
            async.parallel([function (cb) {
              ...
              if (err) return done(err); // An error for 'fido' means the data shouldn't get loaded.

              expect(fido.data).eql(undefined); // Data for 'spot' should still be loaded.
              ...
              connection.get('dogs', 'spot').submitOp([{
                p: ['age'],
                na: 1
          ...
          async.parallel([function (cb) {
            ...
            async.parallel([function (cb) {
              ...
              async.parallel([function (cb) {
                ...
                if (err) return done(err); // Create new ops and test if they are received
                ...
                async.parallel([function (cb) {
                  ...
                  connection.get('cats', 'finn').submitOp([{
                    p: ['age'],
                    na: 1
                  }], cb);
                }], function (err) {
      ...
      it(method + ' will not call back when creating the doc is pending', function (done) {
        ...
        doc[method](done); // HACK: Delay done call to keep from closing the db connection too soon
    ...
    it('doc fetches ops to catch up if it receives multiple future ops', function (done) {
      var backend = this.backend;
      var doc = this.backend.connect().get('dogs', 'fido').on('error', done);
      var doc2 = this.backend.connect().get('dogs', 'fido').on('error', done); // Delaying op replies will cause multiple future ops to be received
