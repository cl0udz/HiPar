...
describe('middleware', function () {
  ...
  describe('readSnapshots', function () {
    function expectFido(request) {
      ...
      expect(request.snapshots[0]).to.have.property('data').eql({
        age: 3
      });
    }
    ...
    beforeEach('Add fido to db', function (done) {
      ...
      this.backend.db.commit('dogs', 'fido', {
        v: 0,
        create: {}
      }, this.snapshot, null, done);
    });
  ...
  describe('reply', function () {
    ...
    it('context has request and reply objects', function (done) {
      ...
      this.backend.use('reply', function (replyContext, next) {
        ...
        expect(replyContext.request).to.eql({
          a: 'qf',
          id: 1,
          c: 'dogs',
          q: {
      ...
      var connection = this.backend.connect();
      connection.createFetchQuery('dogs', {
        age: 3
    ...
    it('can make raw additions to query reply extra', function (done) {
      ...
      connection.createFetchQuery('dogs', {
        age: 3
      }, null, function (err, results, extra) {
        if (err) {
          return done(err);
        }
        ...
        expect(results[0].data).to.eql(snapshot.data);
        expect(extra).to.eql({
          replyMiddlewareValue: 'some value'
        });
        done();
      });
    });
  ...
  describe('access control', function () {
    function setupOpMiddleware(backend) {
      ...
      backend.use('commit', function (request, next) {
        var accountId = request.snapshot.data ? // For created documents, get the accountId from the document data
        request.snapshot.data.accountId : // For deleted documents, get the accountId from before
        request.priorAccountId; // Store the accountId for the document on the op for efficient access control
      ...
      backend.use('op', function (request, next) {
        ...
        var err = {
          message: 'op accountId does not match',
          code: 'ERR_OP_READ_FORBIDDEN'
        };
        return next(err);
      });
    }
    ...
    it('is possible to cache add additional top-level fields on ops for access control', function (done) {
      ...
      connection2.agent.accountId = 'foo'; // Fetching the snapshot here will cause subsequent fetches to get ops
      ...
      connection2.get('dogs', 'fido').fetch(function (err) {
        ...
        connection1.get('dogs', 'fido').create(data, function (err) {
          if (err) return done(err); // This will go through the 'op' middleware and should pass
    ...
    it('op middleware can reject ops', function (done) {
      ...
      var connection1 = this.backend.connect();
      var connection2 = this.backend.connect();
      connection2.agent.accountId = 'baz'; // Fetching the snapshot here will cause subsequent fetches to get ops
      ...
      connection2.get('dogs', 'fido').fetch(function (err) {
        ...
        connection1.get('dogs', 'fido').create(data, function (err) {
          if (err) return done(err); // This will go through the 'op' middleware and fail;
    ...
    it('pubsub subscribe can check top-level fields for access control', function (done) {
      ...
      connection2.get('dogs', 'fido').subscribe(function (err) {
        ...
        connection1.get('dogs', 'fido').create(data, function (err) {
          if (err) return done(err); // The subscribed op will go through the 'op' middleware and should pass
