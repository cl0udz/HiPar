...
module.exports = function (options) {
  ...
  describe('db', function () {
    ...
    require('./client/subscribe')(); // Simplified mock of how submit request applies operations. The
    ...
    function submit(db, collection, id, op, callback) {
      db.getSnapshot(collection, id, {
        $submit: true
      }, null, function (err, snapshot) {
    ...
    describe('getSnapshot', function () {
      ...
      it('getSnapshot returns metadata when option is true', function (done) {
        ...
        commitSnapshotWithMetadata(db, function (err) {
          if (err) return done(err);
          db.getSnapshot('testcollection', 'test', null, {
            metadata: true
          }, function (err, result) {
            if (err) return done(err);
            expect(result.m).eql({
      ...
      it('getSnapshot returns metadata when fields is {$submit: true}', function (done) {
        ...
        commitSnapshotWithMetadata(db, function (err) {
          if (err) return done(err);
          db.getSnapshot('testcollection', 'test', {
            $submit: true
    ...
    describe('getSnapshotBulk', function () {
      ...
      it('getSnapshotBulk returns metadata when option is true', function (done) {
        ...
        commitSnapshotWithMetadata(db, function (err) {
          if (err) return done(err);
          db.getSnapshotBulk('testcollection', ['test2', 'test'], null, {
            metadata: true
          }, function (err, resultMap) {
            if (err) return done(err);
            expect(resultMap.test.m).eql({
              test: 3
            });
            done();
          });
    ...
    describe('getOps', function () {
      ...
      it('getOps returns metadata when option is true', function (done) {
        ...
        submit(db, 'testcollection', 'test', op, function (err) {
          if (err) return done(err);
          db.getOps('testcollection', 'test', null, null, {
            metadata: true
          }, function (err, ops) {
            if (err) return done(err);
            expect(ops[0].m).eql({
    ...
    describe('getOpsBulk', function () {
      it('getOpsBulk returns empty map when there are no ops', function (done) {
        this.db.getOpsBulk('testcollection', {
          test: 0,
          test2: 0
        }, null, null, function (err, opsMap) {
          if (err) return done(err);
          expect(opsMap).eql({
            test: [],
            test2: []
      ...
      it('getOpsToSnapshot returns metadata when option is true', function (done) {
        ...
        submit(db, 'testcollection', 'test', op, function (err) {
          ...
          db.getSnapshot('testcollection', 'test', {
            ...
            if (err) return done(err);
            db.getOpsToSnapshot('testcollection', 'test', 0, snapshot, {
              metadata: true
            }, function (err, ops) {
              if (err) return done(err);
              expect(ops[0].m).eql({
      ...
      it('getQuery argument order', function (done) {
        ...
        async.each(snapshots, function (snapshot, cb) {
          ...
          db.query('testcollection', dbQuery, null, null, function (err, results) {
            if (err) throw err;
            expect(results).eql([snapshots[2], snapshots[0], snapshots[3], snapshots[1]]);
            done();
          });
        });
      });
