...
module.exports = BasicQueryableMemoryDB; // Extension of MemoryDB that supports query filters and sorts on simple
...
function querySnapshot(snapshot, query) {
  ...
  if (snapshot.type == null) return false; // Match any snapshot when there is no query filter

  if (!query.filter) return true; // Check that each property in the filter equals the snapshot data
