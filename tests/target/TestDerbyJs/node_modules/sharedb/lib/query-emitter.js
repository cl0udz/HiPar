...
function QueryEmitter(request, stream, ids, extra) {
  ...
  this.pollDebounce = typeof this.options.pollDebounce === 'number' ? this.options.pollDebounce : typeof this.db.pollDebounce === 'number' ? this.db.pollDebounce : 0;
  this.pollInterval = typeof this.options.pollInterval === 'number' ? this.options.pollInterval : typeof this.db.pollInterval === 'number' ? this.db.pollInterval : 0;
  this._polling = false;
...
module.exports = QueryEmitter; // Start processing events from the stream
...
QueryEmitter.prototype._update = function (op) {
  ...
  var id = op.d;
  var pollCallback = this._defaultCallback; // Check if the op's id matches the query before updating the query results
  ...
  if (this.ids.indexOf(id) !== -1) {
    ...
    pollCallback = function pollCallback(err) {
  ...
  try {
    if (this.db.skipPoll(this.collection, id, op, this.query) || this.skipPoll(this.collection, id, op, this.query)) {
      return pollCallback();
    }
  } catch (err) {
...
QueryEmitter.prototype._flushPoll = function () {
  ...
  if (this._polling || this._pollDebounceId) return; // If another polling event happened while we were polling, call poll again,
  // as the results may have changed

  if (this._pendingPoll) {
    this.queryPoll(); // If a pollInterval is specified, poll if the query doesn't get polled in
...
QueryEmitter.prototype.queryPoll = function (callback) {
  var emitter = this; // Only run a single polling check against mongo at a time per emitter. This
  ...
  this.db.queryPoll(this.collection, this.query, this.options, function (err, ids, extra) {
    ...
    emitter._emitTiming('queryEmitter.poll', start); // Be nice to not have to do this in such a brute force way
...
QueryEmitter.prototype.queryPollDoc = function (id, callback) {
  ...
  this.db.queryPollDoc(this.collection, id, this.query, this.options, function (err, matches) {
    ...
    emitter._emitTiming('queryEmitter.pollDoc', start); // Check if the document was in the previous results set
    ...
    if (i === -1 && matches) {
      ...
      // it at the end
      var index = emitter.ids.push(id) - 1; // We can get the result to send to the client async, since there is a
...
QueryEmitter.prototype.onError = QueryEmitter.prototype.onDiff = QueryEmitter.prototype.onExtra = QueryEmitter.prototype.onOp = function () {
  throw new ShareDBError(ERROR_CODE.ERR_QUERY_EMITTER_LISTENER_NOT_ASSIGNED, 'Required QueryEmitter listener not assigned');
};
