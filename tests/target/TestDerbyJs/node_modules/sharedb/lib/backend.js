...
function Backend(options) {
  ...
  this.pubsub = options.pubsub || new MemoryPubSub(); // This contains any extra databases that can be queried

  this.extraDbs = options.extraDbs || {};
  this.milestoneDb = options.milestoneDb || new NoOpMilestoneDB(); // Map from projected collection -> {type, fields}

  this.projections = {};
  this.suppressPublish = !!options.suppressPublish;
  this.maxSubmitRetries = options.maxSubmitRetries || null; // Map from event name to a list of middleware

  this.middleware = {}; // The number of open agents for monitoring and testing memory leaks
...
Backend.prototype.connect = function (connection, req) {
  ...
  var agent = this.listen(socket.stream, req); // Store a reference to the agent on the connection for convenience. This is
...
Backend.prototype.listen = function (stream, req) {
  ...
  this.trigger(this.MIDDLEWARE_ACTIONS.connect, agent, {
    stream: stream,
    req: req
  }, function (err) {
...
Backend.prototype.trigger = function (action, agent, request, callback) {
  ...
  if (!fns) return callback(); // Copying the triggers we'll fire so they don't get edited while we iterate.
...
Backend.prototype._sanitizeOp = function (agent, projection, collection, id, op, callback) {
  ...
  this.trigger(this.MIDDLEWARE_ACTIONS.op, agent, {
...
Backend.prototype._sanitizeSnapshots = function (agent, projection, collection, snapshots, snapshotType, callback) {
  ...
  this.trigger(this.MIDDLEWARE_ACTIONS.readSnapshots, agent, request, function (err) {
    if (err) return callback(err); // Handle "partial rejection" - "readSnapshots" middleware functions can use
...
Backend.prototype.subscribe = function (agent, index, id, version, options, callback) {
  ...
  if (options) {
    ...
    return callback(new ShareDBError(ERROR_CODE.ERR_DATABASE_METHOD_NOT_IMPLEMENTED, 'Passing options to subscribe has not been implemented'));
  }
...
Backend.prototype.querySubscribe = function (agent, index, query, options, callback) {
  ...
  backend._triggerQuery(agent, index, query, options, function (err, request) {
    ...
    if (request.db.disableSubscribe) {
      return callback(new ShareDBError(ERROR_CODE.ERR_DATABASE_DOES_NOT_SUPPORT_SUBSCRIBE, 'DB does not support subscribe'));
    }
...
Backend.prototype._triggerQuery = function (agent, index, query, options, callback) {
  ...
  backend.trigger(backend.MIDDLEWARE_ACTIONS.query, agent, request, function (err) {
    if (err) return callback(err); // Set the DB reference for the request after the middleware trigger so
...
Backend.prototype.getChannels = function (collection, id) {
  return [this.getCollectionChannel(collection), this.getDocChannel(collection, id)];
};
...
Backend.prototype._fetchSnapshot = function (collection, id, version, callback) {
  ...
  this.milestoneDb.getMilestoneSnapshot(collection, id, version, function (error, milestoneSnapshot) {
    if (error) return callback(error); // Bypass backend.getOps so that we don't call _sanitizeOps. We want to avoid this, because:
    ...
    db.getOps(collection, id, from, version, null, function (error, ops) {
      ...
      backend._buildSnapshotFromOps(id, milestoneSnapshot, ops, function (error, snapshot) {
        ...
        if (version > snapshot.v) {
          return callback(new ShareDBError(ERROR_CODE.ERR_OP_VERSION_NEWER_THAN_CURRENT_SNAPSHOT, 'Requested version exceeds latest snapshot version'));
        }
