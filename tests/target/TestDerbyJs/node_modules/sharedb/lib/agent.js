...
function Agent(backend, stream) {
  ...
  this.stream = stream;
  this.clientId = hat();
  this.connectTime = Date.now(); // We need to track which documents are subscribed by the client. This is a
  // map of collection -> id -> stream

  this.subscribedDocs = {}; // Map from queryId -> emitter

  this.subscribedQueries = {}; // We need to track this manually to make sure we don't reply to messages
  // after the stream was closed.

  this.closed = false; // For custom use in middleware. The agent is a convenient place to cache
  // session state in memory. It is in memory only as long as the session is
  // active, and it is passed to each middleware call

  this.custom = {}; // Initialize the remote client by sending it its agent Id.
...
module.exports = Agent; // Close the agent with the client.
...
Agent.prototype.close = function (err) {
  ...
  if (this.closed) return; // This will end the writable stream and emit 'finish'
...
Agent.prototype._cleanup = function () {
  ...
  if (!this.stream.isServer) this.backend.remoteAgentsCount--; // Clean up doc subscription streams
  ...
  this.subscribedDocs = {}; // Clean up query subscription streams
...
Agent.prototype._subscribeToStream = function (collection, id, stream) {
  ...
  var streams = this.subscribedDocs[collection] || (this.subscribedDocs[collection] = {}); // If already subscribed to this document, destroy the previously subscribed stream
...
Agent.prototype._onOp = function (collection, id, op) {
  if (this._isOwnOp(collection, op)) return; // Ops emitted here are coming directly from pubsub, which emits the same op
...
Agent.prototype._open = function () {
  ...
  this.stream.on('data', function (chunk) {
    ...
    agent.backend.trigger('receive', agent, request, function (err) {
      var callback = function callback(err, message) {
...
Agent.prototype._checkRequest = function (request) {
  if (request.a === 'qf' || request.a === 'qs' || request.a === 'qu') {
    ...
    // Bulk request
    if (request.c != null && typeof request.c !== 'string') return 'Invalid collection';
    if (_typeof(request.b) !== 'object') return 'Invalid bulk subscribe data';
  }
...
Agent.prototype._querySubscribe = function (queryId, collection, query, options, callback) {
  ...
  this.backend.querySubscribe(this, collection, query, options, function (err, emitter, results, extra) {
    ...
    agent._subscribeToQuery(emitter, queryId, collection, query); // No results are returned when ids are passed in as an option. Instead,
...
function getMapResult(snapshotMap) {
  ...
  for (var id in snapshotMap) {
    var mapValue = snapshotMap[id]; // fetchBulk / subscribeBulk map data can have either a Snapshot or an object
    // `{error: Error | string}` as a value.

    if (mapValue.error) {
      // Transform errors to serialization-friendly objects.
      data[id] = {
        error: getReplyErrorObject(mapValue.error)
      };
    } else {
...
Agent.prototype._subscribe = function (collection, id, version, callback) {
  ...
  this.backend.subscribe(this, collection, id, version, function (err, stream, snapshot, ops) {
    if (err) return callback(err); // If we're subscribing from a known version, send any ops committed since
    ...
    agent._subscribeToStream(collection, id, stream); // Snapshot is returned only when subscribing from a null version.
...
function createClientOp(request, clientId) {
  ...
  var src = request.src || clientId; // c, d, and m arguments are intentionally undefined. These are set later

  return request.op ? new EditOp(src, request.seq, request.v, request.op) : request.create ? new CreateOp(src, request.seq, request.v, request.create) : request.del ? new DeleteOp(src, request.seq, request.v, request.del) : undefined;
}
