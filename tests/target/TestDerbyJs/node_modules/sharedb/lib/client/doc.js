...
function Doc(connection, collection, id) {
  ...
  this.pendingFetch = []; // Whether we think we are subscribed on the server. Synchronously set to
...
Doc.prototype._handleSubscribe = function (err, snapshot) {
  ...
  if (err) return this._emitResponseError(err, callback); // Indicate we are subscribed only if the client still wants to be. In the
...
Doc.prototype.unsubscribe = function (callback) {
  this.wantSubscribe = false; // The subscribed state should be conservative in indicating when we are
...
Doc.prototype._sendOp = function () {
  ...
  if (!src) return; // When there is no inflightOp, send the first item in pendingOps. If
...
Doc.prototype._tryCompose = function (op) {
  ...
  if (!last || last.sentAt) return; // Compose an op into a create by applying it. This effectively makes the op
