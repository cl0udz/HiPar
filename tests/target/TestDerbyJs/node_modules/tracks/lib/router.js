...
function mapRoute(from, params) {
  ...
  var queryString = ~i ? params.url.slice(i) : ''; // If the route looks like /:a/:b?/:c/:d?
...
RenderReq.prototype.cancel = function () {
  var options = this.options; // Don't do anything if this is the result of an event, since the
...
RenderReq.prototype.onMatch = function (route, params, next, done) {
  if (!this.page) return next(); // Stop the default browser action, such as clicking a link or submitting a form
