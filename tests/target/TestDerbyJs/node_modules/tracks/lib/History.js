...
var currentPath = window.location.pathname + window.location.search; // Replace the initial state with the current URL immediately,
...
History.prototype.refresh = function () {
  var path = routePath(window.location.href); // Note that we don't pass previous to avoid triggering transitions
...
History.prototype._update = function (historyMethod, relativeUrl, render, state, e) {
  ...
  var path = routePath(url); // TODO: history.push should set the window.location with external urls

  if (!path) return;
  if (render == null) render = true;
  if (state == null) state = {}; // Update the URL
...
History.prototype.page = function () {
  ...
  function redirect(url) {
    if (url === 'back') return history.back(); // TODO: Add support for `basepath` option like Express
...
function routePath(url) {
  ...
  return match && match.protocol === window.location.protocol && match.host === window.location.host && match.pathname + (match.search || '');
}
...
function addListeners(history) {
  ...
  function onClick(e) {
    var el = e.target; // Ignore command click, control click, and non-left click
  ...
  function onSubmit(e) {
    var target = e.target; // Ignore if already prevented
    ...
    if (target.enctype === 'multipart/form-data') return; // Ignore if created by Tracks
  ...
  function onPopState(e) {
    ...
    if (state) {
      ...
      options.method = state.$method; // Note that the post body is only sent on the initial reqest
