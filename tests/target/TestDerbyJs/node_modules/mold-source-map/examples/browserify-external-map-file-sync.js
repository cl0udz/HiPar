...
var path = require('path'),
    ...
    mold = require('..'),
    bundlePath = path.join(__dirname, 'project', 'js', 'build', 'bundle.js') // putting map file right next to bundle file
,
...
function mapFileUrlCommentSync(sourcemap) {
  ...
  sourcemap.mapSources(mold.mapPathRelativeTo(jsRoot)); // write map file and return a sourceMappingUrl that points to it

  fs.writeFileSync(mapFilePath, sourcemap.toJSON(2), 'utf-8'); // Giving just a filename instead of a path will cause the browser to look for the map file 
...
browserify().require(require.resolve('./project/js/main.js'), {
  entry: true
}).bundle({
  debug: true
}).on('error', function (err) {
  console.error(err);
}).pipe(mold.transform(mapFileUrlCommentSync)).pipe(fs.createWriteStream(bundlePath));
