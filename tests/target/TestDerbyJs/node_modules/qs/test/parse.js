...
test('parse()', function (t) {
  ...
  t.test('cannot access Object prototype', function (st) {
    ...
    st.equal(_typeof(Object.prototype.bad), 'undefined');
    st.end();
  });
  ...
  t.test('use number decoder, parses string that has one number with comma option enabled', function (st) {
    var decoder = function decoder(str, defaultDecoder, charset, type) {
      if (!isNaN(Number(str))) {
  ...
  t.test('receives the default decoder as a second argument', function (st) {
    st.plan(1);
    qs.parse('a', {
      decoder: function decoder(str, defaultDecoder) {
  ...
  t.test('handles a custom decoder returning `null`, in the `iso-8859-1` charset, when `interpretNumericEntities`', function (st) {
    st.deepEqual(qs.parse('foo=&bar=' + urlEncodedNumSmiley, {
      charset: 'iso-8859-1',
      decoder: function decoder(str, defaultDecoder, charset) {
