...
test('manual exposed/external modules', function (t) {
  ...
  var b1 = browserify({
    entries: path.join(__dirname, 'expose/index.js'),
    standalone: 'expose'
  }).plugin(require.resolve('../plugin')).external(externalModule);

  var b2 = browserify().plugin(require.resolve('../plugin')).require(externalModule, {
    expose: externalModule
  });
...
test('manual exposed/external modules with cycles', function (t) {
  ...
  var b1 = browserify({
    entries: path.join(__dirname, 'lazy-expose/app.js'),
    standalone: 'expose'
  }).plugin(require.resolve('../plugin')).external(externalModules).on('error', t.fail);

  var b2 = browserify().plugin(require.resolve('../plugin')).require(externalModules.map(function (m) {
    return {
      file: m,
      expose: m
    };
