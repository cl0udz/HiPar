...
var exposedRequireCode = fs.readFileSync(require.resolve('./_exposedRequire'), 'utf8'); // Parse the module and collect require() calls and exports assignments.

function parseModule(row, index, rows, opts) {
  // Holds the `module.exports` variable name.
  var moduleExportsName = toIdentifier('_$' + getModuleName(row.file || '') + '_' + row.id); // browserify is clever about deduping modules with the same source code,
  ...
  var ast; // hack to keep track of `module`/`exports` references correctly.
  ...
  var source = row.source; // Determine if a require() call may not be evaluated according to its linear source position
  ...
  function isOrderUnpredictable(node) {
    while (node = node.parent) {
      ...
      if (node.type === 'FunctionExpression' || node.type === 'ArrowFunctionExpression') {
        ...
        if (node.parent && node.parent.type === 'MemberExpression' && node.parent.object === node && (node.parent.property.name === 'call' || node.parent.property.name === 'apply') && node.parent.parent && node.parent.parent.type === 'CallExpression') {
          continue;
        }
  ...
  var alreadyPredictablyEvaluated = new Set(); // we'll do two walks along the AST in order to detect variables and their references.
  ...
  var magicString = transformAst(source, {
    ...
    scan.visitScope(node); // also collect requires while we're here

    if (isRequire(node)) {
      var moduleExists = function moduleExists(id) {
  ...
  var moduleExportsList = scan.scope(globalScope).getReferences('module').map(function (node) {
    return node.parent;
  }).filter(isModuleExports);
  var exportsList = scan.scope(globalScope).getReferences('exports');
  var moduleList = scan.scope(globalScope).getReferences('module').filter(function (node) {
  ...
  if (moduleExportsList.length === 1 && exportsList.length === 0 && moduleList.length === 0) {
    ...
    if (node.parent.type === 'AssignmentExpression' && node.parent.left === node && node.parent.parent.type === 'ExpressionStatement') {
...
function sortModules(rows) {
  ...
  function visit(mod) {
    ...
    if (hasDeps(mod)) {
      values(mod.deps).map(function attachSortOrder(id) {
      ...
      }).filter(Boolean).sort(compareDependencySortOrder).forEach(function (dep) {
        visit(dep.module);
      });
    }
...
function compareModuleSortOrder(a, b) {
  ...
  if (!a.entry && b.entry) return 1; // Sort entry modules by their `.order`.
  ...
  if (ao && !bo) return -1;
  if (!ao && bo) return 1; // Else sort by ID, so that output is stable.
...
function rewriteModule(row, i, rows) {
  ...
  var requireList = row[kReferences].require; // If `module` is used as a free variable we need to turn it into an object with an `.exports`
  ...
  if (!row[kEvaluateOnDemand]) {
    // on-demand modules have a function wrapper and don't need to be rewritten
  ...
  if (row[kEvaluateOnDemand]) {
    ...
    magicString.prepend('var ' + moduleBaseName + ' = { exports: {} };\n').append('\n' + moduleBaseName + ' = ' + moduleExportsName);
...
function flatten(rows, opts, stream) {
  ...
  moveOnDemandModulesToStart(rows); // Initialize entry modules that are marked as on-demand (because of a dependency cycle
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    var isEntryModule = rows[i].entry && rows[i].hasExports && opts.standalone; // Need this for:
  ...
  if (opts.standalone) {
    ...
    intro += dedent(_templateObject(), rows.exposeName, exposedRequireCode, rows.exposeName, rows.exposeName);
    outro += '\n' + dedent(_templateObject2(), rows.exposeName);
  ...
  if (rows.some(function (mod) {
    return mod[kEvaluateOnDemand];
  })) {
  ...
  var result = '';
  var line = 0;
  var preludePath = path.relative(opts.basedir || process.cwd(), path.join(__dirname, '_prelude'));
  var postludePath = path.relative(opts.basedir || process.cwd(), path.join(__dirname, '_postlude'));
  result += intro;
...
module.exports = function browserPackFlat(opts) {
  ...
  if (typeof opts === 'string' && _typeof(arguments[1]) === 'object') {
...
function getNodeName(node) {
  if (node.type === 'FunctionExpression') node = node.id;else if (node.type === 'ClassExpression') node = node.id;
