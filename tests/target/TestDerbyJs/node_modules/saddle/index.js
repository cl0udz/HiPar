...
mergeInto(STRING_PROPERTIES, UPDATE_PROPERTIES); // CREATE_PROPERTIES map HTML attribute names to an Element DOM property that
...
CREATE_PROPERTIES.value = 'defaultValue'; // http://www.w3.org/html/wg/drafts/html/master/syntax.html#void-elements
...
exports.NAMESPACE_URIS = NAMESPACE_URIS; // Template Classes
...
exports.DynamicAttribute = DynamicAttribute; // Binding Classes
...
Doctype.prototype.get = function () {
  var publicText = this.publicId ? ' PUBLIC "' + this.publicId + '"' : '';
  var systemText = this.systemId ? this.publicId ? ' "' + this.systemId + '"' : ' SYSTEM "' + this.systemId + '"' : '';
  return '<!DOCTYPE ' + this.name + publicText + systemText + '>';
};

Doctype.prototype.appendTo = function () {// Doctype could be created via:
...
function attachText(parent, node, data, template, context) {
  ...
  if (node.nodeType === 3) {
    ...
    data = normalizeLineBreaks(data); // Split adjacent text nodes that would have been merged together in HTML
...
DynamicHtml.prototype.update = function (context, binding) {
  ...
  if (!parent) return; // Get start and end in advance, since binding is mutated in getFragment
...
DynamicAttribute.prototype.update = function (context, binding) {
  ...
  if (propertyName) {
    var propertyValue = STRING_PROPERTIES[binding.name] ? this.stringify(value) : value;
...
Element.prototype.appendTo = function (parent, context) {
  var tagName = this.getTagName(context);
  var element = this.ns ? document.createElementNS(this.ns, tagName) : document.createElement(tagName);
...
Element.prototype.attachTo = function (parent, node, context) {
  ...
  if (!node || node.nodeType !== 1 || node.tagName.toLowerCase() !== tagName.toLowerCase()) {
    throw attachError(parent, node);
  }
...
Element.prototype._bindContent = function (context, element) {
  ...
  var child = this.bindContentToValue && this.content.length === 1 && this.content[0];
...
Block.prototype.update = function (context, binding) {
  ...
  var condition = this.getCondition(context); // Cancel update if prior condition is equivalent to current value

  if (equalConditions(condition, binding.condition)) return;
  binding.condition = condition; // Get start and end in advance, since binding is mutated in getFragment
...
Block.prototype.getCondition = function (context) {
  ...
  var value = this.expression.get(context);
  return _typeof(value) === 'object' ? NaN : value;
};
...
EachBlock.prototype.insert = function (context, binding, index, howMany) {
  var parent = binding.start.parentNode;
  if (!parent) return; // In case we are inserting all of the items, update instead. This is needed
...
EachBlock.prototype.remove = function (context, binding, index, howMany) {
  var parent = binding.start.parentNode;
  if (!parent) return; // In case we are removing all of the items, update instead. This is needed
...
function attachError(parent, node) {
  ...
  return new Error('Attaching bindings failed, because HTML structure ' + 'does not match client rendering.');
}
...
function AttributeBinding(template, context, element, name) {
  ...
  this.name = name;
  this.meta = null;
  var map = element.$bindAttributes || (element.$bindAttributes = new AttributeBindingsMap());
  map[name] = this;
}
...
(function () {
  ...
  if (windowsLength === 1 && unixLength === 1) {
    normalizeLineBreaks = function normalizeLineBreaks(string) {
  ...
  try {
    ...
    setNodeProperty = function setNodeProperty(node, key, value) {
