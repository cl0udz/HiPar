...
describe('mongo db', function () {
  ...
  describe('indexes', function () {
    it('adds ops index', function (done) {
      ...
      this.db.commit('testcollection', 'foo', {
        v: 0,
        create: {}
      }, {}, null, function (err) {
        if (err) return done(err);
        mongo.collection('o_testcollection').indexInformation(function (err, indexes) {
          if (err) return done(err); // Index for getting document(s) ops

          expect(indexes['d_1_v_1']).ok; // Index for checking committed op(s) by src and seq
    ...
    it('respects unique indexes', function (done) {
      var db = this.db;
      this.mongo.collection('testcollection').createIndex({
        x: 1
      }, {
        unique: true
      }, function (err) {
        if (err) return done(err);
        db.commit('testcollection', 'foo', {
          v: 0,
          create: {}
        }, {
          v: 1,
  ...
  describe('query', function () {
    ...
    it('queryPollDoc does not allow $where queries', function (done) {
      this.db.queryPollDoc('testcollection', 'somedoc', {
        $where: 'true'
      }, null, function (err) {
        expect(err).ok;
        done();
    ...
    describe('queryPollDoc correctly filters on _id', function () {
      ...
      beforeEach(function (done) {
        this.db.commit('testcollection', snapshot.id, {
          v: 0,
          create: {}
        }, snapshot, null, done);
      });
      ...
      function test(query, expectedHasDoc, done) {
        this.db.queryPollDoc('testcollection', snapshot.id, query, null, function (err, hasDoc) {
          if (err) done(err);
    ...
    it('$distinct should perform distinct operation', function (done) {
      ...
      var db = this.db;
      db.commit('testcollection', 'test1', {
        v: 0,
        create: {}
      }, snapshots[0], null, function (err) {
        if (err) return done(err);
        db.commit('testcollection', 'test2', {
          v: 0,
          create: {}
        }, snapshots[1], null, function (err) {
          if (err) return done(err);
          db.commit('testcollection', 'test3', {
            v: 0,
            create: {}
          }, snapshots[2], null, function (err) {
            if (err) return done(err);
            var query = {
              $distinct: {
                field: 'y'
...
describe('mongo db connection', function () {
  describe('via url string', function () {
    ...
    it('commit and query', function (done) {
      ...
      var db = this.db;
      db.commit('testcollection', snapshot.id, {
        v: 0,
        create: {}
      }, snapshot, null, function (err) {
        if (err) return done(err);
...
describe('parse query', function () {
  ...
  var addsType = function addsType(query) {
    var queryWithTypeNeNull = shallowClone(query);
    queryWithTypeNeNull._type = {
      $type: 2
    };
  ...
  var doesNotModify = function doesNotModify(query) {
    var parsedQuery = parseQuery(query);
    makeQuerySafe(parsedQuery.query);
  ...
  describe('adds _type: {$type: 2} when necessary', function () {
    it('basic', function () {
      ...
      }); // We don't try to analyze $bitsAllSet
    });
