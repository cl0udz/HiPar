...
function ShareDbMongo(mongo, options) {
  ...
  if (_typeof(mongo) === 'object') {
    options = mongo;
    mongo = options.mongo;
  }

  if (!options) options = {}; // pollDelay is a dodgy hack to work around race conditions replicating the
  // data out to the polling target secondaries. If a separate db is specified
  // for polling, it defaults to 300ms

  this.pollDelay = options.pollDelay != null ? options.pollDelay : options.mongoPoll ? 300 : 0; // By default, we create indexes on any ops collection that is used

  this.disableIndexCreation = options.disableIndexCreation || false; // The getOps() method depends on a separate operations collection, and that
  ...
  this.opIndexes = {}; // Allow $while and $mapReduce queries. These queries let you run arbitrary
  // JS on the server. If users make these queries from the browser, there's
  // security issues.

  this.allowJSQueries = options.allowAllQueries || options.allowJSQueries || false; // Aggregate queries are less dangerous, but you can use them to access any
  // data in the mongo database.

  this.allowAggregateQueries = options.allowAllQueries || options.allowAggregateQueries || false; // Setting this flag to true will attempt to infer a canonical op link for
  ...
  this.getOpsWithoutStrictLinking = options.getOpsWithoutStrictLinking || false; // Track whether the close method has been called
...
ShareDbMongo.prototype.getCollection = function (collectionName, callback) {
  ...
  if (err) return callback(err); // Gotcha: calls back sync if connected or async if not
...
function isLegacyMongoClient(client) {
  ...
  return typeof client.collection === 'function' && typeof client.close === 'function';
}
...
ShareDbMongo.prototype._connect = function (mongo, options) {
  ...
  var finish = function finish(err, client) {
    if (err) throw err;
...
ShareDbMongo.prototype.commit = function (collectionName, id, op, snapshot, options, callback) {
  ...
  this._writeOp(collectionName, id, op, snapshot, function (err, result) {
    ...
    self._writeSnapshot(collectionName, id, snapshot, opId, function (err, succeeded) {
      if (succeeded) return callback(err, succeeded); // Cleanup unsuccessful op if snapshot write failed. This is not
...
ShareDbMongo.prototype.validateCollectionName = function (collectionName) {
  if (collectionName === 'system' || collectionName[0] === 'o' && collectionName[1] === '_') {
...
ShareDbMongo.prototype.getOpCollection = function (collectionName, callback) {
  ...
  this.getDbs(function (err, mongo) {
    ...
    var collection = mongo.collection(name); // Given the potential problems with creating indexes on the fly, it might
...
ShareDbMongo.prototype.getOps = function (collectionName, id, from, to, options, callback) {
  ...
  this._getOpLink(collectionName, id, to, function (err, opLink) {
    if (err) return callback(err); // We need to fetch slightly more ops than requested in order to work backwards along
...
ShareDbMongo.prototype.getOpsBulk = function (collectionName, fromMap, toMap, options, callback) {
  ...
  this._getSnapshotOpLinkBulk(collectionName, ids, function (err, docs) {
    if (err) return callback(err);
    var docMap = getDocMap(docs); // Add empty array for snapshot versions that are up to date and create
    ...
    if (!conditions.length) return callback(null, opsMap); // Otherwise, get all of the ops that are newer
...
ShareDbMongo.prototype.getCommittedOpVersion = function (collectionName, id, snapshot, op, options, callback) {
  ...
  this.getOpCollection(collectionName, function (err, opCollection) {
    ...
    opCollection.find(query).project(projection).sort(sort).limit(1).next(function (err, doc) {
      if (err) return callback(err); // If we find no op with the same src and seq, we definitely don't have
      // any match. This should prevent us from accidentally querying a huge
      // history of ops

      if (!doc) return callback(); // If we do find an op with the same src and seq, we still have to get
...
function filterOps(ops, doc, to) {
  ...
  if (!doc) {
    ...
    var deleteOp = getLatestDeleteOp(ops); // Don't return any ops if we don't find a delete operation, which is the
...
ShareDbMongo.prototype._getOps = function (collectionName, id, from, to, options, callback) {
  this.getOpCollection(collectionName, function (err, opCollection) {
    if (err) return callback(err);
    var query = getOpsQuery(id, from, to); // Exclude the `d` field, which is only for use internal to livedb-mongo.
    // Also exclude the `m` field, which can be used to store metadata on ops
    // for tracking purposes

    var projection = options && options.metadata ? {
      d: 0
    } : {
      d: 0,
      m: 0
...
ShareDbMongo.prototype._getOpsBulk = function (collectionName, conditions, options, callback) {
  this.getOpCollection(collectionName, function (err, opCollection) {
    ...
    var projection = options && options.metadata ? null : {
      m: 0
    };
...
function readOpsBulk(stream, callback) {
  ...
  stream.on('end', function () {
    if (errored) return; // Sort ops for each doc in ascending order by version
...
ShareDbMongo.prototype._getOpLink = function (collectionName, id, to, callback) {
  ...
  this.getOpCollection(collectionName, function (error, collection) {
    if (error) return callback(error); // If to is null, we want the most recent version, so just return the
    ...
    var cursor = collection.find(query).sort({
      v: 1
    }).project(projection);
    getFirstOpWithUniqueVersion(cursor, null, function (error, op) {
      if (error) return callback(error);
      if (op) return callback(null, {
        _o: op.o,
        _v: op.v
...
ShareDbMongo.prototype._query = function (collection, inputQuery, projection, callback) {
  ...
  if (!parsed) return; // Collection operations such as $aggregate run on the whole
  ...
  if (parsed.collectionOperationKey) {
    collectionOperationsMap[parsed.collectionOperationKey](collection, parsed.query, parsed.collectionOperationValue, function (err, extra) {
      if (err) return callback(err);
      callback(null, [], extra);
    });
    return;
  ...
  var cursor = collection.find(parsed.query).project(projection); // Cursor transforms such as $skip transform the cursor into a new
  ...
  if (parsed.cursorOperationKey) {
    cursorOperationsMap[parsed.cursorOperationKey](cursor, parsed.cursorOperationValue, function (err, extra) {
      if (err) return callback(err);
      callback(null, [], extra);
    });
    return;
...
ShareDbMongo.prototype.queryPollDoc = function (collectionName, id, inputQuery, options, callback) {
  ...
  self.getCollectionPoll(collectionName, function (err, collection) {
    ...
    if (!parsed) return; // Run the query against a particular mongo document by adding an _id filter

    var queryId = parsed.query._id;

    if (queryId && _typeof(queryId) === 'object') {
      ...
      if (Array.isArray(queryId.$in) && Object.keys(queryId).length === 1) {
        ...
        parsed.query.$and = parsed.query.$and ? parsed.query.$and.concat({
          _id: id
        }, {
          _id: queryId
        }) : [{
    ...
    collection.find(parsed.query).limit(1).project({
      _id: 1
...
ShareDbMongo.prototype.canPollDoc = function (collectionName, query) {
  ...
  if (query.hasOwnProperty('$sort') || query.hasOwnProperty('$orderby') || query.hasOwnProperty('$limit') || query.hasOwnProperty('$skip') || query.hasOwnProperty('$max') || query.hasOwnProperty('$min') || query.hasOwnProperty('$returnKey')) {
    return false;
  }
...
ShareDbMongo.prototype.skipPoll = function (collectionName, id, op, query) {
  ...
  if (!op.op) return true; // Right now, always re-poll if using a collection operation such as
...
function checkValidMongo(query) {
    ...
    if (key[0] === '$') {
      if (collectionOperationsMap[key]) {
        ...
        if (collectionOperationKey) {
          return ShareDbMongo.onlyOneCollectionOperationError(collectionOperationKey, key);
        }

        collectionOperationKey = key;
      } else if (cursorOperationsMap[key]) {
        if (cursorOperationKey) {
          return ShareDbMongo.onlyOneCursorOperationError(cursorOperationKey, key);
        }
  ...
  if (collectionOperationKey && foundCursorMethod) {
    return ShareDbMongo.cursorAndCollectionMethodError(collectionOperationKey);
  }

  return null;
}

function ParsedQuery(query, collectionOperationKey, collectionOperationValue, cursorTransforms, cursorOperationKey, cursorOperationValue) {
  this.query = query;
...
function parseQuery(inputQuery) {
  ...
  return new ParsedQuery(query, collectionOperationKey, collectionOperationValue, cursorTransforms, cursorOperationKey, cursorOperationValue);
}
...
function makeQuerySafe(query) {
  ...
  if (query.hasOwnProperty('_type')) return; // Deleted documents are kept around so that we can start their version from
...
function deletedDocCouldSatisfyQuery(query) {
  ...
  for (var prop in query) {
    ...
    if (prop === '_id' || prop === '_v' || prop === '_o' || prop === '_m' || prop[0] === '_' && prop[1] === 'm' && prop[2] === '.') {
      continue;
...
function couldMatchNull(clause) {
  if (typeof clause === 'number' || typeof clause === 'boolean' || typeof clause === 'string') {
    return false;
  } else if (clause === null) {
    return true;
...
function castToDoc(id, snapshot, opLink) {
  ...
  var doc = isObject(data) ? shallowClone(data) : data === undefined ? {} : {
    _data: data
  };
  doc._id = id;
...
function isPlainObject(value) {
  return _typeof(value) === 'object' && (Object.getPrototypeOf(value) === Object.prototype || Object.getPrototypeOf(value) === null);
...
function getProjection(fields, options) {
  ...
  if (!fields) {
    return options && options.metadata ? {
      _o: 0
    } : {
      _m: 0,
      _o: 0
...
var collectionOperationsMap = {
  $distinct: function $distinct(collection, query, value, cb) {
    collection.distinct(value.field, query, cb);
  },
  $aggregate: function $aggregate(collection, query, value, cb) {
  ...
  $mapReduce: function $mapReduce(collection, query, value, cb) {
    ...
    collection.mapReduce(value.map, value.reduce, mapReduceOptions, cb);
  }
};
var cursorOperationsMap = {
  $count: function $count(cursor, value, cb) {
    cursor.count(cb);
  },
  $explain: function $explain(cursor, verbosity, cb) {
    cursor.explain(verbosity, cb);
  },
  $map: function $map(cursor, fn, cb) {
    cursor.map(fn, cb);
  }
};
var cursorTransformsMap = {
  $batchSize: function $batchSize(cursor, size) {
    return cursor.batchSize(size);
  },
  $comment: function $comment(cursor, text) {
    return cursor.comment(text);
  },
  $hint: function $hint(cursor, index) {
    return cursor.hint(index);
  },
  $max: function $max(cursor, value) {
    return cursor.max(value);
  },
  $maxScan: function $maxScan(cursor, value) {
    return cursor.maxScan(value);
  },
  $maxTimeMS: function $maxTimeMS(cursor, milliseconds) {
    return cursor.maxTimeMS(milliseconds);
  },
  $min: function $min(cursor, value) {
    return cursor.min(value);
  },
  $noCursorTimeout: function $noCursorTimeout(cursor) {
    // no argument to cursor method
    return cursor.noCursorTimeout();
  },
  $orderby: function $orderby(cursor, value) {
    console.warn('Deprecated: $orderby; Use $sort.');
    return cursor.sort(value);
  },
  $readConcern: function $readConcern(cursor, level) {
    return cursor.readConcern(level);
  },
  $readPref: function $readPref(cursor, value) {
  ...
  $returnKey: function $returnKey(cursor) {
    // no argument to cursor method
    return cursor.returnKey();
  },
  $snapshot: function $snapshot(cursor) {
    // no argument to cursor method
    return cursor.snapshot();
  },
  $sort: function $sort(cursor, value) {
    return cursor.sort(value);
  },
  $skip: function $skip(cursor, value) {
    return cursor.skip(value);
  },
  $limit: function $limit(cursor, value) {
    return cursor.limit(value);
  },
  $showDiskLoc: function $showDiskLoc(cursor, value) {
    console.warn('Deprecated: $showDiskLoc; Use $showRecordId.');
    return cursor.showRecordId(value);
  },
  $showRecordId: function $showRecordId(cursor) {
    // no argument to cursor method
...
ShareDbMongo.$whereDisabledError = function () {
  return {
    code: 4103,
    message: '$where queries disabled'
  };
};
...
ShareDbMongo.$mapReduceDisabledError = function () {
  return {
    code: 4104,
    message: '$mapReduce queries disabled'
  };
};
...
ShareDbMongo.$aggregateDisabledError = function () {
  return {
    code: 4105,
    message: '$aggregate queries disabled'
  };
};
...
ShareDbMongo.$queryDeprecatedError = function () {
  return {
    code: 4106,
    message: '$query property deprecated in queries'
  };
};
...
ShareDbMongo.malformedQueryOperatorError = function (operator) {
  return {
    code: 4107,
    message: 'Malformed query operator: ' + operator
  };
};
