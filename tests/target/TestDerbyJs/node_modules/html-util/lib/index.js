...
if (typeof document !== 'undefined') {
  ...
  replaceEntity = function replaceEntity(match) {
  ...
  replaceEntity = function replaceEntity(match) {
    ...
    var charCode = match.charAt(2) === 'x' || match.charAt(2) === 'X' ? parseInt(match.slice(3, -1), 16) : parseInt(match.slice(2, -1), 10);
...
function minify(html) {
  var minified = '',
      minifyContent = true;
  parse(html, {
    start: function start(tag, tagName, attrs) {
    ...
    text: function text(_text) {
      minified += minifyContent ? trimText(_text) : _text;
    },
    comment: function comment(tag) {
      if (isConditionalComment(tag)) minified += tag;
    },
    other: function other(tag) {
      minified += tag;
    }
  });
  return minified;
