...
exports.emptyTemplate = new saddle.Template([]); // Add ::isUnbound to Template && Binding
...
saddle.Block.prototype.dependencies = function (context, options) {
  if (DependencyOptions.shouldIgnoreTemplate(this, options)) return;
  var dependencies = this.expression.meta && this.expression.meta.blockType === 'on' ? getDependencies(this.expression, context, options) : null;
...
var markerHooks = [{
  emit: function emit(context, node) {
...
function View(views, name, source, options) {
  ...
  this.attributesMap = options && options.attributes && new ViewAttributesMap(options.attributes);
  this.arraysMap = options && options.arrays && new ViewArraysMap(options.arrays); // The empty string is considered true for easier HTML attribute parsing
...
View.prototype._initComponent = function (context) {
  return this._isComponent(context) ? this.componentFactory.init(context) : context;
};
...
View.prototype.dependencies = function (context, options) {
  if (DependencyOptions.shouldIgnoreTemplate(this, options)) return;
  var template = this.template || this.parse(); // We can't figure out relative path dependencies within a component without
  ...
  var viewContext = this._isComponent(context) ? context.componentChild(null) : context;
...
ContextClosure.prototype.equals = function (other) {
  return other instanceof ContextClosure && this.context === other.context && this.template.equals(other.template);
};
...
function Views() {
  ...
  this.tagMap = new ViewsMap(); // TODO: elementMap is deprecated and should be removed with Derby 0.6.0

  this.elementMap = this.tagMap;
}

Views.prototype.find = function (name, namespace) {
  var map = this.nameMap; // Exact match lookup

  var exactName = namespace ? namespace + ':' + name : name;
  var match = map[exactName];
  if (match) return match; // Relative lookup

  var segments = name.split(':');
  var segmentsDepth = segments.length;
  if (namespace) segments = namespace.split(':').concat(segments); // Iterate through segments, leaving the `segmentsDepth` segments and
...
Views.prototype.register = function (name, source, options) {
  ...
  this.nameMap[mapName] = view; // TODO: element is deprecated and should be removed with Derby 0.6.0
...
Views.prototype.serialize = function (options) {
  ...
  for (var name in this.nameMap) {
    ...
    items.push('[function(){return this.template=' + template.serialize() + '},' + serializeObject.args([view.name, minify ? null : view.source, hasKeys(view.options) ? view.options : null]) + ']');
  }

  return 'function(derbyTemplates, views){' + 'var expressions = derbyTemplates.expressions,' + 'templates = derbyTemplates.templates;' + 'views.deserialize([' + items.join(',') + '])}';
};

Views.prototype.findErrorMessage = function (name, contextView) {
  var names = Object.keys(this.nameMap);
  var message = 'Cannot find view "' + name + '" in' + [''].concat(names).join('\n  ') + '\n';
...
AsArray.prototype.emit = function (context, target) {
  var node = traverseAndCreate(context.controller, this.segments);
  var array = node[this.lastSegment] || (node[this.lastSegment] = []); // Iterate backwards, since rendering will usually append

  for (var i = array.length; i--;) {
    var item = array[i]; // Don't add an item if already in the array

    if (item === target) return;
    var mask = this.comparePosition(target, item); // If the emitted target is after the current item in the document,
