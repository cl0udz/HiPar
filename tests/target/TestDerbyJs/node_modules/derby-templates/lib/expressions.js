...
function pathSegments(segments) {
  ...
  for (var i = 0; i < segments.length; i++) {
    ...
    result[i] = _typeof(segment) === 'object' ? segment.item : segment;
  }
...
function renderValue(value, context) {
  return _typeof(value) !== 'object' ? value : value instanceof Template ? renderTemplate(value, context) : Array.isArray(value) ? renderArray(value, context) : renderObject(value, context);
}
...
function renderObject(object, context) {
  return hasTemplateProperty(object) ? renderObjectProperties(object, context) : object;
}
...
Expression.prototype.get = function () {}; // Return the expression's segment list with context objects
...
Expression.prototype._resolvePatch = function (context, segments) {
  return context && context.expression === this && context.item != null ? segments.concat(context) : segments;
};
...
Expression.prototype.isUnbound = function (context) {
  ...
  if (bindType === 'bound') return false; // Otherwise, inherit from the context
...
RelativePathExpression.prototype.resolve = function (context) {
  var relativeContext = context.forRelative(this);
  var base = relativeContext.expression ? relativeContext.expression.resolve(relativeContext) : [];
  if (!base) return;
...
RelativePathExpression.prototype.dependencies = function (context, options) {
  ...
  var relativeContext = context.forRelative(this);
  var dependencies = relativeContext.expression && relativeContext.expression.dependencies(relativeContext, options);
...
AttributePathExpression.prototype.resolve = function (context) {
  var attributeContext = context.forAttribute(this.attribute);
  if (!attributeContext) return; // Attributes may be a template, an expression, or a literal value
...
BracketsExpression.prototype.resolve = function (context) {
  ...
  var inside = this.inside.get(context);
  if (inside == null) return; // Concat the before, inside, and optional after segments

  var base = this.before.resolve(context);
  if (!base) return;
  var segments = this.afterSegments ? base.concat(inside, this.afterSegments) : base.concat(inside);
  return this._resolvePatch(context, segments);
};
...
FnExpression.prototype.get = function (context) {
  var value = this.apply(context); // Lookup property underneath computed value if needed
...
FnExpression.prototype._lookupParent = function (context) {
  ...
  if (parent && parent[this.lastSegment]) return parent; // Otherwise lookup function on page
  ...
  if (parent && parent[this.lastSegment]) return parent; // Throw if not found
...
FnExpression.prototype._applyFn = function (fn, context, extraInputs, thisArg) {
  ...
  if (!this.args) {
    return extraInputs ? fn.apply(thisArg, extraInputs) : fn.call(thisArg);
...
FnExpression.prototype.set = function (context, value) {
  ...
  while (controller) {
    parent = this.parentSegments ? lookup(this.parentSegments, controller) : controller;
