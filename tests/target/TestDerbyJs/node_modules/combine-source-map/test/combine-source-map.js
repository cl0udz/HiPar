...
function checkMappings(foo, sm, lineOffset) {
  ...
  for (var i = 0; i < mappings.length; i++) {
    ...
    if (gen.column !== fooGen.column || gen.line !== fooGen.line + lineOffset) {
      console.error('generated mapping at %s not offset properly:\ninput:  [%s]\noutput:[%s]\n\n', i, inspect(fooGen), inspect(gen));
      genLinesOffset = false;
    }

    if (orig.column !== fooOrig.column || orig.line !== fooOrig.line) {
      console.error('original mapping at %s is not the same as the genrated mapping:\ninput:  [%s]\noutput:[%s]\n\n', i, inspect(fooOrig), inspect(orig));
      origLinesSame = false;
    }
  }

  return {
    genLinesOffset: genLinesOffset,
    origLinesSame: origLinesSame
  };
}
...
test('add one file with inlined source', function (t) {
  ...
  var lineOffset = 3;
  var base64 = combine.create().addFile(file, {
    line: lineOffset
  }).base64();
...
test('add one file without inlined source', function (t) {
  var mapComment = convert.fromObject(foo).setProperty('sourcesContent', []).toComment();
  var file = {
    id: 'xyz',
    source: '(function() {\n\n  console.log(require(\'./bar.js\'));\n\n}).call(this);\n' + '\n' + mapComment,
    sourceFile: 'foo.js'
...
test('add one file with inlined sources from multiple files', function (t) {
  ...
  var base64 = combine.create().addFile({
  ...
  t.deepEqual(sm.sourcesContent, ['console.log(1);', 'console.log(2);', 'console.log(3);', 'console.log(4);'], 'include the correct source file content');
  t.deepEqual(mappingsFromMap(sm), [{
    original: {
      column: 0,
      line: 1
    },
...
test('relative path from multiple files', function (t) {
  ...
  var sm = convert.fromBase64(base64).toObject();
  t.deepEqual(sm.sources, ['src/package1/sub/one.js', 'src/package1/sub/two.js', 'src/package2/sub/three.js', 'src/package2/sub/four.js'], 'include the correct source');
  t.deepEqual(sm.sourcesContent, ['console.log(1);', 'console.log(2);', 'console.log(3);', 'console.log(4);'], 'include the correct source file content');
  t.deepEqual(mappingsFromMap(sm), [{
    original: {
      column: 0,
...
test('relative path when source and file name are the same', function (t) {
  ...
  var sm = convert.fromBase64(base64).toObject();
  t.deepEqual(sm.sources, ['a/b/one.js', 'a/b/two.js'], 'include the correct source');
  t.deepEqual(mappingsFromMap(sm), [{
    original: {
      column: 0,
      line: 1
