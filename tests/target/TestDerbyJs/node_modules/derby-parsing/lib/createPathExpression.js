...
function reducePath(node, segment, afterSegments) {
  ...
  return relative ? new expressions.RelativePathExpression(segments) : createSegmentsExpression(segments);
}
...
function reduceBinaryExpression(node) {
  ...
  if (left instanceof expressions.LiteralExpression && right instanceof expressions.LiteralExpression) {
    var fn = operatorFns.get[operator];
...
function reduceConditionalExpression(node) {
  ...
  if (test instanceof expressions.LiteralExpression && consequent instanceof expressions.LiteralExpression && alternate instanceof expressions.LiteralExpression) {
...
function reduceArrayExpression(node) {
  ...
  return isLiteral ? new expressions.LiteralExpression(literal) : new expressions.ArrayExpression(items);
}
...
function reduceObjectExpression(node) {
  ...
  return isLiteral ? new expressions.LiteralExpression(literal) : new expressions.ObjectExpression(properties);
}

function getKeyName(key) {
  return key.type === Syntax.Identifier ? key.name : key.type === Syntax.Literal ? key.value : unexpected(key);
}
