...
exports.markup = markup; // View.prototype._parse is defined here, so that it doesn't have to
...
templates.View.prototype._parse = function () {
  ...
  try {
    if (this.literal) {
      var source = this.unminified ? this.source : // Remove leading and trailing whitespace only lines by default
      this.source.replace(/^\s*\n/, '').replace(/\s*$/, '');
      template = new templates.Text(source);
    } else if (this.string) {
      template = createStringTemplate(this.source, this);
    } else {
      var source = this.unminified ? this.source : htmlUtil.minify(this.source).replace(/&sp;/g, ' ');
...
function parseHtmlStart(tag, tagName, attributes, selfClosing) {
  ...
  var namespaceUri = lowerTagName === 'svg' ? templates.NAMESPACE_URIS.svg : parseNode.namespaceUri;
...
function parseAttributes(attributes) {
  ...
  for (var key in attributes) {
    ...
    if (parseNode.content.length === 1) {
      ...
      attributesMap[key] = item instanceof templates.Text ? new templates.Attribute(item.data, nsUri) : item instanceof templates.DynamicText ? item.expression instanceof expressions.LiteralExpression ? new templates.Attribute(item.expression.value, nsUri) : new templates.DynamicAttribute(item.expression, nsUri) : new templates.DynamicAttribute(item, nsUri);
...
function parseHtmlEnd(tag, tagName) {
  ...
  if (!(last instanceof templates.DynamicElement && tagName.toLowerCase() === 'tag' || last instanceof templates.Element && last.tagName === tagName)) {
...
function parseBlockExpression(expression) {
  var blockType = expression.meta.blockType; // Block ending

  if (expression.meta.isEnd) {
    parseNode = parseNode.parent; // Validate that the block ending matches an appropriate block start

    var last = parseNode.last();
    var lastExpression = last && (last.expression || last.expressions && last.expressions[0]);

    if (!(lastExpression && blockType === 'end' && lastExpression.meta.blockType || blockType === lastExpression.meta.blockType)) {
...
function createDynamicViewInstance(expression, attributes, hooks, initHooks) {
  var viewInstance = new templates.DynamicViewInstance(expression, attributes, hooks, initHooks); // Wrap the viewInstance in a block with the same expression, so that it is
...
function viewAttributesFromElement(element) {
  ...
  for (var key in element.attributes) {
    ...
    viewAttributes[camelCased] = attribute.expression instanceof templates.Template ? new templates.ViewParent(attribute.expression) : attribute.expression instanceof expressions.Expression ? new expressions.ViewParentExpression(attribute.expression) : attribute.data;
  }
...
function parseAsObjectAttribute(key, value) {
  ...
  if (!(expression instanceof expressions.SequenceExpression && expression.args.length === 2 && expression.args[0] instanceof expressions.PathExpression)) {
...
function elementHooksFromAttributes(attributes, type) {
  ...
  for (var key in attributes) {
    var value = attributes[key]; // Parse `as` assignments
...
function createAttributesExpression(attributes) {
  ...
  return isLiteral ? new expressions.LiteralExpression(literalAttributes) : new expressions.ObjectExpression(dynamicAttributes);
}
...
function parseArrayElement(element, name, viewAttributes) {
  ...
  var camelName = dashToCamelCase(name);
  var viewAttribute = viewAttributes[camelName]; // If viewAttribute is already an ArrayExpression, push the expression for
  // the current array element

  if (viewAttribute instanceof expressions.ArrayExpression) {
    viewAttribute.items.push(expression); // Alternatively, viewAttribute will be an array if its items have all been
    ...
    viewAttributes[camelName] = expression instanceof expressions.LiteralExpression ? [expression.value] : new expressions.ArrayExpression([expression]);
  } else {
    unexpected();
  }
}
...
function parseViewExpression(expression) {
  ...
  var componentHooks = componentHooksFromAttributes(viewAttributes); // A ViewInstance has a static name, and a DynamicViewInstance gets its name
  // at render time

  var viewInstance;

  if (nameExpression instanceof expressions.LiteralExpression) {
    var name = nameExpression.get(); // Will throw if the view can't be found immediately
...
function viewAttributesFromExpression(expression) {
  ...
  var object = expression instanceof expressions.ObjectExpression ? expression.properties : expression instanceof expressions.LiteralExpression ? expression.value : null;
  if (_typeof(object) !== 'object') unexpected();
  var viewAttributes = {};

  for (var key in object) {
    var value = object[key];
    viewAttributes[key] = value instanceof expressions.LiteralExpression ? value.value : value instanceof expressions.Expression ? new expressions.ViewParentExpression(value) : value;
  }
...
function unescapeTextLiteral(text, environment) {
  return environment === 'html' || environment === 'attribute' ? htmlUtil.unescapeEntities(text) : text;
}
...
function createExpression(source) {
  source = source.trim();
  var meta = new expressions.ExpressionMeta(source); // Parse block expression //
  ...
  if (match) {
    ...
    path = match[2];
    as = match[3];
    keyAs = match[4]; // The blocks `else`, `unbound`, and `bound` may not have a path or alias
  } else if (source === 'else' || source === 'unbound' || source === 'bound') {
    meta.blockType = source; // Any source that starts with a `/` is treated as an end block. Either a
    // `{{/}}` with no following characters or a `{{/if}}` style ending is valid
  } else if (source.charAt(0) === '/') {
    meta.isEnd = true;
    meta.blockType = source.slice(1).trim() || 'end'; // Parse value expression //
