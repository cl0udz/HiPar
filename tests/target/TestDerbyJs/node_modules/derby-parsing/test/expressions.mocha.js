...
describe('Expression::get', function () {
  ...
  it('gets literal values', function () {
    ...
    expect(create('0xff').get()).equal(255); // Booleans

    expect(create('true').get()).equal(true);
    expect(create('false').get()).equal(false); // Strings

    expect(create('""').get()).equal('');
    expect(create('\'Howdy\'').get()).equal('Howdy'); // Regular Expressions

    var re = create('/([0-9]+)/').get();
    expect(re).to.be.a(RegExp);
    expect(re.source).equal('([0-9]+)'); // Other
  ...
  it('gets object literals', function () {
    expect(create('{}').get()).eql({});
    expect(create('{foo: 0, bar: 1}').get()).eql({
      foo: 0,
      bar: 1
    });
    expect(create('{foo: 0, bar: {"!": "baz"}}').get()).eql({
      foo: 0,
      bar: {
        '!': 'baz'
      }
  ...
  it('gets object literals containing paths', function () {
    ...
    expect(expression.get(context)).to.eql({
      foo: 2,
      bar: {
        '!': 11,
        baz: 'Hi'
