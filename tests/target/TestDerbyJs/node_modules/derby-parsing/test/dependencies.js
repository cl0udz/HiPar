...
var controller = {
  ...
  passThrough: function passThrough(value) {
    return value;
  },
  informal: {
    greeting: function greeting() {
...
describe('template dependencies', function () {
  ...
  describe('with block', function () {
    it('gets dependencies', function () {
      var template = createTemplate('{{with _page.key as #key}}' + '{{_page.colors[#key].name}}' + '{{/with}}');
      expect(template.dependencies(context)).to.eql([['_page', 'key'], ['_page', 'colors', 'green', 'name']]);
      expect(template.get(context)).to.equal('Green');
    });
  });
  describe('on block', function () {
    it('gets dependencies', function () {
      var template = createTemplate('{{on _page.key}}' + '{{_page.variation}}' + '{{/on}}');
      expect(template.dependencies(context)).to.eql([['_page', 'key'], ['_page', 'variation']]);
      expect(template.get(context)).to.equal('light');
    });
  });
  describe('each block', function () {
    it('gets item alias dependencies', function () {
      var template = createTemplate('{{each _page.keys as #key}}' + '{{#key}}' + '{{/each}}');
      expect(stripContexts(template.dependencies(context))).to.eql([['_page', 'keys'], ['_page', 'keys', {
        item: 0
      }], ['_page', 'keys', {
        item: 1
      }]]);
    ...
    it('gets index alias dependencies', function () {
      var template = createTemplate('{{each _page.keys as #key, #i}}' + '{{#i}}.' + '{{/each}}');
      expect(stripContexts(template.dependencies(context))).to.eql([['_page', 'keys'], ['_page', 'keys'], ['_page', 'keys']]);
      expect(template.get(context)).to.equal('0.1.');
    });
    it('gets alias dependencies from a literal', function () {
      var template = createTemplate('{{each [33, 77] as #key, #i}}' + '{{#i}},{{#key}};' + '{{/each}}');
  ...
  describe('HTML', function () {
    ...
    it.skip('gets DynamicComment dependencies from parsed template', function () {
      ...
      expect(template.dependencies(context)).to.eql([['_page', 'year']]);
    ...
    it('gets DynamicHtml dependencies', function () {
      ...
      expect(template.dependencies(context)).to.eql([['_page', 'html']]);
      expect(template.get(context)).to.equal('<div>Hi</div>');
    });
    ...
    it('gets DynamicElement dependencies', function () {
      ...
      expect(template.dependencies(context)).to.eql([['_page', 'tagName']]);
...
describe('expression dependencies', function () {
  ...
  describe('brackets', function () {
    it('gets bracket + path dependencies', function () {
      ...
      expect(expression.dependencies(context)).to.eql([['_page', 'key'], ['_page', 'colors', 'green', 'name']]);
    });
    it('gets bracket + path + bracket + path dependencies', function () {
      ...
      expect(expression.dependencies(context)).to.eql([['_page', 'key'], ['_page', 'channel'], ['_page', 'colors', 'green', 'rgb', 0]]);
    });
    it('gets bracket + bracket + path dependencies', function () {
      ...
      expect(expression.dependencies(context)).to.eql([['_page', 'key'], ['_page', 'variation'], ['_page', 'colors', 'green', 'light', 'hex']]);
    });
    it('gets nested bracket + path dependencies', function () {
      ...
      expect(expression.dependencies(context)).to.eql([['_page', 'index'], ['_page', 'keys', 1], ['_page', 'colors', 'green', 'name']]);
    });
  });
  describe('fn', function () {
    it('gets path + path dependencies', function () {
      var expression = createExpression('plus(_page.nums[0], _page.nums[1])');
      expect(expression.dependencies(context)).to.eql([['_page', 'nums', 0, '*'], ['_page', 'nums', 1, '*']]);
    });
    it('gets path + fn dependencies', function () {
      var expression = createExpression('plus(_page.nums[0], minus(_page.nums[3], _page.nums[2]))');
      expect(expression.dependencies(context)).to.eql([['_page', 'nums', 0, '*'], ['_page', 'nums', 3, '*'], ['_page', 'nums', 2, '*']]);
    });
    it('gets bracket + bracket dependencies', function () {
      ...
      expect(expression.dependencies(context)).to.eql([['_page', 'first'], ['_page', 'nums', 2, '*'], ['_page', 'second'], ['_page', 'nums', 3, '*']]);
    });
    it('gets fn inside bracket dependencies', function () {
      ...
      expect(expression.dependencies(context)).to.eql([['_page', 'nums', 2, '*'], ['_page', 'nums', 0, '*'], ['_page', 'keys', 1]]);
    });
  });
  ...
  describe('relative paths', function () {
    describe('with block', function () {
      it('gets dependencies', function () {
        ...
        expect(expression.dependencies(blockContext)).to.eql([['_page', 'colors', 'green']]);
      ...
      it('gets subpath dependencies', function () {
        ...
        expect(expression.dependencies(blockContext)).to.eql([['_page', 'colors', 'green', 'name']]);
      ...
      it('gets template in model dependencies', function () {
        ...
        var blockContext = context.child(aliasExpression);
        var expression = createExpression('this');
        expect(expression.dependencies(blockContext)).to.eql([['key'], ['_page', 'colors', 'green', 'name'], ['lightTemplate']]);
      ...
      it('gets subpath from template in model dependencies', function () {
        ...
        expect(expression.dependencies(blockContext)).to.eql([['key'], ['_page', 'colors', 'green', 'name'], ['lightTemplate', 'length']]);
  ...
  describe('aliases', function () {
    ...
    describe('each block', function () {
      it('gets item alias dependencies', function () {
        ...
        expect(expression.dependencies(eachContext)).to.eql([['_page', 'keys', eachContext]]);
      ...
      it('gets subpath from item alias dependencies', function () {
        ...
        expect(expression.dependencies(eachContext)).to.eql([['_page', 'keys', eachContext, 'length']]);
      ...
      it('gets key alias dependencies', function () {
        ...
        expect(expression.dependencies(eachContext)).to.eql([['_page', 'keys']]);
  ...
  describe('view attributes', function () {
    describe('expression values', function () {
      it('gets path dependencies', function () {
        ...
        var expression = createExpression('@color');
        expect(expression.dependencies(viewContext)).to.eql([['_page', 'colors', 'green']]);
      ...
      it('gets subpath from path dependencies', function () {
        ...
        var viewContext = context.viewChild(view, attributes);
        var expression = createExpression('@color.name');
        expect(expression.dependencies(viewContext)).to.eql([['_page', 'colors', 'green', 'name']]);
      ...
      it('gets bracket dependencies', function () {
        ...
        var viewContext = context.viewChild(view, attributes);
        var expression = createExpression('@color');
        expect(expression.dependencies(viewContext)).to.eql([['key'], ['_page', 'colors', 'green']]);
        expect(expression.get(viewContext).name).to.eql('Green');
      });
      it('gets subpath from bracket dependencies', function () {
        ...
        var viewContext = context.viewChild(view, attributes);
        var expression = createExpression('@color.name');
        expect(expression.dependencies(viewContext)).to.eql([['key'], ['_page', 'colors', 'green', 'name']]);
        expect(expression.get(viewContext)).to.eql('Green');
      });
    });
    describe('template values', function () {
      ...
      it('gets template in model attribute dependencies', function () {
        ...
        var viewContext = context.viewChild(view, attributes);
        var expression = createExpression('@color');
        expect(expression.dependencies(viewContext)).to.eql([['key'], ['_page', 'colors', 'green', 'name'], ['lightTemplate']]);
        expect(expression.get(viewContext)).a(templates.Template);
      });
      it('gets subpath from template in model attribute dependencies', function () {
        ...
        var viewContext = context.viewChild(view, attributes);
        var expression = createExpression('@color.length');
        expect(expression.dependencies(viewContext)).to.eql([['key'], ['_page', 'colors', 'green', 'name'], ['lightTemplate', 'length']]);
