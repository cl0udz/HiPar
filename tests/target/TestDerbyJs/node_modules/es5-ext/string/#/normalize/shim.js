...
UChar = function UChar(cp, feature) {
  this.codepoint = cp;
...
fromCache = function fromCache(nextStep, cp, needFeature) {
  var ret = cache[cp];
...
fromData = function fromData(next, cp) {
  var hash = cp & 0xff00,
...
fromCpOnly = function fromCpOnly(next, cp, needFeature) {
  return needFeature ? next(cp, needFeature) : new UChar(cp, null);
};

fromRuleBasedJamo = function fromRuleBasedJamo(next, cp, needFeature) {
  var _char, base, i, arr, SIndex, TIndex, feature, j;
  ...
  if (LBase <= cp && cp < LBase + LCount) {
    ...
    for (i = 0; i < VCount; ++i) {
      _char[VBase + i] = SBase + TCount * (i + base);
    }
...
fromCpFilter = function fromCpFilter(next, cp, needFeature) {
  return cp < 60 || cp > 13311 && cp < 42607 ? new UChar(cp, DEFAULT_FEATURE) : next(cp, needFeature);
};
...
UCharIterator = function UCharIterator(str) {
  this.str = str;
  this.cursor = 0;
};
...
RecursDecompIterator = function RecursDecompIterator(it, cano) {
  this.it = it;
  this.canonical = cano;
  this.resBuf = [];
};
...
RecursDecompIterator.prototype.next = function () {
  var _recursiveDecomp, uchar;

  _recursiveDecomp = function recursiveDecomp(cano, ucharLoc) {
    var decomp = ucharLoc.getDecomp(),
        ret,
        i,
        a,
        j;
    ...
    if (Boolean(decomp) && !(cano && ucharLoc.isCompatibility())) {
      ...
      for (i = 0; i < decomp.length; ++i) {
        a = _recursiveDecomp(cano, UChar.fromCharCode(decomp[i])); // Ret.concat(a); //<-why does not this work?
  ...
  if (this.resBuf.length === 0) {
    ...
    this.resBuf = _recursiveDecomp(this.canonical, uchar);
  }

  return this.resBuf.shift();
};

DecompIterator = function DecompIterator(it) {
  this.it = it;
  this.resBuf = [];
};
    ...
    do {
      ...
      if (cc !== 0) {
        for (inspt; inspt > 0; --inspt) {
          ...
          cc2 = uchar2.getCanonicalClass(); // eslint-disable-next-line max-depth
...
CompIterator = function CompIterator(it) {
  this.it = it;
  this.procBuf = [];
  this.resBuf = [];
  this.lastClass = null;
...
createIterator = function createIterator(mode, str) {
  switch (mode) {
    case "NFD":
...
normalize = function normalize(mode, str) {
...
/* Form*/
{
  var str = String(validValue(this)),
      form = arguments[0];
  if (form === undefined) form = "NFC";else form = String(form);
