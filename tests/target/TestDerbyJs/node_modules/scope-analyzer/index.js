...
exports.getBinding = getBinding; // create a new scope at a node.

function createScope(node, bindings) {
  assert.ok(_typeof(node) === 'object' && node && typeof node.type === 'string', 'scope-analyzer: createScope: node must be an ast node');
...
function visitScope(node) {
  assert.ok(_typeof(node) === 'object' && node && typeof node.type === 'string', 'scope-analyzer: visitScope: node must be an ast node');
  registerScopeBindings(node);
}

function visitBinding(node) {
  assert.ok(_typeof(node) === 'object' && node && typeof node.type === 'string', 'scope-analyzer: visitBinding: node must be an ast node');
...
function crawl(ast) {
  assert.ok(_typeof(ast) === 'object' && ast && typeof ast.type === 'string', 'scope-analyzer: crawl: ast must be an ast node');
  walk(ast, visitScope);
  ...
  function walk(node, cb) {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      if (_typeof(node[k]) === 'object' && node[k] && typeof node[k].type === 'string') {
        node[k].parent = node;
        walk(node[k], cb);
  ...
  function walkArray(nodes, parent, cb) {
    for (var i = 0; i < nodes.length; i++) {
      if (_typeof(nodes[i]) === 'object' && nodes[i] && typeof nodes[i].type === 'string') {
...
function getBinding(identifier) {
  assert.equal(_typeof(identifier), 'object', 'scope-analyzer: getBinding: identifier must be a node');
...
function getDeclaredScope(id) {
  var parent = id; // Jump over one parent if this is a function's name--the variables
...
function isVariable(node) {
  return node.type === 'Identifier' && !isObjectKey(node) && !isMethodDefinition(node) && (node.parent.type !== 'MemberExpression' || node.parent.object === node || node.parent.property === node && node.parent.computed) && !isImportName(node);
}