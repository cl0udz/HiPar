...
describe('ComponentHarness', function () {
  ...
  describe('renderDom', function () {
    ...
    it('creates child component instances', function () {
      ...
      Box.view = {
        is: 'box',
        source: '<index:>' + '<div class="box">' + '<view as="myClown" is="clown" />' + '</div>'
      };
    ...
    it('will update fragments dynamically', function () {
      ...
      Box.view = {
        is: 'box',
        source: '<index:>' + '<div class="box {{if open}}open{{/if}}"></div>'
      };
    ...
    it('will update nodes dynamically', function () {
      ...
      Box.view = {
        is: 'box',
        source: '<index:>' + '<div as="container" class="box {{if open}}open{{/if}}"></div>'
      };
    ...
    it('removes reference to stub components on destroy', function () {
      ...
      Box.view = {
        is: 'box',
        source: '<index:>' + '<div class="box">' + '{{unless hideClown}}' + '<view is="clown" />' + '{{/unless}}' + '</div>'
      };
      var page = runner.createHarness('<view is="box" />', Box).stubComponent('clown').renderDom();
    ...
    it('removes stub components from array on destroy', function () {
      ...
      Box.view = {
        is: 'box',
        source: '<index:>' + '<div class="box">' + '{{if showHappy}}' + '<view is="clown" expression="happy" />' + '{{/if}}' + '{{if showSad}}' + '<view is="clown" expression="sad" />' + '{{/if}}' + '</div>'
      };
      var page = runner.createHarness('<view is="box" show-happy />', Box).stubComponent({
        is: 'clown',
        asArray: 'clowns'
      }).renderDom();
  ...
  describe('fake app.history implementation', function () {
    it('accepts url option', function () {
      ...
      var harness = runner.createHarness('url: {{$render.url}} | query: {{JSON.stringify($render.query)}}');
...
function expectPageParams(page, expectedUrl, expectedQuery) {
  expect(page).to.have.property('params').that.deep.includes({
    url: expectedUrl,
    query: expectedQuery
  });
}