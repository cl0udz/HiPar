...
describe('bindings', function () {
  ...
  describe('basic blocks', function () {
    it('if', function () {
      var app = runner.createHarness().app;
      app.views.register('Body', '{{if _page.nested.value}}' + '{{this}}.' + '{{else}}' + 'otherwise' + '{{/if}}');
      var page = app.createPage();
      var fragment = page.getFragment('Body');
    ...
    it('unless', function () {
      var app = runner.createHarness().app;
      app.views.register('Body', '{{unless _page.nested.value}}' + 'nada' + '{{else}}' + 'otherwise' + '{{/unless}}');
      var page = app.createPage();
      var fragment = page.getFragment('Body');
    ...
    it('each else', function () {
      var app = runner.createHarness().app;
      app.views.register('Body', '{{each _page.items}}' + '{{this}}.' + '{{else}}' + 'otherwise' + '{{/each}}');
      var page = app.createPage();
      var fragment = page.getFragment('Body');
      expect(fragment).html('otherwise');
  ...
  describe('nested blocks', function () {
    it('each containing if', function () {
      var app = runner.createHarness().app;
      app.views.register('Body', '{{each _page.items as #item}}' + '{{if _page.toggle}}' + '{{#item}}.' + '{{/if}}' + '{{/each}}');
      var page = app.createPage();
      var items = page.model.at('_page.items');
  ...
  function testArray(itemTemplate, itemData) {
    it('each on path', function () {
      var app = runner.createHarness().app;
      app.views.register('Body', '<ul>' + '{{each _page.items as #item, #i}}' + itemTemplate + '{{/each}}' + '</ul>');
      testEach(app);
    });
    it('each on alias', function () {
      var app = runner.createHarness().app;
      app.views.register('Body', '{{with _page.items as #items}}' + '<ul>' + '{{each #items as #item, #i}}' + itemTemplate + '{{/each}}' + '</ul>' + '{{/with}}');
      testEach(app);
    });
    it('each on relative path', function () {
      var app = runner.createHarness().app;
      app.views.register('Body', '{{with _page.items}}' + '<ul>' + '{{each this as #item, #i}}' + itemTemplate + '{{/each}}' + '</ul>' + '{{/with}}');
      testEach(app);
    });
    it('each on relative subpath', function () {
      var app = runner.createHarness().app;
      app.views.register('Body', '{{with _page}}' + '<ul>' + '{{each this.items as #item, #i}}' + itemTemplate + '{{/each}}' + '</ul>' + '{{/with}}');
      testEach(app);
    });
    it('each on attribute', function () {
      var app = runner.createHarness().app;
      app.views.register('Body', '<view is="list" items="{{_page.items}}"></view>');
      app.views.register('list', '<ul>' + '{{each @items as #item, #i}}' + itemTemplate + '{{/each}}' + '</ul>');
      testEach(app);
    });
    it('each containing withs', function () {
      var app = runner.createHarness().app;
      app.views.register('Body', '<ul>' + '{{each _page.items as #item, #i}}' + '{{with this}}' + '{{with this}}' + '{{with this}}' + itemTemplate + '{{/with}}' + '{{/with}}' + '{{/with}}' + '{{/each}}' + '</ul>');
      testEach(app);
    });
    it('each containing view instance', function () {
      var app = runner.createHarness().app;
      app.views.register('Body', '<ul>' + '{{each _page.items as #item, #i}}' + '<view is="item"></view>' + '{{/each}}' + '</ul>');
    ...
    function testEach(app) {
      ...
      expect(fragment).html('<ul><li>0. One One</li><li>1. Two Two</li></ul>');
      items.push(itemData[2]);
      expect(fragment).html('<ul><li>0. One One</li><li>1. Two Two</li><li>2. Three Three</li></ul>');
      items.unshift(itemData[3]);
      expect(fragment).html('<ul><li>0. Four Four</li><li>1. One One</li><li>2. Two Two</li><li>3. Three Three</li></ul>');
      items.remove(1, 2);
      expect(fragment).html('<ul><li>0. Four Four</li><li>1. Three Three</li></ul>');
      items.shift();
      expect(fragment).html('<ul><li>0. Three Three</li></ul>');
      items.pop();
      expect(fragment).html('<ul></ul>');
      items.pop();
      expect(fragment).html('<ul></ul>');
      items.push(itemData[0]);
      expect(fragment).html('<ul><li>0. One One</li></ul>');
    }
  }
  ...
  describe('array of strings', function () {
    testArray('<li>{{#i}}. {{#item}} {{this}}</li>', ['One', 'Two', 'Three', 'Four']);
  ...
  it('array chained insertions at index 0', function () {
    var app = runner.createHarness().app;
    app.views.register('Body', '<ul>' + '{{each _data.items as #item}}' + '<li>{{#item}}</li>' + '{{/each}}' + '</ul>');
    var page = app.createPage();
  ...
  it('mutation with number path segments', function () {
    ...
    var app = runner.createHarness().app;
    app.views.register('Body', '<ul>' + '{{each _data.items as #item}}' + '<li>{{#item.label}}</li>' + '{{/each}}' + '</ul>');
    var page = app.createPage();
    var $items = page.model.at('_data.items');
    $items.set([{
      label: 'Red',
    ...
    var fragment = page.getFragment('Body');
    expect(fragment).html('<ul><li>Red</li><li>Green</li><li>Blue</li></ul>'); // Test mutation with a numeric path segment.
