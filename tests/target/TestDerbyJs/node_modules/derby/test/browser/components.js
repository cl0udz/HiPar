...
describe('components', function () {
  describe('destroy', function () {
    it('emits a "destroy" event when the component is removed from the DOM', function (done) {
      ...
      app.views.register('Body', '{{unless _page.hide}}' + '<view is="box" as="box"></view>' + '{{/unless}}');
    ...
    it('emits an event declared in the template with `on-destroy`', function (done) {
      var app = derby.createApp();
      var page = app.createPage();
      app.views.register('Body', '{{unless _page.hide}}' + '<view is="box" on-destroy="destroyBox()"></view>' + '{{/unless}}');
      app.views.register('box', '<div></div>');
  ...
  describe('bind', function () {
    it('calls a function with `this` being the component and passed in arguments', function () {
      ...
      var getArea = function getArea(scale) {
    ...
    function test(getFn, options) {
      ...
      it('calls a function once with `this` being the component', function (done) {
        ...
        var update = function update() {
          expect(this)["instanceof"](Box);
          called = true; // Will error if called more than once:
      ...
      it('calls with the most recent arguments', function (done) {
        ...
        var update = function update(letter, number, cb) {
          expect(this)["instanceof"](Box);
  ...
  describe('dependencies', function () {
    it('gets dependencies rendered inside of components', function () {
      var app = derby.createApp();
      var page = app.createPage();
      app.views.register('Body', '<view is="box" title="{{_page.title}}, friend">' + '{{_page.message}}!' + '</view>');
      app.views.register('box', '<div class="box">' + '<view is="box-title">{{@title}}</view>' + '{{@content}}' + '</div>');
      app.views.register('box-title', '<b>{{@content}}</b>');
      app.component('box', function Box() {});
      app.component('box-title', function BoxTitle() {});
      var view = app.views.find('Body');
      expect(view.dependencies(page.context)).eql([['_page', 'title'], ['_page', 'message']]);
    });
    it('does not return dependencies for local paths within components', function () {
      var app = derby.createApp();
      var page = app.createPage();
      app.views.register('Body', '<view is="box" title="{{_page.title}}"></view>');
      app.views.register('box', '<div class="box">' + '<view is="box-title">{{@title}}</view>' + '{{messages[currentMessage]}}' + '<small>{{#root._page.disclaimer}}</small>' + '</div>');
      app.views.register('box-title', '<b class="{{if show}}visible{{/if}}">' + '{{if happy}}' + '{{emphasize(@content)}}' + '{{else}}' + '{{@content}}' + '{{/if}}' + '</b>');
      app.component('box', function Box() {});
      app.component('box-title', function BoxTitle() {});
      var view = app.views.find('Body');
      expect(view.dependencies(page.context)).eql([['_page', 'title'], ['_page', 'disclaimer']]);
    });
  });
  describe('attribute to model binding', function () {
    it('updates model when path attribute changes', function () {
      ...
      this.app.views.register('Body', '<view is="swatch" value="{{_page.color}}"></view>');
      this.app.views.register('swatch', '<div style="background-color: {{value}}"></div>');
    ...
    it('updates model when expression attribute changes', function () {
      ...
      this.app.views.register('Body', '<view is="swatch" value="{{concat(\'light\', _page.color)}}"></view>');
      this.app.views.register('swatch', '{{@value}}<view is="color" value="{{value}}"></view>');
      this.app.views.register('color', '<div style="background-color: {{value}}"></div>');
    ...
    it('updates model when template attribute changes', function () {
      ...
      this.app.views.register('Body', '<view is="swatch" value="light{{_page.color}}"></view>');
      this.app.views.register('swatch', '<view is="color"></view>');
      this.app.views.register('color', '{{value}}<div style="background-color: {{value}}"></div>');
    ...
    it('updates view expression', function () {
      ...
      this.app.views.register('Body', '<view is="swatch" value="{{view _page.view, {value: _page.color}}}"></view>');
      this.app.views.register('swatch', '<div style="{{value}}">{{value}}</div>');
      this.app.views.register('back', 'background-color: light{{@value}}');
      this.app.views.register('fore', 'color: light{{@value}}');
    ...
    it('updates when template attribute is updated to new value inside component model', function () {
      ...
      this.app.views.register('Body', '<view is="swatch" value="light{{_page.color}}"></view>');
      this.app.views.register('swatch', '<div style="background-color: {{value}}">{{value}}</div>');
    ...
    it('renders template attribute passed through component and partial with correct context', function () {
      this.app = derby.createApp();
      this.page = this.app.createPage();
      this.page.model.set('_page.color', 'blue'); // `Body` uses the `picture-exhibit` component, passing in the `swatch` template as a
      // `@content` attribute. `swatch` refers to a top-level model path, `_page.color`.

      this.app.views.register('Body', '<view is="picture-exhibit" label="Blue Swatch"><view is="swatch"></view></view>');
      this.app.views.register('swatch', '<div style="background-color: {{_page.color}}">{{_page.color}}</div>'); // `picture-exhibit` passes `@content` through as a content attribute to `picture-frame`,
      ...
      this.app.views.register('picture-exhibit', '<view is="picture-frame">{{@content}}</view>' + '<label>{{@label}}</label>');
      this.app.views.register('picture-frame', '<div class="picture-frame">{{@content}}</div>');

      function PictureExhibit() {}

      this.app.component('picture-exhibit', PictureExhibit);
      var fragment = this.page.getFragment('Body');
      expect(fragment).html('<div class="picture-frame">' + '<div style="background-color: blue">blue</div>' + '</div>' + '<label>Blue Swatch</label>');
    });
    it('updates within template content', function () {
      ...
      this.page.model.set('_page.color', 'blue');
      this.app.views.register('Body', '<view is="swatch" width="{{_page.width}}" within>' + 'light{{#color}}' + '</view>');
      this.app.views.register('swatch', '{{with #root._page.color as #color}}' + '<div style="width: {{width}}px; background-color: {{content}}">' + '{{content}}' + '</div>' + '{{/with}}');
    ...
    it('updates within template attribute', function () {
      this.app = derby.createApp();
      this.page = this.app.createPage();
      this.app.views.register('Body', '<view is="swatch">' + '<attribute is="message" within>{{if #show}}Show me!{{else}}Hide me.{{/if}}</attribute>' + '</view>');
      this.app.views.register('swatch', '{{with show as #show}}' + '<div>{{@message}}</div>' + '{{/with}}');
    ...
    it('updates within expression attribute by making it a template', function () {
      this.app = derby.createApp();
      this.page = this.app.createPage();
      this.app.views.register('Body', '<view is="swatch">' + '<attribute is="message" within>{{#show ? "Show me!" : "Hide me."}}</attribute>' + '</view>');
      this.app.views.register('swatch', '{{with show as #show}}' + '<div>{{message}}</div>' + '{{/with}}');
      ...
      swatch.model.set('show', true);
      expect(fragment).html('<div>Show me!</div>'); // getAttribute works, but the rendering context is just inside the
    ...
    it('updates array within template attribute', function () {
      this.app = derby.createApp();
      this.page = this.app.createPage();
      this.app.views.register('Body', '<view is="swatch">' + '<item within>{{if #show}}Show me!{{else}}Hide me.{{/if}}</item>' + '<item>{{if #show}}Show me!{{else}}Hide me.{{/if}}</item>' + '</view>');
      this.app.views.register('swatch', '{{with show as #show}}' + '{{each @items as #item}}' + '{{#item.content}}' + '{{/each}}' + '{{/with}}', {
        arrays: 'item/items'
      });
    ...
    it('updates array within template attribute with content alias', function () {
      ...
      this.page = this.app.createPage();
      this.app.views.register('Body', '<view is="swatch">' + '<item within>{{if #show}}Show me!{{else}}Hide me.{{/if}}</item>' + '<item>{{if #show}}Show me!{{else}}Hide me.{{/if}}</item>' + '</view>');
      this.app.views.register('swatch', '{{with show as #show}}' + '{{each @items as #item}}' + '{{with #item.content as #itemContent}}' + '{{#itemContent}}' + '{{/with}}' + '{{/each}}' + '{{/with}}', {
        arrays: 'item/items'
      });
    ...
    it('updates array within template attribute in model from partial', function () {
      ...
      this.page = this.app.createPage();
      this.app.views.register('Body', '<view is="swatch">' + '<item within>{{if #show}}Show me!{{else}}Hide me.{{/if}}</item>' + '<item>{{if #show}}Show me!{{else}}Hide me.{{/if}}</item>' + '</view>');
      this.app.views.register('swatch', '{{with show as #show}}' + '<view is="swatch-items"></view>' + '{{/with}}', {
        arrays: 'item/items'
      });
      this.app.views.register('swatch-items', '{{each items as #item}}' + '{{#item.content}}' + '{{/each}}');
    ...
    it('updates array within attribute bound to component model path', function () {
      ...
      this.page = this.app.createPage();
      this.app.views.register('Body', '<view is="swatch">' + '<item within>{{if show}}Show me!{{else}}Hide me.{{/if}}</item>' + '<item>{{if show}}Show me!{{else}}Hide me.{{/if}}</item>' + '</view>');
      this.app.views.register('swatch', '{{each @items as #item}}' + '{{#item.content}}' + '{{/each}}', {
        arrays: 'item/items'
      });
    ...
    it('updates array within expression attribute by making it a template', function () {
      this.app = derby.createApp();
      this.page = this.app.createPage();
      this.app.views.register('Body', '<view is="swatch">' + '<item within>{{#show ? "Show me!" : "Hide me."}}</item>' + '<item>{{#show ? "Show me!" : "Hide me."}}</item>' + '</view>');
      this.app.views.register('swatch', '{{with show as #show}}' + '{{each items as #item}}' + '{{#item.content}}' + '{{/each}}' + '{{/with}}', {
        arrays: 'item/items'
      });
  ...
  describe('rendering', function () {
    beforeEach(function () {
      ...
      this.app.views.register('Body', '<view is="box" role="container" title="{{_page.title}}">' + '<view is="box" role="inner1" title="Greeting">Hello.</view>' + '<view is="box" role="inner2"></view>' + '</view>');
      this.app.views.register('box', '<div class="box">' + '<view is="box-title" tip="{{@title}}">{{@title}}</view>' + '{{@content}}' + '</div>');
      this.app.views.register('box-title', '<b title="{{@tip}}">{{@content}}</b>');
    ...
    it('renders a component', function () {
      ...
      expect(html).equal('<div class="box">' + '<b title="Good day">Good day</b>' + '<div class="box"><b title="Greeting">Greeting</b>Hello.</div>' + '<div class="box"><b></b></div>' + '</div>');
    });
    it('sets attributes as values on component model', function () {
      var tests = {
        container: function container(box, boxTitle) {
        ...
        inner1: function inner1(box, boxTitle) {
        ...
        inner2: function inner2(box, boxTitle) {
