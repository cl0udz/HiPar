...
describe('eventmodel', function () {
  beforeEach(function () {
    ...
    var self = this; // This is a helper to update the data model and trigger EM bindings in one go.
    ...
    this.binding = {
      ...
      insert: function insert(index, howMany) {
        self.insertCalled++;
        this.insertArgs = {
          index: index,
          howMany: howMany
  ...
  describe('lists', function () {
    it('Does not update an item binding inside a list item when something is inserted around it', function () {//var ctx = {item:1};
  ...
  describe('array lookup', function () {
    ...
    it('allows chained references', function () {
      ...
      this.set(['x'], 0);
      expect(this.updateCalled).equal(3); // Going back out again to make sure that all the bindings have been updated correctly.

      this.set(['list', 0], 0);
      expect(this.updateCalled).equal(4);
      this.set(['objList', 0, 'listName'], 'superman');
      expect(this.updateCalled).equal(5); // Some things that should not update the binding.
