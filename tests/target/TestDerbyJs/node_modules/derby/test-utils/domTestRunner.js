...
var runner = new DomTestRunner(); // Set up Chai assertion chain methods: `#html` and `#render`
...
DomTestRunner.prototype.installMochaHooks = function (options) {
  options = options || {};
  var jsdomOptions = options.jsdomOptions; // Set up runner's `window` and `document`.
...
function mochaHooksForNode(runner, options) {
  var jsdomOptions = options.jsdomOptions; // Use an indirect require so that Browserify doesn't try to bundle JSDOM.

  var JSDOM = util.serverRequire(module, 'jsdom').JSDOM;
  var nodeGlobal = global; // Keep a direct reference so that we're absolutely sure we clean up our own JSDOM.

  var jsdom;
  global.beforeEach(function () {
    jsdom = new JSDOM('', jsdomOptions);
    runner.window = jsdom.window;
    runner.document = jsdom.window.document; // Set `window` and `document` globals for Derby code that doesn't allow injecting them.

    nodeGlobal.window = runner.window;
    nodeGlobal.document = runner.document; // Initialize "input" and "change" listeners on the document.
