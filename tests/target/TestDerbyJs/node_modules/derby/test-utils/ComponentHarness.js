...
AppForHarness.prototype.loadViews = AppForServer.prototype.loadViews; // `_init()` does setup for loading views from files on the server and loading
...
ComponentHarness.prototype.setup = function (source) {
  this.app.views.register('$harness', source); // Remaining variable arguments are components
...
ComponentHarness.prototype._get = function (render, options) {
  options = options || {};
  var url = options.url || '';
  var page = this.app.createPage(); // Set `page.params`, which is usually created in tracks during `Page#render`:
  ...
  setPageUrl(url); // Fake some methods from tracks/lib/History.js.
  ...
  render(page); // HACK: Implement getting an instance as a side-effect of rendering. This
...
function createStubComponent(options) {
  ...
  StubComponent.prototype.init = asArray ? function () {
    var page = this.page;
    var component = this;
