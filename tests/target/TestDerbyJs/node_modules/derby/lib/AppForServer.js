...
var derbyTemplates = require('../templates'); // Avoid Browserifying these dependencies
...
AppForServer.prototype._initViews = function () {
  ...
  this.views.register('Page', '<!DOCTYPE html>' + '<meta charset="utf-8">' + '<view is="{{$render.prefix}}TitleElement"></view>' + '<view is="{{$render.prefix}}Styles"></view>' + '<view is="{{$render.prefix}}Head"></view>' + '<view is="{{$render.prefix}}BodyElement"></view>', {
    serverOnly: true
  });
  this.views.register('TitleElement', '<title><view is="{{$render.prefix}}Title"></view></title>');
  this.views.register('BodyElement', '<body class="{{$bodyClass($render.ns)}}">' + '<view is="{{$render.prefix}}Body"></view>');
...
AppForServer.prototype.bundle = function (backend, options, cb) {
  ...
  options || (options = {});
  if (options.minify == null) options.minify = util.isProduction; // Turn all of the app's currently registered views into a javascript
  ...
  backend.once('bundle', function (bundle) {
    bundle.require(path.dirname(__dirname), {
      expose: 'derby'
    ...
    bundle.transform(function (filename) {
      ...
      return through(function write() {}, function end() {
...
AppForServer.prototype.writeScripts = function (backend, dir, options, cb) {
  ...
  this.bundle(backend, options, function (err, source, map) {
    ...
    app.scriptUrl = app.scriptBaseUrl + '/derby/' + filename + '.js'; // Write current map and bundle files
    ...
    fs.writeFileSync(app.scriptFilename, source, 'utf8'); // Delete app bundles with same name in development so files don't
...
AppForServer.prototype._viewsSource = function (options) {
  return '/*DERBY_SERIALIZED_VIEWS*/' + 'module.exports = ' + this.views.serialize(options) + ';' + '/*DERBY_SERIALIZED_VIEWS_END*/';
};
...
AppForServer.prototype.serialize = function () {
  ...
  var scriptUrl = this.scriptUrl.indexOf(this.scriptBaseUrl) === 0 ? this.scriptUrl.slice(this.scriptBaseUrl.length) : this.scriptUrl;
  var scriptMapUrl = this.scriptMapUrl.indexOf(this.scriptMapBaseUrl) === 0 ? this.scriptMapUrl.slice(this.scriptMapBaseUrl.length) : this.scriptMapUrl;
...
AppForServer.prototype.loadViews = function (filename, namespace) {
  ...
  if (this.watchFiles) this._watchViews(data.files, filename, namespace); // Make chainable
...
AppForServer.prototype.loadStyles = function (filename, options) {
  ...
  stylesView.source += '<view is="' + filename + '"></view>'; // Make chainable
...
function watchOnce(filenames, callback) {
  ...
  watcher.on('change', function () {
    if (closed) return;
    closed = true; // HACK: chokidar 3.1.1 crashes when you synchronously call close
...
AppForServer.prototype._autoRefresh = function (backend) {
  this.agents = {};
  var app = this; // Auto-refresh is implemented on top of ShareDB's messaging layer.
  //
...
AppForServer.prototype._refreshClients = function () {
  ...
  var views = this.views.serialize({
    minify: true
  });
  var message = {
    derby: 'refreshViews',
