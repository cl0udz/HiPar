...
Page.prototype.destroy = function () {
  ...
  silentModel.destroy('$components'); // Unfetch and unsubscribe from all queries and documents
...
Page.prototype._addModelListeners = function (eventModel) {
  ...
  if (!model) return; // Registering model listeners with the *Immediate events helps to prevent
...
function useLegacyListeners(model) {
  var useLegacy = true; // model.once is broken in older racer, so manually remove event

  var listener = model.on('changeImmediate', function (segments, event) {
    model.removeListener('changeImmediate', listener); // Older Racer emits an array of eventArgs, whereas newer racer emits an event object
...
Page.prototype._addModelListenersLegacy = function (eventModel) {
  var model = this.model;
  if (!model) return; // `util.castSegments(segments)` is needed to cast string segments into
  // numbers, since EventModel#child does typeof checks against segments. This
  // could be done once in Racer's Model#emit, instead of in every listener.
...
Page.prototype._addContextListeners = function (eventModel) {
  ...
  function removeItemContext(context) {// TODO
  }
...
function BindingWrapper(eventModel, expression, binding) {
  ...
  this.ignoreTemplateDependency = binding instanceof components.ComponentAttributeBinding || binding.template instanceof templates.DynamicText && binding instanceof templates.RangeBinding;
...
function patchTextBinding(binding) {
  if (binding instanceof templates.AttributeBinding && binding.name === 'value' && (binding.element.tagName === 'INPUT' || binding.element.tagName === 'TEXTAREA') && documentListeners.inputSupportsSelection(binding.element) && binding.template.expression.resolve(binding.context)) {
