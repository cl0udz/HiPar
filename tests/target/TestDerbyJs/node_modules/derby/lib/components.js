...
function Component(context, data) {
  ...
  model._set(scope, data); // Store a reference to the component's scope such that the expression
  ...
  this.id = id;
  this._scope = scope; // Add reference to this component on the page so that all components
...
Component.prototype.getView = function (viewName) {
  ...
  return viewName ? this.app.views.find(viewName, contextView.namespace) : contextView;
};
...
function setModelAttributes(context, model) {
  ...
  if (!attributes) return; // Set attribute values on component model
...
function createFactory(constructor) {
  ...
  return constructor.singleton || constructor.prototype.singleton ? new SingletonComponentFactory(constructor) : new ComponentFactory(constructor);
}

function emitInitHooks(context, component) {
  if (!context.initHooks) return; // Run initHooks for `on` listeners immediately before init
...
ComponentFactory.prototype.init = function (context) {
  ...
  var component = new this.constructor(context, data); // Detect whether the component constructor already called super by checking
  ...
  setModelAttributes(component.context, component.model); // Do the user-specific initialization. The component constructor should be
...
ComponentFactory.prototype.create = function (context) {
  ...
  component.emit('create', component); // Call the component's create function after its view is rendered
...
function SingletonComponentFactory(constructor) {
  this.constructor = constructor;
  this.component = null; // Disable component from being destroyed, since it is intended to
...
function isBasePrototype(object) {
  return object === Object.prototype || object === Function.prototype || object === null;
}
...
var _extendComponent = Object.setPrototypeOf && Object.getPrototypeOf ? // Modern version, which supports ES6 classes
