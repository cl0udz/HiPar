...
function createAppPage(derby) {
  var Page = derby && derby.Page || PageBase; // Inherit from Page/PageForServer so that we can add controller functions as prototype
...
util.mergeInto(App.prototype, EventEmitter.prototype); // Overriden on server

App.prototype._init = function () {
  this._waitForAttach = true;
  this._cancelAttach = false;
  this.model = new this.derby.Model();
  serializedViews(derbyTemplates, this.views); // Must init async so that app.on('model') listeners can be added.
...
App.prototype._finishInit = function () {
  ...
  this._waitForAttach = false; // Instead of attaching, do a route and render if a link was clicked before
...
App.prototype._contentReady = function () {
  ...
  var isReady = false;
  var app = this; // The ready event handler
...
App.prototype.loadStyles = function () {}; // This function is overriden by requiring 'derby/parsing'


App.prototype.addViews = function () {
  throw new Error('Parsing not available. Registering a view from source should not be used ' + 'in application code. Instead, specify a filename with view.file.');
};
...
App.prototype.component = function (name, constructor, isDependency) {
  ...
  var viewIs, viewFilename, viewSource, viewDependencies; // Always using an object for the static `view` property is preferred

  if (viewProp && _typeof(viewProp) === 'object') {
    viewIs = viewProp.is;
  ...
  var viewName = name || viewIs || viewFilename && path.basename(viewFilename, '.html');
  ...
  var currentConstructor = currentView && currentView.componentFactory ? currentView.componentFactory.constructor : this._pendingComponentMap[mapName]; // Avoid registering the same component twice; we want to avoid the overhead
  ...
  delete this._pendingComponentMap[mapName]; // Make chainable
...
App.prototype.onRoute = function (callback, page, next, done) {
  ...
  this.emit('route', page); // HACK: To update render in transitional routes

  page.model.set('$render.params', page.params);
  page.model.set('$render.url', page.params.url);
  page.model.set('$render.query', page.params.query); // If transitional
...
App.prototype._handleMessage = function (action, message) {
  if (action === 'refreshViews') {
    ...
    var styleElement = document.querySelector('style[data-filename="' + message.filename + '"]');
