...
exports.inputSupportsSelection = inputSupportsSelection; // http://www.whatwg.org/specs/web-apps/current-work/multipage/the-input-element.html#do-not-apply
// TODO: Date types support

function inputSupportsSelection(input) {
  var type = input.type;
  return type === 'text' || type === 'textarea' || type === 'search' || type === 'url' || type === 'tel' || type === 'password';
}
...
var inputValue = function inputValue(input) {
...
function addDocumentListeners(doc) {
  ...
  doc.addEventListener('change', documentChange, true); // Listen to more events for versions of IE with buggy input event implementations
  ...
  if (parseFloat(window.navigator.appVersion.split('MSIE ')[1]) <= 9) {
    ...
    doc.addEventListener('selectionchange', function (e) {
      if (document.activeElement) {
        ...
        }); // selectionchange evts don't have the e.target we need
      }
    }, true);
  ...
  if (input.valueAsNumber !== input.valueAsNumber) {
    ...
    inputValue = function inputValue(input) {
