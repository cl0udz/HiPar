...
require("core-js/modules/web.timers"); // Generated by CoffeeScript 1.7.1
...
randomInt = function randomInt(n) {
  return Math.floor(Math.random() * n);
};

randomArrayElement = function randomArrayElement(array) {
...
messagingMethods = function messagingMethods(options, query, res) {
  ...
  if (type === 'html') {
    ...
    methods = {
      ...
      writeError: function writeError(statusCode, message) {
        methods.writeHead();
    ...
    return {
      ...
      writeRaw: function writeRaw(data) {
...
sendError = function sendError(res, statusCode, message, options) {
  res.writeHead(statusCode, message, options.headers);
  res.end("<html><body><h1>" + message + "</h1></body></html>");
};

bufferPostData = function bufferPostData(req, callback) {
  var data;
  data = [];
...
transformData = function transformData(req, data) {
...
decodeData = function decodeData(req, data) {
  if (req.headers['content-type'] === 'application/json') {
...
order = function order(start, playOld) {
  var base, queue;
  base = start;
  queue = new Array(10);
...
getHostPrefix = function getHostPrefix(options) {
...
BCSession = function BCSession(address, query, headers, options) {
...
BackChannel = function BackChannel(session, res, query) {
  this.res = res;
...
OutgoingArray = function OutgoingArray(id, data, sendcallback, confirmcallback) {
  this.id = id;
  this.data = data;
...
module.exports = browserChannel = function browserChannel(options, onConnect) {
  ...
  createSession = function createSession(address, query, headers) {
  ...
  middleware = function middleware(req, res, next) {
    ...
    if (pathname === "" + base + "/bcsocket.js") {
      ...
      if (req.method === 'POST') {
        ...
        dataError = function dataError(e) {
