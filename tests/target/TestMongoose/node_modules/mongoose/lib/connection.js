...
var Collection = require('./driver').get().Collection;
...
var applyPlugins = require('./helpers/schema/applyPlugins');
...
var noPasswordAuthMechanisms = ['MONGODB-X509'];
/**
...
Object.defineProperty(Connection.prototype, 'readyState', {
  ...
  set: function set(val) {
    ...
    if (this._readyState !== val) {
      this._readyState = val; // [legacy] loop over the otherDbs on this connection and change their state
...
Connection.prototype.startSession = _wrapConnHelper(function startSession(options, cb) {
  ...
  var session = this.client.startSession(options);
  cb(null, session);
});
/**
...
Connection.prototype.dropDatabase = _wrapConnHelper(function dropDatabase(cb) {
  ...
  for (var _i = 0, _Object$keys = Object.keys(this.models); _i < _Object$keys.length; _i++) {
...
function _wrapConnHelper(fn) {
  return function () {
    ...
    var disconnectedError = new MongooseError('Connection ' + this.id + ' was disconnected when calling `' + fn.name + '`');
    return utils.promiseOrCallback(cb, function (cb) {
      ...
      // Re: gh-8534
      immediate(function () {
        if (_this2.readyState === STATES.connecting) {
          _this2.once('open', function () {
            fn.apply(this, argsWithoutCb.concat([cb]));
          });
        } else if (_this2.readyState === STATES.disconnected && _this2.db == null) {
          cb(disconnectedError);
        } else {
          fn.apply(_this2, argsWithoutCb.concat([cb]));
        }
      });
    });
  };
...
Connection.prototype.onOpen = function () {
  this.readyState = STATES.connected; // avoid having the collection subscribe to our event emitter
...
Connection.prototype.openUri = function (uri, options, callback) {
  ...
  if (['string', 'number'].indexOf(_typeof(options)) !== -1) {
  ...
  if (typeof uri !== 'string') {
    throw new MongooseError('The `uri` parameter to `openUri()` must be a ' + "string, got \"".concat(_typeof(uri), "\". Make sure the first parameter to ") + '`mongoose.connect()` or `mongoose.createConnection()` is a string.');
  }

  if (callback != null && typeof callback !== 'function') {
    throw new MongooseError('3rd parameter to `mongoose.connect()` or ' + '`mongoose.createConnection()` must be a function, got "' + _typeof(callback) + '"');
  }
  ...
  var parsePromise = new Promise(function (resolve, reject) {
    parseConnectionString(uri, options, function (err, parsed) {
      if (err) {
        return reject(err);
      }
      ...
      if (dbName) {
        ...
        _this3.name = parsed.defaultDatabase;
      } else {
        _this3.name = get(parsed, 'auth.db', null);
      }

      _this3.host = get(parsed, 'hosts.0.host', 'localhost');
      _this3.port = get(parsed, 'hosts.0.port', 27017);
      _this3.user = _this3.user || get(parsed, 'auth.username');
      _this3.pass = _this3.pass || get(parsed, 'auth.password');
      resolve();
    });
  });

  var _handleReconnect = function _handleReconnect() {
  ...
  var promise = new Promise(function (resolve, reject) {
    ...
    client.connect(function (error) {
      ...
      _this.db = db; // `useUnifiedTopology` events

      var type = get(db, 's.topology.s.description.type', '');

      if (options.useUnifiedTopology) {
        if (type === 'Single') {
          var server = Array.from(db.s.topology.s.servers.values())[0];
          server.s.topology.on('serverHeartbeatSucceeded', function () {
            _handleReconnect();
          });
          server.s.pool.on('reconnect', function () {
            _handleReconnect();
          });
          client.on('serverDescriptionChanged', function (ev) {
            var newDescription = ev.newDescription;

            if (newDescription.type === 'Standalone') {
              _handleReconnect();
            } else if (newDescription.error != null && newDescription.error.name === 'MongoNetworkError' && newDescription.error.message.endsWith('timed out')) {
              _this.emit('timeout');
            } else {
              _this.readyState = STATES.disconnected;
            }
      ...
      client.on('left', function () {
        if (_this.readyState === STATES.connected && get(db, 's.topology.s.coreTopology.s.replicaSetState.topologyType') === 'ReplicaSetNoPrimary') {
          _this.readyState = STATES.disconnected;
        }
      });
  ...
  this.$initialConnection = Promise.all([promise, parsePromise]).then(function (res) {
    return res[0];
  })["catch"](function (err) {
    ...
    if (_this3.listeners('error').length > 0) {
  ...
  if (callback != null) {
    this.$initialConnection = this.$initialConnection.then(function () {
...
Connection.prototype.onClose = function (force) {
  this.readyState = STATES.disconnected; // avoid having the collection subscribe to our event emitter
  // to prevent 0.3 warning
...
Connection.prototype.model = function (name, schema, collection) {
  ...
  if (schema && schema.instanceOfSchema) {
    applyPlugins(schema, this.plugins, null, '$connectionPluginsApplied'); // compile a model

    model = this.base.model(fn || name, schema, collection, opts); // only the first model with this name is cached to allow
  ...
  if (this.models[name] && collection) {
    ...
    var sub = model.__subclass(this, schema, collection); // do not cache the sub model
