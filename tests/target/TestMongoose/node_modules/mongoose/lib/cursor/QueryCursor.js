...
function QueryCursor(query, options) {
  ...
  model.hooks.execPre('find', query, function () {
    _this2._transforms = _this2._transforms.concat(query._transforms.slice());

    if (_this2.options.transform) {
      _this2._transforms.push(options.transform);
    } // Re: gh-8039, you need to set the `cursor.batchSize` option, top-level
    // `batchSize` option doesn't work.


    if (_this2.options.batchSize) {
      _this2.options.cursor = options.cursor || {};
      _this2.options.cursor.batchSize = options.batchSize;
    }

    model.collection.find(query._conditions, _this2.options, function (err, cursor) {
      if (_this._error) {
...
QueryCursor.prototype.close = function (callback) {
  ...
  return utils.promiseOrCallback(callback, function (cb) {
    _this3.cursor.close(function (error) {
      if (error) {
        cb(error);
        return _this3.listeners('error').length > 0 && _this3.emit('error', error);
      }
...
QueryCursor.prototype.next = function (callback) {
  ...
  return utils.promiseOrCallback(callback, function (cb) {
    _next(_this4, function (error, doc) {
      if (error) {
        return cb(error);
      }
...
function _next(ctx, cb) {
  ...
  if (ctx.cursor) {
    return ctx.cursor.next(function (error, doc) {
      ...
      if (!opts.populate) {
        return opts.lean ? callback(null, doc) : _create(ctx, doc, null, callback);
      }
      ...
      ctx.query.model.populate(doc, pop, function (err, doc) {
        ...
        return opts.lean ? callback(null, doc) : _create(ctx, doc, pop, callback);
      });
    });
  } else {
