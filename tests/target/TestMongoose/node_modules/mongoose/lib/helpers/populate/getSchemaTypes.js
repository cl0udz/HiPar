  ...
  function search(parts, schema, subdoc, nestedPath) {
    ...
    while (p--) {
        ...
        if (p !== parts.length && foundschema.schema) {
          ...
          if (schemas != null && schemas.length > 0) {
            ...
            for (var i = 0; i < schemas.length; ++i) {
              var _ret = search(parts.slice(p), schemas[i], subdoc ? mpath.get(trypath, subdoc) : null, nestedPath.concat(parts.slice(0, p)));
          ...
          return search(parts.slice(p), type.schema, null, nestedPath.concat(parts.slice(0, p)));
        }
      }
      ...
      if (topLevelDoc.$__ && topLevelDoc.populated(fullPath) && p < parts.length) {
        ...
        if (model != null) {
          var _ret2 = search(parts.slice(p), model.schema, subdoc ? mpath.get(trypath, subdoc) : null, nestedPath.concat(parts.slice(0, p)));

          if (_ret2) {
            _ret2.$isUnderneathDocArray = _ret2.$isUnderneathDocArray || !model.schema.$isSingleNested;
          }
      ...
      if (_val != null) {
        ...
        var _schema = _model != null ? _model.schema : null;

        if (_schema != null) {
          var _ret3 = search(parts.slice(p), _schema, subdoc ? mpath.get(trypath, subdoc) : null, nestedPath.concat(parts.slice(0, p)));

          if (_ret3) {
            _ret3.$isUnderneathDocArray = _ret3.$isUnderneathDocArray || !_schema.$isSingleNested;
          }
