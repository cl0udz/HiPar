...
var DivergentArrayError = require('./error/divergentArray');
...
var OverwriteModelError = require('./error/overwriteModel');

var PromiseProvider = require('./promise_provider');

var Query = require('./query');

var RemoveOptions = require('./options/removeOptions');

var SaveOptions = require('./options/saveOptions');

var Schema = require('./schema');

var SkipPopulateValue = require('./helpers/populate/SkipPopulateValue');

var TimeoutError = require('./error/timeout');

var ValidationError = require('./error/validation');

var VersionError = require('./error/version');

var ParallelSaveError = require('./error/parallelSave');

var applyQueryMiddleware = require('./helpers/query/applyQueryMiddleware');

var applyHooks = require('./helpers/model/applyHooks');

var applyMethods = require('./helpers/model/applyMethods');

var applyStaticHooks = require('./helpers/model/applyStaticHooks');

var applyStatics = require('./helpers/model/applyStatics');

var applyWriteConcern = require('./helpers/schema/applyWriteConcern');

var assignVals = require('./helpers/populate/assignVals');

var castBulkWrite = require('./helpers/model/castBulkWrite');
...
var getModelsMapForPopulate = require('./helpers/populate/getModelsMapForPopulate');
...
var leanPopulateMap = require('./helpers/populate/leanPopulateMap');

var modifiedPaths = require('./helpers/update/modifiedPaths');

var mpath = require('mpath');

var parallelLimit = require('./helpers/parallelLimit');

var parseProjection = require('./helpers/projection/parseProjection');
...
function _applyCustomWhere(doc, where) {
  ...
  var keys = Object.keys(doc.$where);
  var len = keys.length;
...
Model.prototype.$__handleSave = function (options, callback) {
  ...
  if (this.isNew) {
    // send entire doc
    var obj = this.toObject(saveToObjectOptions);
    ...
    _setIsNew(this, false); // Make it possible to retry the insert
    ...
    if (delta) {
      ...
      this.constructor.exists(this.$__where()).then(function (documentExists) {
        if (!documentExists) throw new DocumentNotFoundError(_this2.$__where(), _this2.constructor.modelName);
...
Model.prototype.$__save = function (options, callback) {
  ...
  this.$__handleSave(options, function (error, result) {
    var hooks = _this3.schema.s.hooks;

    if (error) {
      return hooks.execPost('save:error', _this3, [_this3], {
        error: error
      }, function (error) {
    ...
    var modifiedPaths = _this3.modifiedPaths();
    ...
    if (get(options, 'safe.w') !== 0 && get(options, 'w') !== 0) {
      ...
      if (_this3.$__.version && !_this3.$__.inserting) {
        var doIncrement = VERSION_INC === (VERSION_INC & _this3.$__.version);
        _this3.$__.version = undefined;
        var key = _this3.schema.options.versionKey;
        var version = _this3.$__getValue(key) || 0;

        if (numAffected <= 0) {
          // the update failed. pass an error back
          var err = _this3.$__.$versionError || new VersionError(_this3, version, modifiedPaths);
          return callback(err);
        } // increment version if was successful


        if (doIncrement) {
          _this3.$__setValue(key, version + 1);
        }
      }

      if (result != null && numAffected <= 0) {
        error = new DocumentNotFoundError(result.$where, _this3.constructor.modelName, numAffected, result);
        return hooks.execPost('save:error', _this3, [_this3], {
          error: error
        }, function (error) {
          callback(error, _this3);
    ...
    _this3.$__.saving = undefined;

    _this3.emit('save', _this3, numAffected);

    _this3.constructor.emit('save', _this3, numAffected);
...
function generateVersionError(doc, modifiedPaths) {
  var key = doc.schema.options.versionKey;

  if (!key) {
    return null;
  }

  var version = doc.$__getValue(key) || 0;
...
Model.prototype.save = function (options, fn) {
  ...
  return utils.promiseOrCallback(fn, function (cb) {
    cb = _this4.constructor.$wrapCallback(cb);

    if (parallelSave) {
      _this4.$__handleReject(parallelSave);

      return cb(parallelSave);
    }

    _this4.$__.saveOptions = options;

    _this4.$__save(options, function (error) {
      _this4.$__.saving = undefined;
      delete _this4.$__.saveOptions;
      delete _this4.$__.$versionError;

      if (error) {
        _this4.$__handleReject(error);
...
function shouldSkipVersioning(self, path) {
  ...
  if (!skipVersioning) return false; // Remove any array indexes from the path
...
function operand(self, where, delta, data, val, op) {
  ...
  delta[op][data.path] = val; // disabled versioning?

  if (self.schema.options.versionKey === false) return; // path excluded from versioning?

  if (shouldSkipVersioning(self, data.path)) return; // already marked for versioning?
...
function handleAtomics(self, where, delta, data, value) {
  ...
  var atomics = value[arrayAtomicsSymbol];
...
Model.prototype.$__delta = function () {
  ...
  var divergent = [];
  var d = 0;
  where._id = this._doc._id; // If `_id` is an object, need to depopulate, but also need to be careful
...
function checkDivergentArray(doc, path, array) {
  ...
  if (!(pop && array && array.isMongooseArray)) return; // If the array was populated using options that prevented all
  ...
  if (check) {
    var atomics = array[arrayAtomicsSymbol];
...
Model.prototype.remove = function remove(options, fn) {
  ...
  fn = this.constructor.$handleCallbackError(fn);
  return utils.promiseOrCallback(fn, function (cb) {
    cb = _this5.constructor.$wrapCallback(cb);

    _this5.$__remove(options, cb);
  }, this.constructor.events);
};
/**
 * Alias for remove
...
Model.prototype.deleteOne = function deleteOne(options, fn) {
  ...
  fn = this.constructor.$handleCallbackError(fn);
  return utils.promiseOrCallback(fn, function (cb) {
    cb = _this6.constructor.$wrapCallback(cb);

    _this6.$__deleteOne(options, cb);
  }, this.constructor.events);
};
/*!
 * ignore
...
Model.prototype.$__remove = function $__remove(options, cb) {
  ...
  this[modelCollectionSymbol].deleteOne(where, options, function (err) {
    if (!err) {
      _this7.$__.isDeleted = true;

      _this7.emit('remove', _this7);

      _this7.constructor.emit('remove', _this7);

      return cb(null, _this7);
    }

    _this7.$__.isDeleted = false;
    cb(err);
  });
};
/*!
...
Model.exists = function exists(filter, options, callback) {
  ...
  var query = this.findOne(filter).select({
    _id: 1
...
Model.discriminator = function (name, schema, value) {
  ...
  if (this[subclassedSymbol] != null) {
    ...
    try {
      for (var _iterator = this[subclassedSymbol][Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
...
Model.init = function init(callback) {
  ...
  var autoIndex = utils.getOption('autoIndex', this.schema.options, this.db.config, this.db.base.options);
  var autoCreate = this.schema.options.autoCreate == null ? this.db.config.autoCreate : this.schema.options.autoCreate;

  var _ensureIndexes = autoIndex ? function (cb) {
    return _this8.ensureIndexes({
      _automatic: true
    }, cb);
  } : function (cb) {
    return cb();
  ...
  var _createCollection = autoCreate ? function (cb) {
    return _this8.createCollection({}, cb);
  } : function (cb) {
    return cb();
  };

  this.$init = new Promise(function (resolve, reject) {
    _createCollection(function (error) {
      if (error) {
        return reject(error);
      }

      _ensureIndexes(function (error) {
        if (error) {
          return reject(error);
        }
...
Model.createCollection = function createCollection(options, callback) {
  ...
  var schemaCollation = get(this, 'schema.options.collation', null);
  ...
  return utils.promiseOrCallback(callback, function (cb) {
    cb = _this9.$wrapCallback(cb);

    _this9.db.createCollection(_this9.collection.collectionName, options, utils.tick(function (error) {
      if (error) {
        return cb(error);
      }

      _this9.collection = _this9.db.collection(_this9.collection.collectionName, options);
      cb(null, _this9.collection);
    }));
  }, this.events);
};
/**
...
Model.syncIndexes = function syncIndexes(options, callback) {
  ...
  callback = this.$handleCallbackError(callback);
  return utils.promiseOrCallback(callback, function (cb) {
    cb = _this10.$wrapCallback(cb);

    _this10.createCollection(function (err) {
      if (err) {
        return cb(err);
      }

      _this10.cleanIndexes(function (err, dropped) {
        if (err != null) {
          return cb(err);
        }

        _this10.createIndexes(options, function (err) {
          if (err != null) {
            return cb(err);
          }
...
Model.cleanIndexes = function cleanIndexes(callback) {
  ...
  callback = this.$handleCallbackError(callback);
  return utils.promiseOrCallback(callback, function (cb) {
    var collection = _this11.collection;

    _this11.listIndexes(function (err, indexes) {
      if (err != null) {
        return cb(err);
      }

      var schemaIndexes = _this11.schema.indexes();
      ...
      try {
        for (var _iterator2 = indexes[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
          ...
          var found = false; // Never try to drop `_id` index, MongoDB server doesn't allow it
          ...
          try {
            for (var _iterator3 = schemaIndexes[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
              var schemaIndex = _step3.value;

              if (isIndexEqual(_this11, schemaIndex, index)) {
                found = true;
              }
            }
          } catch (err) {
    ...
    function dropIndexes(toDrop, cb) {
      var remaining = toDrop.length;
      var error = false;
      toDrop.forEach(function (indexName) {
...
function isIndexEqual(model, schemaIndex, dbIndex) {
  var key = schemaIndex[0];

  var options = _decorateDiscriminatorIndexOptions(model, utils.clone(schemaIndex[1])); // If these options are different, need to rebuild the index
  ...
  for (var _i = 0, _optionKeys = optionKeys; _i < _optionKeys.length; _i++) {
    var _key = _optionKeys[_i];

    if (!(_key in options) && !(_key in dbIndex)) {
      continue;
    }

    if (!utils.deepEqual(options[_key], dbIndex[_key])) {
      return false;
    }
  }
  ...
  for (var _i2 = 0; _i2 < schemaIndexKeys.length; ++_i2) {
    if (schemaIndexKeys[_i2] !== dbIndexKeys[_i2]) {
      return false;
    }

    if (!utils.deepEqual(key[schemaIndexKeys[_i2]], dbIndex.key[dbIndexKeys[_i2]])) {
      return false;
    }
  }
...
Model.listIndexes = function init(callback) {
  ...
  var _listIndexes = function _listIndexes(cb) {
    _this12.collection.listIndexes().toArray(cb);
  };

  callback = this.$handleCallbackError(callback);
  return utils.promiseOrCallback(callback, function (cb) {
    cb = _this12.$wrapCallback(cb); // Buffering

    if (_this12.collection.buffer) {
      _this12.collection.addQueue(_listIndexes, [cb]);
    } else {
      _listIndexes(cb);
    }
  }, this.events);
...
Model.ensureIndexes = function ensureIndexes(options, callback) {
  ...
  callback = this.$handleCallbackError(callback);
  return utils.promiseOrCallback(callback, function (cb) {
    cb = _this13.$wrapCallback(cb);

    _ensureIndexes(_this13, options || {}, function (error) {
      if (error) {
        return cb(error);
      }
...
function _ensureIndexes(model, options, callback) {
  ...
  var baseSchema = model.schema._baseSchema;
  var baseSchemaIndexes = baseSchema ? baseSchema.indexes() : [];
  ...
  var create = function create() {
    ...
    if (baseSchemaIndexes.find(function (i) {
      return utils.deepEqual(i, index);
    })) {
      return create();
    }
    ...
    var useCreateIndex = !!model.base.options.useCreateIndex;
    ...
    var methodName = useCreateIndex ? 'createIndex' : 'ensureIndex';
...
function _decorateDiscriminatorIndexOptions(model, indexOptions) {
  ...
  if (model.baseModelName != null && indexOptions.unique && !('partialFilterExpression' in indexOptions) && !('sparse' in indexOptions)) {
    var value = model.schema.discriminatorMapping && model.schema.discriminatorMapping.value || model.modelName;
    indexOptions.partialFilterExpression = _defineProperty({}, model.schema.options.discriminatorKey, value);
  }
...
var _handleSafe = util.deprecate(function _handleSafe(options) {
  if (options.safe) {
...
Model.translateAliases = function translateAliases(fields) {
  ...
  var translate = function translate(key, value) {
    var alias;
    var translated = [];
    var fieldKeys = key.split('.');
    var currentSchema = _this14.schema;

    for (var _i3 in fieldKeys) {
      var name = fieldKeys[_i3];

      if (currentSchema && currentSchema.aliases[name]) {
        alias = currentSchema.aliases[name]; // Alias found,
    ...
    var translatedKey = translated.join('.');
  ...
  if (_typeof(fields) === 'object') {
    ...
    if (fields instanceof Map) {
      ...
      try {
        for (var _iterator5 = new Map(fields)[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
      ...
      for (var _i4 = 0, _Object$keys = Object.keys(fields); _i4 < _Object$keys.length; _i4++) {
        ...
        if (key[0] === '$') {
          if (Array.isArray(fields[key])) {
            for (var _i5 in fields[key]) {
              // Recursively translate nested queries
              fields[key][_i5] = this.translateAliases(fields[key][_i5]);
            }
          }
        }
      }
...
Model.create = function create(doc, options, callback) {
  ...
  if (Array.isArray(doc)) {
    ...
    options = options != null && _typeof(options) === 'object' ? options : {};
  } else {
    var last = arguments[arguments.length - 1];
    options = {}; // Handle falsy callbacks re: #5061
  ...
  return utils.promiseOrCallback(cb, function (cb) {
    ...
    args.forEach(function (doc) {
      toExecute.push(function (callback) {
        var Model = _this15.discriminators && doc[discriminatorKey] != null ? _this15.discriminators[doc[discriminatorKey]] || getDiscriminatorByValue(_this15, doc[discriminatorKey]) : _this15;

        if (Model == null) {
          throw new MongooseError("Discriminator \"".concat(doc[discriminatorKey], "\" not ") + "found for model \"".concat(_this15.modelName, "\""));
        }
    ...
    var numFns = toExecute.length;

    if (numFns === 0) {
      return cb(null, []);
    }

    var _done = function _done(error, res) {
      var savedDocs = [];
      var len = res.length;

      for (var _i6 = 0; _i6 < len; ++_i6) {
        if (res[_i6].doc) {
          savedDocs.push(res[_i6].doc);
        }
      }
      ...
      if (doc instanceof Array) {
        ...
        cb.apply(_this15, [null].concat(savedDocs));
      }
    };

    var _res = [];
    toExecute.forEach(function (fn, i) {
      fn(function (err, res) {
        _res[i] = res;
...
Model.insertMany = function (arr, options, callback) {
  ...
  return utils.promiseOrCallback(callback, function (cb) {
    _this16.$__insertMany(arr, options, cb);
  }, this.events);
};
/*!
 * ignore
...
Model.$__insertMany = function (arr, options, callback) {
  ...
  var lean = get(options, 'lean', false);
  ...
  var toExecute = arr.map(function (doc) {
  ...
  parallelLimit(toExecute, limit, function (error, docs) {
    ...
    _this.collection.insertMany(docObjects, options, function (error, res) {
      ...
      for (var _i7 = 0; _i7 < docAttributes.length; ++_i7) {
        docAttributes[_i7].$__reset();

        _setIsNew(docAttributes[_i7], false);
      }
...
Model.bulkWrite = function (ops, options, callback) {
  ...
  var validations = ops.map(function (op) {
    return castBulkWrite(_this17, op, options);
  });
  callback = this.$handleCallbackError(callback);
  return utils.promiseOrCallback(callback, function (cb) {
    cb = _this17.$wrapCallback(cb);
    each(validations, function (fn, cb) {
      return fn(cb);
    }, function (error) {
      if (error) {
        return cb(error);
      ...
      _this17.collection.bulkWrite(ops, options, function (error, res) {
        if (error) {
          return cb(error);
        }
...
Model.replaceOne = function replaceOne(conditions, doc, options, callback) {
  ...
  var versionKey = get(this, 'schema.options.versionKey', null);
...
function _update(model, op, conditions, doc, options, callback) {
  ...
  callback = model.$handleCallbackError(callback); // gh-2406
...
Model.mapReduce = function mapReduce(o, callback) {
  ...
  return utils.promiseOrCallback(callback, function (cb) {
    ...
    if (o.query) {
      var q = new _this18.Query(o.query);
      q.cast(_this18);
      o.query = q._conditions;
      q = undefined;
    }

    _this18.collection.mapReduce(null, null, o, function (err, res) {
      if (err) {
        return cb(err);
      }
      ...
      if (res.collection) {
        ...
        var model = Model.compile('_mapreduce_' + res.collection.collectionName, Model.mapReduce.schema, res.collection.collectionName, _this18.db, _this18.base);
  ...
  return utils.promiseOrCallback(callback, function (cb) {
    ...
    if (pathsToValidate != null) {
      ...
      paths = paths.filter(function (p) {
        ...
        for (var _i8 = 0; _i8 < pieces.length; ++_i8) {
    ...
    try {
      var _loop = function _loop() {
        ...
        for (var _i9 = 0; _i9 < pieces.length - 1; ++_i9) {
          cur = cur[pieces[_i9]];
        }

        var val = get(obj, path, void 0);
        ...
        schematype.doValidate(val, function (err) {
          if (err) {
            ...
            if (err instanceof ValidationError) {
              for (var _i10 = 0, _Object$keys2 = Object.keys(err.errors); _i10 < _Object$keys2.length; _i10++) {
                var _err = _Object$keys2[_i10];
                error.addError("".concat(path, ".").concat(err.errors[_err].path), _err);
              }
            } else {
...
Model.geoSearch = function (conditions, options, callback) {
  ...
  return utils.promiseOrCallback(callback, function (cb) {
    ...
    _this20.collection.geoHaystackSearch(options.near[0], options.near[1], options, function (err, res) {
      if (err) {
        return cb(err);
      }
      ...
      for (var _i11 = 0; _i11 < res.results.length; ++_i11) {
        var temp = res.results[_i11];
        res.results[_i11] = new _this20();

        res.results[_i11].init(temp, {}, init);
      }
    });
  }, this.events);
};
...
Model.populate = function (docs, paths, callback) {
  ...
  var _this = this; // normalized paths


  paths = utils.populate(paths); // data that should persist across subPopulate calls

  var cache = {};
  callback = this.$handleCallbackError(callback);
  return utils.promiseOrCallback(callback, function (cb) {
    cb = _this21.$wrapCallback(cb);
...
function _populate(model, docs, paths, cache, callback) {
  ...
  for (var _i12 = 0; _i12 < length; ++_i12) {
    populate(model, docs, paths[_i12], next);
  }
...
function populate(model, docs, options, callback) {
  ...
  for (var _i13 = 0; _i13 < len; ++_i13) {
    ...
    if (mod.foreignField.size === 1) {
      ...
      try {
        for (var _iterator8 = mod.foreignField[Symbol.iterator](), _step8; !(_iteratorNormalCompletion8 = (_step8 = _iterator8.next()).done); _iteratorNormalCompletion8 = true) {
          var _foreignField = _step8.value;
          ...
          if (_foreignField !== '_id' || !match['_id']) {
            ...
            $or.push(_defineProperty({}, _foreignField, {
              $in: ids
            }));
          }
        }
  ...
  function _done() {
    ...
    try {
      for (var _iterator9 = params[Symbol.iterator](), _step9; !(_iteratorNormalCompletion9 = (_step9 = _iterator9.next()).done); _iteratorNormalCompletion9 = true) {
        var arr = _step9.value;
        var _mod = arr[0];
        var _assignmentOpts = arr[3];

        _assign(model, vals, _mod, _assignmentOpts);
      }
    } catch (err) {
...
function _execPopulateQuery(mod, match, select, assignmentOpts, callback) {
  ...
  var queryOptions = Object.assign({}, mod.options.options);

  if (mod.count) {
    delete queryOptions.skip;
  }

  var query = mod.model.find(match, select, queryOptions); // If we're doing virtual populate and projection is inclusive and foreign
  ...
  try {
    for (var _iterator10 = mod.foreignField[Symbol.iterator](), _step10; !(_iteratorNormalCompletion10 = (_step10 = _iterator10.next()).done); _iteratorNormalCompletion10 = true) {
      var _foreignField2 = _step10.value;

      if (_foreignField2 !== '_id' && query.selectedInclusively() && !isPathSelectedInclusive(query._fields, _foreignField2)) {
  ...
  if (mod.count) {
    ...
    try {
      for (var _iterator11 = mod.foreignField[Symbol.iterator](), _step11; !(_iteratorNormalCompletion11 = (_step11 = _iterator11.next()).done); _iteratorNormalCompletion11 = true) {
        var foreignField = _step11.value;
...
function _assign(model, vals, mod, assignmentOpts) {
  ...
  var rawDocs = {};
  var key;
  var val; // Clone because `assignRawDocsToIdStructure` will mutate the array

  var allIds = utils.clone(mod.allIds); // optimization:
  ...
  for (var _i15 = 0; _i15 < len; _i15++) {
    ...
    try {
      for (var _iterator12 = mod.foreignField[Symbol.iterator](), _step12; !(_iteratorNormalCompletion12 = (_step12 = _iterator12.next()).done); _iteratorNormalCompletion12 = true) {
        var foreignField = _step12.value;
          ...
          for (var j = 0; j < _valLength; ++j) {
            ...
            if (rawDocs[key]) {
              if (Array.isArray(rawDocs[key])) {
                ...
                rawOrder[key].push(_i15);
              } else {
                rawDocs[key] = [rawDocs[key], val];
                rawOrder[key] = [rawOrder[key], _i15];
              }
            } else {
...
Model.compile = function compile(name, schema, collectionName, connection, base) {
  ...
  if (typeof name === 'function' && name.prototype instanceof Model) {
    ...
    model = function model(doc, fields, skipId) {
      ...
      var discriminatorKey = model.schema.options.discriminatorKey;
      ...
      var Discriminator = model.discriminators[doc[discriminatorKey]] || getDiscriminatorByValue(model, doc[discriminatorKey]);
  ...
  var _userProvidedOptions = schema._userProvidedOptions || {}; // `bufferCommands` is true by default...


  var bufferCommands = true; // First, take the global option
  ...
  model.prototype[modelCollectionSymbol] = model.prototype.collection; // apply methods and statics
  ...
  model.collection = model.prototype.collection; // Create custom query constructor
...
function applyQueryMethods(model, methods) {
  for (var _i16 in methods) {
    model.Query.prototype[_i16] = methods[_i16];
  }
}
/*!
...
Model.__subclass = function subclass(conn, schema, collection) {
  ...
  if (_this.discriminators != null) {
    ...
    for (var _i17 = 0, _Object$keys3 = Object.keys(_this.discriminators); _i17 < _Object$keys3.length; _i17++) {
      var key = _Object$keys3[_i17];
  ...
  Model.collection = Model.prototype.collection; // Errors handled internally, so ignore
...
Model.inspect = function () {
  return "Model { ".concat(this.modelName, " }");
};
