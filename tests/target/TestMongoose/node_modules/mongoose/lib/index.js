...
if (global.MONGOOSE_DRIVER_PATH) {
  var deprecationWarning = 'The `MONGOOSE_DRIVER_PATH` global property is ' + 'deprecated. Use `mongoose.driver.set()` instead.';

  var setDriver = require('util').deprecate(function () {
...
var SchemaTypes = require('./schema/index');

var VirtualType = require('./virtualtype');

var STATES = require('./connectionstate');

var VALID_OPTIONS = require('./validoptions');
...
var removeSubdocs = require('./plugins/removeSubdocs');
...
function Mongoose(options) {
  this.connections = [];
  this.models = {};
  this.modelSchemas = {}; // default global options
  ...
  if (!options || !options[defaultMongooseSymbol]) {
    ...
    for (var _i = 0, _arr = ['Schema', 'model']; _i < _arr.length; _i++) {
      var key = _arr[_i];
...
Mongoose.prototype.set = function (key, value) {
  var _mongoose = this instanceof Mongoose ? this : mongoose;

  if (VALID_OPTIONS.indexOf(key) === -1) throw new Error("`".concat(key, "` is an invalid option."));
...
Mongoose.prototype.createConnection = function (uri, options, callback) {
  ...
  var conn = new Connection(_mongoose);
...
Mongoose.prototype.connect = function (uri, options, callback) {
  ...
  var conn = _mongoose.connection;
  return conn.openUri(uri, options, callback).then(function () {
    return _mongoose;
  });
};
/**
...
Mongoose.prototype.disconnect = function (callback) {
  ...
  return utils.promiseOrCallback(callback, function (cb) {
    var remaining = _mongoose.connections.length;

    if (remaining <= 0) {
      return cb(null);
    }

    _mongoose.connections.forEach(function (conn) {
...
Mongoose.prototype.model = function (name, schema, collection, skipInit) {
  ...
  var sub; // connection.model() may be passing a different schema for
  ...
  if (_mongoose.models[name] && options.cache !== false) {
    ...
    if (collection && collection !== _mongoose.models[name].collection.name) {
      ...
      sub = model.__subclass(_mongoose.connection, schema, collection); // do not cache the sub model
  ...
  var connection = options.connection || _mongoose.connection;
...
Mongoose.prototype.modelNames = function () {
  ...
  var names = Object.keys(_mongoose.models);
  return names;
};
/**
...
Mongoose.prototype.isValidObjectId = function (v) {
  ...
  var base = this || mongoose;
  var ObjectId = base.driver.get().ObjectId;
...
var mongoose = module.exports = exports = new Mongoose(_defineProperty({}, defaultMongooseSymbol, true));