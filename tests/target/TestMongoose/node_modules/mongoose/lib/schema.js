...
var SchemaTypeOptions = require('./options/SchemaTypeOptions');

var VirtualOptions = require('./options/VirtualOptions');

var VirtualType = require('./virtualtype');

var addAutoId = require('./helpers/schema/addAutoId');
  ...
  for (var i = 0; i < keys.length; ++i) {
    ...
    if (!(utils.isPOJO(obj[key]) || obj[key] instanceof SchemaTypeOptions)) {
      ...
      if (!this.options.typePojoToMixed && utils.isPOJO(obj[key][this.options.typeKey])) {
        ...
        var schemaWrappedPath = Object.assign({}, obj[key], {
          type: new Schema(obj[key][this.options.typeKey])
        });
...
reserved.collection = reserved.db = reserved.errors = reserved.init = reserved.isModified = reserved.isNew = reserved.get = reserved.modelName = reserved.save = reserved.schema = reserved.toObject = reserved.validate = reserved.remove = reserved.populated = 1;
/*!
...
Schema.prototype.path = function (path, obj) {
  ...
  if (schemaType.$isSchemaMap) {
    ...
    if (utils.hasUserDefinedProperty(obj, 'of')) {
      var isInlineSchema = utils.isPOJO(obj.of) && Object.keys(obj.of).length > 0 && !utils.hasUserDefinedProperty(obj.of, this.options.typeKey);
      _mapType = isInlineSchema ? new Schema(obj.of) : obj.of;
    }

    this.paths[_mapPath] = this.interpretAsType(_mapPath, _mapType, this.options);
    schemaType.$__schemaType = this.paths[_mapPath];
  }
  ...
  if (schemaType.$isSingleNested) {
    ...
    for (var _key2 in schemaType.schema.subpaths) {
      this.singleNestedPaths[path + '.' + _key2] = schemaType.schema.subpaths[_key2];
    }
  ...
  if (schemaType.$isMongooseArray && schemaType.caster instanceof SchemaType) {
    ...
    for (var _i = 0, _toAdd = toAdd; _i < _toAdd.length; _i++) {
      var _schemaType2 = _toAdd[_i];
      this.subpaths[_schemaType2.path] = _schemaType2;
    }
  }

  if (schemaType.$isMongooseDocumentArray) {
    for (var _i2 = 0, _Object$keys = Object.keys(schemaType.schema.paths); _i2 < _Object$keys.length; _i2++) {
      var _key3 = _Object$keys[_i2];
      this.subpaths[path + '.' + _key3] = schemaType.schema.paths[_key3];
      schemaType.schema.paths[_key3].$isUnderneathDocArray = true;
    }

    for (var _i3 = 0, _Object$keys2 = Object.keys(schemaType.schema.subpaths); _i3 < _Object$keys2.length; _i3++) {
      var _key4 = _Object$keys2[_i3];
      this.subpaths[path + '.' + _key4] = schemaType.schema.subpaths[_key4];
      schemaType.schema.subpaths[_key4].$isUnderneathDocArray = true;
    }

    for (var _i4 = 0, _Object$keys3 = Object.keys(schemaType.schema.singleNestedPaths); _i4 < _Object$keys3.length; _i4++) {
      var _key5 = _Object$keys3[_i4];
      this.subpaths[path + '.' + _key5] = schemaType.schema.singleNestedPaths[_key5];
      schemaType.schema.singleNestedPaths[_key5].$isUnderneathDocArray = true;
    }
  }
...
function gatherChildSchemas(schema) {
  ...
  for (var _i5 = 0, _Object$keys4 = Object.keys(schema.paths); _i5 < _Object$keys4.length; _i5++) {
    var path = _Object$keys4[_i5];
...
function getMapPath(schema, path) {
  for (var _i6 = 0, _Object$keys5 = Object.keys(schema.paths); _i6 < _Object$keys5.length; _i6++) {
    var _path = _Object$keys5[_i6];
...
Schema.prototype.post = function (name) {
  if (name instanceof RegExp) {
    ...
    try {
      for (var _iterator6 = hookNames[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
        var fn = _step6.value;
  ...
  if (Array.isArray(name)) {
    ...
    try {
      for (var _iterator7 = name[Symbol.iterator](), _step7; !(_iteratorNormalCompletion7 = (_step7 = _iterator7.next()).done); _iteratorNormalCompletion7 = true) {
        var el = _step7.value;
...
Schema.prototype.virtual = function (name, options) {
  ...
  for (var i = 0; i < parts.length - 1; ++i) {
    if (this.paths[cur] != null && this.paths[cur].$isMongooseDocumentArray) {
      var _ret = function () {
        var remnant = parts.slice(i + 1).join('.');

        var v = _this3.paths[cur].schema.virtual(remnant);

        v.get(function (v, virtual, doc) {
          var parent = doc.__parentArray[arrayParentSymbol];
          var path = cur + '.' + doc.__index + '.' + remnant;
  ...
  if (Array.isArray(path)) {
    path.forEach(function (name) {
      ...
      if (this.nested[name]) {
        ...
        try {
          for (var _iterator8 = allKeys[Symbol.iterator](), _step8; !(_iteratorNormalCompletion8 = (_step8 = _iterator8.next()).done); _iteratorNormalCompletion8 = true) {
