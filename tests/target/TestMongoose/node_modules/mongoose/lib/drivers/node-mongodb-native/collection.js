...
NativeCollection.prototype.onOpen = function () {
  var _this = this; // always get a new collection in case the user changed host:port
  ...
  return _this.conn.db.collection(_this.name, function (err, c) {
    if (err) return callback(err); // discover if this collection exists and if it is capped

    _this.conn.db.listCollections({
      name: _this.name
      ...
      if (exists) {
        ...
        // create
        var opts = Object.assign({}, _this.opts.capped);
        opts.capped = true;
...
function iter(i) {
  NativeCollection.prototype[i] = function () {
    ...
    var debug = get(_this, 'conn.base.options.debug');
    var lastArg = arguments[arguments.length - 1]; // If user force closed, queueing will hang forever. See #5664
    ...
    if (this.buffer) {
      ...
      return new this.Promise(function (resolve, reject) {
        _this2.addQueue(i, [].concat(args).concat([function (err, res) {
          if (err != null) {
            return reject(err);
          }
...
for (var _i = 0, _Object$keys = Object.keys(Collection.prototype); _i < _Object$keys.length; _i++) {
