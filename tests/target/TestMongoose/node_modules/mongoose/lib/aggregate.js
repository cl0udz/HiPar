...
Aggregate.prototype.addFields = function (arg) {
  ...
  if (_typeof(arg) === 'object' && !util.isArray(arg)) {
...
Aggregate.prototype.explain = function (callback) {
  ...
  return utils.promiseOrCallback(callback, function (cb) {
    ...
    model.hooks.execPre('aggregate', _this, function (error) {
      ...
      _this.options.explain = true;
      model.collection.aggregate(_this._pipeline, _this.options || {}).explain(function (error, result) {
        var _opts = {
          error: error
        };
        return model.hooks.execPost('aggregate', _this, [result], _opts, function (error) {
          if (error) {
            return cb(error);
          }
...
Aggregate.prototype.exec = function (callback) {
  ...
  return utils.promiseOrCallback(callback, function (cb) {
    ...
    model.hooks.execPre('aggregate', _this2, function (error) {
      ...
      if (!_this2._pipeline.length) {
        return cb(new Error('Aggregate has empty pipeline'));
      }

      var options = utils.clone(_this2.options || {});
      collection.aggregate(_this2._pipeline, options, function (error, cursor) {
        if (error) {
          var _opts2 = {
            error: error
          };
          return model.hooks.execPost('aggregate', _this2, [null], _opts2, function (error) {
            if (error) {
              return cb(error);
            }
...
if (Symbol.asyncIterator != null) {
  Aggregate.prototype[Symbol.asyncIterator] = function () {
    ...
    }).exec().transformNull().map(function (doc) {
      return doc == null ? {
        done: true
      } : {
        value: doc,
...
function prepareDiscriminatorPipeline(aggregate) {
  ...
  var discriminatorMapping = schema && schema.discriminatorMapping;

  if (discriminatorMapping && !discriminatorMapping.isRoot) {
    var originalPipeline = aggregate._pipeline;
    var discriminatorKey = discriminatorMapping.key;
    var discriminatorValue = discriminatorMapping.value; // If the first pipeline stage is a match and it doesn't specify a `__t`
    // key, add the discriminator key to it. This allows for potential
    // aggregation query optimizations not to be disturbed by this feature.

    if (originalPipeline[0] && originalPipeline[0].$match && !originalPipeline[0].$match[discriminatorKey]) {
      originalPipeline[0].$match[discriminatorKey] = discriminatorValue; // `originalPipeline` is a ref, so there's no need for
