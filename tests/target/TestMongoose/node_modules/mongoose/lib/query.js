...
var ReadPreference = require('./driver').get().ReadPreference;

var applyGlobalMaxTimeMS = require('./helpers/query/applyGlobalMaxTimeMS');

var applyWriteConcern = require('./helpers/schema/applyWriteConcern');

var cast = require('./cast');

var castArrayFilters = require('./helpers/update/castArrayFilters');

var castUpdate = require('./helpers/query/castUpdate');

var completeMany = require('./helpers/query/completeMany');

var get = require('./helpers/get');

var getDiscriminatorByValue = require('./helpers/discriminator/getDiscriminatorByValue');

var hasDollarKeys = require('./helpers/query/hasDollarKeys');

var helpers = require('./queryhelpers');

var isInclusive = require('./helpers/projection/isInclusive');

var mquery = require('mquery');

var parseProjection = require('./helpers/projection/parseProjection');

var selectPopulatedFields = require('./helpers/query/selectPopulatedFields');

var setDefaultsOnInsert = require('./helpers/setDefaultsOnInsert');

var slice = require('sliced');

var updateValidators = require('./helpers/updateValidators');

var util = require('util');

var utils = require('./utils');

var wrapThunk = require('./helpers/query/wrapThunk');
/**
...
function Query(conditions, options, model, collection) {
  ...
  this._executionCount = 0; // this is the case where we have a CustomQuery, we need to check if we got
...
var setSafe = util.deprecate(function setSafe(options, safe) {
  options.safe = safe;
...
Query.prototype._find = wrapThunk(function (callback) {
  ...
  applyGlobalMaxTimeMS(this.options, this.model); // Separate options to pass down to `completeMany()` in case we need to
...
Query.prototype._completeOne = function (doc, res, callback) {
  ...
  var userProvidedFields = this._userProvidedFields || {}; // `populate`, `lean`

  var mongooseOptions = this._mongooseOptions; // `rawResult`
...
Query.prototype._findOne = wrapThunk(function (callback) {
  ...
  this._fields = this._castFields(this._fields);
  applyGlobalMaxTimeMS(this.options, this.model); // don't pass in the conditions because we already merged them in
  ...
  Query.base.findOne.call(this, {}, function (err, doc) {
    ...
    _this3._completeOne(doc, null, _wrapThunkCallback(_this3, callback));
  });
});
/**
...
Query.prototype.__distinct = wrapThunk(function __distinct(callback) {
  ...
  this._collection.collection.distinct(this._distinct, this._conditions, callback);
});
/**
...
Query.prototype._findOneAndDelete = wrapThunk(function (callback) {
  ...
  this._collection.collection.findOneAndDelete(filter, options, _wrapThunkCallback(this, function (err, res) {
    if (err) {
      return callback(err);
    }

    var doc = res.value;
    return _this4._completeOne(doc, res, callback);
  }));
});
/**
...
Query.prototype._findOneAndReplace = wrapThunk(function (callback) {
  ...
  var fields = null;
  var castedDoc = new this.model(this._update, null, true);
  ...
  castedDoc.validate(function (err) {
    if (err != null) {
      return callback(err);
    }
    ...
    _this5._collection.collection.findOneAndReplace(filter, castedDoc, options, _wrapThunkCallback(_this5, function (err, res) {
      if (err) {
        return callback(err);
      }

      var doc = res.value;
      return _this5._completeOne(doc, res, callback);
    }));
  });
});
/*!
...
function _getOption(query, option, def) {
  var opts = query._optionsForExec(query.model);
...
Query.prototype._findAndModify = function (type, callback) {
  ...
  var opts = this._optionsForExec(model);
  ...
  var conn = get(model, 'collection.conn', {});
  ...
  if (useFindAndModify === false) {
    ...
    var updateMethod = isOverwriting ? 'findOneAndReplace' : 'findOneAndUpdate';
    ...
    if (runValidators) {
      this.validate(this._update, opts, isOverwriting, function (error) {
        ...
        if (_this6._update && _this6._update.toBSON) {
          _this6._update = _this6._update.toBSON();
        }

        collection[updateMethod](castedQuery, _this6._update, opts, _wrapThunkCallback(_this, function (error, res) {
          return cb(error, res ? res.value : res, res);
        }));
      });
    } else {
...
var _legacyFindAndModify = util.deprecate(function (filter, update, opts, cb) {
  ...
  var sort = opts != null && Array.isArray(opts.sort) ? opts.sort : [];

  var _cb = _wrapThunkCallback(this, function (error, res) {
    return cb(error, res ? res.value : res, res);
  });
...
Query.prototype._mergeUpdate = function (doc) {
  if (doc == null || _typeof(doc) === 'object' && Object.keys(doc).length === 0) {
    return;
  }
...
function _updateThunk(op, callback) {
  ...
  if (isOverwriting) {
    ...
    this._update = setDefaultsOnInsert(this._conditions, this.model.schema, this._update, options);
  }

  var runValidators = _getOption(this, 'runValidators', false);

  if (runValidators) {
    this.validate(this._update, options, isOverwriting, function (err) {
      if (err) {
        return callback(err);
      }

      if (_this7._update.toBSON) {
        _this7._update = _this7._update.toBSON();
      }

      _this7._collection[op](castedQuery, _this7._update, options, callback);
    });
    return null;
  }
  ...
  return utils.promiseOrCallback(callback, function (cb) {
    try {
      if (isOverwriting) {
        ...
        updateValidators(_this8, _this8.model.schema, castedDoc, options, cb);
      }
    } catch (err) {
      process.nextTick(function () {
        cb(err);
...
Query.prototype.orFail = function (err) {
  ...
  this.map(function (res) {
    switch (_this9.op) {
      case 'find':
        if (res.length === 0) {
          throw _orFailError(err, _this9);
        }
...
Query.prototype.exec = function exec(op, callback) {
  ...
  return utils.promiseOrCallback(callback, function (cb) {
    cb = _this10.model.$wrapCallback(cb);
    ...
    _this10._hooks.execPre('exec', _this10, [], function (error) {
      if (error != null) {
        return cb(error);
      }

      _this10[_this10.op].call(_this10, function (error, res) {
        if (error) {
          return cb(error);
        }

        _this10._hooks.execPost('exec', _this10, [], {}, function (error) {
          if (error) {
            return cb(error);
          }
...
function _wrapThunkCallback(query, cb) {
  return function (error, res) {
    ...
    try {
      for (var _iterator = query._transforms[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var fn = _step.value;
...
Query.prototype._castUpdate = function _castUpdate(obj, overwrite) {
  ...
  if (schema != null && utils.hasUserDefinedProperty(filter, schema.options.discriminatorKey) && _typeof(filter[schema.options.discriminatorKey]) !== 'object' && schema.discriminators != null) {
    var discriminatorValue = filter[schema.options.discriminatorKey];
    var byValue = getDiscriminatorByValue(this.model, discriminatorValue);
...
Query.prototype.populate = function () {
  ...
  var res = utils.populate.apply(null, arguments); // Propagate readConcern and readPreference and lean from parent query,
  // unless one already specified

  if (this.options != null) {
    var readConcern = this.options.readConcern;
    var readPref = this.options.readPreference;
  ...
  if (opts.lean != null) {
    ...
    for (var _i2 = 0; _i2 < res.length; ++_i2) {
      if (get(res[_i2], 'options.lean') == null) {
        res[_i2].options = res[_i2].options || {};
        res[_i2].options.lean = lean;
      }
    }
  }
  ...
  for (var _i3 = 0; _i3 < res.length; ++_i3) {
    var path = res[_i3].path;

    if (pop[path] && pop[path].populate && res[_i3].populate) {
      res[_i3].populate = pop[path].populate.concat(res[_i3].populate);
    }

    pop[res[_i3].path] = res[_i3];
  }
...
Query.prototype._applyPaths = function applyPaths() {
  ...
  var _selectPopulatedPaths = true;
...
Query.prototype.cursor = function cursor(opts) {
  ...
  var options = Object.assign({}, this._optionsForExec(), {
...
Query.prototype.near = function () {
  var params = [];
  var sphere = this._mongooseOptions.nearSphere; // TODO refactor
...
if (Symbol.asyncIterator != null) {
  Query.prototype[Symbol.asyncIterator] = function () {
    return this.cursor().transformNull().map(function (doc) {
      return doc == null ? {
        done: true
      } : {
        value: doc,
