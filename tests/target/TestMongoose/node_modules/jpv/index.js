...
var compareCommon = function compareCommon(value, pattern) {
  /*
  * Special for debugging
  * */
  var res = function res(result) {
    if (!result) {
  ...
  */
  ...
  if (typeof pattern === 'string') {
    // Native Types
    var nativeMatches = pattern.match(/^(\!)?\((.*)\)(\?)?$/i);

    if (nativeMatches !== null) {
      var match = value === null ? nativeMatches[2] === 'null' : _typeof(value) === nativeMatches[2]; // Negation ? Operator
    ...
    var logicalMatches = pattern.match(/^(\!)?\[(.*)\](\?)?$/i);

    if (logicalMatches !== null && logicalMatches[2] in expressions) {
      var _match = String(value).match(expressions[logicalMatches[2]]) !== null; // Negation ? Operator
      ...
      return res(logicalMatches[1] === '!' ? !_match : _match);
    } // Functional Regex


    var functionalRegexMatches = pattern.match(/^(\!)?\{\/(.*)\/([a-z]*)\}(\?)?$/i);

    if (functionalRegexMatches !== null) {
      var _match2 = String(value).match(new RegExp(functionalRegexMatches[2], functionalRegexMatches[3])) !== null; // Negation ? Operator
      ...
      return res(functionalRegexMatches[1] === '!' ? !_match2 : _match2);
    } // Functional Fixed


    var functionalFixedMatches = pattern.match(/^(\!)?\{(.*)\}(\?)?$/i);

    if (functionalFixedMatches !== null) {
      var _match3 = String(value) === String(functionalFixedMatches[2]); // Negation ? Operator
      ...
      return res(functionalFixedMatches[1] === '!' ? !_match3 : _match3);
    } // Fixed Type
  ...
  if (typeof pattern === 'number' || _typeof(pattern) === 'symbol' || typeof pattern === 'boolean') {
    return res(pattern === value);
  } // pattern = object


  if (_typeof(pattern) === 'object') {
    if (_typeof(pattern) !== _typeof(value)) {
      return res(false);
    }
  ...
  throw 'error: don`t use ' + _typeof(pattern) + ' as a pattern value, insted of it use string format';
};
/**
 * Compare in standard
...
var compareStandard = function compareStandard(value, pattern) {
...
var compareStrict = function compareStrict(value, pattern) {
  // when pattern is undefined
  if (typeof pattern === 'undefined') {
    log("The value of [" + JSON.stringify(value) + "] does not match with [" + JSON.stringify(pattern) + "]");
    return false;
...
var compareExistence = function compareExistence(obj1, obj2) {
  // when no object for pattern
  if (typeof obj2 === 'undefined') {
    if (typeof obj1 === 'string') {
      if (obj1.match(/^(\!)?\[(.*)\]\?$/i) !== null || obj1.match(/^(\!)?\((.*)\)\?$/i) !== null || obj1.match(/^(\!)?\{(\/.*\/[a-z]*)\}\?$/i) !== null || obj1.match(/^(\!)?\{(.*)\}\?$/i) !== null) {
        return true;
      }
    }
...
var iterate = function iterate(obj1, obj2, valid, cb) {
  /*
  * Already not valid
  * */
...
var standardValidate = function standardValidate(json, pattern) {
...
var strictValidate = function strictValidate(json, pattern) {
...
module.exports = {
   ...
   */
  validate: function validate(json, pattern, option) {
    // when strict is set
    if (option !== null) {
