...
function (_OperationBase) {
  ...
  function CommandOperationV2(parent, options, operationOptions) {
    ...
    _this2 = _possibleConstructorReturn(this, _getPrototypeOf(CommandOperationV2).call(this, options));
    _this2.ns = parent.s.namespace.withCollection('$cmd');
    _this2.readPreference = resolveReadPreference(parent, _this2.options);
    _this2.readConcern = resolveReadConcern(parent, _this2.options);
    _this2.writeConcern = resolveWriteConcern(parent, _this2.options);
    _this2.explain = false;

    if (operationOptions && typeof operationOptions.fullResponse === 'boolean') {
      _this2.fullResponse = true;
    } // TODO: A lot of our code depends on having the read preference in the options. This should
    //       go away, but also requires massive test rewrites.


    _this2.options.readPreference = _this2.readPreference; // TODO(NODE-2056): make logger another "inheritable" property

    if (parent.s.logger) {
      _this2.logger = parent.s.logger;
    } else if (parent.s.db && parent.s.db.logger) {
      _this2.logger = parent.s.db.logger;
    }
  ...
  _createClass(CommandOperationV2, [{
    ...
    value: function executeCommand(server, cmd, callback) {
      var _this = this; // TODO: consider making this a non-enumerable property
