...
function (_Readable) {
  ...
  function CoreCursor(topology, ns, cmd, options) {
    ...
    _this3 = _possibleConstructorReturn(this, _getPrototypeOf(CoreCursor).call(this, {
      objectMode: true
    }));
    options = options || {};
    ...
    if (ns instanceof OperationBase) {
      _this3.operation = ns;
      ns = _this3.operation.ns.toString();
      options = _this3.operation.options;
      cmd = _this3.operation.cmd ? _this3.operation.cmd : {};
    } // Cursor pool


    _this3.pool = null; // Cursor server

    _this3.server = null; // Do we have a not connected handler

    _this3.disconnectHandler = options.disconnectHandler; // Set local values

    _this3.bson = topology.s.bson;
    _this3.ns = ns;
    _this3.namespace = MongoDBNamespace.fromString(ns);
    _this3.cmd = cmd;
    _this3.options = options;
    _this3.topology = topology; // All internal state
    ...
    if (_typeof(options.session) === 'object') {
      _this3.cursorState.session = options.session;
    ...
    if (typeof topologyOptions.promoteLongs === 'boolean') {
      _this3.cursorState.promoteLongs = topologyOptions.promoteLongs;
    } else if (typeof options.promoteLongs === 'boolean') {
      _this3.cursorState.promoteLongs = options.promoteLongs;
    } // Add promoteValues to cursor state


    if (typeof topologyOptions.promoteValues === 'boolean') {
      _this3.cursorState.promoteValues = topologyOptions.promoteValues;
    } else if (typeof options.promoteValues === 'boolean') {
      _this3.cursorState.promoteValues = options.promoteValues;
    } // Add promoteBuffers to cursor state


    if (typeof topologyOptions.promoteBuffers === 'boolean') {
      _this3.cursorState.promoteBuffers = topologyOptions.promoteBuffers;
    } else if (typeof options.promoteBuffers === 'boolean') {
      _this3.cursorState.promoteBuffers = options.promoteBuffers;
    }

    if (topologyOptions.reconnect) {
      _this3.cursorState.reconnect = topologyOptions.reconnect;
    } // Logger


    _this3.logger = Logger('Cursor', topologyOptions); //
    // Did we pass in a cursor id

    if (typeof cmd === 'number') {
      _this3.cursorState.cursorId = Long.fromNumber(cmd);
      _this3.cursorState.lastCursorId = _this3.cursorState.cursorId;
    } else if (cmd instanceof Long) {
      _this3.cursorState.cursorId = cmd;
      _this3.cursorState.lastCursorId = cmd;
    } // TODO: remove as part of NODE-2104


    if (_this3.operation) {
      _this3.operation.cursorState = _this3.cursorState;
    }
  ...
  _createClass(CoreCursor, [{
    ...
    value: function _read() {
      ...
      this._next(function (err, result) {
        if (err) {
          if (_this.listeners('error') && _this.listeners('error').length > 0) {
            _this.emit('error', err);
          }

          if (!_this.isDead()) _this.close(); // Emit end event
        ...
        if (_this.cursorState.streamOptions && typeof _this.cursorState.streamOptions.transform === 'function' && result != null) {
          return _this.push(_this.cursorState.streamOptions.transform(result));
        } // If we provided a map function


        if (_this.cursorState.transforms && typeof _this.cursorState.transforms.doc === 'function' && result != null) {
          return _this.push(_this.cursorState.transforms.doc(result));
        } // Return the result


        _this.push(result);

        if (result === null && _this.isDead()) {
    ...
    value: function _initializeCursor(callback) {
      ...
      if (isUnifiedTopology(cursor.topology) && cursor.topology.shouldCheckForSessionSupport()) {
        cursor.topology.selectServer(ReadPreference.primaryPreferred, function (err) {
          ...
          _this2._initializeCursor(callback);
        });
        return;
      }
