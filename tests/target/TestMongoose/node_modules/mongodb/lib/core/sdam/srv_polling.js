...
function (_EventEmitter) {
  ...
  function SrvPoller(options) {
    ...
    _this3 = _possibleConstructorReturn(this, _getPrototypeOf(SrvPoller).call(this));

    if (!options || !options.srvHost) {
      throw new TypeError('options for SrvPoller must exist and include srvHost');
    }

    _this3.srvHost = options.srvHost;
    _this3.rescanSrvIntervalMS = 60000;
    _this3.heartbeatFrequencyMS = options.heartbeatFrequencyMS || 10000;
    _this3.logger = Logger('srvPoller', options);
    _this3.haMode = false;
    _this3.generation = 0;
  ...
  _createClass(SrvPoller, [{
    ...
    value: function _poll() {
      ...
      dns.resolveSrv(this.srvAddress, function (err, srvRecords) {
        if (generation !== _this2.generation) {
          return;
        }

        if (err) {
          _this2.failure('DNS error', err);
        ...
        srvRecords.forEach(function (record) {
          if (matchesParentDomain(record.name, _this2.srvHost)) {
            finalAddresses.push(record);
          } else {
            _this2.parentDomainMismatch(record);
          }
        });

        if (!finalAddresses.length) {
          _this2.failure('No valid addresses found at host');

          return;
        }

        _this2.success(finalAddresses);
      });
    }
  }, {
    key: "srvAddress",
