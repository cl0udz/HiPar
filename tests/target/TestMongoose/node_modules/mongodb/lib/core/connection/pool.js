...
function connectionFailureHandler(pool, event, err, conn) {
  if (conn) {
    ...
    if (pool.state !== DRAINING && pool.state !== DESTROYED && pool.options.legacyCompatMode === false) {
      ...
      pool.emit('drain', err); // wait to flush work items so this server isn't selected again immediately

      process.nextTick(function () {
        return conn.flush(err);
      });
      return;
...
function createConnection(pool, callback) {
  ...
  connect(pool.options, pool.s.cancellationToken, function (err, connection) {
    pool.connectingConnections--;
