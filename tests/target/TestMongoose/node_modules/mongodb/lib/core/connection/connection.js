...
function (_EventEmitter) {
  ...
  function Connection(socket, options) {
    ...
    _this3 = _possibleConstructorReturn(this, _getPrototypeOf(Connection).call(this));
    options = options || {};
    ...
    _this3.logger = Logger('Connection', options);
    _this3.bson = options.bson;
    _this3.tag = options.tag;
    _this3.maxBsonMessageSize = options.maxBsonMessageSize || DEFAULT_MAX_BSON_MESSAGE_SIZE;
    _this3.port = options.port || 27017;
    _this3.host = options.host || 'localhost';
    _this3.socketTimeout = typeof options.socketTimeout === 'number' ? options.socketTimeout : 360000; // These values are inspected directly in tests, but maybe not necessary to keep around

    _this3.keepAlive = typeof options.keepAlive === 'boolean' ? options.keepAlive : true;
    _this3.keepAliveInitialDelay = typeof options.keepAliveInitialDelay === 'number' ? options.keepAliveInitialDelay : 300000;
    _this3.connectionTimeout = typeof options.connectionTimeout === 'number' ? options.connectionTimeout : 30000;

    if (_this3.keepAliveInitialDelay > _this3.socketTimeout) {
      _this3.keepAliveInitialDelay = Math.round(_this3.socketTimeout / 2);
    } // Debug information


    if (_this3.logger.isDebug()) {
      _this3.logger.debug("creating connection ".concat(_this3.id, " with options [").concat(JSON.stringify(debugOptions(DEBUG_FIELDS, options)), "]"));
    } // Response options


    _this3.responseOptions = {
    ...
    _this3.queue = []; // Internal state

    _this3.writeStream = null;
    _this3.destroyed = false;
    _this3.timedOut = false; // Create hash method

    var hash = crypto.createHash('sha1');
    hash.update(_this3.address);
    _this3.hashedName = hash.digest('hex'); // All operations in flight on the connection

    _this3.workItems = []; // setup socket

    _this3.socket = socket;

    _this3.socket.once('error', errorHandler(_assertThisInitialized(_this3)));

    _this3.socket.once('timeout', timeoutHandler(_assertThisInitialized(_this3)));

    _this3.socket.once('close', closeHandler(_assertThisInitialized(_this3)));

    _this3.socket.on('data', dataHandler(_assertThisInitialized(_this3)));

    if (connectionAccounting) {
      addConnection(_this3.id, _assertThisInitialized(_this3));
    }
