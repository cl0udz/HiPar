...
var commands = ['add', 'append', 'apply', 'batch', 'exists', 'geospatialMonteCarlo', 'get', 'info', 'operate', 'put', 'query', 'remove', 'scan', 'sindex', 'udf'];
...
.option('verbose', {
  ...
  "default": false
}).example('put --update joe name=Joe age=42', 'Updates an existing record under the key "joe"; fails if the record does not exist.').example('info version', 'Query a random cluster node for it\'s build version').example('query --background --range x 1 5 --udf scan increment 2', 'Run a background query over the records in the specified range (1 <= x < 5) and apply the scan:increment UDF on the records').commandDir('./', {
  include: function include(filename) {
    return commands.some(function (cmd) {
      return cmd === path.basename(filename, '.js');
    });
  }
}).demandCommand().strict(true).help().version(VERSION).env('AEROSPIKE').parse();