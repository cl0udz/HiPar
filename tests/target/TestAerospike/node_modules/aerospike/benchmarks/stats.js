...
var OPERATION_TIME_DIFF = 1; // histograms
...
function timeHistogram(operations) {
  operations.map(function (op) {
    var elapsed = op[OPERATION_TIME_DIFF];
...
function printConfigTable(config, print, prefix) {
  ...
  configTable.push({
    iterations: config.iterations === undefined ? 'undefined' : config.iterations
  });
  configTable.push({
  ...
  configTable.push({
    time: config.time === undefined ? 'undefined' : timeUnits(config.time)
  });
...
function printTransactions(transactions, print, prefix) {
  ...
  var columns = Object.keys(transactions).map(function (key) {
    return transactions[key];
  });
  var totals = columns.map(function (col) {
    return numberFormat(col.count, 0);
  });
  table.push({
    Total: totals
  });
  var avgTps = columns.map(function (col) {
    return numberFormat(col.tps, 0);
  });
  table.push({
    TPS: avgTps
  });
  var minTps = columns.map(function (col) {
    return numberFormat(col.min_tps, 0);
  });
  table.push({
    'Min TPS': minTps
  });
  var maxTps = columns.map(function (col) {
    return numberFormat(col.max_tps, 0);
  });
  table.push({
    'Max TPS': maxTps
  });
...
function aggregateIntervalStats(statName, tx) {
  var stats = trans[statName] = trans[statName] || {
    count: 0,
    max_tps: 0,
    min_tps: Infinity
  };
