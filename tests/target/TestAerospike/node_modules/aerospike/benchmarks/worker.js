...
argv.ttl = stats.parseTimeToSecs(argv.ttl); // variables to track memory growth(RSS) of worker process.
...
function getPromise(key, done) {
  var timeStart = process.hrtime();
  client.get(key).then(function () {
    return done(0, process.hrtime(timeStart), READ);
  })["catch"](function (err) {
    return done(err.code, process.hrtime(timeStart), READ);
  });
...
function putPromise(options, done) {
  var timeStart = process.hrtime();
  client.put(options.key, options.record, metadata).then(function () {
    return done(0, process.hrtime(timeStart), WRITE);
  })["catch"](function (err) {
    return done(err.code, process.hrtime(timeStart), WRITE);
  });
...
function run(options) {
  ...
  var completed = 0; // @ TO-DO optimization.
...
var runLongRunningJob = function runLongRunningJob(options) {
...
var monitorMemory = function monitorMemory() {
