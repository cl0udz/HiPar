...
function printIntervalStats() {
  if (rwWorkers > 0) {
    logger.info('%s read(tps=%d timeouts=%d errors=%d) write(tps=%d timeouts=%d errors=%d) ', new Date().toString(), intervalStats[0][0], intervalStats[0][1], intervalStats[0][2], intervalStats[1][0], intervalStats[1][1], intervalStats[1][2]);
  }

  if (queryWorkers) {
    logger.info('%s query(records = %d timeouts = %d errors = %d)', new Date().toString(), intervalStats[2][0], intervalStats[2][1], intervalStats[2][2]);
  }

  if (scanWorkers) {
    logger.info('%s scan(records = %d timeouts = %d errors = %d)', new Date().toString(), intervalStats[3][0], intervalStats[3][1], intervalStats[3][2]);
  }
}
...
if (!argv.silent) {
  logger.info('namespace: %s, set: %s, worker processes: %s, keys: %s, read: %s%%, write: %s%%, promises: %s', argv.namespace, argv.set, argv.processes, keyrange, ROPSPCT, WOPSPCT, argv.promises);
}
/**
