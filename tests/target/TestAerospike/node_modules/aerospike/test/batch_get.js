...
describe('client.batchGet()', function () {
  ...
  it('should successfully read 10 records', function () {
    ...
    return putgen.put(numberOfRecords, kgen, rgen, mgen).then(function (records) {
      ...
      return client.batchGet(keys).then(function (results) {
        expect(results.length).to.equal(numberOfRecords);
        results.forEach(function (result) {
          var putRecord = records.find(function (record) {
            return record.key.key === result.record.key.key;
          });
  ...
  it('returns an empty array when no keys are passed', function () {
    client.batchGet([]).then(function (results) {
