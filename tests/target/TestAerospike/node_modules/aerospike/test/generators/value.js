...
var Double = Aerospike.Double; // Returns a random integer between min (included) and max (excluded)
...
function _double(options) {
  var opt = merge(_double.defaults, options);
  var seq = opt.min;
  var step = opt.step;
...
function array(options) {
  ...
  return function () {
    return opt.values.map(function (gen) {
      return gen();
    });
  };
}
