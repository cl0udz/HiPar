...
var perfdata = require('./perfdata');

var fs = require('fs');

var mega = 1024 * 1024; // bytes in a MB
...
describe('client.scan()', function () {
  ...
  function executeScan(onData, done) {
    ...
    var timer = perfdata.interval(reportingInterval, function (ms) {
      ...
      console.log('%d ms: %d records received (%d rps; mem: %d MB, heap: %d / %d MB)', ms, received, throughput, rss, heapUsed, heapTotal);
    });
  ...
  it('scans ' + numberOfRecords + ' records with busy loop in WebWorker', function (done) {
    ...
    var timer = perfdata.interval(reportingInterval, function (ms) {
      ...
      var heapUsed = Math.round(memUsage.heapUsed / mega);
      var heapTotal = Math.round(memUsage.heapTotal / mega);
      console.log('%d ms: %d records processed (%d rps; mem: %d MB, heap: %d / %d MB)', ms, processed, throughput, rss, heapUsed, heapTotal);
    });

    var worker = function worker(record, meta, key) {
  ...
  it('scans ' + numberOfRecords + ' records with file IO', function (done) {
    ...
    var fileAppend = function fileAppend(record) {
