...
var NUMERIC = Aerospike.indexDataType.NUMERIC;
var STRING = Aerospike.indexDataType.STRING;
var GEO2DSPHERE = Aerospike.indexDataType.GEO2DSPHERE;
var LIST = Aerospike.indexType.LIST;
var MAPVALUES = Aerospike.indexType.MAPVALUES;
var MAPKEYS = Aerospike.indexType.MAPKEYS;
...
describe('Queries', function () {
  ...
  var samples = [{
    ...
    name: 'point map match',
    mg: {
      a: GeoJSON.Point(103.913, 1.308),
      b: GeoJSON.Point(105.913, 3.308)
    }
  }, {
    name: 'point map non-match',
    mg: {
      a: GeoJSON.Point(-122.101, 37.421),
      b: GeoJSON.Point(-120.101, 39.421)
    }
  }, {
    name: 'region match',
    ...
    name: 'region list match',
    lg: [GeoJSON.Polygon([102.913, 0.308], [102.913, 2.308], [104.913, 2.308], [104.913, 0.308], [102.913, 0.308])]
  }, {
    name: 'region list non-match',
    lg: [GeoJSON.Polygon([-121.101, 36.421], [-121.101, 38.421], [-123.101, 38.421], [-123.101, 36.421], [-121.101, 36.421])]
  }, {
    name: 'region map match',
    mg: {
      a: GeoJSON.Polygon([102.913, 0.308], [102.913, 2.308], [104.913, 2.308], [104.913, 0.308], [102.913, 0.308])
    }
  }, {
    name: 'region map non-match',
    mg: [GeoJSON.Polygon([-121.101, 36.421], [-121.101, 38.421], [-123.101, 38.421], [-123.101, 36.421], [-121.101, 36.421])]
  }, {
    name: 'aggregate',
    value: 10
  }, {
  ...
  var indexes = [['qidxName', 'name', STRING], ['qidxInt', 'i', NUMERIC], ['qidxIntList', 'li', NUMERIC, LIST], ['qidxIntMap', 'mi', NUMERIC, MAPVALUES], ['qidxStr', 's', STRING], ['qidxStrList', 'ls', STRING, LIST], ['qidxStrMap', 'ms', STRING, MAPVALUES], ['qidxStrMapKeys', 'mks', STRING, MAPKEYS], ['qidxGeo', 'g', GEO2DSPHERE], ['qidxGeoList', 'lg', GEO2DSPHERE, LIST], ['qidxGeoMap', 'mg', GEO2DSPHERE, MAPVALUES]];
  ...
  before(function () {
    var sampleGen = function sampleGen() {
      return samples.pop();
    };
    ...
    return Promise.all([putgen.put(numberOfSamples, kgen, sampleGen, mgen).then(function () {
      return Promise.all(indexes.map(function (idx) {
        return helper.index.create(idx[0], testSet, idx[1], idx[2], idx[3]);
      }));
  ...
  after(function () {
    return helper.udf.remove('udf.lua').then(function () {
      return Promise.all(indexes.map(function (idx) {
        return helper.index.remove(idx[0]);
      }));
    });
  });
  describe('client.query()', function () {
    ...
    it('creates a query without specifying the set', function () {
      var namespace = 'test';
      var query = client.query(namespace, {
        select: ['i']
      });
      expect(query).to.be["instanceof"](Query);
      expect(query.ns).to.equal('test');
  ...
  describe('query.foreach()', function () {
    it('should apply a stream UDF to filter the results', function (done) {
      ...
      stream.on('data', function (record) {
        return results.push(record.bins);
      });
      stream.on('end', function () {
    ...
    it('should raise client errors asynchronously', function (done) {
      var invalidPolicy = new Aerospike.QueryPolicy({
        totalTimeout: 'not a valid timeout'
      });
      var query = client.query('test');
      var stream = query.foreach(invalidPolicy); // if error is raised synchronously we will never reach here
    ...
    context('filter predicates', function () {
      ...
      describe('filter.geoWithinGeoJSONRegion()', function () {
        it('should match locations within a GeoJSON region', function (done) {
          var region = new GeoJSON({
            type: 'Polygon',
            coordinates: [[[103, 1.3], [104, 1.3], [104, 1.4], [103, 1.4], [103, 1.3]]]
          });
          var args = {
            filters: [filter.geoWithinGeoJSONRegion('g', region)]
          };
        ...
        it('should match locations in a list within a GeoJSON region', function (done) {
          ...
          var args = {
            filters: [filter.geoWithinGeoJSONRegion('lg', region, LIST)]
          };
        ...
        it('should match locations in a map within a GeoJSON region', function (done) {
          ...
          var args = {
            filters: [filter.geoWithinGeoJSONRegion('mg', region, MAPVALUES)]
          };
      ...
      describe('filter.geoWithinRadius()', function () {
        it('should match locations within a radius from another location', function (done) {
          var args = {
            filters: [filter.geoWithinRadius('g', 103.9135, 1.3085, 15000)]
          };
          verifyQueryResults(args, 'point match', done);
        });
        it('should match locations in a list within a radius from another location', function (done) {
          var args = {
            filters: [filter.geoWithinRadius('lg', 103.9135, 1.3085, 15000, LIST)]
          };
          verifyQueryResults(args, 'point list match', done);
        });
        it('should match locations in a map within a radius from another location', function (done) {
          var args = {
            filters: [filter.geoWithinRadius('mg', 103.9135, 1.3085, 15000, MAPVALUES)]
          };
          verifyQueryResults(args, 'point map match', done);
        });
      });
      describe('filter.geoContainsGeoJSONPoint()', function () {
        it('should match regions that contain a GeoJSON point', function (done) {
          ...
          var args = {
            filters: [filter.geoContainsGeoJSONPoint('g', point)]
          };
        ...
        it('should match regions in a list that contain a GeoJSON point', function (done) {
          ...
          var args = {
            filters: [filter.geoContainsGeoJSONPoint('lg', point, LIST)]
          };
        ...
        it('should match regions in a map that contain a GeoJSON point', function (done) {
          ...
          var args = {
            filters: [filter.geoContainsGeoJSONPoint('mg', point, MAPVALUES)]
          };
  ...
  describe('query.apply()', function () {
    ...
    it('returns a Promise that resolves to the result of the aggregation', function () {
      ...
      var query = client.query(helper.namespace, testSet, args);
      return query.apply('udf', 'count').then(function (result) {
        expect(result).to.equal(3);
      });
    });
  });
  describe('query.background()', function () {
    ...
    it('returns a Promise that resolves to a Job', function () {
      ...
      var query = client.query(helper.namespace, testSet, args);
      return query.background('udf', 'noop').then(function (job) {
        expect(job).to.be["instanceof"](Job);
      });
    });
  });
  ...
  context('legacy scan interface', function () {
    ...
    ['UDF', 'concurrent', 'percentage', 'priority'].forEach(function (key) {
      it('should throw an exception if the query options contain key "' + key + '"', function () {
        ...
        args[key] = 'foo';
        expect(function () {
          return client.query(helper.namespace, testSet, args);
        }).to["throw"]('Invalid query arguments');
      });
    });
  });
});