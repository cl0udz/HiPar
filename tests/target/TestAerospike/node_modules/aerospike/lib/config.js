...
function () {
  ...
  function Config(config) {
    ...
    this.hosts = config.hosts || process.env.AEROSPIKE_HOSTS || "localhost:".concat(this.port);
    /**
    ...
    if (_typeof(config.policies) === 'object') {
    ...
    if (_typeof(config.modlua) === 'object') {
      /**
    ...
    if (_typeof(config.sharedMemory) === 'object') {
  ...
  _createClass(Config, [{
    key: "setDefaultPolicies",
    value: function setDefaultPolicies(policies) {
      for (var type in policies) {
        var values = policies[type];
