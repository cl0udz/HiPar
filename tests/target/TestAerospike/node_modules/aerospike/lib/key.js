...
Key.prototype.equals = function (other) {
  return this.ns === other.ns && (!isSet(this.set) && !isSet(other.set) || this.set === other.set) && (!isSet(this.key) && !isSet(other.key) || this.key === other.key) && (!isSet(this.digest) || !isSet(other.digest) || this.digest.equals(other.digest));
};
...
function isValidNamespace(ns) {
  return typeof ns === 'string' && ns.length > 0 && ns.length <= 32;
}

function isValidSetName(set) {
  return typeof set === 'string' && set.length > 0 && set.length <= 64;
}

function isValidKey(key) {
  return typeof key === 'string' && key.length > 0 || Number.isInteger(key) || Buffer.isBuffer(key) && Buffer.byteLength(key) > 0;
}
