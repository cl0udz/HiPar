...
function attemptReconnect(self) {
  return function () {
    ...
    function _connectHandler(self) {
      return function () {
        ...
        reauthenticate(self, this, function () {
          ...
          self.availableConnections.push(connection); // Emit reconnect event
    ...
    var connection = new Connection(messageHandler(self), self.options); // Add handlers
    ...
    connection.on('parseError', _connectionFailureHandler(self, 'parseError')); // On connection

    connection.on('connect', _connectHandler(self)); // Attempt connection
...
Pool.prototype.connect = function () {
  ...
  connection.once('connect', function (connection) {
    if (self.state == DESTROYED || self.state == DESTROYING) return self.destroy(); // Apply any store credentials
        ...
        if (connection.isConnected()) {
          ...
          if (workItem.monitoring) {
            ...
            if (!foundValidConnection) {
              ...
              self.queue.unshift(workItem); // // Flush any monitoring operations in the queue, failing fast
              // flushMonitoringOperations(self.queue);
