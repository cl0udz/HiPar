...
var BSON = retrieveBSON(),
    Long = BSON.Long; // Write concern fields
...
var hasWriteConcern = function hasWriteConcern(writeConcern) {
...
var cloneWriteConcern = function cloneWriteConcern(writeConcern) {
  var wc = {};
...
var aggregateWriteOperationResults = function aggregateWriteOperationResults(opType, ops, results, connection) {
  var finalResult = {
    ok: 1,
    n: 0
  };
...
var executeOrdered = function executeOrdered(opType, command, ismaster, ns, bson, pool, ops, options, callback) {
  var _ops = ops.slice(0); // Collect all the getLastErrors


  var getLastErrors = []; // Execute an operation

  var executeOp = function executeOp(list, _callback) {
    ...
    try {
      ...
      var commands = [op]; // Add getLastOrdered
      ...
      commands.push(getLastErrorOp); // getLastError callback

      var getLastErrorCallback = function getLastErrorCallback(err, result) {
        if (err) return callback(err); // Get the document

        var doc = result.result; // Save the getLastError document
...
var executeUnordered = function executeUnordered(opType, command, ismaster, ns, bson, pool, ops, options, callback) {
  ...
  for (var i = 0; i < ops.length; i++) {
    ...
    try {
      // Add binary message to list of commands to execute
      var commands = [op]; // If write concern 0 don't fire getLastError
      ...
      if (hasWriteConcern(writeConcern)) {
        ...
        commands.push(getLastErrorOp); // Give the result from getLastError the right index

        var callbackOp = function callbackOp(_index) {
          return function (err, result) {
            if (err) error = err; // Update the number of operations executed

            totalOps = totalOps - 1; // Save the getLastError document

            if (!err) getLastErrors[_index] = result.result; // Check if we are done
  ...
  if (writeConcern && writeConcern.w == 0 && callback) {
    callback(null, new CommandResult({
      ok: 1
    }, null));
  }
};
