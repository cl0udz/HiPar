...
var executeWrite = function executeWrite(pool, bson, type, opsField, ns, ops, options, callback) {
  ...
  if (type == 'insert') queryOptions.checkKeys = true; // Ensure we support serialization of functions
...
var executeFindCommand = function executeFindCommand(bson, ns, cmd, cursorState, topology, options) {
  ...
  if (cmd.limit) findCmd.limit = cmd.limit; // Add a batchSize

  if (typeof cmd.batchSize == 'number') findCmd.batchSize = Math.abs(cmd.batchSize); // Check if we wish to have a singleBatch
