...
function connectNewServers(self, servers, callback) {
  // Count lefts
  var count = servers.length; // Handle events
      ...
      if (event == 'connect' && !self.authenticating) {
        ...
        applyAuthenticationContexts(self, _self, function () {
          ...
          if (result) {
            ...
            _self.on('parseError', handleEvent(self, 'parseError')); // Rexecute any stalled operation
...
function topologyMonitor(self, options) {
  ...
  var _haInterval = options.haInterval ? options.haInterval : self.s.haInterval; // Count of initial sweep servers to check


  var count = servers.length; // Each server is monitored in parallel in their own timeout loop

  var monitorServer = function monitorServer(_host, _self, _options) {
    var intervalId = _process(function () {
      ...
      count = count - 1; // Do we already have server connection available for this host

      var _server = _self.s.replicaSetState.get(_host); // Check if we have a known server connection and reuse
      ...
      if (_server) {
        return pingServer(_self, _server, function (err) {
          ...
          if (_process === setTimeout) {
            if (_self.state == CONNECTING && (self.s.replicaSetState.hasSecondary() && self.s.options.secondaryOnlyConnectionAllowed || self.s.replicaSetState.hasPrimary())) {
              _self.state = CONNECTED; // Emit connected sign
            ...
            if (_self.state == DISCONNECTED && (self.s.replicaSetState.hasSecondary() && self.s.options.secondaryOnlyConnectionAllowed || self.s.replicaSetState.hasPrimary())) {
              _self.state = CONNECTED; // Rexecute any stalled operation
    ...
    }, _haInterval); // Add the intervalId to our list of intervalIds
    ...
    if (event == 'connect') {
      ...
      applyAuthenticationContexts(self, _this, function () {
        ...
        if (result == true) {
          ...
          if (self.state === CONNECTING && self.s.replicaSetState.hasPrimary() || self.s.replicaSetState.hasSecondary() && self.s.options.secondaryOnlyConnectionAllowed) {
            // We are connected
