...
exports['large stream - callback return true'] = function (test) {
  var fs = from(function (i, next) {
    this.emit('data', i);
    if (i >= 99999) return this.emit('end');
    return true;
  });
  spec(fs).readable().validateOnExit();
  read(fs, function (err, arr) {
...
exports['simple (not strictly pausable) setTimeout'] = function (test) {
  ...
  var t = from(function (i, n) {
    var self = this;
    setTimeout(function () {
      if (_expected.length) self.emit('data', _expected.shift());else if (!self.ended) self.emit('end');
      n();
    }, 3);
  });
  /*
  ...
  */
  ...
  spec(t).readable().pausable({
    strict: false
  }).validateOnExit(); //pause(t)

  var paused = false;
  var i = setInterval(function () {
    if (!paused) t.pause();else t.resume();
    paused = !paused;
  }, 2);
  t.on('end', function () {
    clearInterval(i);
