...
UI.prototype._applyLayoutDSL = function (str) {
  var _this = this,
      rows = str.split('\n'),
      leftColumnWidth = 0; // simple heuristic for layout, make sure the
...
UI.prototype.rowToString = function (row, lines) {
  ...
  rrows.forEach(function (rrow, r) {
    ...
    rrow.forEach(function (col, c) {
      ...
      str += ts;
      if (row[c].padding && row[c].padding[right]) str += new Array(row[c].padding[right] + 1).join(' '); // if prior row is span, try to render the
...
UI.prototype._renderInline = function (source, previousLine, paddingLeft) {
  ...
  for (var i = 0, tc, sc; i < Math.max(source.length, target.length); i++) {
    tc = target.charAt(i) || ' ';
    sc = source.charAt(i) || ' '; // we tried to overwrite a character in the other string.

    if (tc !== ' ' && sc !== ' ') return source; // there is not enough whitespace to maintain padding.

    if (sc !== ' ' && i < paddingLeft + target.length) return source; // :thumbsup:

    if (tc === ' ') str += sc;else str += tc;
  }
...
UI.prototype._rasterize = function (row) {
  var _this = this,
      ...
      wrapped; // word wrap all columns, and create
...
UI.prototype._columnWidths = function (row) {
  var _this = this,
      ...
      unset = row.length,
      unsetWidth,
      remainingWidth = this.width; // column widths can be set in config.
  ...
  widths.forEach(function (w, i) {
    if (!_this.wrap) widths[i] = row[i].width || row[i].text.length;else if (w === undefined) widths[i] = Math.max(unsetWidth, _minWidth(row[i]));
  });
  return widths;
