...
var mockCars = ['rsx', 'tsx', 'civic', 'integra'];
describe('Test Multipart Form Single Field Submissions', function () {
  it('submit multipart user data with POST', function (done) {
    request(app).post('/fields/user').field('firstName', mockUser.firstName).field('lastName', mockUser.lastName).field('email', mockUser.email).expect('Content-Type', /json/).expect(200, {
...
describe('Test Multipart Form Array Field Submissions', function () {
  it('submit array of data with POST', function (done) {
    ...
    req.expect(200).end(function (err, res) {
      ...
      var responseMatchesRequest = res.body.join(',') === mockCars.join(',');
