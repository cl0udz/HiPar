...
TokenParser.prototype = {
   ...
   * @private
   */
  parseVar: function parseVar(token, match, lastState) {
    var self = this;
    match = match.split('.');
   ...
   * @private
   */
  checkMatch: function checkMatch(match) {
    var temp = match[0];
    var result;
...
exports.parse = function (swig, source, opts, tags, filters) {
  ...
  var tagStrip = new RegExp('^' + escapedTagOpen + '-?\\s*-?|-?\\s*-?' + escapedTagClose + '$', 'g');
  var tagStripBefore = new RegExp('^' + escapedTagOpen + '-');
  var tagStripAfter = new RegExp('-' + escapedTagClose + '$');
  var varStrip = new RegExp('^' + escapedVarOpen + '-?\\s*-?|-?\\s*-?' + escapedVarClose + '$', 'g');
  ...
  var anyChar = '[\\s\\S]*?'; // Split the template source based on variable, tag, and comment blocks
  // /(\{%[\s\S]*?%\}|\{\{[\s\S]*?\}\}|\{#[\s\S]*?#\})/

  var splitter = new RegExp('(' + escapedTagOpen + anyChar + escapedTagClose + '|' + escapedVarOpen + anyChar + escapedVarClose + '|' + escapeRegExp(cmtOpen) + anyChar + escapeRegExp(cmtClose) + ')');
  var line = 1;
  var stack = [];
  var parent = null;
  var tokens = [];
