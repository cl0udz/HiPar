...
function makeExamplePipe() {
  return es.connect(es.map(function (data, callback) {
...
exports['read array then map'] = function (test) {
  ...
  ,
      ...
      read = [],
      pipe = es.connect(first, es.map(function (data, callback) {
    callback(null, {
      data: data
    });
  }), es.map(function (data, callback) {
...
exports['connect returns a stream'] = function (test) {
  var rw = es.connect(es.map(function (data, callback) {
    callback(null, data * 2);
  }), es.map(function (data, callback) {
  ...
  var array = [190, 24, 6, 7, 40, 57, 4, 6],
      _array = [],
      c = es.connect(es.readArray(array), rw, es.log('after rw:'), es.writeArray(function (err, _array) {
    it(_array).deepEqual(array.map(function (e) {
      return e * 10;
    }));
    test.done();
  }));
