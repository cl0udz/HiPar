...
var es = require('../'),
    ...
    from = require('from'),
    through = require('through'); //REFACTOR THIS TEST TO USE es.readArray and es.writeArray
...
function pauseStream(prob, delay) {
  var pauseIf = 'number' == typeof prob ? function () {
    return Math.random() < prob;
  } : 'function' == typeof prob ? prob : 0.1;
  var delayer = !delay ? process.nextTick : 'number' == typeof delay ? function (next) {
...
exports['simple map applied to a stream'] = function (test) {
  var input = [1, 2, 3, 7, 5, 3, 1, 9, 0, 2, 4, 6]; //create event stream from

  var doubler = es.map(function (data, cb) {
    cb(null, data * 2);
  });
  spec(doubler).through().validateOnExit(); //a map is only a middle man, so it is both readable and writable
...
exports['emit error calledback'] = function (test) {
  var err = new Error('INTENSIONAL ERROR'),
      mapper = es.map(function (data, callback) {
    callback(err);
  });
  mapper.on('error', function (_err) {
    it(_err).equal(err);
...
exports['map applied to a stream with filtering'] = function (test) {
  ...
  var doubler = es.map(function (data, callback) {
    if (data % 2) callback(null, data * 2);else callback();
  });
  readStream(doubler, function (err, output) {
