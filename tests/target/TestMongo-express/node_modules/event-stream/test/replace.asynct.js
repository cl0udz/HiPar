...
exports['fizz buzz'] = function (test) {
  var readThis = d.map(1, 100, function (i) {
    return !(i % 3 || i % 5) ? "FB" : !(i % 3) ? "F" : !(i % 5) ? "B" : '' + i;
...
exports['fizz buzz replace'] = function (test) {
  ...
  var replace = es.replace('7', 'seven'); //  var x = spec(replace).through()

  split.pipe(replace).pipe(es.join(function (err, string) {
...
exports['fizz buzz replace whitespace using regexp'] = function (test) {
  ...
  var replaceTrailing = es.replace(/[\s]*$/, ''); //  var x = spec(replace).through()

  split.pipe(replaceLeading).pipe(replaceTrailing).pipe(es.join(function (err, string) {
    it(string).equal(fizzbuzz);
  }));
