...
test('flag boolean', function (t) {
  ...
  t.deepEqual(_typeof(argv.t), 'boolean');
  t.end();
});
test('flag boolean value', function (t) {
  ...
  t.deepEqual(_typeof(argv.verbose), 'boolean');
  t.deepEqual(_typeof(argv.t), 'boolean');
  t.end();
});
...
test('strings', function (t) {
  ...
  t.equal(s, '0001234');
  t.equal(_typeof(s), 'string');
  var x = parse(['-x', '56'], {
    string: 'x'
  }).x;
  t.equal(x, '56');
  t.equal(_typeof(x), 'string');
  t.end();
});
...
test('string and alias', function (t) {
  ...
  t.equal(_typeof(x.str), 'string');
  t.equal(x.s, '000123');
  t.equal(_typeof(x.s), 'string');
  ...
  t.equal(_typeof(y.str), 'string');
  t.equal(y.s, '000123');
  t.equal(_typeof(y.s), 'string');
  t.end();
});
