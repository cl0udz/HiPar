...
var Server = function Server(host, port, options) {
  options = options || {};
  if (!(this instanceof Server)) return new Server(host, port, options);
  EventEmitter.call(this);
  var self = this; // Filter the options

  options = filterOptions(options, legalOptionNames); // Stored options
  ...
  var server = new CServer(clonedOptions); // Define the internal properties

  this.s = {
    // Create an instance of a server instance from mongodb-core
    server: server // Server capabilities
    ,
    sCapabilities: null // Cloned options
    ,
    clonedOptions: clonedOptions // Reconnect
    ,
    reconnect: clonedOptions.reconnect // Emit error
    ,
    emitError: clonedOptions.emitError // Pool size
    ,
    poolSize: clonedOptions.size // Store Options
    ,
    storeOptions: storeOptions // Store
    ,
    store: store // Host
    ,
    host: host // Port
...
var define = Server.define = new Define('Server', Server, false); // BSON property
...
Server.prototype.connect = function (db, _options, callback) {
  ...
  var connectErrorHandler = function connectErrorHandler() {
    return function (err) {
      ...
      self.s.server.removeListener('connect', connectErrorHandler); // Try to callback
  ...
  var connectHandler = function connectHandler() {
    ...
    self.s.server.on('close', errorHandler('close')); // Only called on destroy

    self.s.server.on('destroy', destroyHandler); // relay the event
    ...
    self.s.server.on('monitoring', relay('monitoring')); // Emit open event
  ...
  self.s.server.once('connect', connectHandler); // Reconnect server

  self.s.server.on('reconnect', reconnectHandler);
  self.s.server.on('reconnectFailed', reconnectFailedHandler); // Start connection
...
Server.prototype.close = function (forceClosed) {
  this.s.server.destroy(); // We need to wash out all stored processes
