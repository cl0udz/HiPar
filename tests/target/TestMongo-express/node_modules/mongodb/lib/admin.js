...
var serverStatus = function serverStatus(self, callback) {
  self.s.db.executeDbAdminCommand({
    serverStatus: 1
  }, function (err, doc) {
...
var profilingLevel = function profilingLevel(self, callback) {
  self.s.db.executeDbAdminCommand({
    profile: -1
  }, function (err, doc) {
    if (err == null && doc.ok === 1) {
...
Admin.prototype.authenticate = function (username, password, options, callback) {
  ...
  options.authdb = 'admin'; // Execute using callback

  if (typeof callback == 'function') return this.s.db.authenticate(username, password, options, callback); // Return a Promise
...
var writeConcern = function writeConcern(options, db) {
...
var setProfilingLevel = function setProfilingLevel(self, level, callback) {
  var command = {};
  var profile = 0;
...
var profilingInfo = function profilingInfo(self, callback) {
  try {
...
var replSetGetStatus = function replSetGetStatus(self, callback) {
  self.s.db.executeDbAdminCommand({
    replSetGetStatus: 1
  }, function (err, doc) {
