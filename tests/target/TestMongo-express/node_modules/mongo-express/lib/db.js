...
var connect = function connect(config) {
  ...
  function processOpenDatabase(err, db) {
    ...
    connectionData.mainConn = db; //Check if admin features are on
  ...
  if (config.mongodb.connectionString) {
    ...
    if (Array.isArray(host)) {
      ...
      db = new mongodb.Db('local', new mongodb.ReplSet(host), {
        safe: true,
        w: 0
      });
      db.open(processOpenDatabase);
    } else {
      db = new mongodb.Db('local', new mongodb.Server(host, port, dbOptions), {
        safe: true
      });
      db.open(processOpenDatabase);
    }
