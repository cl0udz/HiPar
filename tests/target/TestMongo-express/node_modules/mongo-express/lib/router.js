...
var router = function router(config) {
  ...
  appRouter.use('/public', express["static"](__dirname + '/../build/')); // Set request size limit
  ...
  var buildBaseHref = function buildBaseHref(originalUrl, reqUrl) {
    if (reqUrl === '/') {
      return addTrailingSlash(originalUrl);
    }

    var idx = originalUrl.lastIndexOf(reqUrl);
    var rootPath = originalUrl.substring(0, idx);
  ...
  appRouter.all('*', function (req, res, next) {
    ...
    res.locals.gridFSBuckets = utils.colsToGrid(mongo.collections); // Flash messages
  ...
  appRouter.param('document', function (req, res, next, id) {
    ...
    var findStraighId = function findStraighId(id) {
  ...
  var mongoMiddleware = function mongoMiddleware(req, res, next) {
    ...
    req.gridFSBuckets = utils.colsToGrid(mongo.collections); //Allow page handlers to request an update for collection list
  ...
  var configuredRoutes = routes(config);
  ...
  appRouter.post('/checkValid', mongoMiddleware, configuredRoutes.checkValid); // Collection level routes
  ...
  appRouter.get('/db/:database/updateCollections', mongoMiddleware, configuredRoutes.updateCollections); // GridFS
  ...
  appRouter.post('/db/:database', mongoMiddleware, configuredRoutes.addCollection); // Document routes
  ...
  appRouter["delete"]('/db/:database/:collection/:document', mongoMiddleware, configuredRoutes.deleteDocument); // Property routes
