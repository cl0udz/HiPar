...
var buildCollectionURL = utils.buildCollectionURL;
...
var routes = function routes(config) {
  ...
  exp._buildMongoQuery = function (req) {
    ...
    if (key && value) {
      ...
      value = converters[type](value); // 2. then set query to it
  ...
  exp.viewCollection = function (req, res) {
    ...
    var edKey = function edKey(doc, defaultKey) {
          ...
          req.collection.count(query, null, function (err, count) {
            ...
            for (var i in items) {
              ...
              if (utils.roughSizeOfObject(items[i]) > config.options.maxRowSize) {
                for (var _prop in items[i]) {
                  if (_prop !== '_id' && utils.roughSizeOfObject(items[i][_prop]) > 200) {
                    items[i][_prop] = {
                      attribu: _prop,
                      display: '*** LARGE ROW ***',
                      humanSz: utils.bytesToSize(utils.roughSizeOfObject(items[i][_prop])),
                      maxSize: utils.bytesToSize(config.options.maxRowSize),
                      preview: JSON.stringify(items[i][_prop]).substr(0, 25),
                      roughSz: utils.roughSizeOfObject(items[i][_prop]),
                      _id: items[i]._id
                    };
                  }
                }
            ...
            var ctx = {
              ...
              // All used columns
              count: count,
              // total number of docs returned by the query
              stats: stats,
  ...
  exp.exportCollection = function (req, res) {
    res.setHeader('Content-Disposition', 'attachment; filename="' + encodeURI(req.collectionName) + '"; filename*=UTF-8\'\'' + encodeURI(req.collectionName));
  ...
  exp.addCollection = function (req, res) {
    ...
    var validation = validateCollectionName(name);
  ...
  exp.importCollection = function (req, res) {
    ...
    if (keys.some(function (key) {
      return req.files[key].mimetype !== 'application/octet-stream' || !req.files[key].data || !req.files[key].data.toString;
    })) {
    ...
    for (var _i = 0, _keys = keys; _i < _keys.length; _i++) {
      ...
      var str = "[".concat(fileContent.replace(/}\s*\n\s*\{/gm, '},{').replace(/\{"\$oid":"(.+?)"}/gm, '"$1"'), "]");
    ...
    req.collection.insert(docs).then(function (stats) {
      res.status(200).send("".concat(stats.result.n, " document(s) inserted"));
    })["catch"](function (err) {
      console.error(err);
