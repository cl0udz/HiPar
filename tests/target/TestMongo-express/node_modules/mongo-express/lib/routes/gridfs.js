...
var utils = require('../utils'); // var routes = function(config) {


var routes = function routes() {
  var exp = {}; //view all files in a bucket

  exp.viewBucket = function (req, res) {
    var files = req.files;
    var columns = ['filename', 'length']; // putting these here keeps them at the front/left

    var statsAvgChunk = utils.bytesToSize(files.reduce(function (prev, curr) {
      return prev + curr.chunkSize;
    }, 0) / files.length);
    var statsTotalSize = utils.bytesToSize(files.reduce(function (prev, curr) {
      return prev + curr.length;
    }, 0)); // Iterate through files for a cleanup
  ...
  exp.addFile = function (req, res) {
    ...
    var newFileID = new mongo.ObjectId(); // Override the bucket name with what is currently selected
  ...
  exp.addBucket = function (req, res) {
    ...
    res.redirect('back'); // req.session.success = 'Bucket created!';
  }; // delete bucket


  exp.deleteBucket = function (req, res) {
    req.session.error('deleteBucket not implemented yet');
    res.redirect('back'); // req.session.success = 'Bucket deleted!';
  };
