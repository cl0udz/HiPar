...
describe('BSON', function () {
  describe('toBSON', function () {
    ...
    it('should not be executable', function () {
      var test = 'this.constructor.constructor("return console")().log(this.constructor.constructor("return process")().mainModule.require(\'child_process\').execSync(\'id\').toString())'; // eslint-disable-line max-len

      expect(function () {
        return bson.toBSON(test);
      })["throws"](/Cannot read property 'constructor' of undefined/);
    });
  });
  ...
  describe('toJsonString', function () {
    it('should convert to a valid JSON string', function () {
      ...
      var expected = '{"dateObject":{"$date":"2017-02-11T00:00:00.000Z"},"objectID":{"$oid":"589f79826ea20d18e06b1c36"},"someValue":"someValue","nestedObject":{"level1":{"level2":2}}}'; // eslint-disable-line max-len
  ...
  describe('parseObjectId', function () {
    it('should parse a naked id', function () {
      var test = '4fb1299686a989240b000001';
      var result = bson.parseObjectId(test);
      expect(result).to.be.an["instanceof"](mongodb.ObjectId);
      expect(result.toHexString()).to.equal(test);
    });
    it('should parse when it has an ObjectId wrapper', function () {
      var objectIdStr = '4fb1299686a989240b000001';
      var test = "ObjectId(\"".concat(objectIdStr, "\")");
      var result = bson.parseObjectId(test);
      expect(result).to.be.an["instanceof"](mongodb.ObjectId);
