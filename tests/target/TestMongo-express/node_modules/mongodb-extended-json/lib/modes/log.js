...
var moment = require('moment'); // @todo (kangas) Remove need for eslint overrride

/* eslint no-cond-assign:0 */


function toStrictQuotes(str) {
  return str // wrap field names in double quotes
  .replace(/([{,])\s*([^,{\s\'"]+)\s*:/g, '$1 "$2":');
}

function toStrictSimple(str) {
  return str // Timestamps
  .replace(/Timestamp (\d+)\|(\d+)/g, '{ "$timestamp": { "t": $1, "i": $2 } }') // MinKey
  .replace(/MinKey/g, '{ "$minKey": 1 }') // MaxKey
  .replace(/MaxKey/g, '{ "$maxKey": 1 }') // ObjectIds
