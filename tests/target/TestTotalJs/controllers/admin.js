...
var ALLOW = ['/api/dependencies/', '/api/pages/preview/', '/api/upload/', '/api/nav/', '/api/files/', '/stats/', '/live/', '/api/widgets/'];
var DDOS = {};
var WS = null;
global.ADMIN = {};
...
exports.install = function () {
  ...
  ROUTE('#admin/api/dashboard/online/', json_dashboard_online); // Internal

  ROUTE('#admin/api/dependencies/', ['*Settings --> dependencies']); // MODEL: /models/widgets.js
  ...
  ROUTE('#admin/api/widgets/{id}/backups/', json_backups); // MODEL: /models/widgets.js

  ROUTE('#admin/api/widgetsglobals/', ['*WidgetGlobals --> read']);
  ROUTE('#admin/api/widgetsglobals/', ['*WidgetGlobals --> save', 'post'], 30); // MODEL: /models/pages.js
  ...
  ROUTE('#admin/api/pages/links/', json_pages_links); // MODEL: /models/pages.js

  ROUTE('#admin/api/pagesglobals/', ['*PageGlobals --> read']);
  ROUTE('#admin/api/pagesglobals/', ['*PageGlobals --> save', 'post'], 30); // MODEL: /models/posts.js
  ...
  ROUTE('#admin/api/posts/{id}/backups/', json_backups); // MODEL: /models/notices.js
  ...
  ROUTE('#admin/api/notices/preview/', view_notices_preview, ['json']); // MODEL: /models/subscribers.js
  ...
  ROUTE('#admin/api/subscribers/toggle/', ['*Subscriber --> toggle']); // MODEL: /models/newsletters.js
  ...
  ROUTE('#admin/api/newsletters/state/', json_newsletter_state); // MODEL: /models/navigations.js

  ROUTE('#admin/api/nav/{id}/', ['*Navigation --> read']);
  ROUTE('#admin/api/nav/', ['*Navigation --> save', 'post']); // MODEL: /models/settings.js

  ROUTE('#admin/api/settings/', ['*Settings --> read']);
  ROUTE('#admin/api/settings/', ['*Settings --> save', 'post']); // ESHOP
  ...
  ROUTE('#admin/api/products/export/', json_products_export, ['*Product']); // MODEL: /models/orders.js
  ...
  ROUTE('#admin/api/orders/dependencies/', ['*Order --> dependencies']); // MODEL: /models/users.js
  ...
  ROUTE('#admin/api/users/{id}/stats/', ['*User --> stats']); // Files

  ROUTE('#admin/api/files/', ['*File --> query']);
  ROUTE('#admin/api/files/clear/', ['*File --> clear']); // Other

  ROUTE('#admin/api/contactforms/stats/', ['*Contact --> stats']); // Websocket

  WEBSOCKET('#admin/live/', socket, ['json']); // Login
...
function login() {
  ...
  if (F.global.config.users[key]) {
    OPERATION('admin.notify', {
      type: 'admin.login',
      message: self.body.name
    });
...
ON('controller', function (controller, name) {
  ...
  var ddos = DDOS[controller.ip]; // 5 failed attempts
...
function json_backups(id) {
  var self = this;
  NOSQL(self.req.split[self.req.split.length - 3]).backups(function (n) {
    return n.data.id === id;
  }, self.callback());
}
...
function json_products_export() {
  var self = this;
  self.$workflow('export', function (err, response) {
    return self.binary(Buffer.from(response), 'applications/json', 'binary', 'products.json');
  });
}
