...
exports.install = function () {
  ...
  ROUTE('#logoff', redirect_logoff, ['authorize']); // Payment process
...
function view_category() {
  ...
  } else self.title(self.sitemap_name('category'));
...
function view_detail(linker) {
  ...
  $GET('Product', options, function (err, response) {
    if (err) return self.invalid().push(err); // Binds a sitemap
...
function view_signin() {
  var self = this;
  var hash = self.query.hash; // Auto-login
...
function paypal_process(id) {
  ...
  paypal.detail(self, function (err, data) {
    ...
    if (success) self.$workflow('paid', function () {
      return self.redirect(url + '?paid=1');
    });else self.redirect(url + '?paid=0');
  });
}