    ...
    self.find('[data-b]').each(function () {
      ...
      if (!obj) {
        ...
        if (el.attrd('b-template') === 'true') {
          ...
          var str = '';
          if (tmp.length) str = tmp.html();else str = el.html();
        ...
        } else obj.html = html ? self.prepare(html) : undefined;
      ...
      for (var i = 0, length = arr.length; i < length; i++) {
        p += (p ? '.' : '') + arr[i];
        if (keys[p]) keys[p].push(obj);else keys[p] = [obj];
      }
    });
...
COMPONENT('grid', 'filter:true;external:false;fillcount:50;filterlabel:Filtering values ...;boolean:true|on|yes;pluralizepages:# pages,# page,# pages,# pages;pluralizeitems:# items,# item,# items,# items;pagination:false;rowheight:30', function (self, config) {
  ...
  self.make = function () {
    ...
    pagination = config.pagination ? VIRTUALIZE(self.find('.ui-grid-footer'), {
      page: 'input',
      first: 'button[name="first"]',
      last: 'button[name="last"]',
      prev: 'button[name="prev"]',
    ...
    self.event('change', '.ui-grid-filter', function () {
      var el = $(this).parent();
      if (this.value) options.filter[this.name] = this.value;else delete options.filter[this.name];
    ...
    self.event('change', '.ui-grid-input', function () {
      ...
      if (isNaN(page) || page < 0 || page > cache.pages || page === cache.page) return;
      scroll = true;
      cache.page = page;
      self.operation('pagination');
    });
  ...
  self.checked = function (value) {
    if (typeof value === 'boolean') self.find('input[type="checkbox"]').prop('checked', value);else return tbody.find('input:checked');
  };
  ...
  self.meta = function (html) {
    ...
    for (var i = 0; i < options.columns.length; i++) {
      ...
      if (typeof column.header === 'string' && column.header.indexOf('{{') !== -1) column.header = Tangular.compile(column.header);
      if (typeof column.template === 'string') column.template = column.template.indexOf('{{') === -1 ? new Function('a', 'b', 'return \'' + column.template + '\'') : Tangular.compile(column.template);
    }
  ...
  self.rebuild = function (init) {
    ...
    for (var i = 0, length = columns.length; i < length; i++) {
      var col = columns[i];
      if (typeof col.size !== 'string') size += col.size || 1;
      col.sorting = null;
      if (typeof col.render === 'string') col.render = FN(col.render);
      if (typeof col.header === 'string') col.header = FN(col.header);
    ...
    for (var i = 0, length = columns.length; i < length; i++) {
      ...
      if (col.filter === false) filter.push('<th class="ui-grid-columnfilterempty ui-grid-columnfilter{1}" style="width:{0}">&nbsp;</th>'.format(width, self.cls(1, col.classfilter, col["class"])));else filter.push('<th class="ui-grid-columnfilter{4}" style="width:{0}"><input type="text" placeholder="{3}" name="{2}" autocomplete="off" class="ui-grid-filter" /></th>'.format(width, i, col.name, col.filter || config.filterlabel, self.cls(1, col.classfilter, col["class"])));
    }
  ...
  self.fill = function () {
    if (config.autosize === false || filled) return;
    filled = true;
  ...
  self.sort = function (data) {
    ...
    if (data.column.sorting === 'desc') {
      ...
      if (config.external) self.operation('sort');else self.refresh();
    } else if (data.column) {
  ...
  self.can = function (row) {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      if (type === 'number') {
        if (val2 == null) val2 = filtercache[column] = self.parseNumber(filter);
        if (val2.length === 1 && val !== val2[0]) return false;
        if (val < val2[0] || val > val2[1]) return false;
      } else if (type === 'string') {
        if (val2 == null) {
        ...
        if (val2 == null) val2 = filtercache[column] = config["boolean"].indexOf(filter.replace(/\s/g, '')) !== -1;
        ...
        if (val2.length === 1 && val.format('yyyyMMdd') !== val2[0].format('yyyyMMdd')) return false;
        if (val < val2[0] || val > val2[1]) return false;
      } else return false;
    }
  ...
  self.parseDate = function (val) {
    ...
    if (index === -1) {
      ...
      if (val.length === 4) return [new Date(+val, 0, 1), new Date(+val + 1, 0, 1)];
  ...
  self.reset = function () {
    ...
    options.lastsortelement = null;
    if (options.lastsort) options.lastsort.sorting = null;
    options.lastsort = null;
  };
  ...
  self.setter = function (value) {
    ...
    if (config.external) {
      ...
      for (var i = 0, length = value.items.length; i < length; i++) {
        if (isFilter && !self.can(value.items[i])) continue;
...
COMPONENT('contextmenu', function (self) {
  ...
  self.make = function () {
    ...
    $(document).on('touchstart mousedown', function (e) {
      if (is && self.target !== e.target && !self.target.contains(e.target)) self.hide(1);
    });
  };
  ...
  self.show = function (orientation, target, items, callback, offsetX, offsetY) {
    ...
    var type = _typeof(items);

    var item;
    if (type === 'string') items = self.get(items);else if (type === 'function') {
      callback = items;
    ...
    for (var i = 0, length = items.length; i < length; i++) {
      ...
      if (item.icon) {
        if (item.icon.substring(0, 3) !== 'fa-') item.icon = 'fa-' + item.icon;
      } else item.icon = 'fa-caret-right';
    ...
    var options = {
      left: orientation === 'center' ? Math.ceil(offset.left - self.element.width() / 2 + target.innerWidth() / 2) : orientation === 'left' ? offset.left - 8 + (offsetX || 0) : offset.left - self.element.width() + target.innerWidth() + (offsetX || 0) + 8,
      top: offset.top + target.innerHeight() + 10 + (offsetY || 0)
    };
    self.css(options);
    if (is) return;
    self.rclass('hidden');
...
COMPONENT('textbox', function (self, config) {
  ...
  self.validate = function (value) {
    if (!config.required || config.disabled) return true;
    if (self.type === 'date') return value instanceof Date && !isNaN(value.getTime());
    if (value == null) value = '';else value = value.toString();
  ...
  self.redraw = function () {
    ...
    if (!icon2 && self.type === 'date') icon2 = 'calendar';else if (self.type === 'search') {
      icon2 = 'search ui-textbox-control-icon';

      self.setter2 = function (value) {
        if (self.$stateremoved && !value) return;
    ...
    if (config.label) content = config.label;
  ...
  self.configure = function (key, value, init) {
    ...
    switch (key) {
      ...
      case 'type':
        self.type = value;
        if (value === 'password') value = 'password';else self.type = 'text';
        redraw = true;
        break;
  ...
  self.state = function (type) {
    ...
    if (invalid === self.$oldstate) return;
...
COMPONENT('inlineform', function (self, config) {
  ...
  self.submit = function () {
    if (config.submit) EXEC(config.submit, self);else self.hide();
  };
  ...
  self.make = function () {
    var icon;
    if (config.icon) icon = '<i class="fa fa-{0}"></i>'.format(config.icon);else icon = '<i></i>';
    ...
    self.rclass('hidden');
    self.replace(el);
    header = self.virtualize({
      title: '.ui-inlineform-title > span',
      icon: '.ui-inlineform-title > i'
    });
    self.find('button').on('click', function () {
      ...
      switch (this.name) {
        case 'submit':
          if (el.hasClass('exec')) self.hide();else self.submit(self.hide);
          break;
  ...
  self.toggle = function (el, position, offsetX, offsetY) {
    if (self.hclass('hidden')) self.show(el, position, offsetX, offsetY);else self.hide();
  };
  ...
  self.show = function (el, position, offsetX, offsetY) {
    ...
    if (offsetX) offset.left += offsetX;
    if (offsetY) offset.top += offsetY;
...
COMPONENT('form', function (self, config) {
  ...
  if (!W.$$form) {
    ...
    $(document).on('click', '.ui-form-container', function (e) {
      var el = $(e.target);
      if (!(el.hclass('ui-form-container-padding') || el.hclass('ui-form-container'))) return;
  ...
  self.resize = function () {
    if (!config.center || self.hclass('hidden')) return;
  ...
  self.make = function () {
    ...
    self.replace(el);
    header = self.virtualize({
      title: '.ui-form-title > span',
      icon: '.ui-form-title > i'
    });
  ...
  self.setter = function (value) {
    ...
    if (self.hclass('hidden') === isHidden) return;
    ...
    if (W.$$form_level < 1) W.$$form_level = 1;
    W.$$form_level++;
    ...
    }, 300); // Fixes a problem with freezing of scrolling in Chrome
...
COMPONENT('dropdown', function (self, config) {
  ...
  self.validate = function (value) {
    ...
    if (type === 'undefined' || type === 'object') value = '';else value = value.toString();
    EMIT('reflow', self.name);
  ...
  self.bind = function (path, arr) {
    ...
    for (var i = 0, length = arr.length; i < length; i++) {
      ...
      if (item.length) builder.push(template.format(item, value === item ? ' selected="selected"' : '', item));else builder.push(template.format(item[propValue], value === item[propValue] ? ' selected="selected"' : '', item[propText]));
    }
  ...
  self.redraw = function () {
    ...
    if (label) {
      ...
      self.html(builder.join(''));
    } else self.html(html).aclass('ui-dropdown-values');
...
COMPONENT('validation', 'delay:100;flags:visible', function (self, config) {
  ...
  self.configure = function (key, value, init) {
    switch (key) {
      case 'selector':
        if (!init) elements = self.find(value || def);
        break;
  ...
  self.state = function () {
    setTimeout2(self.id, function () {
      ...
      if (!disabled && config["if"]) disabled = !EVALUATE(self.path, config["if"]);
...
COMPONENT('textarea', function (self, config) {
  ...
  self.validate = function (value) {
    if (config.disabled || !config.required) return true;
    if (value == null) value = '';else value = value.toString();
    return value.length > 0;
  };
...
COMPONENT('importer', function (self, config) {
  ...
  self.setter = function (value) {
    if (config["if"] !== value) return;

    if (imported) {
      if (config.reload) EXEC(config.reload);else self.setter = null;
      return;
    }

    imported = true;
    IMPORT(config.url, function () {
      if (config.reload) EXEC(config.reload);else self.remove();
    });
  };
});
COMPONENT('codemirror', 'linenumbers:false;required:false;trim:false;tabs:false', function (self, config) {
  ...
  self.make = function () {
    ...
    if (config.tabs) options.indentWithTabs = true;
    ...
    editor.on('change', function (a, b) {
      if (config.disabled || !can[b.origin]) return;
      setTimeout2(self.id, function () {
...
COMPONENT('nosqlcounter', 'count:0;height:80', function (self, config) {
  ...
  self.configure = function (key, value) {
    switch (key) {
      case 'months':
        if (value instanceof Array) months = value;else months = value.split(',').trim();
        break;
    }
  };
  ...
  self.redraw = function (maxbars) {
    ...
    for (var i = 0, length = stats.length; i < length; i++) {
      var item = stats[i];
      var val = item.value;
      if (val > 999) val = (val / 1000).format(1, 2) + 'K';
      sum += val + ',';
      var h = max === 0 ? 0 : item.value / max * (100 - min);
      h += min;
      cls = item.value ? '' : 'empty';
      if (item.id === current) cls += (cls ? ' ' : '') + 'current';
      if (i === maxbars - 1) cls += (cls ? ' ' : '') + 'last';
      var w = bar.format(2, '');
  ...
  self.setter = function (value) {
    ...
    } else self.redraw(WIDTH() === 'xs' ? config.count / 2 : config.count, value);
  };
});
COMPONENT('keyvalue', 'maxlength:100', function (self, config) {
  ...
  self.configure = function (key, value, init, prev) {
    ...
    switch (key) {
      ...
      case 'icon':
        if (value && prev) self.find('i').rclass('fa').aclass('fa fa-' + value);else redraw = true;
        break;
  ...
  self.redraw = function () {
    var icon = config.icon;
    var label = config.label || content;
    if (icon) icon = '<i class="fa fa-{0}"></i>'.format(icon);
    empty.value = '';
  ...
  self.make = function () {
    ...
    self.event('change keypress', 'input', function (e) {
      if (config.disabled || e.type !== 'change' && e.which !== 13) return;
      var el = $(this);
...
COMPONENT('expander', function (self, config) {
  ...
  self.toggle = function (v) {
    if (v == null) v = !self.hclass('ui-expander-expanded');
    if (v === prev) return;
    prev = v;
...
COMPONENT('disable', function (self, config) {
  ...
  self.configure = function (key, value) {
    if (key === 'validate') validate = value.split(',').trim();
  };

  self.setter = function (value) {
    var is = true;
    if (config["if"]) is = EVALUATE(self.path, config["if"]);else is = !value;
...
COMPONENT('textboxlist', 'maxlength:100', function (self, config) {
  ...
  self.configure = function (key, value, init, prev) {
    ...
    switch (key) {
      ...
      case 'icon':
        if (value && prev) self.find('i').rclass().aclass(value);else redraw = true;
        break;
    }
  ...
  self.redraw = function () {
    ...
    if (config.icon) icon = '<i class="fa fa-{0}"></i>'.format(config.icon);
    empty.value = '';
  ...
  self.make = function () {
    ...
    empty.value = '';
    empty.disabled = config.disabled;
    if (config.disabled) self.aclass('ui-disabled');
    content = self.html();
    ...
    self.event('change keypress', 'input', function (e) {
      ...
      if (base) {
        if (!raw || raw.indexOf(value) === -1) self.push(self.path, value, 2);
        this.value = '';
        self.change(true);
        return;
      }
...
COMPONENT('dropdowncheckbox', 'checkicon:check;visible:0;alltext:All selected;limit:0;selectedtext:{0} selected', function (self, config) {
  ...
  self.redraw = function () {
    ...
    if (content.length) self.html('<div class="ui-dropdowncheckbox-label{0}">{1}{2}:</div>'.format(config.required ? ' ui-dropdowncheckbox-required' : '', config.icon ? '<i class="fa fa-' + config.icon + '"></i>' : '', content) + html);else self.html(html);
  ...
  self.make = function () {
    ...
    self.redraw();
    if (config.items) self.reconfigure({
      items: config.items
    });else if (config.datasource) self.reconfigure({
  ...
  self.bind = function (path, value) {
    ...
    if (value !== undefined) prepared = true;

    if (!value || !value.length) {
      var h = config.empty || '&nbsp;';
      if (h === self.old) return;
    ...
    for (var i = 0, length = value.length; i < length; i++) {
      ...
      var item = {
        value: isString ? value[i] : value[i][kv],
        text: isString ? value[i] : value[i][kt],
        index: i
      };
    ...
    if (h === self.old) return;
    self.old = h;
    if (render) container.rclass(clsempty).html(render);else container.aclass(clsempty).html(config.empty);
    self.refresh();
  };
  ...
  self.setter = function (value) {
    ...
    container.find('.ui-dropdowncheckbox-item').each(function () {
      ...
      var index = +el.attr('data-index');
      var checked = false;
      if (!value || !value.length) checked = false;else if (data[index]) checked = data[index];
    ...
    if (!label && config.placeholder) {
      ...
      if (count == data.length && config.alltext !== 'null' && config.alltext) label = config.alltext;else if (config.visible && count > config.visible) label = config.selectedtext.format(count, data.length);
  ...
  if (W.$dropdowncheckboxevent) return;
...
COMPONENT('snackbar', 'timeout:3000;button:Dismiss', function (self, config) {
  ...
  self.make = function () {
    ...
    virtual = self.virtualize({
      body: '.ui-snackbar-body',
      button: '.ui-snackbar-dismiss'
    });
...
COMPONENT('repeater', 'hidden:true;check:true', function (self, config) {
  ...
  self.configure = function (key, value) {
    if (key === 'filter') filter = value ? GET(value) : null;
  };
...
COMPONENT('crop', 'dragdrop:true;format:{0}', function (self, config) {
  ...
  self.make = function () {
    ...
    self.event('mousemove mouseup', function (e) {
      ...
      if (self.disabled || !can || !is) return;
      var rect = canvas.getBoundingClientRect();
      var x = e.clientX - rect.left;
      var y = e.clientY - rect.top;
    ...
    self.event('keydown', 'input', function () {
      ...
      setTimeout2(self.id, function () {
        ...
        container.find('li').each(function () {
          if (value && this.getAttribute('data-search').toSearch().indexOf(value) === -1) hide.push(this);else show.push(this);
        });
        $(hide).aclass('hidden');
        $(show).rclass('hidden');
...
COMPONENT('multioptions', function (self) {
  ...
  self.make = function () {
    ...
    self.event('click', '.multioptions-operation', function (e) {
      ...
      if (type === 'number') {
        ...
        if (!step) step = 1;
        if (min) min = min.parseInt();
        if (max) max = max.parseInt();
        var value;

        if (name === 'plus') {
          value = input.val().parseInt() + step;
          if (max !== 0 && max && value > max) value = max;
          input.val(value);
        } else {
          value = input.val().parseInt() - step;
          if (min !== 0 && min && value < min) value = min;
          input.val(value);
        }
  ...
  self.mapping = function (key, label, def, type, max, min, step, validator) {
    if (typeof type === 'number') {
      ...
      max = type;
      type = 'number';
    } else if (!type) type = def instanceof Date ? 'date' : _typeof(def);
    ...
    if (type instanceof Array) {
      ...
      type.forEach(function (val) {
        values.push({
          text: val.text === undefined ? val : val.text,
          value: val.value === undefined ? val : val.value
        });
      });
      type = 'array';
    }
    ...
    if (validator && typeof validator !== 'function') validator = null;
    dep[key] = values;
    mapping[key] = {
      name: key,
      label: label,
  ...
  self.save = function () {
    ...
    Object.keys(mapping).forEach(function (key) {
      ...
      if (el.hclass('ui-multioptions-select')) {
        ...
        if (obj[key] && obj[key].value) obj[key] = obj[key].value;
        return;
      }
    });
    skip = true;
  ...
  self.setter = function (options) {
    ...
    var builder = [];
    Object.keys(mapping).forEach(function (key) {
      var option = mapping[key]; // option.name
      // option.label
...
COMPONENT('fileupload', function (self, config) {
  ...
  self.configure = function (key, value, init) {
    ...
    switch (key) {
      ...
      case 'accept':
        var el = $('#' + id);
        if (value) el.prop('accept', value);else el.removeProp('accept');
        break;

      case 'multiple':
        var el = $('#' + id);
        if (value) el.prop('multiple', true);else el.removeProp('multiple');
        break;
  ...
  self.upload = function (files) {
    ...
    UPLOAD(config.url, data, function (response, err) {
      ...
      if (config.array) self.push(response);else self.set(response);
    });
  };
...
COMPONENT('suggestion', function (self, config) {
  ...
  self.show = function (orientation, target, items, callback) {
    ...
    var options = {
      left: orientation === 'center' ? Math.ceil(offset.left - self.element.width() / 2 + target.innerWidth() / 2) : orientation === 'left' ? offset.left - 8 : offset.left - self.element.width() + target.innerWidth(),
      top: offset.top + target.innerHeight() + 10
    };
    self.css(options);
    if (is) return;
    self.rclass('hidden');
...
COMPONENT('calendar', 'today:Set today;firstday:0;close:Close;yearselect:true;monthselect:true;yearfrom:-70 years;yearto:5 years', function (self, config) {
  ...
  self.configure = function (key, value) {
    switch (key) {
      case 'days':
        if (value instanceof Array) self.days = value;else self.days = value.split(',').trim();
      ...
      case 'months':
        if (value instanceof Array) self.months = value;else self.months = value.split(',').trim();
        self.months_short = [];

        for (var i = 0, length = self.months.length; i < length; i++) {
          var m = self.months[i];
          MONTHS[i] = m;
          if (m.length > 4) m = m.substring(0, 3) + '.';
      ...
      case 'yearfrom':
        if (value.indexOf('current') !== -1) self.years_from = parseInt(new Date().format('yyyy'));else self.years_from = parseInt(new Date().add(value).format('yyyy'));
        break;

      case 'yearto':
        if (value.indexOf('current') !== -1) self.years_to = parseInt(new Date().format('yyyy'));else self.years_to = parseInt(new Date().add(value).format('yyyy'));
        break;
    }
  };
    ...
    for (var i = 0; i < days + frm; i++) {
      ...
      if (i >= frm) {
        ...
        if (!obj.isFuture && year === ty) {
          if (tm < month) obj.isFuture = true;else if (tm === month) obj.isFuture = td < index;
        }
      } else {
        indexEmpty++;
      ...
      if (!obj.isEmpty) cur = d.add(i + ' days');
  ...
  self.show = function (el, value, callback, offset) {
    ...
    if (!el) return self.hide();
    var off = el.offset();
    var h = el.innerHeight();
    self.css({
      left: off.left + (offset || 0),
      top: off.top + h + 12
    });
    self.rclass('hidden');
    self.click = callback;
  ...
  self.make = function () {
    ...
    self.event('click', 'button', function (e) {
      ...
      if (current_year < self.years_from || current_year > self.years_to) return;
      skipDay = true;
      self.date(dt);
    });
  ...
  self.date = function (value) {
    ...
    if (typeof value === 'string') value = value.parseDate();
    ...
    if (!value) value = DATETIME = new Date();
...
COMPONENT('progress', 'animate:true', function (self, config) {
  ...
  self.setter = function (value) {
    ...
    if (value > 100) value = 100;else if (value < 0) value = 0;
    old = value >> 0;
    if (config.animate) container.stop().animate({
      width: old + '%'
    }, 80).show();else container.css({
      width: old + '%'
    });
    container.html(old + '%');
  };
...
COMPONENT('textboxtags', function (self, config) {
  ...
  var refresh = false;
  var W = window;
  if (!W.$textboxtagstemplate) W.$textboxtagstemplate = Tangular.compile('<div class="ui-textboxtags-tag" data-name="{{ name }}">{{ name }}<i class="fa fa-times-circle ui-textboxtags-remove"></i></div>');
  ...
  self.configure = function (key, value, init) {
    ...
    switch (key) {
      ...
      case 'icon':
        ...
        if (fa.length && value) fa.rclass().aclass('fa fa-' + value);else redraw = true;
        break;
  ...
  self.make = function () {
    ...
    self.event('click', '.ui-textboxtags-remove', function (e) {
      ...
      var el = $(this);
      var arr = self.get();
      if (isString) arr = self.split(arr);
    ...
    self.event('keydown', 'input', function (e) {
      ...
      if (config.uppercase) value = value.toUpperCase();else if (config.lowercase) value = value.toLowerCase();
      if (isString) arr = self.split(arr);
      if (!(arr instanceof Array)) arr = [];
      if (arr.indexOf(value) === -1) arr.push(value);else return;
      this.value = '';
      self.set(isString ? arr.join(', ') : arr);
      self.change(true);
    });
  ...
  self.setter = function (value) {
    if (!refresh && NOTMODIFIED(self.id, value)) return;
    refresh = false;
...
COMPONENT('websocket', 'reconnect:3000', function (self, config) {
  ...
  self.make = function () {
    ...
    if (!url.match(/^(ws|wss):\/\//)) url = (location.protocol.length === 6 ? 'wss' : 'ws') + '://' + location.host + (url.substring(0, 1) !== '/' ? '/' : '') + url;
...
COMPONENT('donutchart', 'format:{{ value | format(0) }};tooltip:true;presentation:true;animate:true', function (self, config) {
  ...
  self.redraw = function (width, value) {
    ...
    if (!force && NOTMODIFIED(self.id, items)) return;
    var size = width;
    var half = size / 2;
    ...
    for (var i = 0, length = items.length; i < length; i++) {
      ...
      if (item.end === 360) item.end = 359.99;
...
COMPONENT('barchart', 'paddingbars:5;limit:0;paddinggroup:10;radius:2;offsetX:10;offsetY:10;templateY:{{ value | format(0) }};templateX:{{ value }};height:0', function (self, config) {
  ...
  self.make = function () {
    ...
    self.event('click mouseenter', 'rect', function (e) {
      ...
      if (index === self.$selectedindex && e.type === 'mouseenter') return;
      self.$selectedindex = index;
      var arr = index.split(',');
      var item = self.get()[+arr[0]];
      var value = item.values[+arr[1]];
      selected.text(templateY({
        value: value.y
      }));
  ...
  self.setter = function (value) {
    ...
    if (config.limit) maxY = config.limit;
  ...
  self.register = function (shortcut, callback, prevent) {
    shortcut.split(',').trim().forEach(function (shortcut) {
      ...
      shortcut.split('+').trim().forEach(function (item) {
        ...
        if (num) builder.push('e.which===' + num);else builder.push('e.key===\'{0}\''.format(item));
      });
      items.push({
        shortcut: alias.join('+'),
        fn: new Function('e', 'return ' + builder.join('&&')),
        callback: callback,
        prevent: prevent
      });
      length = items.length;
...
COMPONENT('features', 'height:37', function (self, config) {
    ...
    self.event('keydown', 'input', function (e) {
      ...
      switch (e.which) {
        ...
        case 13:
          ...
          if (sel.length && self.callback) self.callback(self.items[+sel.attr('data-index')]);
          self.hide();
          break;
        ...
        case 38:
          ...
          o = true;
          selectedindex--;
          if (selectedindex < 0) selectedindex = 0;else self.move();
          break;
        ...
        case 40:
          ...
          o = true;
          selectedindex++;
          if (selectedindex >= resultscount) selectedindex = resultscount;else self.move();
          break;
      }
  ...
  self.search = function (value) {
    if (!value) {
      if (self.oldsearch === value) return;
  ...
  self.move = function () {
    ...
    container.find('li').each(function () {
      var el = $(this);
      if (el.hclass('hidden')) return;
      var is = selectedindex === counter;
      el.tclass('selected', is);

      if (is) {
        var t = config.height * counter - config.height;
        if (t + config.height * 5 > h) scroller.scrollTop(t);else scroller.scrollTop(0);
      }
  ...
  self.show = function (items, callback) {
    ...
    var item;
    if (type === 'string') items = self.get(items);
...
COMPONENT('listing', 'count:20', function (self, config) {
  ...
  self.make = function () {
    self.find('script').each(function (index) {
      ...
      if (index) layout = T;else self.template = T;
    });
...
COMPONENT('autocomplete', 'height:200', function (self, config) {
  ...
  function keydown(e) {
    ...
    if (c !== 38 && c !== 40 && c !== 13) {
      ...
      searchtimeout = setTimeout(function () {
        var val = input.value;
        if (!val) return self.render(EMPTYARRAY);
        if (searchvalue === val) return;
        searchvalue = val;
        self.resize();
        onSearch(val, function (value) {
          self.render(value);
        });
      }, 200);
      return;
  ...
  self.resize = function () {
    if (!offsetter || !old) return;
    var offset = offsetter.offset();
    offset.top += offsetter.height();
    offset.width = offsetter.width();
    if (margin.left) offset.left += margin.left;
    if (margin.top) offset.top += margin.top;
    if (margin.width) offset.width += margin.width;
    self.css(offset);
  };
  ...
  self.attachelement = function (element, input, search, callback, top, left, width) {
    clearTimeout(searchtimeout);
    if (input.setter) input = input.find('input');else input = $(input);
