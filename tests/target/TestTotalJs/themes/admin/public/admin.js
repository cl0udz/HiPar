...
function refresh_filebrowser(target, type, clear) {
  ...
  item.filename = null;
  item.type = type;
  if (clear === true) item.clear = clear;
...
$(window).on('resize', refresh_height); // Tangular helpers
...
Tangular.register('counter', function (value) {
  if (value > 999999) return (value / 1000000).format(2) + ' M';
  if (value > 9999) return (value / 10000).format(2) + ' K';
