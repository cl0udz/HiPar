...
UPTODATE('1 day');
var common = {}; // Online statistics for visitors

(function () {
  if (navigator.onLine != null && !navigator.onLine) return;
  var options = {};
  options.type = 'GET';
  options.headers = {
    'x-ping': location.pathname,
    'x-cookies': navigator.cookieEnabled ? '1' : '0',
...
function refresh_category() {
  ...
  el.find('a').each(function () {
    ...
    if (linker.indexOf(el.attr('href')) !== -1) {
      el.aclass('selected');
      var next = el.next();
      if (next.length && next.is('nav')) el.find('.fa').rclass('fa-caret-right').aclass('fa-caret-down');
    }
  });
}
...
function refresh_prices() {
  ...
  FIND('shoppingcart', function (com) {
    ...
    items.each(function () {
      ...
      var p;
      if (discount) p = discount;else if (priceold && price < priceold) p = 100 - price / (priceold / 100);
