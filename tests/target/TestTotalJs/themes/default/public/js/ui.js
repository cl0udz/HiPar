...
COMPONENT('enter', 'validate:true', function (self, config) {
  ...
  self.make = function () {
    self.event('keydown', 'input', function (e) {
      if (e.which === 13 && (!config.validate || MAIN.can(self.path))) EXEC(config.exec, self);
    });
  };
});
...
COMPONENT('shoppingcart', 'discount:0;expiration:6 days', function (self, config) {
  ...
  self.modify = function (response) {
    ...
    for (var i = 0; i < datasource.items.length; i++) {
      ...
      if (res == null) datasource.items[i] = null;else {
        var price = res.prices.findItem('id', item.idvariant); // Price not found
  ...
  self.upd = function (token, count) {
    ...
    if (item) {
      if (item.count === count) return count;
  ...
  self.sum = function (init) {
    ...
    if (config.discount) datasource.sum = datasource.price - datasource.price / 100 * config.discount;else datasource.sum = datasource.price;
...
COMPONENT('modificator', function (self) {
  ...
  self.click = function (path, schema) {
    if (path.substring(0, 1) === '%') path = 'jctmp.' + path.substring(1);
    var fn = db[schema];
  ...
  self.register = function (name, fn) {
    ...
    for (var i = 0, length = paths.length; i < length; i++) {
      ...
      for (var j = 0, jl = arr.length; j < jl; j++) {
        var obj = arr[j];
        if (obj.init || obj.schema !== name) continue;
        obj.init = true;
        obj.event.type = 'init';
        obj.event.bindtype = -1;
...
COMPONENT('mobilecarousel,mobilecarousel@1', 'count:1;selector:.col-sm-4;margin:15;snapping:true;animate:5000', function (self, config) {
  ...
  self.animate = function () {
    ...
    index += increment;
    if (index === count - 1) increment = -1;else if (index === 0) increment = 1;
    skip = true;
    anim = true;
    container.animate({
  ...
  self.snap = function () {
    ...
    container.stop().animate({
      scrollLeft: off * (width + config.margin)
    }, 200);
    setTimeout(function () {
      skip = false;
    }, 500);
