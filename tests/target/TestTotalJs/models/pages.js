...
var REGEXP_HTML_CLASS = /(\s)class=".*?"/g;
var REGEXP_HTML_ATTR = /(\s)data-cms-id="[a-z0-9]+"|data-cms-widget="[a-z0-9]+"/g;
var REGEXP_HTML_DIV = /<div\s>/g;
var REGEXP_GLOBAL = /\$[0-9a-z-_]+/gi;
...
NEWSCHEMA('Page').make(function (schema) {
  ...
  schema.setSave(function ($) {
    ...
    if (!model.navigations.length) model.navigations = null;
    model.body = U.minifyHTML(model.body);
    model.search = ((model.title || '') + ' ' + (model.keywords || '') + ' ' + model.search).keywords(true, true).join(' ').max(1000); // Sanitizes URL

    if (!model.ispartial && !model.url.startsWith('http:') && !model.url.startsWith('https:')) {
      model.url = U.path(model.url);
      if (model.url[0] !== '/') model.url = '/' + model.url;
    }

    model.oldurl = undefined;
    var db = isUpdate ? nosql.modify(model).where('id', model.id).backup(user).log('Update: ' + model.id, user) : nosql.insert(model).log('Create: ' + model.id, user); // Update a URL in all navigations where this page is used

    if (!model.ispartial) $WORKFLOW('Navigation', 'page', {
      page: model
    });
    db.callback(function () {
      ...
      if (model.replacelink && model.url !== oldurl && oldurl) $WORKFLOW('Page', 'replacelinks', {
        url: model.url,
        oldurl: oldurl
      });else setTimeout2('pages', refresh, 1000);
      $.success();
    });
  ...
  schema.addWorkflow('url', function ($) {
    ...
    if (!model.url || model.url === '---') {
      ...
      } else model.url = model.name.slug();
    }
  ...
  schema.addWorkflow('stats', function ($) {
    ...
    if ($.id) nosql.monthly($.id, $.callback);else nosql.monthly($.callback);
  });
});
NEWSCHEMA('PageGlobals').make(function (schema) {
  ...
  schema.addWorkflow('add', function ($) {
    ...
    pending.push($.options); // For multiple usage at the time
...
function parseGlobals(val) {
  ...
  for (var i = 0, length = arr.length; i < length; i++) {
    var str = arr[i];
    if (!str || str[0] === '#' || str.substring(0, 2) === '//') continue;
    ...
    if (name && value) GLOBALS[name] = value;
  }
}
...
String.prototype.CMSrender = function (settings, callback, controller) {
  ...
  if (!F.global.widgets.$ready) {
    ...
    setTimeout(function (body, settings, callback) {
      return body.CMSrender(settings, callback, controller);
    }, 500, body, settings, callback);
    return;
  }
  ...
  settings.wait(function (item, next) {
    ...
    if (!widget || !widget.total || !widget.total.render) return next();
    var beg = '<div';
    var end = '</div>';
    var pos = eindex + 1;
    var count = 0;
    ...
    return callback(body.CMSglobals().CMStidy());
  });
};
...
function widgetsettings(widget, settings) {
  ...
  for (var i = 0, length = keys.length; i < length; i++) {
    var key = keys[i];
    if (settings[key] == null) obj[key] = widget.def[key];else obj[key] = settings[key];
  }
...
String.prototype.CMStidy = function () {
  ...
  while (true) {
    ...
    while (true) {
      if (body[--index] === '<' || index <= 0) break;
    }

    if (index === beg || index <= 0) return;
    ...
    while (true) {
      ...
      if (str.substring(0, tag.length) === tag) count++;
      index++;
    }
  }
  ...
  return body.replace(REGEXP_HTML_ATTR, '').replace(REGEXP_HTML_CLASS, function (text) {
    ...
    for (var i = 0, length = arr.length; i < length; i++) {
      var cls = arr[i];
      if (cls[0] === 'C' && cls[1] === 'M' && cls[2] === 'S' && cls !== 'CMS_hidden') continue;
      builder += (builder ? ' ' : '') + cls;
    }
...
function loadpartial(page, callback, controller) {
  var output = {};
  if (!page.partial || !page.partial.length) return callback(output);
  var nosql = NOSQL('pages');
...
Controller.prototype.CMSpage = function (callback, cache) {
  ...
  self.memorize('cachecms' + self.url, cache || '1 minute', cache === false, function () {
    ...
    if (self.repository.page) NOSQL('pages').counter.hit('all').hit(self.repository.page.id);
  });
  return self;
};
