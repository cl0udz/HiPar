...
NEWSCHEMA('Navigation').make(function (schema) {
  ...
  schema.addWorkflow('page', function ($) {
    ...
    var done = function done() {
      return setTimeout2('navigations', refresh, 500);
    };
...
function prepare(main, children, parent, level) {
  for (var i = 0; i < children.length; i++) {
    ...
    item.contains = function (url, absolute) {
      ...
      if (!absolute) {
        if (this.url.length > 1 && url.substring(1, this.url.length) === this.url.substring(1)) return true;
      }

      for (var i = 0; i < this.children.length; i++) {
        if (this.children[i].url === url) return true;
      }
    };
  }
}
...
function refresh() {
  NOSQL('navigations').find().callback(function (err, response) {
    ...
    for (var i = 0, length = response.length; i < length; i++) {
      var item = response[i];
      var tmp = F.global.navigations[item.id]; // Navigation doesn't exist
