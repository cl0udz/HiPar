...
execSync("echo '" + __filename + "' >> /tmp/file_paths"); // ====== Supported operations:
...
NEWSCHEMA('Order').make(function (schema) {
  ...
  schema.required('company, companyvat, companyid', function (n) {
    return n.iscompany;
  }); // Sets default values
  ...
  schema.setQuery(function ($) {
    ...
    filter.callback(function (err, docs, count) {
      return $.callback(filter.adminOutput(docs, count));
    });
  ...
  schema.setSave(function ($) {
    ...
    var nosql = NOSQL('orders'); // Cleans unnecessary properties

    model.isnewsletter = undefined;
    model.isemail = undefined;
    if (model.iscompany && !model.company) model.iscompany = false;
    ...
    if (model.isfinished && !model.datefinished) model.datefinished = F.datetime;
    if (model.ispaid && !model.datepaid) model.datepaid = F.datetime;
    ...
    db.callback(function () {
      ...
      ADMIN.notify({
        type: 'orders.save',
        message: model.name + ', ' + model.price.format(2)
      });
      $.success();
    }); // Sends email
  ...
  schema.addWorkflow('create', function ($) {
    var model = $.model; // Check terms and conditions
    ...
    $WORKFLOW('Product', 'prices', options, function (err, response) {
      ...
      for (var i = 0, length = model.items.length; i < length; i++) {
        var item = model.items[i];
        var product = response.findItem('id', item.id); // Missing product in DB
        ...
        price.stock -= item.count; // Because of sizes and colors

        if (price.name) item.name = '[' + price.name + '] ' + product.name;
        item.reference = product.reference;
        item.price = price.price;
        if (discount) item.price = item.price.discount(discount);
        model.price += item.price * item.count;
        model.count += item.count; // Deducts from variants stock

        if (stock[price.id]) stock[price.id] += item.count;else stock[price.id] = item.count; // Deducts from the global stock

        if (stocksum[item.id]) stocksum[item.id] += item.count;else stocksum[item.id] = item.count; // Stats for each ordered item

        counter.hit(item.id, item.count);
        counter.hit(price.id, item.count); // Add to list of ordered products
      ...
      NOSQL('products').modify({
        prices: function prices(val) {
          for (var i = 0; i < val.length; i++) {
            var price = val[i];
            if (stock[price.id]) price.stock -= stock[price.id];
          }

          return val;
        },
        stock: function stock(val, doc) {
          return val - stocksum[doc.id];
        }
      })["in"]('id', options.id).log('Update stock, order #: ' + model.id, model.name + ' (customer)'); // Writes stats

      counter.hit('all'); // Stats of user orders
      ...
      model.isnewsletter = undefined;
      model.isemail = undefined;
      model.isterms = undefined; // Inserts order into the database

      db.insert(model);
      $.success(true, model.id);
      EMIT('orders.save', model); // Sends email
  ...
  schema.setRemove(function ($) {
    var id = $.body.id;
    var user = $.user.name;
    NOSQL('orders').remove().backup(user).log('Remove: ' + id, user).where('id', id).callback(function () {
      return $.success();
    });
  });
  schema.addWorkflow('toggle', function ($) {
    ...
    if (log) NOSQL('orders').modify(upd).log(log + ': ' + arr.join(', '), user)["in"]('id', arr).callback(function () {
      return $.success();
    });else $.success();
  }); // Clears DB
  ...
  schema.addWorkflow('clear', function ($) {
    var user = $.user.name;
    NOSQL('orders').remove().backup(user).log('Clear all orders', user).callback(function () {
      return $.success();
    });
  }); // Stats
  ...
  schema.addOperation('dashboard', function (error, model, options, callback) {
    ...
    NOSQL('orders').find().prepare(prepare).callback(function () {
  ...
  schema.addWorkflow('paid', function ($) {
    NOSQL('orders').modify({
      ispaid: true,
      datepaid: F.datetime
    }).where('ispaid', false).where('id', $.id || $.options.id).callback(function (err, count) {
