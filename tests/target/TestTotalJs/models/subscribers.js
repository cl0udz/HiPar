...
NEWSCHEMA('Subscriber').make(function (schema) {
  ...
  schema.define('language', 'String(2)'); // Saves the model into the database
  ...
  schema.setRemove(function ($) {
    var id = $.body.id;
    var user = $.user.name;
    NOSQL('subscribers').remove().backup(user).log('Remove: ' + id, user).where('email', id).callback(function () {
      return $.success();
    });
  }); // Performs download
  ...
  schema.addWorkflow('unsubscribe', function ($) {
    ADMIN.notify({
      type: 'subscribers.unsubscribe',
      message: $.query.email
    });
    NOSQL('subscribers').modify({
      unsubscribed: true,
      dateupdated: F.datetime
