...
NEWSCHEMA('File').make(function (schema) {
  ...
  schema.addWorkflow('clear', function ($) {
    ...
    var count = 0;
    var db = {}; // This can be a longer operation, therefore respond
      ...
      files.wait(function (file, next) {
        databases.wait(function (filename, resume) {
          ...
          CLEANUP(Fs.createReadStream(filename).on('data', function (chunk) {
            ...
            if (tmp.length && Buffer.concat(tmp, tmp[0].length + (tmp[1] ? tmp[1].length : 0)).indexOf(file.buffer) !== -1) file.is = true;
          }), resume);
        }, function () {
          if (!file.is) {
            remove.push(file);
        ...
        remove.wait(function (item, next) {
          return storage.remove(item.id, next);
        }, function () {
          return ADMIN.notify({
            type: 'files.clear',
            message: count + ''
