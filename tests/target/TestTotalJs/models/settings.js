...
NEWSCHEMA('Settings').make(function (schema) {
  ...
  schema.define('smtpoptions', 'JSON'); // PayPal account

  schema.define('paypaluser', String);
  schema.define('paypalpassword', String);
  schema.define('paypalsignature', String);
  schema.define('paypaldebug', Boolean); // Saves settings into the file

  schema.setSave(function ($) {
    var model = $.model;
    var settings = U.extend({}, model.$clean());
    if (settings.url.endsWith('/')) settings.url = settings.url.substring(0, settings.url.length - 1);
    settings.datebackup = F.datetime;
    NOSQL('settings_backup').insert(JSON.parse(JSON.stringify(settings)));
    settings.datebackup = undefined; // Writes settings into the file
  ...
  schema.setGet(function ($) {
    Fs.readFile(filename, function (err, data) {
      ...
      } else settings = data.toString('utf8').parseJSON(true);
  ...
  schema.addWorkflow('smtp', function ($) {
    var model = $.model;
    if (model.smtp) F.useSMTP(model.smtp, model.smtpoptions ? model.smtpoptions.parseJSON() : '', function (err) {
      return err ? $.invalid(err) : $.success();
    });else $.success();
  ...
  schema.addWorkflow('load', function ($) {
    schema.get(null, function (err, settings) {
      ...
      F.config.url = settings.url; // Refreshes internal informations

      !settings.users && (settings.users = []); // Adds an admin (service) account
      ...
      settings.users = users; // Rewrites internal framework settings

      F.config['mail-address-from'] = settings.emailsender;
      F.config['mail-address-reply'] = settings.emailreply; // Currency settings
