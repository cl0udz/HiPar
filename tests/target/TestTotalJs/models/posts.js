...
NEWSCHEMA('Post').make(function (schema) {
  ...
  schema.define('signals', '[String(30)]'); // Gets listing
...
function refresh() {
  ...
  var prepare = function prepare(doc) {
    return doc.ispublished && categories[doc.category] && (categories[doc.category] += 1);
  };

  config.posts && config.posts.forEach(function (item) {
    return categories[item.id] = 0;
  });
  NOSQL('posts').find().prepare(prepare).callback(function () {
    var output = [];
    Object.keys(categories).forEach(function (key) {
      var category = config.posts.findItem('id', key);
      category && output.push({
        id: key,
        name: category.name,
        linker: key.slug(),
