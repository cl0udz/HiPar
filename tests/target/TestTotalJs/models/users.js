...
var COOKIE = '__user';
var SECRET = '06eshopToTal11';
var ONLINE = {}; // Creates a cookie

exports.login = function (controller, id) {
  controller.cookie(COOKIE, F.encrypt({
    id: id,
    ip: controller.ip
  }, SECRET, true), '6 days');
}; // Removes a cookie
...
exports.session = function (user) {
  return ONLINE[user.id] = {
    ...
    email: user.email,
    ticks: F.datetime.add('30 minutes').getTime(),
    phone: user.phone,
...
NEWSCHEMA('User').make(function (schema) {
  ...
  schema.define('isconfirmed', Boolean); // Gets a specific user
  ...
  schema.setSave(function ($) {
    ...
    NOSQL('users').modify(model).backup(user).log('Update: ' + model.id, user).where('id', model.id).callback(function (count) {
      if (model.isblocked) // Logout user
        delete ONLINE[model.id];else if (ONLINE[model.id]) // Modifies session
      ...
      if (count) {
        ...
        ADMIN.notify({
          type: 'users.save',
          message: model.firstname + ' ' + model.lastname
        });
      }
    });
  ...
  schema.setRemove(function ($) {
    var id = $.body.id;
    var user = $.user.name;
    NOSQL('users').remove().backup(user).log('Remove: ' + id, user).where('id', id).callback(function () {
      return $.success();
    });
  }); // Clears DB
  ...
  schema.addWorkflow('clear', function ($) {
    var user = $.user.name;
    NOSQL('users').remove().backup(user).log('Clear all users', user).callback(function () {
      return $.success();
    });
  }); // Gets listing
...
NEWSCHEMA('UserSettings').make(function (schema) {
  ...
  schema.define('passwordreply', 'String(30)'); // Validation

  schema.required('password', function (model) {
    return model.ispassword;
  });
  schema.setSave(function ($) {
    var model = $.model;
    ...
    if (model.ispassword) model.password = model.password.hash('sha1');else model.password = undefined;
    ...
    NOSQL('users').modify(model).where('id', $.user.id).log('Update account, IP: ' + $.controller.ip, $.user.name).callback(function () {
      return $.success();
    });
  });
  schema.setGet(function ($) {
...
NEWSCHEMA('UserLogin').make(function (schema) {
  ...
  schema.setPrepare(function (name, value) {
    if (name === 'password') return value.hash('sha1');
  });
...
NEWSCHEMA('UserPassword').make(function (schema) {
  ...
  schema.addWorkflow('exec', function (error, model, options, callback, controller) {
    $GET('User', model, function (err, response) {
      ...
      response.hash = F.encrypt({
        id: response.id,
        expire: F.datetime.add('2 days').getTime()
      });
...
ON('service', function (counter) {
  ...
  for (var i = 0, length = users.length; i < length; i++) {
    ...
    if (user.ticks < ticks) delete ONLINE[users[i]];
  }
