...
NEWSCHEMA('Newsletter').make(function (schema) {
  ...
  schema.define('widgets', '[Object]'); // Gets listing
  ...
  schema.setRemove(function ($) {
    var id = $.body.id;
    var user = $.user.name;
    NOSQL('newsletters').remove().backup(user).log('Remove: ' + id, user).where('id', id).callback(function () {
      return $.success();
    });
  ...
  schema.setSave(function ($) {
    ...
    db.callback(function () {
      ADMIN.notify({
        type: 'newsletters.save',
        message: model.name
      });
      EMIT('newsletter.save', model);
      if ($.model.send) $.model.$workflow('send', $.callback);else $.success();
    });
  }); // Clears database
    ...
    newsletter.body.CMSrender(newsletter.widgets, function (body) {
      ...
      NOSQL('subscribers').find().where('unsubscribed', false).callback(function (err, response) {
        ...
        response.limit(10, function (items, next) {
          ...
          ADMIN.notify({
            type: 'newsletters.percentage',
            message: F.global.newsletter.percentage + ''
          }); // Sends email
          // Each 100 email waits 1 minute ....

          if (sum % 100 === 0) setTimeout(function () {
            return Mail.send2(messages, next);
          }, 60000);else Mail.send2(messages, function () {
            return setTimeout(next, 2000);
          });
        }, function () {
          ADMIN.notify({
            type: 'newsletters.sent',
            message: repository.page.name
          });
          NOSQL('newsletters').modify({
            count: count,
            datesent: F.datetime
          }).where('id', F.global.newsletter.id);
...
function prepare_urladdress(body) {
  return body.replace(REG_URL, function (text) {
    return text[0] === 'h' ? 'href="' + F.global.config.url + '/' : 'src="' + F.global.config.url + '/';
  });
}

function internal_notvalidaddress(err, message) {
  if (err) console.log('---> problem in email', message.addressTo);
}