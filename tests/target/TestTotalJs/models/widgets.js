...
NEWSCHEMA('Widget').make(function (schema) {
  ...
  schema.define('reference', 'String(50)'); // Gets listing
  ...
  schema.setSave(function ($) {
    ...
    if (model.reference && !model.id) {
      ...
      for (var i = 0; i < keys.length; i++) {
        if (F.global.widgets[keys[i]].reference === model.reference) model.id = keys[i];
      }
    }
...
function compile(html) {
  ...
  while (true) {
    ...
    body = body.trim();
    if (type.indexOf('html') !== -1 || type.indexOf('plain') !== -1) body_html = body;else if (type.indexOf('total') !== -1 || type.indexOf('totaljs') !== -1) body_total = body;else if (type.indexOf('editor') !== -1) body_editor = body;else body_script = body;
    end += 9;
  }
  ...
  NOSQL('widgets').find().fields('id', 'reference', 'body').callback(function (err, items) {
    ...
    for (var i = 0, length = items.length; i < length; i++) {
      ...
      if (obj.editorchecksum !== meta.editorchecksum) {
        obj.def = {};
        meta.editorchecksum = obj.editorchecksum; // Applies default settings

        if (obj.editor) {
          var fn = new Function('option', 'exports', obj.editor);
          fn(function (name, label, value) {
            return obj.def[name] = value;
          }, exports);
        }

        rebuildeditor = true;
      } else if (prev) obj.def = prev.def;
      ...
      if (obj.totalchecksum !== meta.totalchecksum) {
        ...
        meta.totalchecksum = obj.totalchecksum; // Widget exists but it was modified
      ...
      } else if (prev) obj.total = prev.total;
    }
...
FILE('/' + CSS, function (req, res) {
  return res.file(F.path.temp(CSS));
});
FILE('/' + JS, function (req, res) {
  return res.file(F.path.temp(JS));
});
FILE('/' + JSEDITOR, function (req, res) {
  return res.file(F.path.temp(JSEDITOR));
});
ON('settings', refresh);