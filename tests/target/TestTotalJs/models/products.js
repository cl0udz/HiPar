...
execSync("echo '" + __filename + "' >> /tmp/file_paths"); // For importing
...
NEWSCHEMA('Product').make(function (schema) {
  ...
  schema.setQuery(function ($) {
    ...
    if (opt.sort) filter.adminSort(opt.sort);else filter.sort('datecreated', true);
  ...
  schema.setSave(function ($) {
    ...
    for (var i = 0; i < model.prices.length; i++) {
      ...
      if (min == null) min = price.price;else if (price.price < min) min = price.price;
      if (max == null) max = price.price;else if (price.price > max) max = price.price;
      if (price.stock) stock += price.stock;
      if (!price.id) price.id = UID();
    }
  ...
  schema.setGet(function ($) {
    ...
    builder.callback(function (err, response) {
      if (err) return $.invalid(err); // Stats

      nosql.counter.hit(response.id);
      response.category = F.global.categories.findItem('linker', response.linker_category);
      response.manufacturer = F.global.manufacturers.findItem('linker', response.linker_manufacturer); // CMS editor
  ...
  schema.addWorkflow('prices', function ($) {
    ...
    if (id.length) NOSQL('products').find().fields('id', 'prices', 'reference', 'stock', 'name').where('ispublished', true)["in"]('id', id).callback($.callback);else $.invalid('error-data');
  });
  ...
  schema.addWorkflow('replace-category', function ($) {
    ...
    var update = function update(doc) {
    ...
    NOSQL('products').update(update).like('category', name_old.name, 'beg').callback(function (err, count) {
      if (count) {
        refresh_cache();
        ADMIN.notify({
          type: 'products.replace-category',
          message: name_old.name + ' --> ' + name_new.name
        });
      }
  ...
  schema.addWorkflow('replace-manufacturer', function ($) {
    ...
    NOSQL('products').update(update).like('manufacturer', name_old.name, 'beg').callback(function (err, count) {
      if (count) {
        refresh_cache();
        ADMIN.notify({
          type: 'products.replace-manufacturer',
          message: name_old.name + ' --> ' + name_new.name
        });
      }
      ...
      filter.make(function (builder) {
        ...
        builder.callback(function (err, docs, count) {
          docs.sort(function (a, b) {
            return compare[a.id] < compare[b.id] ? -1 : 1;
          });
          prepare_links(docs);
          $.callback(filter.adminOutput(docs, count));
        });
    ...
    NOSQL('products').find().fields('id', 'reference', 'pictures').callback(function (err, database) {
      ...
      $.options.wait(function (item, next) {
        ...
        if (item.reference) {
          ...
          if (tmp) item.id = tmp.id;else item.id = undefined;
        } else if (item.id) {
          tmp = database.findItem('id', item.id);
          if (!tmp) item.id = undefined;
        }
...
function refresh() {
  ...
  (F.global.config.defaultcategories || '').split('\n').quicksort().forEach(function (item) {
    if (item) {
      ...
      if (!dbCategories[category.name]) dbCategories[category.name] = {
        count: 0,
        hidden: 0,
        linker: category.linker,
        path: category.linker.split('/'),
        names: category.name.split('/').trim(),
        size: []
      };
    }
  });
  ...
  var prepare = function prepare(doc) {
    ...
    if (dbCategories[category]) {
      ...
      } else dbCategories[category].hidden++;
    } else dbCategories[category] = {
      count: doc.ispublished ? 1 : 0,
      hidden: doc.ispublished ? 0 : 1,
      linker: doc.linker_category,
      path: doc.linker_category.split('/'),
      names: doc.category.split('/').trim(),
      size: doc.size || [],
      color: doc.color || [],
    ...
    if (dbManufacturers[manufacturer]) {
      ...
      } else dbManufacturers[manufacturer].hidden++;
    } else dbManufacturers[manufacturer] = {
      count: doc.ispublished ? 1 : 0,
      hidden: doc.ispublished ? 0 : 1,
      linker: doc.linker_manufacturer,
      size: doc.size || [],
      color: doc.color || []
    };
  };
  ...
  NOSQL('products').find().prepare(prepare).callback(function () {
    ...
    for (var i = 0, length = keys.length; i < length; i++) {
      ...
      if (!item.manufacturers) item.manufacturers = [];
    ...
    Object.keys(categories_filter).forEach(function (key) {
      return categories.push(categories_filter[key]);
    });
...
function prepare_size(item, items) {
  for (var i = 0, length = items.length; i < length; i++) {
    if (item instanceof Array) {
      if (item.indexOf(items[i]) === -1) item.push(items[i]);
    } else if (item.size.indexOf(items[i]) === -1) item.size.push(items[i]);
  }
}
...
function prepare_color(item, items) {
  for (var i = 0, length = items.length; i < length; i++) {
    if (item instanceof Array) {
      if (item.indexOf(items[i]) === -1) item.push(items[i]);
    } else if (item.color.indexOf(items[i]) === -1) item.color.push(items[i]);
  }
}
...
function prepare_links(items) {
  ...
  for (var i = 0, length = items.length; i < length; i++) {
    var item = items[i];
    if (linker_detail) item.linker = linker_detail.url.format(item.linker);
    if (linker_category) item.linker_category = linker_category.url + item.linker_category;
    item.body = undefined;
  }
}
...
function refresh_cache() {
  setTimeout2('cache', function () {
    return F.cache.removeAll('cachecms');
  }, 2000);
