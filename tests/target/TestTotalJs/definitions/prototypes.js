...
F.prototypes(function (proto) {
  // Converting values
  var convert = function convert(value, type) {
    if (type === undefined || type === String) return value;
    if (type === Number) return value.trim().parseFloat();
    if (type === Date) return value.trim().parseDate();
    if (type === Boolean) return value.trim().parseBoolean();
    return value;
  ...
  proto.DatabaseBuilder.adminFilter = function (name, obj, type, key) {
    ...
    if (type === undefined || type === String) return builder.search(key, value);
