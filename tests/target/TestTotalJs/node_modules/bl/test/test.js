...
var tape = require('tape'),
    ...
    encodings = ('hex utf8 utf-8 ascii binary base64' + (process.browser ? '' : ' ucs2 ucs-2 utf16le utf-16le')).split(' '); // run the indexOf tests
...
tape('multiple bytes from crazy nested buffer lists', function (t) {
  var bl = new BufferList();
  bl.append(new BufferList([new BufferList([new BufferList(Buffer.from('abc')), Buffer.from('d'), new BufferList(Buffer.from('efg'))]), new BufferList([Buffer.from('hi')]), new BufferList(Buffer.from('j'))]));
  t.equal(bl.length, 10);
  t.equal(bl.slice(0, 10).toString('ascii'), 'abcdefghij');
  t.equal(bl.slice(3, 10).toString('ascii'), 'defghij');
  t.equal(bl.slice(3, 6).toString('ascii'), 'def');
...
tape('append chainable (test results)', function (t) {
  var bl = new BufferList('abc').append([new BufferList('def')]).append(new BufferList([Buffer.from('ghi'), new BufferList('jkl')])).append([Buffer.from('mnop'), new BufferList([Buffer.from('qrstu'), Buffer.from('vwxyz')])]);
  t.equal(bl.length, 26);
  t.equal(bl.slice().toString('ascii'), 'abcdefghijklmnopqrstuvwxyz');
  t.end();
});
...
!process.browser && tape('test stream', function (t) {
  var random = crypto.randomBytes(65534),
    ...
    bl.pipe(fs.createWriteStream('/tmp/bl_test_rnd_out.dat')).on('close', function () {
...
!process.browser && tape('destroy with pipe before read end', function (t) {
  t.plan(2);
  var bl = new BufferList();
  fs.createReadStream(__dirname + '/test.js').pipe(bl);
  bl.destroy();
  t.equal(bl._bufs.length, 0);
  t.equal(bl.length, 0);
});
...
!process.browser && tape('destroy with pipe after read end', function (t) {
  t.plan(2);
  var bl = new BufferList();
  fs.createReadStream(__dirname + '/test.js').on('end', onEnd).pipe(bl);
