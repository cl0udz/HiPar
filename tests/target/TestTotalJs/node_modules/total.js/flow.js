...
MP.on = function (name, fn) {
  ...
  if (self.$events[name]) self.$events[name].push(fn);else self.$events[name] = [fn];
  return self;
};
...
MP.removeAllListeners = function (name) {
  if (this.$events) {
    if (name === true) this.$events = {};else if (name) this.$events[name] = undefined;else this.$events = {};
  }
...
MP.send = function (outputindex) {
  ...
  if (!self.main.$can(false, self.toid, outputindex)) return count;

  for (var i = 0; i < outputs.length; i++) {
    var output = outputs[i];
    if (output.disabled || output.paused) continue;
...
FP.register = function (name, declaration) {
  var self = this;
  if (typeof declaration === 'string') declaration = new Function('instance', declaration);
  var cache;
...
FP.use = function (schema, callback) {
  var self = this;
  if (typeof schema === 'string') schema = schema.parseJSON(true); // schema.COMPONENT_ID.component = 'condition';
  ...
  if (schema) {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      schema[key].cache = {};
      if (!component) err.push(key, '"' + instance.component + '" component not found.');
    }

    self.meta.flow = schema;
  } else err.push('schema', 'Flow schema is invalid.');
...
FP.$can = function (isinput, id, index) {
  var self = this;
  if (!self.meta.flow.paused) return true;
  var key = (isinput ? 'input' : 'output') + D + id + D + index;
  if (!self.meta.flow.paused[key]) return true;
}; // path = ID__INPUTINDEX
...
FP.trigger2 = function (path, data, controller) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (flow.component === path[0]) obj = self.trigger(key + D + (path.length === 1 ? 0 : path[1]), data, controller, events);
  }
