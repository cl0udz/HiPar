...
var REGAPPEND = /\/--[a-z0-9]+/i;
var REGAPPENDREPLACE = /\/--/g;
var META = {};
META.version = 1;
  ...
  async.push(function (next) {
    ...
    U.ls(F.path.root(CONF.directory_bundles), function (files) {
      ...
      files.wait(function (filename, resume) {
        if (!filename.endsWith('.bundle')) return resume();
        var dbpath = CONF.directory_databases;
        F.restore(filename, target, resume, function (p, dir) {
          if (dir) {
            if (!p.startsWith(dbpath) && META.directories.indexOf(p) === -1) META.directories.push(p);
          } else {
            var dirname = p.substring(0, p.length - U.getName(p).length);
            if (dirname && dirname !== '/') dirs[dirname] = true; // handle files in bundle to merge
            ...
            if (META.files.indexOf(p) === -1) META.files.push(p);
          }
  ...
  async.push(function (next) {
    U.ls(path, function (files, dirs) {
      ...
      for (var i = 0, length = files.length; i < length; i++) {
        ...
        var type = 0;
        if (file.startsWith(CONF.directory_databases) || file.startsWith('/flow/') || file.startsWith('/dashboard/')) type = 1;else if (REGAPPEND.test(file)) {
          file = file.replace(REGAPPENDREPLACE, '/');
          type = 3;
        } else if (file.startsWith(CONF.directory_public)) type = 2;
        Files.push({
          name: file,
          filename: files[i],
          type: type
...
function cleanFiles(callback) {
  ...
  if (meta.directories && meta.directories.length) {
    ...
    for (var i = 0, length = meta.directories.length; i < length; i++) {
      try {
        if (!blacklist[meta.directories[i]]) Fs.rmdirSync(Path.join(path, meta.directories[i]));
      } catch (e) {}
    }
  }
...
function createDirectories(dirs, callback) {
  ...
  for (var i = 0, length = dirs.length; i < length; i++) {
    ...
    if (META.directories.indexOf(p) === -1) META.directories.push(p);
...
function copyFiles(files, callback) {
  ...
  files.wait(function (file, next) {
    if (!META.ignore(file.name)) return next();
    ...
    if (file.type !== 1 && META.files.indexOf(p) === -1) META.files.push(p);
    if (exists && (ext === 'resource' || !ext && file.name.substring(1, 7) === 'config' || INTERNAL[file.name])) append = true;

    if (CONSOLE && exists) {
      CONF.allow_debug && F.consoledebug(append ? 'EXT:' : 'REW:', p);
    } else F.consoledebug(append ? 'EXT:' : 'COP:', p);

    if (append) {
      Fs.appendFile(filename, '\n' + Fs.readFileSync(file.filename).toString('utf8'), next);
    } else copyFile(file.filename, filename, next);

    if (CONSOLE && exists) CONF.allow_debug && F.consoledebug('REW:', p);else F.consoledebug('COP:', p);
  }, callback);
}
