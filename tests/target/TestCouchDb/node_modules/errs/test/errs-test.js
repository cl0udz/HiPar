...
vows.describe('errs').addBatch({
  "Using errs module": {
    "the register() method": {
      "should register the prototype": function shouldRegisterThePrototype() {
        errs.register('named', fixtures.NamedError);
        assert.equal(errs.registered['named'], fixtures.NamedError);
      },
      "should register an error without providing its name": function shouldRegisterAnErrorWithoutProvidingItsName() {
    ...
    "the create() method with": {
      ...
      "an object": {
        ...
        "that has a name": {
          ...
          "should respect that name in the stack trace": function shouldRespectThatNameInTheStackTrace(err) {
    ...
    "the handle() method": {
      "with a callback": {
        ...
        "should invoke the callback with the error": function shouldInvokeTheCallbackWithTheError(_, err) {
      ...
      "with an EventEmitter (i.e. stream)": {
        ...
        "should emit the `error` event": function shouldEmitTheErrorEvent(_, err) {
          assert.equal(err, this.err);
        }
      },
  ...
  "Using errs module": {
    "the unregister() method": {
      "should unregister the prototype": function shouldUnregisterThePrototype() {
  ...
  "Using errs module": {
    "the merge() method": {
      "supports": {
        "an undefined error": function anUndefinedError() {
          var err = errs.merge(undefined, {
            message: 'oh noes!'
          });
          assert.equal(err.message, 'oh noes!');
          assert.instanceOf(err, Error);
        },
        "a null error": function aNullError() {
          var err = errs.merge(null, {
            message: 'oh noes!'
          });
          assert.equal(err.message, 'oh noes!');
          assert.instanceOf(err, Error);
        },
        "a false error": function aFalseError() {
          var err = errs.merge(false, {
            message: 'oh noes!'
          });
          assert.equal(err.message, 'oh noes!');
          assert.instanceOf(err, Error);
        },
        "a string error": function aStringError() {
          var err = errs.merge('wat', {
            message: 'oh noes!'
          });
          assert.equal(err.message, 'oh noes!');
          assert.instanceOf(err, Error);
      ...
      "should preserve custom properties": function shouldPreserveCustomProperties() {
      ...
      "should have a stack trace": function shouldHaveAStackTrace() {
        var err = new Error('Msg!');
        err = errs.merge(err, {});
        assert.isTrue(Array.isArray(err.stacktrace));
      },
      "should preserve message specified in create": function shouldPreserveMessageSpecifiedInCreate() {
        var err = new Error('Msg!');
        err = errs.merge(err, {
          message: "Override!"
        });
      ...
      "should preserve properties specified": function shouldPreservePropertiesSpecified() {
        var err = new Error('Msg!');
        err = errs.merge(err, {
          ns: "test"
        });
      ...
      "with a truthy value": function withATruthyValue() {
      ...
      "with a truthy stack": function withATruthyStack() {
        var err = errs.merge({
          stack: true
        }, {
          message: 'Override!',
      ...
      "with an Array stack": function withAnArrayStack() {
        var err = errs.merge({
          stack: []
        }, {
          message: 'Override!',
  ...
  "Using errs module": {
    "Error.prototype.toJSON": {
      "should exist": function shouldExist() {
      ...
      "should be writable": function shouldBeWritable() {
