...
'use strict'; // Core routines for event emitters
...
var Changes = require('./stream').Changes;

var querystring = require('querystring'); // Use the library timeout functions, primarily so the test suite can catch errors.


var setTimeout = lib.setTimeout;
var clearTimeout = lib.clearTimeout;
...
var RESPONSE_GRACE_TIME = 5000; // https://console.bluemix.net/docs/services/Cloudant/api/database.html#get-changes

var CHANGES_PARAMETERS = ['conflicts', 'descending', 'doc_ids', 'feed', 'filter', 'heartbeat', 'include_docs', 'limit', 'seq_interval', 'since', 'style', 'timeout', // additional parameters as defined in http://docs.couchdb.org/en/2.0.0/api/database/changes.html
'att_encoding_info', 'attachments', 'last-event-id', 'view']; // https://console.bluemix.net/docs/services/Cloudant/api/advanced.html#-get-_db_updates-

var DB_UPDATES_PARAMETERS = ['descending', 'feed', 'heartbeat', 'limit', 'since', 'timeout'];
...
function Feed(opts) {
  ...
  if (typeof opts.httpAgent === 'function') {
    self.httpAgent = opts.httpAgent;
  } else {
    self.httpAgent = request.defaults(opts.httpAgent || {});
  }
...
Feed.prototype.start = Feed.prototype.follow = function follow_feed() {
  var self = this;
...
Feed.prototype.confirm = function confirm_feed() {
  ...
  req = self.httpAgent(req, db_response);
  ...
  function db_response(er, resp, body) {
    ...
    self.emit('confirm', db); // `/_db_updates` does not support since=-1, we use since="now" instead.
...
Feed.prototype.query = function query_feed() {
  ...
  if (query_params.view) {
    query_params.filter = '_view';
  }
  ...
  var now = new Date();
  var feed_ts = lib.JDUP(now);
  var feed_id = process.env.follow_debug ? feed_ts.match(/\.(\d\d\d)Z$/)[1] : feed_ts;
  self.log.debug('Feed query ' + feed_id + ': ' + lib.scrub_creds(feed_url));
  var feed_request = self.httpAgent(req);
  feed_request.on('response', function (res) {
    // Simulate the old onResponse option.
    on_feed_response(null, res, res.body);
  });
  feed_request.on('error', on_feed_response); // The response headers must arrive within one heartbeat.
  ...
  function on_feed_response(er, resp, body) {
    ...
    if (resp.statusCode === 404) {
      destroy_response(resp);
      self.log.warn('Database not found. Stopping changes feed after seq ' + self.since);
      var del_er = new Error('Database deleted after change: ' + self.since);
      del_er.deleted = true;
...
Feed.prototype.prep = function prep_request(changes_stream) {
  ...
  self.pending.wait_timer = null; // Just re-run the pause or resume to do the needful on changes_stream (self.pending.request).
...
Feed.prototype.wait = function wait_for_event() {
  ...
  msg += ': ' + self.db_safe;
  self.log.debug(msg);
  self.pending.wait_timer = setTimeout(function () {
    self.on_timeout();
  }, timeout_ms);
};
...
Feed.prototype.pause = function () {
  var self = this;
  var was_paused = self.is_paused; // Emit pause after pausing the stream, to allow listeners to react.
...
Feed.prototype.resume = function () {
  var self = this;
  var was_paused = self.is_paused; // Emit resume before resuming the data feed, to allow listeners to prepare.
...
Feed.prototype.on_couch_data = function on_couch_data(change) {
  ...
  change = JSON.parse(change); // self.log.debug('Object:\n' + util.inspect(change));

  if ('last_seq' in change) {
    self.log.debug('Found last_seq in change. Retrying from seq ' + change.last_seq);
  ...
  self.on_change(change); // on_change() might work its way all the way to a "change" event, and the listener
...
Feed.prototype.stop = function (val) {
  var self = this;
  self.log.debug('Stop'); // Die with no errors.
...
Feed.prototype.on_change = function on_change(change) {
  ...
  if (result) {
    ...
    self.log.debug('Builtin filter FAIL for change: ' + change.seq); // Even with a filtered change, a "catchup" event might still be appropriate.
...
Feed.prototype.restart = function restart() {
  var self = this;
  self.emit('restart'); // Kill ourselves and then start up once again
