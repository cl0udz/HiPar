...
Changes.prototype.write_continuous = function (data) {
  ...
  self.log.debug('write: ' + util.inspect({
    'data': data,
    'buf': buf
  ...
  while ((offset = buf.indexOf('\n')) >= 0) {
    ...
    if (json === '') {// Heartbeats (empty strings) are fine, but otherwise confirm valid JSON.
    } else if (json[0] !== '{') {
...
Changes.prototype.end = function (data, encoding) {
  var self = this;
  self.is_ending = true;
  self.writable = false; // Always call write, even with no data, so it can fire the "end" event.
  ...
  if (self.feed === 'longpoll') {
    ...
    self.changes = changes.results.map(function (change) {
...
Changes.prototype.destroy = function () {
  ...
  if (self.source && self.source.__isRequestRequest && self.source.response && typeof self.source.response.destroy === 'function') {
    self.source.response.destroy();
  }
};

Changes.prototype.destroySoon = function () {
  throw new Error('not implemented'); // return self.request.destroySoon()
}; //
...
Changes.prototype.normalize_data = function (data, encoding) {
  ...
  if (self.expect === null) {
    self.expect = self.feed === 'longpoll' ? DEFS.longpoll_header : '';
  }
  ...
  var expected_remainder = self.expect.substr(expected_part.length);
...
Changes.prototype.error = function (er) {
  ...
  self.readable = false;
  self.writable = false;
  self.emit('error', er); // The write() method sometimes returns this value, so if there was an error, make write() return false.
