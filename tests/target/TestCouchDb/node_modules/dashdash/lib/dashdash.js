...
function makeIndent(arg, deflen, name) {
  if (arg === null || arg === undefined) return space(deflen);else if (typeof arg === 'number') return space(arg);else if (typeof arg === 'string') return arg;else assert.fail('invalid "' + name + '": not a string or number: ' + arg);
}
/**
...
function Parser(config) {
  ...
  var self = this; // Allow interspersed arguments (true by default).

  this.interspersed = config.interspersed !== undefined ? config.interspersed : true; // Don't allow unknown flags (true by default).
...
Parser.prototype.parse = function parse(inputs) {
  var self = this; // Old API was `parse([argv, [slice]])`
  ...
  assert.optionalArrayOfString(inputs.argv, 'inputs.argv'); //assert.optionalNumber(slice, 'slice');
  ...
  outer: while (i < args.length) {
    var arg = args[i]; // End of options marker.
    ...
    if (arg === '--') {
      ...
      if (!option) {
        if (!this.allowUnknown) throw new Error(format('unknown option: "--%s"', name));else if (this.interspersed) _args.push(arg);else break outer;
      } else {
      ...
      _args.push(arg); // An arg and interspersed args are not allowed, so done options.
  ...
  this.options.forEach(function (o) {
    ...
    if (nameSort === 'length') {
      names.sort(function (a, b) {
        if (a.length < b.length) return -1;else if (b.length < a.length) return 1;else return 0;
      });
    }
...
var BASH_COMPLETION_TEMPLATE_PATH = path.join(__dirname, '../etc/dashdash.bash_completion.in');
/**
...
function bashCompletionFromOptions(args) {
  ...
  assert.string(args.name, 'args.name');
  assert.optionalString(args.specExtra, 'args.specExtra');
  assert.optionalArrayOfString(args.argtypes, 'args.argtypes'); // Gather template data.
...
function synopsisFromOpt(o) {
  ...
  var names = o.names || [o.name]; // `type` here could be undefined if, for example, the command has a
