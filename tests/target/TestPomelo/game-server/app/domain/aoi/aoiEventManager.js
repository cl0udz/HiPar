...
var exp = module.exports; //Add event for aoi
...
function onMobAdd(params) {
  ...
  var ids = area.aoi.getIdsByRange({
    x: mob.x,
    y: mob.y
  }, mob.range, [EntityType.PLAYER])[EntityType.PLAYER];
...
function onPlayerUpdate(params) {
  ...
  if (params.removeObjs.length > 0) {
    messageService.pushMessageToPlayer(uid, 'onRemoveEntities', {
