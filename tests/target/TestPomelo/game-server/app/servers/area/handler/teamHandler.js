...
var Handler = function Handler(app) {
  ...
  this.teamNameArr.length = Object.keys(this.teamNameArr).length; // utils.myPrint('teamNameArr = ', JSON.stringify(this.teamNameArr));
};
/**
...
Handler.prototype.createTeam = function (msg, session, next) {
  ...
  this.app.rpc.manager.teamRemote.createTeam(session, args, function (err, ret) {
    utils.myPrint("ret.result = ", ret.result);
    utils.myPrint("typeof ret.result = ", _typeof(ret.result));
    result = ret.result;
...
Handler.prototype.disbandTeam = function (msg, session, next) {
  ...
  this.app.rpc.manager.teamRemote.disbandTeamById(session, args, function (err, ret) {
    result = ret.result;
...
Handler.prototype.applyJoinTeamReply = function (msg, session, next) {
  ...
  if (msg.reply === consts.TEAM.JOIN_TEAM_REPLY.ACCEPT) {
    ...
    messageService.pushMessageToPlayer({
      ...
      reply: consts.TEAM.JOIN_TEAM_REPLY.REJECT
    });
  }
...
Handler.prototype.kickOut = function (msg, session, next) {
  ...
  var args = {
    captainId: captainId,
    teamId: msg.teamId,
    kickedPlayerId: msg.kickedPlayerId
  };
  this.app.rpc.manager.teamRemote.kickOut(session, args, function (err, ret) {});
  next();
};
/**
...
Handler.prototype.leaveTeam = function (msg, session, next) {
  ...
  var result = consts.TEAM.FAILED;
  utils.myPrint("player.teamId = ", player.teamId);
  utils.myPrint("typeof player.teamId = ", _typeof(player.teamId));
  utils.myPrint("msg.teamId = ", msg.teamId);
  utils.myPrint("typeof msg.teamId = ", _typeof(msg.teamId));
  ...
  this.app.rpc.manager.teamRemote.leaveTeamById(session, args, function (err, ret) {
    result = ret.result;
    utils.myPrint("1 ~ result = ", result);
