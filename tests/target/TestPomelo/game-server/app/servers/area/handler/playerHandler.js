...
handler.enterScene = function (msg, session, next) {
  ...
  userDao.getPlayerAllInfo(playerId, function (err, player) {
    ...
    var map = area.map; // temporary code
    ...
    if (player.teamId > consts.TEAM.TEAM_ID_NONE) {
      ...
      pomelo.app.rpc.manager.teamRemote.updateMemberInfo(session, memberInfo, function (err, ret) {});
    }
  });
};
/**
...
handler.move = function (msg, session, next) {
  ...
  if (timer.addAction(action)) {
    player.isMoving = true; //Update state

    if (player.x !== path[0].x || player.y !== path[0].y) {
      timer.updateObject({
        id: player.entityId,
        type: consts.EntityType.PLAYER
      }, {
        x: player.x,
        y: player.y
      }, path[0]);
...
handler.pickItem = function (msg, session, next) {
  ...
  player.target = target.entityId; // next();
