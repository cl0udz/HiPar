...
module.exports = {
  ...
  RES_CODE: {
    ...
    // success
    ERR_FAIL: -1,
    // Failded without specific reason
  ...
  PLAYER: {
    ...
    reviveTime: 5000,
    RECOVER_WAIT: 10000,
    //You must wait for at lest 10s to start recover hp.
  ...
  TEAM: {
    TEAM_ID_NONE: 0,
    // player without team(not in any team)
    PLAYER_ID_NONE: 0,
    // none player id in a team(placeholder)
    AREA_ID_NONE: 0,
    // none area id (placeholder)
    USER_ID_NONE: 0,
    // none user id (placeholder)
    SERVER_ID_NONE: 0,
    // none server id (placeholder)
    PLAYER_INFO_NONE: '',
    // none player info	(placeholder)
    JOIN_TEAM_RET_CODE: {
      OK: 1,
      // join ok
      NO_POSITION: -1,
   ...
   */
  isPickable: function isPickable(entity) {
   ...
   */
  isAttackable: function isAttackable(entity) {
