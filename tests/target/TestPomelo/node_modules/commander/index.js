...
Command.prototype.option = function (flags, description, fn, defaultValue) {
  var self = this,
      ...
      oname = option.name(),
      name = camelcase(oname); // default as 3rd arg

  if ('function' != typeof fn) defaultValue = fn, fn = null; // preassign default value only for --no-*, [optional], or <required>
  ...
  this.on(oname, function (val) {
    // coercion
    if (null != val && fn) val = fn(val); // unassigned or bool
...
Command.prototype.parse = function (argv) {
  ...
  this._name = this._name || basename(argv[1]); // process argv
...
Command.prototype.executeSubCommand = function (argv, args, unknown) {
  ...
  if ('help' == args[0] && 1 == args.length) this.help(); // <cmd> --help
  ...
  var bin = basename(argv[1]) + '-' + args[0]; // check for ./<bin> first

  var local = path.join(dir, bin); // run it
  ...
  for (var i = 0; i < len; ++i) {
    ...
    if (option) {
      ...
      if (option.required) {
        ...
        this.emit(option.name(), arg); // optional arg
        ...
        this.emit(option.name(), arg); // bool
      } else {
...
Command.prototype.optionHelp = function () {
  var width = this.largestOptionLength(); // Prepend the help information
