...
var http = require('http'),
    ...
    utils = require('./utils'); // environment
...
Server.prototype.handle = function (req, res, out) {
  ...
  function next(err) {
    ...
    if (!layer || res.headerSent) {
      ...
      if (out) return out(err); // error

      if (err) {
        var msg = 'production' == env ? 'Internal Server Error' : err.stack || err.toString(); // output to stderr in a non-test env

        if ('test' != env) console.error(err.stack || err.toString()); // unable to respond
    ...
    try {
      ...
      removed = layer.route;
      req.url = req.url.substr(removed.length); // Ensure leading slash
