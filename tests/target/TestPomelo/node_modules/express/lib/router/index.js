...
Router.prototype.match = function (method, url) {
  return this.find(function (route) {
    return route.match(url) && (route.method == method || method == 'all');
  });
};
/**
...
Router.prototype._dispatch = function (req, res, next) {
  ...
  (function pass(i, err) {
    ...
    req.route = route = self._match(req, i); // implied OPTIONS

    if (!route && 'OPTIONS' == req.method) return self._options(req, res); // no route

    if (!route) return next(err); // we have a route
    // start at param 0
...
Router.prototype._match = function (req, i) {
  var method = req.method.toLowerCase(),
      ...
      captures,
      route,
      keys; // pass HEAD to GET routes

  if ('head' == method) method = 'get'; // routes for this method
  ...
  if (routes = routes[method]) {
    ...
    for (var len = routes.length; i < len; ++i) {
      ...
      if (captures = route.match(path)) {
        keys = route.keys;
        route.params = []; // params from capture groups
...
Router.prototype._route = function (method, path, callbacks) {
  var app = this.app,
      callbacks = utils.flatten(toArray(arguments, 2)); // ensure path was given

  if (!path) throw new Error('app.' + method + '() requires a path'); // create the route
