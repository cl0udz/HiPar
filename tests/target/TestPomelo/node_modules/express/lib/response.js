...
res.send = function (body, headers, status) {
  ...
  status = status || this.statusCode; // allow 0 args as 204

  if (!arguments.length || undefined === body) status = 204; // determine content type
  ...
  if (undefined !== body && !this.header('Content-Length')) {
    this.header('Content-Length', Buffer.isBuffer(body) ? body.length : Buffer.byteLength(body));
...
res.sendfile = function (path, options, fn) {
  ...
  options = options || {}; // support function as second arg
...
res.attachment = function (filename) {
  ...
  this.header('Content-Disposition', filename ? 'attachment; filename="' + basename(filename) + '"' : 'attachment');
  return this;
};
/**
...
res.download = function (path, filename, fn, fn2) {
  var self = this; // support callback as second arg
...
res.clearCookie = function (name, options) {
  ...
  this.cookie(name, '', options ? utils.merge(options, opts) : opts);
};
/**
...
res.redirect = function (url, status) {
  ...
  map.__proto__ = app.redirects; // Attempt mapped redirect

  var mapped = 'function' == typeof map[url] ? map[url](req, this) : map[url]; // Perform redirect

  url = mapped || url; // Relative

  if (!~url.indexOf('://')) {
    // Respect mount-point
    if ('/' != base && 0 != url.indexOf(base)) url = base + url; // Absolute
...
res.local = function (name, val) {
  ...
  return undefined === val ? this._locals[name] : this._locals[name] = val;
};
/**
...
res.locals = res.helpers = function (obj) {
  if (obj) {
    for (var key in obj) {
