...
var Event = require('../../consts/consts').Event; // max member num in a team
...
function Team(teamId) {
  ...
  this.playerNum = 0;
  this.captainId = 0;
  this.playerDataArray = new Array(MAX_MEMBER_NUM); // team channel, push msg within the team
...
Team.prototype.disbandTeam = function () {
  ...
  for (var i in arr) {
    ...
    playerIdArray.push(playerId); //rpc invoke
    ...
    var params = {
      ...
      args: [{
        playerId: playerId,
        instanceId: arr[i].playerData.instanceId
      }]
    };
    utils.myPrint('playerId = ', playerId);
...
Team.prototype.removePlayer = function (playerId, cb) {
  ...
  var _this = this; // async network operation
  ...
  var params = {
    ...
    args: [{
      ...
      instanceId: tmpData.playerData.instanceId
    }]
  };
  utils.myPrint('params = ', JSON.stringify(params));
