...
var Player = function Player(opts) {
  ...
  this.range = opts.range || 2; // player's team id, default 0(not in any team).

  this.teamId = consts.TEAM.TEAM_ID_NONE; // is the team captain, default false

  this.isCaptain = consts.TEAM.NO; // game copy flag
...
module.exports = Player; //emit the event 'died' after it is died
...
Player.prototype.updateTeamMemberInfo = function () {
  if (this.teamId > consts.TEAM.TEAM_ID_NONE) {
    ...
    pomelo.app.rpc.manager.teamRemote.updateMemberInfo(null, memberInfo, function (err, ret) {});
  }
...
Player.prototype.handOverTask = function (taskIds) {
  ...
  for (var i = 0; i < length; i++) {
    ...
    task.save(); // delete this.curTasks[id];
  }
};
/**
