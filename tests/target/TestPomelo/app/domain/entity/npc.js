...
var Npc = function Npc(opts) {
  ...
  this.x = opts.x;
  this.y = opts.y; // console.error('NPC ~ x, y = ', this.x, this.y);
};
...
Npc.prototype.talk = function (player) {
  if (!formula.inRange(player, this, TALK_RANGE)) {
    return {
      result: consts.NPC.NOT_IN_RANGE,
      distance: TALK_RANGE
    };
  }
...
Npc.prototype.traverse = function (route, msg) {
  var player = this.area.getEntity(msg.player); //If don't need task test, just change area.
  ...
  TaskDao.getTaskByIds(player.id, TraverseTask[msg.kindId], function (err, tasks) {
    if (tasks && tasks.length > 0) {
      var task = tasks[0]; //For test only
