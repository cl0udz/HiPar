...
var Instance = function Instance(opts) {
  this.areaId = opts.id;
  this.type = opts.type;
  this.map = opts.map; //The map from player to entity
  ...
  this.channel = null;
  this.playerNum = 0;
  this.emptyTime = Date.now(); //Init AOI

  this.aoi = aoiManager.getService(opts);
  this.aiManager = ai.createManager({
    area: this
  });
...
Instance.prototype.start = function () {
  aoiEventManager.addEvent(this, this.aoi.aoi); //Init mob zones
...
Instance.prototype.removeEntity = function (entityId) {
  ...
  if (!e) return true; //If the entity belong to a subzone, remove it
