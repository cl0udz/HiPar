...
Move.prototype.update = function () {
  ...
  while (travelDistance > 0) {
    if (distance <= travelDistance) {
      ...
      pos = path[index];
      index++; //If the index exceed the last point, means the move is finished
  ...
  this.entity.y = Math.floor(pos.y); //Update the aoi module
  ...
  if (this.entity.type === consts.EntityType.PLAYER) {
    ...
    if (this.tickNumber % 10 === 0) {
      messageService.pushMessageToPlayer({
        uid: this.entity.userId,
        sid: this.entity.serverId
      }, 'onPathCheckout', {
