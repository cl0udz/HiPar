...
Map.prototype.initWeightMap = function () {
  ...
  for (i = 0; i < collisions.length; i++) {
    ...
    if (!!points && points.length > 0) {
      ...
      y2 = Math.ceil(maxy / this.tileH); //regular the poinit to not exceed the map

      x1 = x1 < 0 ? 0 : x1;
      y1 = y1 < 0 ? 0 : y1;
      x2 = x2 > this.rectW ? this.rectW : x2;
      y2 = y2 > this.rectH ? this.rectH : y2; //For all the tile in the polygon's externally rect, check if the tile is in the collision

      for (x = x1; x < x2; x++) {
        for (y = y1; y < y2; y++) {
          p = {
            x: x * this.tileW + this.tileW / 2,
            y: y * this.tileH + this.tileH / 2
          };
          l = this.tileW / 4;
          p1 = {
            x: p.x - l,
          ...
          if (geometry.isInPolygon(p1, polygon) || geometry.isInPolygon(p2, polygon) || geometry.isInPolygon(p3, polygon) || geometry.isInPolygon(p4, polygon)) {
      ...
      y2 = Math.ceil((collision.y + collision.height) / this.tileH); //regular the poinit to not exceed the map
...
Map.prototype.findPath = function (x, y, x1, y1, useCache) {
  ...
  if (this._checkLinePath(x, y, x1, y1)) {
    return {
      ...
      cost: formula.distance(x, y, x1, y1)
    };
  }
  ...
  var ty2 = Math.floor(y1 / this.tileH); //Use cache to get path
