...
var MoveToTarget = require('../action/moveToTarget'); //var FindNearbyPlayer = require('../action/findNearbyPlayer');
...
var Brain = function Brain(blackboard) {
  this.blackboard = blackboard; //try attack and move to target action
  ...
  var attack = new TryAndAdjust({
    ...
    tryAction: new TryAttack({
      blackboard: blackboard,
      getSkillId: function getSkillId(bb) {
        return 1; //normal attack
      }
    })
  }); //loop attack action

  var checkTarget = function checkTarget(bb) {
  ...
  var haveTarget = function haveTarget(bb) {
  ...
  this.action.addChild(attackIfHaveTarget); //this.action.addChild(findTarget);
