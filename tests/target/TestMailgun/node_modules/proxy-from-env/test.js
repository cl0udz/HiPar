...
var getProxyForUrl = require('./').getProxyForUrl; // Runs the callback with process.env temporarily set to env.
...
function testProxyUrl(env, expected, input) {
  assert(_typeof(env) === 'object' && env !== null); // Copy object to make sure that the in param does not get modified between
  // the call of this function and the use of it below.

  env = JSON.parse(JSON.stringify(env));
  var title = 'getProxyForUrl(' + JSON.stringify(input) + ')' + ' === ' + JSON.stringify(expected); // Save call stack for later use.

  var stack = {};
  Error.captureStackTrace(stack, testProxyUrl); // Only use the last stack frame because that shows where this function is
...
describe('getProxyForUrl', function () {
  ...
  describe('http_proxy and HTTP_PROXY', function () {
    ...
    testProxyUrl(env, 'http://http-proxy', parseUrl('http://example')); // jscs:disable requireCamelCaseOrUpperCaseIdentifiers

    env.http_proxy = 'http://priority'; // jscs:enable requireCamelCaseOrUpperCaseIdentifiers

    testProxyUrl(env, 'http://priority', 'http://example');
  });
  describe('http_proxy with non-sensical value', function () {
    var env = {}; // Crazy values should be passed as-is. It is the responsibility of the
    ...
    testProxyUrl(env, 'Crazy \n!() { ::// }', 'http://wow'); // The implementation assumes that the HTTP_PROXY environment variable is
  ...
  describe('https_proxy and HTTPS_PROXY', function () {
    var env = {}; // Assert that there is no fall back to http_proxy

    env.HTTP_PROXY = 'http://unexpected.proxy';
    testProxyUrl(env, '', 'https://example');
    env.HTTPS_PROXY = 'http://https-proxy';
    testProxyUrl(env, 'http://https-proxy', 'https://example'); // jscs:disable requireCamelCaseOrUpperCaseIdentifiers

    env.https_proxy = 'http://priority'; // jscs:enable requireCamelCaseOrUpperCaseIdentifiers

    testProxyUrl(env, 'http://priority', 'https://example');
  });
  describe('ftp_proxy', function () {
    var env = {}; // Something else than http_proxy / https, as a sanity check.
  ...
  describe('all_proxy', function () {
    ...
    testProxyUrl(env, 'http://catch-all', 'https://example'); // jscs:disable requireCamelCaseOrUpperCaseIdentifiers

    env.all_proxy = 'http://priority'; // jscs:enable requireCamelCaseOrUpperCaseIdentifiers
  ...
  describe('all_proxy without scheme', function () {
    ...
    testProxyUrl(env, 'https://noscheme', 'https://example'); // The module does not impose restrictions on the scheme.

    testProxyUrl(env, 'bogus-scheme://noscheme', 'bogus-scheme://example'); // But the URL should still be valid.

    testProxyUrl(env, '', 'bogus');
  });
  describe('no_proxy empty', function () {
    var env = {};
    env.HTTPS_PROXY = 'http://proxy'; // NO_PROXY set but empty.

    env.NO_PROXY = '';
    testProxyUrl(env, 'http://proxy', 'https://example'); // No entries in NO_PROXY (comma).

    env.NO_PROXY = ',';
    testProxyUrl(env, 'http://proxy', 'https://example'); // No entries in NO_PROXY (whitespace).

    env.NO_PROXY = ' ';
    testProxyUrl(env, 'http://proxy', 'https://example'); // No entries in NO_PROXY (multiple whitespace / commas).
  ...
  describe('no_proxy=127.0.0.1 does NOT match localhost', function () {
    ...
    testProxyUrl(env, '', 'http://127.0.0.1'); // We're not performing DNS queries, so this shouldn't match.
