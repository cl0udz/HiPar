...
(function () {
  ...
  main = function main(libOptions) {
    ...
    traverse = function traverse(options) {
    ...
    parse = function parse(input, arg$) {
      var slice, obj, positional, restPositional, overrideRequired, prop, setValue, setDefaults, checkRequired, mutuallyExclusiveError, checkMutuallyExclusive, _checkDependency, checkDependencies, checkProp, args, key, value, option, ref$, i$, len$, arg, that, result, _short, argName, usingAssign, val, flags, len, j$, len1$, i, flag, opt, name, valPrime, negated, noedName;
      ...
      mutuallyExclusiveError = function mutuallyExclusiveError(first, second) {
      ...
      checkMutuallyExclusive = function checkMutuallyExclusive() {
      ...
      _checkDependency = function checkDependency(option) {
        ...
        for (i$ = 0, len$ = targetOptionNames.length; i$ < len$; ++i$) {
          ...
          if (targetOption && _checkDependency(targetOption)) {
            if (type === 'or') {
              return true;
            }
          } else if (type === 'and') {
    ...
    return {
      parse: parse,
      parseArgv: function parseArgv(it) {
        return parse(it, {
          slice: 2
        });
      },
