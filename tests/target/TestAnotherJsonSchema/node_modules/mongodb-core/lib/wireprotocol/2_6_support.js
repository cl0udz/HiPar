...
WireProtocol.prototype.killCursor = function (bson, ns, cursorState, pool, callback) {
  var cursorId = cursorState.cursorId; // Create a kill cursor command

  var killCursor = new KillCursor(bson, ns, [cursorId]); // Build killCursor options
...
var setupClassicFind = function setupClassicFind(bson, ns, cmd, cursorState, topology, options) {
  ...
  var findCmd = {}; // We have a Mongos topology, check if we need to add a readPreference
  ...
  findCmd['$query'] = cmd.query; // Throw on majority readConcern passed in
