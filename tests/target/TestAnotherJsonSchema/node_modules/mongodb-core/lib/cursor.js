...
Cursor.prototype._find = function (callback) {
  ...
  var queryCallback = function queryCallback(err, r) {
    ...
    if (Array.isArray(result.documents) && result.documents.length === 1 && (!self.cmd.find || self.cmd.find && self.cmd.virtual === false) && (typeof result.documents[0].cursor !== 'string' || result.documents[0]['$err'] || result.documents[0]['errmsg'] || Array.isArray(result.documents[0].result))) {
      ...
      if (result.documents[0].cursor != null && typeof result.documents[0].cursor !== 'string') {
        ...
        self.cursorState.operationTime = result.documents[0].operationTime; // If we have a firstBatch set it
...
Cursor.prototype._getmore = function (callback) {
  ...
  var pool = this.server.s.pool; // We have a wire protocol handler
...
Cursor.prototype.kill = function (callback) {
  ...
  var pool = this.server.s.pool; // Execute command
...
var nextFunction = function nextFunction(self, callback) {
  ...
  if (self.cursorState.cursorId == null) {
    ...
    if (self.topology.isDestroyed()) return callback(new MongoNetworkError('connection destroyed, not possible to instantiate cursor')); // query, cmd, options, cursorState, callback
...
function initializeCursor(cursor, callback) {
  ...
  if (!cursor.topology.isConnected(cursor.options)) {
    ...
    if (cursor.disconnectHandler != null) {
      ...
      return cursor.disconnectHandler.addObjectAndMethod('cursor', cursor, 'next', [callback], callback);
    }
  }

  return cursor.topology.selectServer(cursor.options, function (err, server) {
    if (err) {
    ...
    cursor.server = server; // Set as init

    cursor.cursorState.init = true; // error if collation not supported
    ...
    try {
      cursor.query = cursor.server.wireProtocolHandler.command(cursor.bson, cursor.ns, cursor.cmd, cursor.cursorState, cursor.topology, cursor.options);
