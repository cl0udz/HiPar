...
function () {
  ...
  _createClass(Cursor, [{
    ...
    value: function kill(callback) {
      // Set cursor to dead
      this.s.dead = true;
      this.s.killed = true; // Remove documents

      this.s.documents = []; // If no cursor id just return
      ...
      var pool = this.s.server.s.pool; // Execute command
    ...
    value: function readBufferedDocuments(number) {
      var unreadDocumentsLength = this.s.documents.length - this.s.cursorIndex;
      var length = number < unreadDocumentsLength ? number : unreadDocumentsLength;
      var elements = this.s.documents.slice(this.s.cursorIndex, this.s.cursorIndex + length); // Transform the doc with passed in transformation method if provided
      ...
      this.s.cursorIndex = this.s.cursorIndex + elements.length; // Return elements
...
Cursor.prototype._find = deprecate(function (callback) {
  return _find(_this, callback);
}, '_find() is deprecated, please stop using it');
Cursor.prototype._getmore = deprecate(function (callback) {
  return _getmore(_this, callback);
}, '_getmore() is deprecated, please stop using it');

function _getmore(cursor, callback) {
  if (cursor.logger.isDebug()) {
    cursor.logger.debug("schedule getMore call for query [".concat(JSON.stringify(cursor.query), "]"));
  } // Determine if it's a raw query


  var raw = cursor.options.raw || cursor.cmd.raw; // Set the current batchSize

  var batchSize = cursor.s.batchSize;
  ...
  var pool = cursor.s.server.s.pool; // We have a wire protocol handler

  cursor.s.server.s.wireProtocolHandler.getMore(cursor.bson, cursor.ns, cursor.s, batchSize, raw, pool, cursor.options, callback);
}
...
function _find(cursor, callback) {
  ...
  var queryCallback = function queryCallback(err, r) {
    ...
    var result = r.message; // Query failure bit set
    ...
    if (Array.isArray(result.documents) && result.documents.length === 1 && (!cursor.cmd.find || cursor.cmd.find && cursor.cmd.virtual === false) && (result.documents[0].cursor !== 'string' || result.documents[0]['$err'] || result.documents[0]['errmsg'] || Array.isArray(result.documents[0].result))) {
      ...
      if (result.documents[0].cursor != null && typeof result.documents[0].cursor !== 'string') {
        ...
        cursor.s.lastCursorId = cursor.s.cursorId; // If we have a firstBatch set it
    ...
    cursor.s.lastCursorId = result.cursorId; // Transform the results with passed in transformation method if provided
  ...
  if (_typeof(cursor.s.session) === 'object') {
...
function isConnectionDead(cursor, callback) {
  if (cursor.pool && cursor.pool.isDestroyed()) {
    ...
    var err = new MongoNetworkError("connection to host ".concat(cursor.pool.host, ":").concat(cursor.pool.port, " was destroyed"));

    _setCursorNotifiedImpl(cursor, function () {
      return callback(err);
    });
...
function _setCursorNotifiedImpl(cursor, callback) {
  ...
  if (cursor._endSession) {
    return cursor._endSession(undefined, function () {
      return callback();
    });
  }
...
function initializeCursorAndRetryNext(cursor, callback) {
  cursor.topology.selectServer(readPreferenceServerSelector(cursor.options.readPreference || ReadPreference.primary), function (err, server) {
    if (err) {
      callback(err, null);
      return;
    }
    ...
    cursor.s.init = true; // check if server supports collation
    ...
    if (cursor.cmd && cursor.cmd.collation && cursor.server.description.maxWireVersion < 5) {
      callback(new MongoError("server ".concat(cursor.server.name, " does not support collation")));
      return;
    }

    try {
      cursor.query = cursor.s.server.s.wireProtocolHandler.command(cursor.bson, cursor.ns, cursor.cmd, cursor.s, cursor.topology, cursor.options);
...
function nextFunction(cursor, callback) {
  ...
  if (isCursorKilled(cursor, callback)) return; // Cursor is dead but not marked killed, return null

  if (isCursorDeadButNotkilled(cursor, callback)) return; // We have a dead and killed cursor, attempting to call next should error

  if (isCursorDeadAndKilled(cursor, callback)) return; // We have just started the cursor
  ...
  if (cursor.s.cursorId == null) {
    ...
    if (isConnectionDead(cursor, callback)) return; // query, cmd, options, s, callback
    ...
    return _find(cursor, function (err) {
      ...
      if (cursor.s.documents.length === 0 && cursor.s.cursorId && cursor.s.cursorId.isZero() && !cursor.cmd.tailable && !cursor.cmd.awaitData) {
  ...
  if (cursor.s.limit > 0 && cursor.s.currentLimit >= cursor.s.limit) {
    ...
    cursor.kill(); // Set cursor in dead and notified state

    setCursorDeadAndNotified(cursor, callback);
    return;
  }

  if (cursor.s.documents.length === cursor.s.cursorIndex && cursor.cmd.tailable && Long.ZERO.equals(cursor.s.cursorId)) {
  ...
  if (cursor.s.cursorIndex === cursor.s.documents.length && !Long.ZERO.equals(cursor.s.cursorId)) {
    ...
    cursor.s.documents = [];
    cursor.s.cursorIndex = 0; // Check if connection is dead and return if not possible to

    if (isConnectionDead(cursor, callback)) return; // Execute the next get more
    ...
    return _getmore(cursor, function (err, doc, connection) {
      ...
      cursor.connection = connection; // Tailable cursor getMore result, notify owner about it
      // No attempt is made here to retry, this is left to the user of the
      // core module to handle to keep core simple

      if (cursor.s.documents.length === 0 && cursor.cmd.tailable && Long.ZERO.equals(cursor.s.cursorId)) {
      ...
      } else if (cursor.s.documents.length === 0 && cursor.cmd.tailable && !Long.ZERO.equals(cursor.s.cursorId)) {
  ...
  cursor.s.currentLimit += 1; // Get the document

  var doc = cursor.s.documents[cursor.s.cursorIndex++]; // Doc overflow
