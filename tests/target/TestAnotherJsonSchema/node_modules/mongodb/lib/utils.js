...
function resolveReadPreference(options, sources) {
  ...
  var db = sources.db;
  var coll = sources.collection;
  var defaultReadPreference = sources["default"];
  ...
  } else if (readPreference && !(readPreference instanceof ReadPreference) && _typeof(readPreference) === 'object') {
...
function decorateWithCollation(command, target, options) {
  var topology = target.s && target.s.topology;

  if (!topology) {
    throw new TypeError('parameter "target" is missing a topology');
  }

  var capabilities = target.s.topology.capabilities();

  if (options.collation && _typeof(options.collation) === 'object') {
    if (capabilities && capabilities.commandsTakeCollation) {
      command.collation = options.collation;
    } else {
      throw new MongoError("server ".concat(topology.s.coreTopology.name, " does not support collation"));
    }
  }
}
/**
