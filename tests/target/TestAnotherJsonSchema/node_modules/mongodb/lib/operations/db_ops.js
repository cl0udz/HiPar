...
function addUser(db, username, password, options, callback) {
  var Db = require('../db'); // Did the user destroy the topology
  ...
  executeAuthCreateUserCommand(db, username, password, options, function (err, r) {
    ...
    if (err && err.code === -5000) {
      ...
      var collection = _db.collection(CONSTANTS.SYSTEM_USER_COLLECTION); // Check if we are inserting the first user


      count(collection, {}, finalOptions, function (err, count) {
...
function createCollection(db, name, options, callback) {
  var Collection = require('../collection'); // Get the write concern options


  var finalOptions = applyWriteConcern(Object.assign({}, options), {
    db: db
  }, options); // Did the user destroy the topology

  if (db.serverConfig && db.serverConfig.isDestroyed()) {
    return callback(new MongoError('topology was destroyed'));
  }

  var listCollectionOptions = Object.assign({}, finalOptions, {
    nameOnly: true
  ...
  db.listCollections({
    ...
    if (collections.length > 0 && finalOptions.strict) {
      ...
      try {
        return handleCallback(callback, null, new Collection(db, db.s.topology, db.s.databaseName, name, db.s.pkFactory, options));
      } catch (err) {
    ...
    applyWriteConcern(cmd, {
      db: db
    }, options); // Add all optional parameters
    ...
    executeCommand(db, cmd, finalOptions, function (err) {
      if (err) return handleCallback(callback, err);
      handleCallback(callback, null, new Collection(db, db.s.topology, db.s.databaseName, name, db.s.pkFactory, options));
    });
  });
}
/**
...
function createIndex(db, name, fieldOrSpec, options, callback) {
  ...
  createIndexUsingCreateIndexes(db, name, fieldOrSpec, finalOptions, function (err, result) {
    ...
    db.s.topology.insert("".concat(db.s.databaseName, ".").concat(CONSTANTS.SYSTEM_INDEX_COLLECTION), doc, finalOptions, function (err, result) {
...
function executeCommand(db, command, options, callback) {
  ...
  var dbName = options.dbName || options.authdb || db.s.databaseName; // Convert the readPreference if its not a write
  ...
  db.s.topology.command("".concat(dbName, ".$cmd"), command, options, function (err, result) {
    if (err) return handleCallback(callback, err);
...
function executeDbAdminCommand(db, command, options, callback) {
  db.s.topology.command('admin.$cmd', command, options, function (err, result) {
    // Did the user destroy the topology
    if (db.serverConfig && db.serverConfig.isDestroyed()) {
      return callback(new MongoError('topology was destroyed'));
    }
...
function removeUser(db, username, options, callback) {
  var Db = require('../db'); // Attempt to execute command
  ...
  executeAuthRemoveUserCommand(db, username, options, function (err, result) {
    if (err && err.code === -5000) {
      ...
      var _db2 = options.dbName ? new Db(options.dbName, _db2.s.topology, _db2.s.options) : _db2; // Fetch a user collection


      var collection = _db2.collection(CONSTANTS.SYSTEM_USER_COLLECTION); // Locate the user
...
function setProfilingLevel(db, level, options, callback) {
  ...
  executeCommand(db, command, options, function (err, doc) {
...
module.exports = {
  ...
  listCollectionsTransforms: listCollectionsTransforms,
