...
var Map = require('mongodb-core').BSON.Map;

var executeOperation = require('./utils').executeOperation;

var count = require('./operations/cursor_ops').count;

var each = require('./operations/cursor_ops').each;

var hasNext = require('./operations/cursor_ops').hasNext;

var next = require('./operations/cursor_ops').next;

var toArray = require('./operations/cursor_ops').toArray;
/**
...
function Cursor(bson, ns, cmd, options, topology, topologyOptions) {
  ...
  this.sortValue = this.s.cmd.sort; // Get the batchSize
...
Cursor.prototype._initImplicitSession = function () {
  if (!this.s.explicitlyIgnoreSession && !this.s.session && this.s.topology.hasSessionSupport()) {
    this.s.session = this.s.topology.startSession({
      owner: this
    });
...
Cursor.prototype.count = function (applySkipLimit, opts, callback) {
  if (this.s.cmd.query == null) throw MongoError.create({
...
Cursor.prototype.close = function (options, callback) {
  ...
  var completeClose = function completeClose() {
    ...
    _this2.emit('close'); // Callback if provided


    if (typeof callback === 'function') {
      return handleCallback(callback, null, _this2);
    } // Return a Promise


    return new _this2.s.promiseLibrary(function (resolve) {
      resolve();
    });
  };
...
Cursor.prototype._read = function () {
  ...
  this.next(function (err, result) {
    if (err) {
      if (_this3.listeners('error') && _this3.listeners('error').length > 0) {
        _this3.emit('error', err);
      }

      if (!_this3.isDead()) _this3.close(); // Emit end event

      _this3.emit('end');

      return _this3.emit('finish');
    } // If we provided a transformation method


    if (typeof _this3.s.streamOptions.transform === 'function' && result != null) {
      return _this3.push(_this3.s.streamOptions.transform(result));
    } // If we provided a map function


    if (_this3.cursorState.transforms && typeof _this3.cursorState.transforms.doc === 'function' && result != null) {
      return _this3.push(_this3.cursorState.transforms.doc(result));
    } // Return the result


    _this3.push(result);

    if (result === null && _this3.isDead()) {
