...
$(document).ready(function () {
  ...
  $(document).on('click', '#product_opt_add', function (e) {
    ...
    html += '</div></div></li>'; // append data

    $('#product_opt_wrapper').append(html); // add to the stored json string

    optJson[optName] = {
      optName: optName,
      optLabel: optLabel,
      optType: optType,
      optOptions: $.grep(optOptions.split(','), function (n) {
        return n === 0 || n;
      })
    }; // write new json back to field

    $('#productOptions').val(JSON.stringify(optJson)); // clear inputs
  ...
  $('#settingsForm').validator().on('submit', function (e) {
    if (!e.isDefaultPrevented()) {
      e.preventDefault(); // set hidden elements from codemirror editors
  ...
  $(document).on('click', '#validatePermalink', function (e) {
    if ($('#productPermalink').val() !== '') {
      $.ajax({
        ...
        data: {
          permalink: $('#productPermalink').val(),
          docId: $('#productId').val()
        }
      }).done(function (msg) {
        showNotification(msg.message, 'success');
  ...
  $(document).on('click', '#orderCreate', function (e) {
    ...
    if ($('#createOrderForm').validator('validate').has('.has-error').length === 0) {
      $.ajax({
        ...
        showNotification(result.message, 'success');
        window.location = "/admin/order/view/".concat(result.orderId);
      }).fail(function (msg) {
        showNotification(msg.responseJSON.message, 'danger');
      });
    }
  ...
  $(document).on('click', '#uploadButton', function (e) {
    ...
    formData.append('productId', $('#productId').val()); // Upload file
  ...
  $('#global-search-value').on('keyup', function (e) {
  ...
  $('body').on('click', '.gr-click', function (e) {
    $('#global-search-value').val();
    var url = $(e.currentTarget).closest('.global-result').attr('data-url');
...
function globalSearch() {
  ...
  $.ajax({
    ...
    res.customers.forEach(function (value) {
      hasResult = true;
      var result = "\n            <li class=\"list-group-item global-result text-center\" data-url=\"/admin/customer/view/".concat(value._id, "\">\n                <div class=\"row\">\n                <div class=\"col global-result-type gr-click\"><i class=\"feather\" data-feather=\"user\"></i> Customer</div>\n                <div class=\"col global-result-detail gr-click\">").concat(value.firstName, " ").concat(value.lastName, "</div>\n                <div class=\"col global-result-detail gr-click\">").concat(value.email, "</div>\n                </div>\n            </li>");
      $('#global-search-results').append(result);
    });
    res.orders.forEach(function (value) {
      hasResult = true;
      var result = "\n            <li class=\"list-group-item global-result text-center\" data-url=\"/admin/order/view/".concat(value._id, "\">\n                <div class=\"row\">\n                    <div class=\"col global-result-type gr-click\"><i class=\"feather\" data-feather=\"package\"></i> Order</div>\n                    <div class=\"col global-result-detail gr-click\">").concat(value.orderFirstname, " ").concat(value.orderLastname, "</div>\n                    <div class=\"col global-result-detail gr-click\">").concat(moment(value.orderDate).format('YYYY/MM/DD'), "</div>\n                    <div class=\"col global-result-detail gr-click\">").concat(value.orderEmail, "</div>\n                </div>\n            </li>");
      $('#global-search-results').append(result);
    });
    res.products.forEach(function (value) {
      hasResult = true;
      var result = "<li class=\"list-group-item global-result text-center\" data-url=\"/admin/product/edit/".concat(value._id, "\">\n                <div class=\"row\">\n                    <div class=\"col global-result-type gr-click\"><i class=\"feather\" data-feather=\"tag\"></i> Product</div>\n                    <div class=\"col global-result-detail gr-click\">").concat(value.productTitle, "</div>\n                    <div class=\"col global-result-detail gr-click\">").concat($('#currencySymbol').val()).concat(numeral(value.productPrice).format('0.00'), "</div>\n                </div>\n            </li>");
      $('#global-search-results').append(result);
    });
