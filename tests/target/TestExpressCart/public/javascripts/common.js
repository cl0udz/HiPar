    ...
    if ($('#shipping-form').validator('validate').has('.has-error').length === 0) {
      ...
      var handler = window.StripeCheckout.configure({
        ...
        token: function token(_token) {
          ...
          $('#shipping-form').append('<input type="hidden" name="stripeToken" value="' + _token.id + '" />');
...
function showNotification(msg, type, reloadPage, redirect) {
  // defaults to false
  reloadPage = reloadPage || false; // defaults to null
...
function slugify(str) {
  ...
  $slug = trimmed.replace(/[^a-z0-9-æøå]/gi, '-').replace(/-+/g, '-').replace(/^-|-$/g, '').replace(/æ/gi, 'ae').replace(/ø/gi, 'oe').replace(/å/gi, 'a');
