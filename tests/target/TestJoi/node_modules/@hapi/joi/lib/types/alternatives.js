...
module.exports = Any.extend({
  ...
  args: function args(schema) {
    ...
    if (schemas.length === 1) {
      if (Array.isArray(schemas[0])) {
        return schema["try"].apply(schema, _toConsumableArray(schemas[0]));
      }
    }

    return schema["try"].apply(schema, schemas);
  },
  validate: function validate(value, helpers) {
    ...
    if (schema._flags.match) {
      ...
      for (var i = 0; i < schema.$_terms.matches.length; ++i) {
        ...
        var localState = state.nest(item.schema, "match.".concat(i));
        localState.snapshot();
    ...
    for (var _i = 0; _i < schema.$_terms.matches.length; ++_i) {
      ...
      if (_item.schema) {
        var _localState = state.nest(_item.schema, "match.".concat(_i));
      ...
      for (var j = 0; j < tests.length; ++j) {
        ...
        var id = "match.".concat(_i).concat(_item["switch"] ? '.' + j : '');

        if (!is.$_match(input, state.nest(is, "".concat(id, ".is")), prefs)) {
          if (otherwise) {
            return otherwise.$_validate(value, state.nest(otherwise, "".concat(id, ".otherwise")), prefs);
          }
        } else if (then) {
          return then.$_validate(value, state.nest(then, "".concat(id, ".then")), prefs);
        }
      }
    }
  ...
  rules: {
    conditional: {
      method: function method(condition, options) {
  ...
  if (!complex.length) {
    return {
      errors: error('alternatives.types', {
        types: _toConsumableArray(valids)
      })
    };
...
internals.unmatched = function (failures, error) {
  ...
  try {
    for (var _iterator6 = failures[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
      ...
      errors.push.apply(errors, _toConsumableArray(failure.reports));
    }
  } catch (err) {
