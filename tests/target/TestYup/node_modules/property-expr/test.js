...
a.strictEqual(getter('["foo"]["bar"][1]')(obj), 'bux');
a.strictEqual(getter('[1]')([1, 'bux']), 'bux'); // safe access
...
a.strictEqual(getter('foo.FE43-D880-21AE', true)(obj), 3); // -- Setters --
...
a.strictEqual(obj.foo.bar[1], 'baz'); // -- Cache --
...
cache.clear();
a.strictEqual(cache._size, 0);
a.strictEqual(cache.get('a'), undefined); // -- split --

var parts = expression.split('foo.baz["bar"][1]');
a.strictEqual(parts.length, 4); // -- join --

var parts = expression.split('foo.baz["bar"][1]');
a.strictEqual(expression.join(['0', 'baz', '"bar"', 1]), '[0].baz["bar"][1]');
a.strictEqual(expression.join(parts), 'foo.baz["bar"][1]'); // -- forEach --
...
a.strictEqual(count, 3); // -- normalizePath --

a.deepEqual(expression.normalizePath('foo[ " bux\'s " ].bar["baz"][ 9 ][ \' s \' ]'), ['foo', ' bux\'s ', 'bar', 'baz', '9', ' s ']);
