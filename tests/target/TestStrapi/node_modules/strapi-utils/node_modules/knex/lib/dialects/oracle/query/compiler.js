...
(0, _lodash.assign)(QueryCompiler_Oracle.prototype, {
  ...
  insert() {
    ...
    sql.sql = 'begin ' + (0, _lodash.map)(insertData.values, value => {
      let returningHelper;
      ...
      subSql += returning ? ` returning ROWID into ${this.formatter.parameter(returningHelper)}` : ''; // pre bind position because subSql is an execute immediate parameter
  ...
  columnInfo() {
    ...
    const table = this.client.customWrapIdentifier(this.single.table, _lodash.identity); // Node oracle drivers doesn't support LONG type (which is data_default type)
  ...
  select() {
    let query = this.with();
    const statements = (0, _lodash.map)(components, component => {
  ...
  _addReturningToSqlAndConvert(sql, returning, tableName) {
    ...
    const returningValues = Array.isArray(returning) ? returning : [returning];
    const returningHelper = new _utils.ReturningHelper(returningValues.join(':'));
