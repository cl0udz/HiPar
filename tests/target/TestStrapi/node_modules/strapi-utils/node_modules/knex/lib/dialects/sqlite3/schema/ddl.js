...
function SQLite3_DDL(client, tableCompiler, pragma, connection) {
  ...
  this.formatter = client && client.config && client.config.wrapIdentifier ? client.config.wrapIdentifier : value => value;
}
...
(0, _lodash.assign)(SQLite3_DDL.prototype, {
  ...
  getColumn: _bluebird.default.method(function (column) {
    const currentCol = (0, _lodash.find)(this.pragma, col => {
  ...
  getTableSql() {
    ...
    return this.trx.raw(`SELECT name, sql FROM sqlite_master WHERE type="table" AND name="${this.tableName()}"`).then(result => {
  ...
  renameColumn: _bluebird.default.method(function (from, to) {
    return this.client.transaction(trx => {
      ...
      return this.getColumn(from).bind(this).then(this.getTableSql).then(function (sql) {
        ...
        const _invert = (0, _lodash.invert)(this.client.postProcessResponse((0, _lodash.invert)({
          from,
          to
        }))),
  ...
  dropColumn: _bluebird.default.method(function (columns) {
    return this.client.transaction(trx => {
      this.trx = trx;
      return _bluebird.default.all(columns.map(column => this.getColumn(column))).bind(this).then(this.getTableSql).then(function (sql) {
        const createTable = sql[0];
        ...
        const mappedColumns = Object.keys(this.client.postProcessResponse((0, _lodash.fromPairs)(columns.map(column => [column, column]))));
        return _bluebird.default.bind(this).then(this.createTempTable(createTable)).then(this.copyData).then(this.dropOriginal).then(function () {
          return this.trx.raw(newSql);
        }).then(this.reinsertData(row => (0, _lodash.omit)(row, ...mappedColumns))).then(this.dropTempTable);
      });
    }, {
      connection: this.connection
    });
