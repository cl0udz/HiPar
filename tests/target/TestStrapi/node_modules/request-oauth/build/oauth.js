...
function oauth(options) {
  return build(options.oauth, options.url, options.method, options.url.query, options.form, options.body);
}
...
function build(oauth, uri, method, query, form, body) {
  ...
  if (transport === 'body' && (method !== 'POST' || form === undefined)) {
    return new Error('oauth: transport_method: body requires POST ' + 'and content-type: application/x-www-form-urlencoded');
  }

  if (oauth.body_hash && form === undefined) {
    var sig = oauth.signature_method || 'HMAC-SHA1';
    if (['HMAC-SHA1', 'RSA-SHA1'].indexOf(sig) < 0) {
      return new Error('oauth: ' + oauth.signature_method + ' signature_method' + ' not supported with body_hash signing');
    }
...
function buildParams(oauth, uri, method, query, form) {
  ...
  oa.oauth_signature = _oauth.sign(oa.oauth_signature_method, method, baseurl, params, consumer_secret_or_private_key, token_secret);
