...
test('A route could support multiple versions (find) / 2', function (t) {
  t.plan(7);
  var findMyWay = FindMyWay();
...
test('A route could support multiple versions (find) / 3', function (t) {
  t.plan(10);
  var findMyWay = FindMyWay();
  findMyWay.on('GET', '/test/:id/hello', {
    version: '1.2.3'
  }, noop);
  findMyWay.on('GET', '/test/:id/hello', {
    version: '3.2.0'
  }, noop);
  findMyWay.on('GET', '/test/name/hello', {
    version: '4.0.0'
  }, noop);
...
test('A route could support multiple versions (find) / 4', function (t) {
  t.plan(8);
  var findMyWay = FindMyWay();
  findMyWay.on('GET', '/test/*', {
    version: '1.2.3'
...
test('A route could support multiple versions (find) / 5', function (t) {
  t.plan(1);
  var findMyWay = FindMyWay();
  findMyWay.on('GET', '/', {
    version: '1.2.3'
...
test('Find with a version but without versioned routes', function (t) {
  t.plan(1);
  var findMyWay = FindMyWay();
  findMyWay.on('GET', '/', noop);
  t.notOk(findMyWay.find('GET', '/', '1.x'));
});
test('A route could support multiple versions (lookup)', function (t) {
  t.plan(7);
  var findMyWay = FindMyWay({
    defaultRoute: function defaultRoute(req, res) {
      var versions = ['2.x', '2.3.4', '3.2.1'];
  ...
  findMyWay.on('GET', '/', {
    ...
    var versions = ['1.x', '1.2.3'];
    t.ok(versions.indexOf(req.headers['accept-version']) > -1);
  });
  findMyWay.on('GET', '/', {
    version: '3.2.0'
  }, function (req, res) {
    var versions = ['3.x', '3.2.0'];
    t.ok(versions.indexOf(req.headers['accept-version']) > -1);
  });
  findMyWay.lookup({
    method: 'GET',
...
test('It should always choose the highest version of a route', function (t) {
  t.plan(3);
  var findMyWay = FindMyWay();
  findMyWay.on('GET', '/', {
    version: '2.3.0'
  ...
  findMyWay.lookup({
    ...
    url: '/',
    headers: {
      'accept-version': '4.3.x'
    }
  }, null);
});
test('Declare the same route with and without version', function (t) {
  t.plan(2);
  var findMyWay = FindMyWay();
  findMyWay.on('GET', '/', noop);
  findMyWay.on('GET', '/', {
...
test('It should throw if you declare multiple times the same route', function (t) {
  t.plan(1);
  var findMyWay = FindMyWay();
  findMyWay.on('GET', '/', {
    version: '1.2.3'
