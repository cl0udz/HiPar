...
var Benchmark = require('benchmark'); // The default number of samples for Benchmark seems to be low enough that it
...
var findMyWay = new FindMyWay();
findMyWay.on('GET', '/', function () {
  return true;
});
findMyWay.on('GET', '/user/:id', function () {
  return true;
});
findMyWay.on('GET', '/user/:id/static', function () {
  return true;
});
findMyWay.on('GET', '/customer/:name-:surname', function () {
  return true;
});
findMyWay.on('GET', '/at/:hour(^\\d+)h:minute(^\\d+)m', function () {
  return true;
});
findMyWay.on('GET', '/abc/def/ghi/lmn/opq/rst/uvz', function () {
  return true;
});
findMyWay.on('GET', '/', {
  version: '1.2.0'
...
suite.add('lookup static route', function () {
  findMyWay.lookup({
    method: 'GET',
    url: '/',
    headers: {}
