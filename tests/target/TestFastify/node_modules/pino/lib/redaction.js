...
var validate = validator({
  ...
  ERR_INVALID_PATH: function ERR_INVALID_PATH(s) {
    return `pino – redact paths array contains an invalid path (${s})`;
  }
});
var CENSOR = '[Redacted]';
...
function redaction(opts, serialize) {
  ...
  var shape = paths.reduce(function (o, str) {
    ...
    var nextPath = `${str.substr(index, str.length - 1)}`;
  ...
  var topCensor = function topCensor() {
    return typeof censor === 'function' ? serialize(censor(...arguments)) : serialize(censor);
  };

  return [...Object.keys(shape), ...Object.getOwnPropertySymbols(shape)].reduce(function (o, k) {
    // top level key:
