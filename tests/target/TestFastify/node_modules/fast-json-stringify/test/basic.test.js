...
function buildTest(schema, toStringify) {
  test(`render a ${schema.title} as JSON`, function (t) {
    t.plan(5);
...
test('skip or coerce numbers and integers that are not numbers', function (t) {
  var stringify = build({
    title: 'basic',
    type: 'object',
    properties: {
...
test('additionalProperties - throw on unknown type', function (t) {
  t.plan(1);
...
test('patternProperties - throw on unknown type', function (t) {
  t.plan(1);
...
test('render a single quote as JSON', function (t) {
  t.plan(2);
  var schema = {
    type: 'string'
  };
  var toStringify = '" double quote';
  var validate = validator(schema);
  var stringify = build(schema);
  var output = stringify(toStringify);
  t.equal(output, JSON.stringify(toStringify));
