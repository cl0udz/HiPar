...
TimeTree.prototype.untrackNode = function (node) {
  ...
  var labelNode = this.tableLabel.get(node.label);
...
TimeTree.prototype.getParent = function (parent) {
  ...
  var parentNode = this.tableLabel.get(parent);
...
TimeTree.prototype.add = function (parent, child, start) {
  ...
  var parentNode = this.getParent(parent);
  var nodeId = "".concat(child, "-").concat(Math.random());
  var childNode = {
    id: nodeId,
    parent: parent,
    start: start,
...
TimeTree.prototype.prittyPrint = function () {
  var decorateText = function decorateText(node) {
    node.label = "".concat(node.label, " ").concat(node.diff, " ms");

    if (node.nodes.length > 0) {
      node.nodes = node.nodes.map(function (_) {
        return decorateText(_);
      });
    }
  ...
  var out = decorateText(this.toJSON());
  return archy(out);
};
