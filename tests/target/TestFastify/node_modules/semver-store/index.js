...
SemVerStore.prototype.del = function (version) {
  ...
  var major = version.slice(0, firstDot);
  var minor = secondDot === -1 ? version.slice(firstDot + 1) : version.slice(firstDot + 1, secondDot);
  var patch = secondDot === -1 ? 'x' : version.slice(secondDot + 1); // check existence of major node

  var majorNode = this.tree.children[major];
  if (majorNode == null) return this; // if minor is the wildcard, then remove the full major node
  ...
  if (minorNode == null) return this; // if patch is the wildcard, then remove the full minor node
  ...
  if (patchNode == null) return this; // Specific delete

  this.tree.children[major].children[minor].removeChild(patch); // check if the minor node has no more children, if so removes it
