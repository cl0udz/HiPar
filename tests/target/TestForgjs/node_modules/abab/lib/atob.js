...
function atob(input) {
  ...
  input = String(input); // "Remove all space characters from input."

  input = input.replace(/[ \t\n\f\r]/g, ''); // "If the length of input divides by 4 leaving no remainder, then: if
  ...
  var output = ''; // "Let buffer be a buffer that can have bits appended to it, initially
  // empty."
  //
  ...
  var buffer = 0;
  var accumulatedBits = 0; // "While position does not point past the end of input, run these
  // substeps:"
  ...
  for (var i = 0; i < input.length; i++) {
    ...
    buffer <<= 6;
    buffer |= atobLookup(input[i]); // "If buffer has accumulated 24 bits, interpret them as three 8-bit
