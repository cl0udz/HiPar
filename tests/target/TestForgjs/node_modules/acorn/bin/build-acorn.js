...
browserify({
  standalone: "acorn"
}).plugin(require('browserify-derequire')).transform(babelify).require("./src/index.js", {
  entry: true
...
browserify({
  standalone: "acorn.loose"
}).plugin(require('browserify-derequire')).transform(acornShimPrepare).transform(babelify).require("./src/loose/index.js", {
  entry: true
}).bundle().on("error", function (err) {
  console.log("Error: " + err.message);
}).pipe(acornShimComplete()).pipe(fs.createWriteStream("dist/acorn_loose.js"));

browserify({
  standalone: "acorn.walk"
}).plugin(require('browserify-derequire')).transform(acornShimPrepare).transform(babelify).require("./src/walk/index.js", {
  entry: true
}).bundle().on("error", function (err) {
  console.log("Error: " + err.message);
}).pipe(acornShimComplete()).pipe(fs.createWriteStream("dist/walk.js"));

babel.transformFile("./src/bin/acorn.js", function (err, result) {
  if (err) return console.log("Error: " + err.message);
  fs.writeFile("bin/acorn", result.code, function (err) {
    if (err) return console.log("Error: " + err.message); // Make bin/acorn executable
