...
function isSpace(ch) {
  return ch < 14 && ch > 8 || ch === 32 || ch === 160 || (0, _.isNewLine)(ch);
}
...
lp.readToken = function () {
  for (;;) {
    try {
      ...
      if (!(e instanceof SyntaxError)) throw e; // Try to skip some text, based on the error message, and then continue
      ...
      if (/unterminated/i.test(msg)) {
        ...
        while (pos < this.input.length) {
          ...
          if (ch === 34 || ch === 39 || (0, _.isNewLine)(ch)) break;
        }
      ...
      if (replace) {
        if (this.options.locations) replace.loc = new _.SourceLocation(this.toks, (0, _.getLineInfo)(this.input, replace.start), (0, _.getLineInfo)(this.input, replace.end));
        return replace;
      }
    }
  }
