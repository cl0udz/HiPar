...
lp.parseMaybeUnary = function (noIn) {
  if (this.tok.type.prefix) {
    ...
    this.next();
    _node.argument = this.parseMaybeUnary(noIn);
    return this.finishNode(_node, "SpreadElement");
  }
  ...
  while (this.tok.type.postfix && !this.canInsertSemicolon()) {
    var _node2 = this.startNodeAt(start);

    _node2.operator = this.tok.value;
    _node2.prefix = false;
    _node2.argument = this.checkLVal(expr);
    this.next();
    expr = this.finishNode(_node2, "UpdateExpression");
  }
...
lp.parseSubscripts = function (base, start, noCalls, startIndent, line) {
  for (;;) {
    ...
    if (this.eat(_.tokTypes.dot)) {
      ...
      _node4.callee = base;
      _node4.arguments = this.parseExprList(_.tokTypes.parenR);
  ...
  switch (this.tok.type) {
    ...
    case _.tokTypes.parenL:
      ...
      if (this.eat(_.tokTypes.arrow)) {
        return this.parseArrowExpression(this.startNodeAt(parenStart), inner.expressions || ((0, _parseutil.isDummy)(inner) ? [] : [inner]));
      }
