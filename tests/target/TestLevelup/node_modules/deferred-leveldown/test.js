...
require('abstract-leveldown/test/del-test').args = noop; // This fails on "iterator has db reference" test, as expected because
// the return value of db.iterator() depends on whether the db is open.

require('abstract-leveldown/test/iterator-test').args = noop; // Test abstract-leveldown compliance

suite(testCommon); // Custom tests
...
test('single operation', function (t) {
  var called = false;
  var db = {
    put: function put(key, value, options, callback) {
...
test('many operations', function (t) {
  ...
  var db = {
    ...
    batch: function batch(arr, options, callback) {
      if (batches++ === 0) {
        t.deepEqual(arr, [{
          type: 'put',
          key: 'k1',
...
test('keys and values should not be serialized', function (t) {
  var DATA = [];
  var ITEMS = [123, 'a string', Buffer.from('w00t'), {
    an: 'object'
  }];
  ...
  t.test('clear', function (t) {
    var calls = [];
    var ld = Db('clear', function (opts, cb) {
      calls.push(opts);
    });
    ITEMS.forEach(function (key) {
      ld.clear({
...
test('non-deferred approximateSize', function (t) {
  ...
  var db = {
    ...
    approximateSize: function approximateSize(start, end, callback) {
      t.is(start, 'bar');
      t.is(end, 'foo');
      process.nextTick(callback);
    }
...
test('non-deferred compactRange', function (t) {
  ...
  var db = {
    ...
    compactRange: function compactRange(start, end, callback) {
      t.is(start, 'bar');
      t.is(end, 'foo');
      process.nextTick(callback);
    }
