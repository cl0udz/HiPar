...
var legacyRangeOptions = ['start', 'end']; // Test the suite itself as well as the default implementation,
...
test('test open() extensibility', function (t) {
  ...
  var expectedCb = function expectedCb() {};
...
test('test batch([]) (array-form) with empty array is asynchronous', function (t) {
  ...
  test.batch([], function (err) {
    ...
    t.ok(async, 'callback is asynchronous'); // Assert that asynchronicity is provided by batch() rather than _batch()
...
test('test chained batch() extensibility', function (t) {
  ...
  t.deepEqual(spy.getCall(0).args[0][0], {
    type: 'put',
    key: 'foo',
    value: 'bar'
  }, 'got expected array argument[0]');
  t.deepEqual(spy.getCall(0).args[0][1], {
    type: 'del',
    key: 'bang'
  }, 'got expected array argument[1]');
  t.deepEqual(spy.getCall(0).args[1], {}, 'got expected options argument');
  t.equal(spy.getCall(0).args[2], expectedCb, 'got expected callback argument');
  ...
  t.deepEqual(spy.getCall(1).args[0][0], {
    type: 'put',
    key: 'foo',
    value: 'bar'
  }, 'got expected array argument[0]');
  t.deepEqual(spy.getCall(1).args[0][1], {
    type: 'del',
    key: 'bang'
  }, 'got expected array argument[1]');
  t.deepEqual(spy.getCall(1).args[1], expectedOptions, 'got expected options argument');
...
test('test AbstractChainedBatch#write() extensibility', function (t) {
  ...
  t.equal(spy.getCall(0).args.length, 2, 'got two arguments');
  t.same(spy.getCall(0).args[0], {}, 'got options'); // awkward here cause of nextTick & an internal wrapped cb
...
test('test AbstractIterator#next() extensibility', function (t) {
  ...
  t.equal(spy.getCall(0).args.length, 1, 'got one arguments'); // awkward here cause of nextTick & an internal wrapped cb
...
test('test serialization extensibility (put)', function (t) {
  ...
  var Test = implement(AbstractLevelDOWN, {
    _put: spy,
    _serializeKey: function _serializeKey(key) {
      t.equal(key, 'no');
      return 'foo';
    },
    _serializeValue: function _serializeValue(value) {
      t.equal(value, 'nope');
      return 'bar';
    }
  });
...
test('test serialization extensibility (iterator seek)', function (t) {
  t.plan(3);
  var spy = sinon.spy();
  var TestIterator = implement(AbstractIterator, {
    _seek: spy
  });
  var Test = implement(AbstractLevelDOWN, {
    _iterator: function _iterator() {
...
test('test serialization extensibility (clear range options)', function (t) {
  ...
  rangeOptions.forEach(function (key) {
    var Test = implement(AbstractLevelDOWN, {
      ...
      _clear: function _clear(options, callback) {
...
test('.status', function (t) {
  ...
  t.test('open error', function (t) {
    var Test = implement(AbstractLevelDOWN, {
      _open: function _open(options, cb) {
        cb(new Error());
      }
    });
    var test = new Test('foobar');
...
test('_setupIteratorOptions', function (t) {
  ...
  t.test('does not delete empty buffers', function (t) {
    var options = setupOptions(function () {
      return Buffer.from('');
    });
    keys.forEach(function (key) {
