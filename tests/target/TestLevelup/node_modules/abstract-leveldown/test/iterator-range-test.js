...
exports.range = function (test, testCommon) {
  function rangeTest(name, opts, expected) {
    ...
    if (!opts.reverse && !('limit' in opts)) {
      ...
      rangeTest(name + ' (flipped)', reverseOpts, expected.slice().reverse());
    }
  }
  ...
  rangeTest('test iterator with gt=50 and reverse=true', {
    gt: '50',
    reverse: true
  }, data.slice().reverse().slice(0, 49)); // end='0', starting key is actually '00' so it should avoid it
  ...
  rangeTest('test iterator with limit=20 and start=79 and reverse=true - legacy', {
    ...
    start: '79',
    reverse: true
  }, data.slice().reverse().slice(20, 40)); // the default limit value from levelup is -1
