...
exports.nonErrorKeys = function (test, testCommon) {
  // valid falsey keys
  makePutGetDelSuccessfulTest(test, '`0` key', 0, 'foo 0'); // standard String key

  makePutGetDelSuccessfulTest(test, 'long String key', 'some long string that I\'m using as a key for this unit test, cross your fingers human, we\'re going in!', 'foo');
...
exports.nonErrorValues = function (test, testCommon) {
  ...
  makePutGetDelSuccessfulTest(test, '`NaN` value', 'foo NaN', NaN); // all of the following result in an empty-string value:

  makePutGetDelSuccessfulTest(test, 'empty String value', 'foo', '', '');
  makePutGetDelSuccessfulTest(test, 'empty Buffer value', 'foo', Buffer.alloc(0), ''); // note that an implementation may return the value as an array

  makePutGetDelSuccessfulTest(test, 'empty Array value', 'foo', [], ''); // standard String value

  makePutGetDelSuccessfulTest(test, 'long String value', 'foo', 'some long string that I\'m using as a key for this unit test, cross your fingers human, we\'re going in!'); // standard Buffer value

  makePutGetDelSuccessfulTest(test, 'Buffer value', 'foo', testBuffer); // non-empty Array as a key
