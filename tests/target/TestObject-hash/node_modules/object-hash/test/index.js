...
describe('hash', function () {
  ...
  it('sugar methods should be equivalent', function () {
    ...
    assert.equal(hash.keys(obj), hash(obj, {
      excludeValues: true
    }), 'keys');
    assert.equal(hash.sha1(obj), hash(obj, {
      algorithm: 'sha1'
    }), 'sha1');
    assert.equal(hash.MD5(obj), hash(obj, {
      algorithm: 'md5'
    }), 'md5');
    assert.equal(hash.keysMD5(obj), hash(obj, {
      algorithm: 'md5',
      excludeValues: true
    }), 'keys md5');
  });
  ...
  it("recursive objects don't blow up stack", function () {
    ...
    assert.doesNotThrow(function () {
      hash(hash1);
    }, /Maximum call stack size exceeded/, 'Should not throw an stack size exceeded exception');
  });
  ...
  it("various hashes in crypto.getHashes() should be supported", function () {
    ...
    for (var i = 0; i < hashes.length; i++) {
      assert.ok(hash(obj, {
        algorithm: hashes[i]
      }), 'Algorithm ' + hashes[i] + ' should be supported');
    }
  });
  ...
  it('distinguishes async functions based on their properties', function () {
    ...
    try {
      ...
      if (err.name === 'SyntaxError') return this.skip('Not available on Node 6');else throw err;
    }
  ...
  it('excludeKeys works', function () {
    ...
    ha = hash({
      ...
      excludeKeys: function excludeKeys(key) {
        return key === 'b';
      }
    });
    hb = hash({
