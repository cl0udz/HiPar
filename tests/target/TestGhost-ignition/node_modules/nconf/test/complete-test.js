...
var completeTest = helpers.fixture('complete-test.json'),
    complete = helpers.fixture('complete.json'); // prime the process.env
...
process.env.json_obj = JSON.stringify({
  foo: 'bar',
  baz: 'foo'
});
...
vows.describe('nconf/multiple-stores').addBatch({
  "When using the nconf with multiple providers": {
    ...
    "json vars": {
      ...
      "are present": function arePresent(err, data) {
        assert.isNull(err);
    ...
    "and saving *synchronously*": {
      ...
      "correct return value": function correctReturnValue(topic) {
      ...
      "the file": {
        ...
        "saved correctly": function savedCorrectly(err, data) {
          data = JSON.parse(data);
          Object.keys(data).forEach(function (key) {
  ...
  "When using the nconf with multiple providers": {
    "and saving *asynchronously*": {
      ...
      "correct return value": function correctReturnValue(err, data) {
        assert.isNull(err);
        Object.keys(data).forEach(function (key) {
          assert.deepEqual(data[key], nconf.get(key));
        });
  ...
  "When using env with lowerCase:true": {
    ...
    "env vars": {
      "keys also available as lower case": function keysAlsoAvailableAsLowerCase() {
  ...
  "When using env with parseValues:true": {
    ...
    "env vars": {
      "JSON keys properly parsed": function JSONKeysProperlyParsed() {
        Object.keys(process.env).forEach(function (key) {
  ...
  "When using env with transform:fn": {
    ...
    "env vars": {
      "port key/value properly transformed": function portKeyValueProperlyTransformed() {
  ...
  "When using env with a transform:fn that drops an entry": {
    ...
    "env vars": {
      "port key/value properly transformed": function portKeyValueProperlyTransformed() {
        assert.equal(_typeof(nconf.get('FOO')), 'undefined');
      }
    }
  },
  teardown: function teardown() {
  ...
  "When using env with a bad transform:fn": {
    ...
    "env vars": {
      "port key/value throws transformation error": function portKeyValueThrowsTransformationError(err) {
  ...
  "When using env with a bad transform:fn": {
    ...
    "env vars": {
      "can access to nested values": function canAccessToNestedValues(err) {
        assert.deepEqual(nconf.get('NESTED'), {
          VALUE: 'nested',
