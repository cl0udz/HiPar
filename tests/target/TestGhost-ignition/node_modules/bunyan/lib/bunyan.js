...
function Logger(options, _childOptions, _childSimple) {
  ...
  if (options.serializers && (_typeof(options.serializers) !== 'object' || Array.isArray(options.serializers))) {
...
function mkRecord(log, minLevel, args) {
  ...
  } else if (_typeof(args[0]) !== 'object' || Array.isArray(args[0])) {
    // `log.<level>(msg, ...)`
    fields = null;
    msgArgs = args.slice();
...
if (mv) {
  RotatingFileStream = function RotatingFileStream(options) {
    ...
    assert.equal(_typeof(this.count), 'number', format('rotating-file stream "count" is not a number: %j (%s) in %j', this.count, _typeof(this.count), this));
