...
function GelfStream(host, port, options) {
  if (options == null && _typeof(port) === 'object') {
    options = port;
    port = null;

    if (options == null && _typeof(host) === 'object') {
      options = host;
      host = null;
    }
  }
...
function mapGelfLevel(bunyanLevel) {
  switch (bunyanLevel) {
    ...
    /*bunyan.INFO*/
    :
      return gelfling.INFO;

    case 40
    /*bunyan.WARN*/
    :
      return gelfling.WARNING;

    case 50
    /*bunyan.ERROR*/
    :
      return gelfling.ERROR;

    case 60
    /*bunyan.FATAL*/
    :
      return gelfling.EMERGENCY;
...
function flatten(obj, into, prefix, sep) {
  ...
  for (key in obj) {
    ...
    prop = obj[key];
    if (_typeof(prop) === 'object' && !(prop instanceof Date) && !(prop instanceof RegExp)) flatten(prop, into, prefix + key + sep, sep);else into[prefix + key] = prop;
  }
...
function bunyanToGelf(log) {
  ...
  if (log.err && log.err.stack && (errFile = log.err.stack.match(/\n\s+at .+ \(([^:]+)\:([0-9]+)/)) != null) {
    if (errFile[1]) gelfMsg.file = errFile[1];
    if (errFile[2]) gelfMsg.line = errFile[2];
  }

  for (key in flattenedLog) {
    if (ignoreFields.indexOf(key) < 0 && gelfMsg[key] == null) gelfMsg[key] = flattenedLog[key];
  }
