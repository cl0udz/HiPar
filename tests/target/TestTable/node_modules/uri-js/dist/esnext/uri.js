...
var _regexpsUri = _interopRequireDefault(require("./regexps-uri"));

var _regexpsIri = _interopRequireDefault(require("./regexps-iri"));
...
function _normalizeIPv6(host, protocol) {
  ...
  if (address) {
    var _address$toLowerCase$ = address.toLowerCase().split('::').reverse(),
        _address$toLowerCase$2 = _slicedToArray(_address$toLowerCase$, 2),
...
function parse(uriString) {
  ...
  var components = {};
  var protocol = options.iri !== false ? _regexpsIri.default : _regexpsUri.default;
  ...
  if (matches) {
    ...
    if (!options.unicodeSupport && (!schemeHandler || !schemeHandler.unicodeSupport)) {
      ...
      if (components.host && (options.domainHost || schemeHandler && schemeHandler.domainHost)) {
        ...
        try {
          components.host = _punycode.default.toASCII(components.host.replace(protocol.PCT_ENCODED, pctDecChars).toLowerCase());
        } catch (e) {
      ...
      _normalizeComponentEncoding(components, _regexpsUri.default);
    } else {
      //normalize encodings
...
function serialize(components) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  var protocol = options.iri ? _regexpsIri.default : _regexpsUri.default;
  ...
  if (components.host) {
    ...
    //if host component is a domain name
        ...
        try {
          components.host = !options.iri ? _punycode.default.toASCII(components.host.replace(protocol.PCT_ENCODED, pctDecChars).toLowerCase()) : _punycode.default.toUnicode(components.host);
        } catch (e) {
...
function resolveComponents(base, relative) {
  ...
  var skipNormalization = arguments.length > 3 ? arguments[3] : undefined;
  var target = {};
...
function resolve(baseURI, relativeURI, options) {
  var schemelessOptions = (0, _util.assign)({
    scheme: 'null'
  }, options);
...
function escapeComponent(str, options) {
  return str && str.toString().replace(!options || !options.iri ? _regexpsUri.default.ESCAPE : _regexpsIri.default.ESCAPE, pctEncChar);
}

;

function unescapeComponent(str, options) {
  return str && str.toString().replace(!options || !options.iri ? _regexpsUri.default.PCT_ENCODED : _regexpsIri.default.PCT_ENCODED, pctDecChars);
}
