...
function findScenarios() {
  return fs.readdirSync(path.join(__dirname, 'bson-corpus')).filter(function (x) {
    return x.indexOf('json') !== -1;
  }).map(function (x) {
    return [x, fs.readFileSync(path.join(__dirname, 'bson-corpus', x), 'utf8')];
  }).map(function (x) {
    return [path.basename(x[0], '.json'), JSON.parse(x[1])];
  });
}

function nativeToBson(_native) {
  var b = new BSON();
  var serializeOptions = {
    ignoreUndefined: false
  };
  return b.serialize(_native, serializeOptions);
}
...
function nativeToCEJSON(_native2) {
  return extJSON.stringify(_native2);
}

function nativeToREJSON(_native3) {
  return extJSON.stringify(_native3, {
    relaxed: true
  });
}
...
var skip = {
  'Timestamp with high-order bit set on both seconds and increment': 'Current BSON implementation of timestamp/long cannot hold these values - 1 too large.'
};
var modifiedDoubles = {
  '+1.0': {
    canonical_extjson: '{"d":{"$numberDouble":"1"}}'
  },
  '-1.0': {
    canonical_extjson: '{"d":{"$numberDouble":"-1"}}'
  },
...
describe('BSON Corpus Tests:', function () {
  findScenarios().forEach(function (scenario) {
    var scenarioData = scenario[1],
        deprecated = scenarioData.deprecated,
        description = scenarioData.description,
        valid = scenarioData.valid || []; // since doubles are formatted differently in JS than in corpus, overwrite expected results
    ...
    valid.forEach(function (v) {
      ...
      if (skip.hasOwnProperty(v.description)) {
        it.skip(v.description, function () {});
        return;
      }

      it(description + ' - ' + v.description, function (done) {
        ...
        var nativeFromCB = bsonToNative(cB),
            nativeFromCEJ = jsonToNative(cEJ); // round tripped EJSON should match the original

        expect(nativeToCEJSON(nativeFromCEJ)).to.equal(cEJ); // invalid, but still parseable, EJSON. if provided, make sure that we
        // properly convert it to canonical EJSON and BSON.

        if (v.degenerate_extjson) {
          var dEJ = normalize(v.degenerate_extjson),
        ...
        if (!v.lossy) expect(nativeToBson(nativeFromCEJ)).to.deep.equal(cB); // the reverse direction, BSON -> native -> EJSON, should match canonical EJSON.

        expect(nativeToCEJSON(nativeFromCB)).to.equal(cEJ);

        if (v.relaxed_extjson) {
          var rEJ = normalize(v.relaxed_extjson); // BSON -> native -> relaxed EJSON matches provided

          expect(nativeToREJSON(nativeFromCB)).to.equal(rEJ); // relaxed EJSON -> native -> relaxed EJSON unchanged
