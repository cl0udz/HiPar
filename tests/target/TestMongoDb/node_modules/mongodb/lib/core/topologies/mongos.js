...
var cloneOptions = require('./shared').cloneOptions;
...
Mongos.prototype.connect = function (options) {
  ...
  var servers = this.s.seedlist.map(function (x) {
    var server = new Server(Object.assign({}, self.s.options, x, options, {
      reconnect: false,
      monitoring: false,
      parent: self,
    ...
    if (event === 'connect') {
      ...
      if (self.ismaster.msg === 'isdbgrid') {
        ...
        for (var _i = 0; _i < handlers.length; _i++) {
          _this.removeAllListeners(handlers[_i]);
...
function connectProxies(self, servers) {
  ...
  servers.forEach(function (server) {
    return connect(server, timeoutInterval++);
  });
}
...
function pickProxy(self, session) {
  ...
  var proxy; // We have no connectedProxies pick first of the connected ones
...
Mongos.prototype.destroy = function (options, callback) {
  ...
  var proxies = this.connectedProxies.concat(this.connectingProxies);
  var serverCount = proxies.length;
  ...
  var serverDestroyed = function serverDestroyed() {
    ...
    emitTopologyDescriptionChanged(_this2);
  ...
  proxies.forEach(function (server) {
    // Emit the sdam event
    _this2.emit('serverClosed', {
      topologyId: _this2.id,
      address: server.name
    });

    destroyServer(server, options, serverDestroyed);
    moveServerFrom(_this2.connectedProxies, _this2.disconnectedProxies, server);
  });
};
/**
...
function executeWriteOperation(args, options, callback) {
  ...
  var op = args.op;
  var ns = args.ns;
  var ops = args.ops; // Pick a server

  var server = pickProxy(self, options.session); // No server found error out
...
Mongos.prototype.command = function (ns, cmd, options, callback) {
  ...
  var cb = function cb(err, result) {
    ...
    if (willRetryWrite) {
      var newOptions = Object.assign({}, clonedOptions, {
        retrying: true
      });
      return _this3.command(ns, cmd, newOptions, callback);
    }
...
Mongos.prototype.selectServer = function (selector, options, callback) {
  ...
  options = options || {};
  var server = pickProxy(this, options.session);
