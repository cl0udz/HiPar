...
module.exports = function (url, options, callback) {
  ...
  dns.resolveSrv(srvAddress, function (err, addresses) {
    ...
    var connectionStrings = addresses.map(function (address, i) {
      if (i === 0) return "".concat(base).concat(address.name, ":").concat(address.port);else return "".concat(address.name, ":").concat(address.port);
    });
...
function parseConnectionString(url, options) {
  ...
  var auth = authPart.split(':', 2); // Decode the authentication URI components and verify integrity
      ...
      default:
        {
          ...
          logger.warn("".concat(name, " is not supported as a connection string option"));
        }
        break;
    }
