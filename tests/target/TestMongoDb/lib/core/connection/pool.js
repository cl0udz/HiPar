...
var stateTransition = makeStateMachine((_makeStateMachine = {}, _defineProperty(_makeStateMachine, DISCONNECTED, [CONNECTING, DRAINING, DISCONNECTED]), _defineProperty(_makeStateMachine, CONNECTING, [CONNECTING, CONNECTED, DRAINING, DISCONNECTED]), _defineProperty(_makeStateMachine, CONNECTED, [CONNECTED, DISCONNECTED, DRAINING]), _defineProperty(_makeStateMachine, DRAINING, [DRAINING, DESTROYING, DESTROYED]), _defineProperty(_makeStateMachine, DESTROYING, [DESTROYING, DESTROYED]), _defineProperty(_makeStateMachine, DESTROYED, [DESTROYED]), _makeStateMachine));
var CONNECTION_EVENTS = new Set(['error', 'close', 'timeout', 'parseError', 'connect', 'message']);
var _id = 0;
/**
...
function connectionFailureHandler(pool, event, err, conn) {
  if (conn) {
    ...
    while (conn.workItems.length > 0) {
      var workItem = conn.workItems.shift();
  ...
  var totalConnections = totalConnectionCount(pool);
...
function attemptReconnect(pool, callback) {
  return function () {
    ...
    if (pool.retriesLeft <= 0) {
      pool.destroy();
      var error = new MongoTimeoutError("failed to reconnect after ".concat(pool.options.reconnectTries, " attempts with interval ").concat(pool.options.reconnectInterval, " ms"), pool.reconnectError);
    ...
    createConnection(pool, function (err, conn) {
      if (err == null) {
        pool.reconnectId = null;
        pool.retriesLeft = pool.options.reconnectTries;
    ...
    if (workItem && !workItem.immediateRelease) {
      ...
      if (workItem.command && message.documents[0]) {
        ...
        if (responseDoc.writeConcernError) {
          var err = new MongoWriteConcernError(responseDoc.writeConcernError, responseDoc);
...
Pool.prototype.connect = function (callback) {
  ...
  createConnection(this, function (err, conn) {
    if (err) {
      if (typeof callback === 'function') {
        _this.destroy();
    ...
    stateTransition(_this, CONNECTED); // create min connections

    if (_this.minSize) {
      for (var i = 0; i < _this.minSize; i++) {
        createConnection(_this);
      }
    }
...
function destroy(self, connections, options, callback) {
  ...
  eachAsync(connections, function (conn, cb) {
    var _iteratorNormalCompletion = true;
    var _didIteratorError = false;
    var _iteratorError = undefined;

    try {
      for (var _iterator = CONNECTION_EVENTS[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
...
Pool.prototype.reset = function (callback) {
  ...
  eachAsync(connections, function (conn, cb) {
    ...
    try {
      for (var _iterator2 = CONNECTION_EVENTS[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
    ...
    resetPoolState(_this2); // create a new connection, this will ultimately trigger execution

    createConnection(_this2, function () {
      if (typeof callback === 'function') {
        callback(null, null);
      }
    });
...
function serializeCommand(self, command, callback) {
  var originalCommandBuffer = command.toBin(); // Check whether we and the server have agreed to use a compressor

  var shouldCompress = !!self.options.agreedCompressor;
...
Pool.prototype.write = function (command, options, cb) {
  ...
  if (self.options.monitorCommands) {
    ...
    operation.cb = function (err, reply) {
      if (err) {
        self.emit('commandFailed', new apm.CommandFailedEvent(_this3, command, err, operation.started));
      } else {
        if (reply && reply.result && (reply.result.ok === 0 || reply.result.$err)) {
          self.emit('commandFailed', new apm.CommandFailedEvent(_this3, command, reply.result, operation.started));
        } else {
          self.emit('commandSucceeded', new apm.CommandSucceededEvent(_this3, command, reply, operation.started));
        }
      }
  ...
  serializeCommand(self, command, function (err, serializedBuffers) {
...
function createConnection(pool, callback) {
  ...
  connect(pool.options, function (err, connection) {
    pool.connectingConnections--;

    if (err) {
      if (pool.logger.isDebug()) {
        pool.logger.debug("connection attempt failed with error [".concat(JSON.stringify(err), "]"));
...
function _execute(self) {
  return function () {
    ...
    while (true) {
      ...
      var totalConnections = totalConnectionCount(self); // No available connections available, flush any monitoring ops
      ...
      var connection = null;
      var connections = self.availableConnections.filter(function (conn) {
        return conn.workItems.length === 0;
      ...
      if (Array.isArray(buffer)) {
        for (var _i = 0; _i < buffer.length; _i++) {
          writeSuccessful = connection.write(buffer[_i]);
        }
      } else {
